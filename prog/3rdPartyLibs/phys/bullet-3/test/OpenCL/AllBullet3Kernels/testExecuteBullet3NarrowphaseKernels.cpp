...
//		snprintf(flags,sizeof(flags),"-g -s \"%s\"","C:/develop/bullet3_experiments2/opencl/gpu_narrowphase/kernels/satClipHullContacts.cl");
//#endif
...
unsigned char* openFile(const char* fileName, int* sizeInBytesPtr)
{
	*sizeInBytesPtr = 0;
...

	{
		...
		{
			snprintf(relativeFileName, sizeof(relativeFileName), "%s%s", prefix[i], "unittest_data.zip");
	...
	if (!buffer)
	{
		...
		for (int i = 0; !f && i < numPrefixes; i++)
		{
			snprintf(relativeFileName, sizeof(relativeFileName), "%s%s", prefix[i], fileName);
			f = fopen(relativeFileName, "rb");
		}
