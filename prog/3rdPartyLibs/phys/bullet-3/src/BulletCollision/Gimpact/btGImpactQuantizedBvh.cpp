	...
	GIM_BVH_DATA_ARRAY& primitive_boxes, int startIndex, int endIndex)
{
	...
	{
		btVector3 center = (primitive_boxes[i].m_bound.m_max + primitive_boxes[i].m_bound.m_min);
		means += center;
	}
	...
	int endIndex, int splitAxis)
{
	...
	for (i = startIndex; i < endIndex; i++)
		splitValue += (primitive_boxes[i].m_bound.m_max[splitAxis] + primitive_boxes[i].m_bound.m_min[splitAxis]);
	splitValue /= (btScalar)numIndices;

	//sort leafNodes so all values larger then splitValue comes first, and smaller values start from 'splitIndex'.
	for (i = startIndex; i < endIndex; i++)
	{
		btScalar center = (primitive_boxes[i].m_bound.m_max[splitAxis] + primitive_boxes[i].m_bound.m_min[splitAxis]);
