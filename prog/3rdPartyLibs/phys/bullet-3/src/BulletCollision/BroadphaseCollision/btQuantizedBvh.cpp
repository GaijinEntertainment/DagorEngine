...
void btQuantizedBvh::setQuantizationValues(const btVector3& bvhAabbMin, const btVector3& bvhAabbMax, btScalar quantizationMargin)
{
	...
	btVector3 aabbSize = bvhAabbMax - bvhAabbMin + 2.f * clampValue;
	m_bvhQuantization = btVector3(btScalar(65533.0), btScalar(65533.0), btScalar(65533.0)) / aabbSize;
	m_bvhAabbMin = bvhAabbMin - clampValue;
	m_bvhAabbMax = m_bvhAabbMin +
		btVector3(65533.f / m_bvhQuantization.getX(), 65533.f / m_bvhQuantization.getY(), 65533.f / m_bvhQuantization.getZ());

	m_useQuantization = true;

	{
		unsigned short vecIn[4];
		btVector3 v;
		{
...
int btQuantizedBvh::sortAndCalcSplittingIndex(int startIndex, int endIndex, int splitAxis)
{
	...
	btScalar splitValue = 0.0f; //splitValue = mean_center[splitAxis]*2, so we compare later with center*2
	for (i = startIndex; i < endIndex; i++)
		splitValue += (getAabbMax(i)[splitAxis]+getAabbMin(i)[splitAxis]);
	splitValue /= (btScalar)numIndices;

	//sort leafNodes so all values larger then splitValue comes first, and smaller values start from 'splitIndex'.
	for (i = startIndex; i < endIndex; i++)
	{
		btScalar center = (getAabbMax(i)[splitAxis] + getAabbMin(i)[splitAxis]);
...
int btQuantizedBvh::calcSplittingAxis(int startIndex, int endIndex)
{
	...
	for (i = startIndex; i < endIndex; i++)
	{
		btVector3 center = (getAabbMax(i) + getAabbMin(i));
		means += center;
	}
