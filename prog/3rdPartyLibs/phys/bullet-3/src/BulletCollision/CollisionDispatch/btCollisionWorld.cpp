...
void btCollisionWorld::updateSingleAabb(btCollisionObject* colObj)
{
	...
	{
		if (auto *h = colObj->getBroadphaseHandle())
			bp->setAabb(h, minAabb, maxAabb, m_dispatcher1);
	}
	else
	{
		...
		colObj->setIslandTag(-1);
		removeCollisionObject(colObj);
...
{
	BT_PROFILE0("updateAabbs");

	for (int i = (int)m_collisionObjects.size() - 1; i >= 0; i--)
	{
...
{
	BT_PROFILE0("calculateOverlappingPairs");
	m_broadphasePairCache->calculateOverlappingPairs(m_dispatcher1);
}

void btCollisionWorld::performDiscreteCollisionDetection()
{
	BT_PROFILE0("performDiscreteCollisionDetection");
	...
	{
		BT_PROFILE0("dispatchAllCollisionPairs");
		if (dispatcher)
	...
	else
	{
		...
		else
		{
			if (collisionShape->isCompound())
			{
				...
				BT_PROFILE0("convexSweepCompound");
				const btCompoundShape* compoundShape = static_cast<const btCompoundShape*>(collisionShape);
