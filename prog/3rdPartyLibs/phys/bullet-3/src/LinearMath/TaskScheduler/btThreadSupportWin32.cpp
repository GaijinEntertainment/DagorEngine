...
void btThreadSupportWin32::startThreads(const ConstructionInfo& threadConstructionInfo)
{
	...
	for (int i = 0; i < m_numThreads; i++)
	{
		...
		snprintf(threadStatus.m_eventStartHandleName, sizeof(threadStatus.m_eventStartHandleName),
		  "es%.8s%d%d", threadConstructionInfo.m_uniqueName, uniqueId, i);
		threadStatus.m_eventStartHandle = CreateEventA(0, false, false, threadStatus.m_eventStartHandleName);

		snprintf(threadStatus.m_eventCompleteHandleName, sizeof(threadStatus.m_eventCompleteHandleName),
