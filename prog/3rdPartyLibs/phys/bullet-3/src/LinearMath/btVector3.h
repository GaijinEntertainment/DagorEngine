...
#define bt_sel(a, b, c) _mm_or_ps(_mm_and_ps(c, b), _mm_andnot_ps(c, a))
...
#if defined(__SPU__) && defined(_TARGET_C0)
	btScalar m_floats[4];
...
public:
	...
	{
#ifdef BT_DEBUG
		m_floats[0] = m_floats[1] = m_floats[2] = m_floats[3] = 0;
#endif
	}
...
SIMD_FORCE_INLINE btVector3 chooseMaxAxis(const btVector3& dir, const btVector3* vertices)
{
#if defined(BT_USE_SSE_IN_API) && defined (BT_USE_SSE)
	btVector3 dots = dir.dot3(vertices[0],vertices[1],vertices[2]);
	// 0 > 1, 1 > 2, 2 > 0, ignore
	__m128 gt0 = _mm_cmpgt_ps(dots.mVec128, bt_pshufd_ps(dots.mVec128, BT_SHUFFLE(1, 2, 0, 3)));
	__m128 gt01 = bt_splat_ps(gt0, 0);
	__m128 gt12 = bt_splat_ps(gt0, 1);
	__m128 gt20 = bt_splat_ps(gt0, 2);
	__m128 res = bt_sel(bt_sel(vertices[2].mVec128, vertices[1].mVec128, gt12),
	                    bt_sel(vertices[0].mVec128, vertices[2].mVec128, gt20),
	                    gt01);
