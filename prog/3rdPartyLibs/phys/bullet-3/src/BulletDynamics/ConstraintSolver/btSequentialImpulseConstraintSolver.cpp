...
#if !defined(USE_FMA) && (defined(__FMA__) || defined(_WIN32))
#if !defined(__FMA__) && defined(__clang__) // immintrin.h is already included by btCpuFeatureUtility.h -> intin.h -> x86intrin.h -> immintrin.h
#include <avxintrin.h> // __m256
#include <fmaintrin.h> // _mm_fnmadd_ps, ...
#else
#include <immintrin.h>
#endif
#define USE_FMA 1
...
#if defined(BT_ALLOW_SSE4) && USE_FMA && (defined(__clang__) || defined(__GNUC__))
__attribute__((target("sse4.1,fma")))
#endif
...
void btSequentialImpulseConstraintSolver::setupSolverFunctions(bool useSimd)
{
	...
	if (useSimd)
	{
		...
		if (cpuFeatures & btCpuFeatureUtility::CPU_FEATURE_SSE4_1)
#if USE_FMA
