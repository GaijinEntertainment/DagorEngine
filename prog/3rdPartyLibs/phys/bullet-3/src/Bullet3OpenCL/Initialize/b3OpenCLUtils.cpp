...
#else
		snprintf(binaryFileName, B3_MAX_STRING_LENGTH, "%s/%s.%s.%s.bin", sCachedBinaryPath, strippedName, deviceName, driverVersion);
#endif
	}
			...
			if (binaryFileValid)
			{
				...
				if (srcFileHandle == INVALID_HANDLE_VALUE)
				{
					...
					for (int i = 0; (srcFileHandle == INVALID_HANDLE_VALUE) && i < 5; i++)
					{
						...
						snprintf(relativeFileName, sizeof(relativeFileName), "%s%s", prefix[i], clFileNameForCaching);
...
#else
		snprintf(compileFlags, flagsize, "%s %s", flags, additionalMacros);
#endif
