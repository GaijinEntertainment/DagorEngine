...
int b3QuantizedBvh::sortAndCalcSplittingIndex(int startIndex, int endIndex, int splitAxis)
{
	...
	int numIndices = endIndex - startIndex;
	b3Scalar splitValue = 0.0f; //splitValue = mean_center[splitAxis]*2, so we compare later with center*2

	for (i = startIndex; i < endIndex; i++)
		splitValue += (getAabbMax(i)[splitAxis] + getAabbMin(i)[splitAxis]);
	splitValue /= (b3Scalar)numIndices;

	//sort leafNodes so all values larger then splitValue comes first, and smaller values start from 'splitIndex'.
	for (i = startIndex; i < endIndex; i++)
	{
		b3Scalar center = (getAabbMax(i)[splitAxis] + getAabbMin(i)[splitAxis]);
...
int b3QuantizedBvh::calcSplittingAxis(int startIndex, int endIndex)
{
	...
	for (i = startIndex; i < endIndex; i++)
	{
		b3Vector3 center = (getAabbMax(i) + getAabbMin(i));
		means += center;
	}
