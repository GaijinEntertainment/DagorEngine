...
static int _TIFFReserveLargeEnoughWriteBuffer(TIFF* tif, uint32_t strip_or_tile)
{
    ...
    {
        TIFFErrorExt(tif->tif_clientdata, module, "Strip bytecount array pointer is NULL");
        return 0;
    }
    /* (uint32_t)(-1) is NOSTRIP/NOTILE and is also >= td_nstrips */
    if( strip_or_tile >= td->td_nstrips )
    {
        TIFFErrorExt(tif->tif_clientdata, module, "Strip/tile number not valid");
        return 0;
    }
...
TIFFAppendToStrip(TIFF* tif, uint32_t strip, uint8_t* data, tmsize_t cc)
{
        ...
        td->td_stripbytecount_p[strip] > 0 )
    {
        ...
        while( toCopy > 0 )
        {
            ...
            offsetRead += chunkSize;
            offsetWrite += chunkSize;
            td->td_stripbytecount_p[strip] += chunkSize;
            toCopy -= chunkSize;
        }
        _TIFFfree(temp);
