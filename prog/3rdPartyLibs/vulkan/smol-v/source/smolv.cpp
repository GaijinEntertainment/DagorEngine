...
struct OpCounterSorter
{
  inline bool operator()(OpCounter a, OpCounter b) { return a.second > b.second; }
};
...
void smolv::StatsPrint(const Stats* stats)
{
	...
	std::sort(counts, counts + kKnownOpsCount, OpCounterSorter());
	std::sort(sizes, sizes + kKnownOpsCount, OpCounterSorter());
	std::sort(sizesSmol, sizesSmol + kKnownOpsCount, OpCounterSorter());
