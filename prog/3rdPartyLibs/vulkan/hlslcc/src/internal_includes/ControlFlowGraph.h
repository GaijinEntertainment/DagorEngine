...
public:
  ...
  const BasicBlockStorage &AllBlocks() const
  {
...
public:
  ...
  typedef std::unordered_set<uint32_t> RegisterSet;
  ...
  typedef std::unordered_set<const Instruction *> ConnectionSet;
  ...
  typedef std::unordered_map<uint32_t, ReachableDefinitionsPerVariable> ReachableVariables; // A VisibleDefinitionSet for each variable*component.

  const Instruction *First() const
  {
    return m_First;
  }
  ...
  const RegisterSet &UEVar() const
  {
    return m_UEVar;
  }
  const RegisterSet &VarKill() const
  {
    return m_VarKill;
  }

  const ConnectionSet &Preceding() const
  {
    return m_Preceding;
  }
  const ConnectionSet &Succeeding() const
  {
    return m_Succeeding;
  }

  const ReachableVariables &DEDef() const
  {
    return m_DEDef;
  }
  const ReachableVariables &Reachable() const
  {
    return m_Reachable;
  }
...
private:
  ...
  const Instruction *m_First = nullptr; // The first instruction in the basic block
  const Instruction *m_Last = nullptr; // The last instruction in the basic block. Either OPCODE_RET or a branch/jump/loop instruction
