...
void TranslateOperandSwizzleWithMask(HLSLCrossCompilerContext* psContext, const Operand* psOperand, uint32_t ui32ComponentMask, int iRebase)
{
  ...
  uint32_t srcMask = psOperand->GetAccessMask();
  bool needsSwizzle = false;
  if (psOperand->iWriteMaskEnabled && psOperand->iNumComponents != 1)
    needsSwizzle = true;
  else if (!psOperand->iWriteMaskEnabled && psOperand->iNumComponents==0 && psOperand->eType == OPERAND_TYPE_TEMP)
    needsSwizzle = true;
...
{
	if (psContext->psShader.eTargetLanguage == LANG_METAL) // WTF?
	{
...
static void printImmediate32(HLSLCrossCompilerContext *psContext, uint32_t value, SHADER_VARIABLE_TYPE eType)
{
	...
	case SVT_FLOAT16:
	{
		//we expect that data is not changed between shaders
		//but PrintFloat can't handle "raw int" data, breaking it due to float->str conversion
		//so we must use intBitsToFloat to avoid this

		//DXBC does not directly say what data type IMM value have,
		//use a exponent check to try determine it
		static const uint32_t exponentMask = 0x7f800000;
		uint32_t exponent = value & exponentMask;
		if ((exponent != 0 && exponent != exponentMask) || value == 0)
			HLSLcc::PrintFloat(glsl, *((float *)(&value)));
		else
			bformata(glsl, "intBitsToFloat(%d)", value);
	}
	break;
	case SVT_BOOL:
		if (value == 0)
...
uint32_t getStrideSizeForDynamicIndexing(const ShaderVarType* var)
{
  if (var->Class == SVC_STRUCT)
  {
    if (var->Members.size())
    {
      const ShaderVarType* lastMember = &var->Members[0];
      for (uint32_t i = 1; i < var->Members.size(); ++i)
      {
        const ShaderVarType* thisMember = &var->Members[i];
        if (lastMember->Offset < thisMember->Offset)
          lastMember = thisMember;
      }
      return lastMember->Offset + getStrideSizeForDynamicIndexing(lastMember);
    }
    return 1;
  }
  else if (var->Class == SVC_SCALAR || var->Class == SVC_VECTOR)
  {
    uint32_t baseSize = 0;
    ...
    if (var->Class == SVC_VECTOR)
    {
      baseSize *= var->Columns;
    }
    if (var->Class == SVC_MATRIX_ROWS)
    {
      baseSize *= var->Rows * 4;
    }
    if (var->Class == SVC_MATRIX_ROWS)
    {
      baseSize *= var->Columns * 4;
    }

    // align to register with
    baseSize = ((baseSize + 15) & ~15);
    if (var->Elements)
      baseSize *= var->Elements;
    return baseSize;
  }
...
void ToGLSL::TranslateVariableNameWithMask(const Operand* psOperand, uint32_t ui32TOFlag, uint32_t* pui32IgnoreSwizzle, uint32_t ui32CompMask, int *piRebase, const char* temp_name)
{
      ...
      if (temp_name)
      {
		...
		case OPERAND_TYPE_CONSTANT_BUFFER:
		{
            ...
            if (psVarType->Parent && psVarType->Parent->Elements > 1 && index == -1 && psOperand->m_SubOperands[1])
            {
              ...
              TranslateOperand(psOperand->m_SubOperands[1].get(), TO_FLAG_UNSIGNED_INTEGER, 1, temp_name);
              bformata(glsl, " / %uu", getStrideSizeForDynamicIndexing(psVarType->Parent) / 16);
		...
		case OPERAND_TYPE_IMMEDIATE_CONSTANT_BUFFER:
		{
			...
			if (psOperand->m_Size == 1 && !(ui32TOFlag & TO_FLAG_FORCE_SWIZZLE))
				*pui32IgnoreSwizzle = 1;
			break;
		}
		...
		case OPERAND_TYPE_INPUT_CONTROL_POINT:
		{
			...
			else
			{
				...
				// Check for scalar
				if ((psContext->psShader.abScalarInput[(int)psOperand->GetRegisterSpace(psContext->psShader.eShaderType, psContext->psShader.asPhases[psContext->currentPhase].ePhase)][psOperand->ui32RegisterNumber] & psOperand->GetAccessMask()) != 0)
					*pui32IgnoreSwizzle = 1;
			}
			break;
		}
			...
			*piRebase = psIn->iRebase;
			...
			case NAME_VERTEX_ID:
				if ((psContext->flags & HLSLCC_FLAG_VULKAN_BINDINGS) != 0)
					bcatcstr(glsl, "(gl_VertexIndex - gl_BaseVertexARB)");
				else
					bcatcstr(glsl, "(gl_VertexIndex - gl_BaseVertexARB)");
				//*pui32IgnoreSwizzle = 1;
				break;
...
void ToGLSL::TranslateOperand(const Operand* psOperand, uint32_t ui32TOFlag, uint32_t ui32ComponentMask , const char* temp_override)
{
	bstring glsl = *psContext->currentGLSLString;
	...
	{
		TranslateVariableNameWithMask(psOperand, ui32TOFlag, &ui32IgnoreSwizzle, OPERAND_4_COMPONENT_MASK_ALL, &iRebase, temp_override);
		return;
	}
	...
	TranslateVariableNameWithMask(psOperand, ui32TOFlag, &ui32IgnoreSwizzle, ui32ComponentMask, &iRebase, temp_override);
...
std::string TextureSamplerName(ShaderInfo& psShaderInfo, const uint32_t ui32TextureRegisterNumber, const uint32_t ui32SamplerRegisterNumber, const int bZCompare)
{
  ...
  const ResourceBinding* psTextureBinding = nullptr;
  bool foundTexture = psShaderInfo.GetResourceFromBindingPoint(RGROUP_TEXTURE, ui32TextureRegisterNumber, &psTextureBinding);

  const ResourceBinding* psSamplerBinding = nullptr;
  bool foundSampler = psShaderInfo.GetResourceFromBindingPoint(RGROUP_SAMPLER, ui32SamplerRegisterNumber, &psSamplerBinding);
