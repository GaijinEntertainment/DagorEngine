...
static void DecodeNameToken(const uint32_t* pui32NameToken, Operand& psOperand)
{
...
static void MarkTextureAsShadow(std::vector<Declaration> &declarations, const Operand& psTextureOperand)
{
  auto ref = find_if(declarations.begin(), declarations.end(), [=](const Declaration& decl)
  {
    return (decl.eOpcode == OPCODE_DCL_RESOURCE)
        && (decl.asOperands[0].eType == OPERAND_TYPE_RESOURCE &&
            decl.asOperands[0].ui32RegisterNumber == psTextureOperand.ui32RegisterNumber);
  });

  if (ref != declarations.end())
    ref->ui32IsShadowTex = 1;
}

static void MarkTextureSamplerPair(ShaderInfo& psShaderInfo,
                                   std::vector<Declaration> & declarations,
                                   const Operand& psTextureOperand,
                                   const Operand& psSamplerOperand,
                                   TextureSamplerPairs& samplers)
{
...
static uint32_t DecodeOperand(const uint32_t *pui32Tokens, Operand& psOperand)
{
  int i;
  ...
  {
    copy((float*)(&pui32Tokens[ui32NumTokens]), (float*)(&pui32Tokens[ui32NumTokens + psOperand.iNumComponents]), psOperand.afImmediates);
    ui32NumTokens += psOperand.iNumComponents;
  }
  else if (psOperand.eType == OPERAND_TYPE_IMMEDIATE64)
  {
    copy((double*)(&pui32Tokens[ui32NumTokens]), (double*)(&pui32Tokens[ui32NumTokens + psOperand.iNumComponents * 2]), psOperand.adImmediates);
    ui32NumTokens += psOperand.iNumComponents * 2;
  }

  if (psOperand.eType == OPERAND_TYPE_OUTPUT_DEPTH_GREATER_EQUAL ||
     psOperand.eType == OPERAND_TYPE_OUTPUT_DEPTH_LESS_EQUAL)
  {
    psOperand.ui32RegisterNumber = UINT32_MAX;
    psOperand.ui32CompMask = UINT32_MAX;
  }
  ...
  for (i = 0; i < psOperand.iIndexDims; ++i)
  {
    ...
    case OPERAND_INDEX_RELATIVE:
      ...
      DecodeOperand(pui32Tokens + ui32NumTokens, *psOperand.m_SubOperands[i]);
    ...
    case OPERAND_INDEX_RELATIVE:
    case OPERAND_INDEX_IMMEDIATE32_PLUS_RELATIVE:
      for (int j = 0; j < psOperand.iNumComponents; j++)
...
static const uint32_t* DecodeDeclaration(Shader& psShader, const uint32_t* pui32Token, Declaration& psDecl, ShaderPhase& psPhase)
{
  ...
  case OPCODE_DCL_INDEX_RANGE:
  {
    ...
    RegisterSpace regSpace = psDecl.asOperands[0].GetRegisterSpace(psShader.eShaderType, psPhase.ePhase);
    if (psDecl.asOperands[0].eType == OPERAND_TYPE_INPUT)
    {
      ...
      psShader.aIndexedInput[(int)regSpace][reg] = indexRange;
      psShader.aIndexedInputParents[(int)regSpace][reg] = reg;

      //-1 means don't declare this input because it falls in
      //the range of an already declared array.
      for (i = reg + 1; i < reg + indexRange; ++i)
      {
        psShader.aIndexedInput[(int)regSpace][i] = -1;
        psShader.aIndexedInputParents[(int)regSpace][i] = reg;
      }
    }

    if (psDecl.asOperands[0].eType == OPERAND_TYPE_OUTPUT)
    {
      psShader.aIndexedOutput[(int)regSpace][psDecl.asOperands[0].ui32RegisterNumber] = true;;
    }
    break;
  }
...
static const uint32_t* DecodeInstruction(const uint32_t* pui32Token, Instruction* psInst, Shader& psShader, ShaderPhase *psPhase)
{
  uint32_t ui32TokenLength = DecodeInstructionLength(*pui32Token);
  const uint32_t bExtended = DecodeIsOpcodeExtended(*pui32Token);
  const OPCODE_TYPE eOpcode = DecodeOpcodeType(*pui32Token);
  ...
  if (eOpcode < NUM_OPCODES && eOpcode >= 0)
  {
    psShader.aiOpcodeUsed[eOpcode] = true;
  }
  ...
  case OPCODE_IMUL:
  case OPCODE_UDIV:
    psInst->ui32FirstSrc = 2; // those have 2 outputs and 2 inputs
  case OPCODE_MAD:
  case OPCODE_MOVC:
  case OPCODE_IMAD:
  case OPCODE_LOD:
  ...
  case OPCODE_SAMPLE_B:
  {
    ...
    {
      MarkTextureAsShadow(psPhase->psDecl, psInst->asOperands[2]);
    }
...
static const uint32_t* DecodeShaderPhase(const uint32_t* pui32Tokens,
                                         Shader& psShader,
                                         ...
                                         ShaderPhase& psPhase)
{
  ...
  for(;;) //Keep going until we reach the first non-declaration token, or the end of the shader.
  {
    Declaration newDecl;
    ...
    if (pui32Result)
    {
      psPhase.psDecl.push_back(newDecl);
  ...
  while (pui32CurrentToken < (psShader.pui32FirstToken + ui32ShaderLength))
  {
    ...
    const uint32_t* nextInstr = DecodeInstruction(pui32CurrentToken, &newInstruction, psShader, &psPhase);
    ...
    if (newInstruction.eOpcode == OPCODE_HS_FORK_PHASE || newInstruction.eOpcode == OPCODE_HS_JOIN_PHASE)
    {
      return pui32CurrentToken;
    }
    psPhase.psInst.push_back(newInstruction);
...
static const void AllocateHullPhaseArrays(const uint32_t* pui32Tokens,
                                          Shader& psShader)
{
  const uint32_t* pui32CurrentToken = pui32Tokens;
  const uint32_t ui32ShaderLength = psShader.ui32ShaderLength;
  ...
  for(auto&& phase : psShader.asPhases)
    phase.ui32InstanceCount = 1;
}

static const uint32_t* DecodeHullShader(const uint32_t* pui32Tokens, Shader& psShader)
{
  const uint32_t* pui32CurrentToken = pui32Tokens;
  const uint32_t ui32ShaderLength = psShader.ui32ShaderLength;
  ...
  auto psPhase = &psShader.asPhases[2];
...
static void Decode(const uint32_t* pui32Tokens, Shader& psShader)
{
  const uint32_t* pui32CurrentToken = pui32Tokens;
...
Shader* DecodeDXBC(uint32_t* data, uint32_t decodeFlags)
{
  ...
  uint32_t* chunkOffsets = (uint32_t*)(header + 1);
  uint32_t chunkCount = header->chunkCount;
  uint32_t* shaderChunk = nullptr;

  for (uint32_t i = 0; i < chunkCount; ++i)
  {
    DXBCChunkHeader* chunk = (DXBCChunkHeader*)((uint8_t*)data + chunkOffsets[i]);
  ...
  if (shaderChunk)
  {
    uint32_t ui32MajorVersion = DecodeProgramMajorVersion(*shaderChunk);
    uint32_t ui32MinorVersion = DecodeProgramMinorVersion(*shaderChunk);

    auto psShader = make_unique<Shader>();
