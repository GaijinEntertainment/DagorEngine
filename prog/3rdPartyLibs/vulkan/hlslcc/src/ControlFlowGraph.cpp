...
const BasicBlock &ControlFlowGraph::Build(const Instruction *firstInstruction)
{
  ...
  {
    hadChanges = false;
    for(auto&& bb : m_BlockStorage)
      if (bb->RebuildReachable())
        hadChanges = true;
  } while (hadChanges);
...
{
  auto itr = m_BlockMap.find(Utils::GetNextNonLabelInstruction(instruction));
  ...
  , m_Last(NULL)
{
  ...
  m_Graph.m_BlockStorage.emplace_back(this);

  m_Graph.m_BlockMap.insert(make_pair(psFirst, this));
  ...
  if (psPrecedingBlockHead != NULL)
  {
    ...
    prec->m_Succeeding.insert(psFirst);
  }
