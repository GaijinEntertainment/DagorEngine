...
static bool SetVectorType(std::vector<SHADER_VARIABLE_TYPE> &aeTempVecType, uint32_t regBaseIndex, uint32_t componentMask, SHADER_VARIABLE_TYPE eType)
{
...
{
  for (uint32_t i = 0; i < psInst->ui32NumOperands; i++)
...
{
  SetPartialDataTypes(std::vector<SHADER_VARIABLE_TYPE>& _aeTempVec)
    : m_TempVec(_aeTempVec)
  {}
  std::vector<SHADER_VARIABLE_TYPE>& m_TempVec;

  template<typename ItrType>
  void operator()(ItrType inst, Operand *psOperand, uint32_t ui32OperandType) const
  {
    if (psOperand->eType != OPERAND_TYPE_TEMP)
      return;
    ...
    if (ui32OperandType == FEO_FLAG_SUBOPERAND)
    {
      ...
      uint32_t reg = psOperand->ui32RegisterNumber;
      SHADER_VARIABLE_TYPE newType = OperandPrecisionToShaderVariableType(psOperand->eMinPrecision, SVT_INT_AMBIGUOUS);
      for (uint32_t i = 0; i < 4; i++)
      {
        if (!(mask & (1 << i)))
          continue;
        if (m_TempVec[reg * 4 + i] == SVT_VOID)
          m_TempVec[reg * 4 + i] = newType;
      }
    }
    else
    {
      ...
      uint32_t mask = psOperand->GetAccessMask();
      uint32_t reg = psOperand->ui32RegisterNumber;
      SHADER_VARIABLE_TYPE newType = OperandPrecisionToShaderVariableType(psOperand->eMinPrecision, SVT_VOID);
...
{
  WritebackDataTypes(HLSLCrossCompilerContext& _ctx, std::vector<SHADER_VARIABLE_TYPE>& _aeTempVec)
    : m_Context(_ctx)
    , m_TempVec(_aeTempVec)
  {}
  ...
  void operator()(ItrType inst, Operand* psOperand, uint32_t /*ui32OperandType*/) const
  {
    ...
    uint32_t reg = psOperand->ui32RegisterNumber;
    uint32_t mask = psOperand->GetAccessMask();
    SHADER_VARIABLE_TYPE dtype = SVT_VOID;
    for (uint32_t i = 0; i < 4; i++)
    {
      if (!(mask & (1 << i)))
        continue;
      ...
      ASSERT(dtype == SVT_VOID || dtype == m_TempVec[reg * 4 + i]);

      dtype = m_TempVec[reg * 4 + i];
...
std::vector<SHADER_VARIABLE_TYPE> HLSLcc::DataTypeAnalysis::SetDataTypes(HLSLCrossCompilerContext* psContext, std::vector<Instruction> & instructions, uint32_t ui32TempCount)
{
  uint32_t i;
  ...
  std::vector<SHADER_VARIABLE_TYPE> aeTempVecType(ui32TempCount * 4, SVT_VOID);

  if (ui32TempCount == 0)
    return aeTempVecType;
  ...
  ForEachOperand(instructions.begin(), instructions.end(), FEO_FLAG_ALL, SetPartialDataTypes(aeTempVecType));
  ...
  //	if (psContext->psShader.ui32MajorVersion <= 3)
  {
    ...
    for(auto&& inst : instructions)
    {
      ...
      case OPCODE_LD_MS:
      {
        ...
        MarkOperandAs(psInst->asOperands[0], TypeFlagsToSVTType(ResourceReturnTypeToFlag(psBinding->ui32ReturnType))
          , aeTempVecType);
        MarkOperandAs(psInst->asOperands[1], SVT_UINT, aeTempVecType);
        break;
      }
        ...
        if (psInst->eOpcode == OPCODE_MOV || psInst->eOpcode == OPCODE_MOVC)
        {
          ...
          if (dataType != SVT_VOID)
          {
            ...
            for (k = 0; k < psInst->ui32NumOperands; k++)
            {
              ...
              mask = psInst->asOperands[k].GetAccessMask();
              madeProgress = SetVectorType(aeTempVecType, psInst->asOperands[k].ui32RegisterNumber * 4, mask, dataType);
  ...
  for(auto&& t : aeTempVecType)
    if (t == SVT_FORCED_INT || t == SVT_INT_AMBIGUOUS)
      t = SVT_INT;

  ForEachOperand(instructions.begin(), instructions.end(), FEO_FLAG_ALL, WritebackDataTypes(*psContext, aeTempVecType));
