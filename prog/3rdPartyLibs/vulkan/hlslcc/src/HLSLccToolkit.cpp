...
namespace HLSLcc
{
	...
	bool IsAddOneInstruction(const Instruction *psInst)
	{
		...
		{
			temp = &psInst->asOperands[1];
			constant = &psInst->asOperands[2];
		}
		else
		{
			temp = &psInst->asOperands[2];
			constant = &psInst->asOperands[1];
		}
		if (temp->ui32RegisterNumber != psInst->asOperands[0].ui32RegisterNumber)
			return false;
    if (temp->aeDataType[0] != SVT_UINT && temp->aeDataType[0] != SVT_INT)
      return false;
		if (constant->eType != OPERAND_TYPE_IMMEDIATE32)
			return false;

		if (*(int *)&constant->afImmediates[0] != 1)
			return false;
