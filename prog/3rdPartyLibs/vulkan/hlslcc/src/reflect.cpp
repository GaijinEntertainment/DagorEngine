...
static const uint32_t* ReadResourceBinding(const uint32_t* pui32FirstResourceToken, const uint32_t* pui32Tokens, ResourceBinding* psBinding, uint32_t decodeFlags)
{
	...
	psBinding->isIntUAV = false;
	size_t nameLen = psBinding->name.length();
	if ((nameLen > 12) && (psBinding->name.rfind("__hlslcc_int") == (nameLen - 12)))
		psBinding->isIntUAV = true;
                                          ...
                                          ConstantBuffer* psBuffer)
{
  ...
  for (uint32_t i = 0; i < ui32VarCount; ++i)
  {
    ...
    if (ui32DefaultValueOffset)
    {
      ...
      copy(pui32DefaultValToken, pui32DefaultValToken + ui32NumDefaultValues, psVar->pui32DefaultValues.begin());
    }
  }
				   ...
				   uint32_t decodeFlags)
{
  ...
  if (decodeFlags & HLSLCC_FLAG_MAP_CONST_BUFFER_TO_REGISTER_ARRAY)
  {
    uint32_t id = 0;
    for (auto& cbuffer : psShaderInfo->psConstantBuffers)
    {
      ShaderVar var;
      var.haveDefaultValue = 0;
      var.name = "constData" + std::to_string(id++);
      var.ui32StartOffset = 0;
      var.ui32Size = cbuffer.ui32TotalSizeInBytes;

      var.sType.Class = SVC_VECTOR;
      var.sType.Type = SVT_FLOAT;
      var.sType.Rows = 1;
      var.sType.Columns = 4;
      var.sType.Elements = (var.ui32Size + 15) / 16;
      var.sType.MemberCount = 0;
      var.sType.Offset = 0;
      var.sType.name = var.name;
      var.sType.fullName = var.sType.name;
      var.sType.m_IsUsed = true;

      cbuffer.asVars.clear();
      cbuffer.asVars.push_back(var);
    }
  }
}
