...
{
	uint32_t p;
	auto &bindingArray = aui32StructuredBufferBindingPoints;
...
void ShaderPhase::ResolveUAVProperties()
{
  ...
  Instruction *psFirstInstruction = &psInst[0];
	...
	for (decl = psFirstDeclaration; decl <= psLastDeclaration; decl++)
	{
		Instruction *inst;
		uint32_t uavReg;
		if (decl->eOpcode != OPCODE_DCL_UNORDERED_ACCESS_VIEW_TYPED &&
			decl->eOpcode != OPCODE_DCL_UNORDERED_ACCESS_VIEW_STRUCTURED &&
			decl->eOpcode != OPCODE_DCL_UNORDERED_ACCESS_VIEW_RAW)
			continue;

		uavReg = decl->asOperands[0].ui32RegisterNumber;

		for (inst = psFirstInstruction; inst <= psLastInstruction; inst++)
		{
			uint32_t opIndex;
			...
			case OPCODE_LD_UAV_TYPED:
				...
				numComponents = inst->asOperands[0].GetNumSwizzleElements(); // get component count from the write target
				break;

			case OPCODE_STORE_UAV_TYPED:
				ASSERT(inst->asOperands[0].eType == OPERAND_TYPE_UNORDERED_ACCESS_VIEW);
				opIndex = 0;
			...
			if(inst->asOperands[opIndex].eType != OPERAND_TYPE_UNORDERED_ACCESS_VIEW)
				continue;

			// Check the instruction is operating on the declared uav
			if (inst->asOperands[opIndex].ui32RegisterNumber != uavReg)
				continue;

			decl->sUAV.ui32AccessFlags |= accessFlags;

			// get the max components accessed, but only for typed (texture) UAVs
			if (numComponents > decl->sUAV.ui32NumComponents && decl->eOpcode == OPCODE_DCL_UNORDERED_ACCESS_VIEW_TYPED)
			{
				decl->sUAV.ui32NumComponents = numComponents;
			}
		}
	}
}
...
static void CoalesceTemps(Shader& psShader, ShaderPhase *psPhase, uint32_t ui32MaxOrigTemps)
{
	...
	for (i = 0; i < psPhase->ui32TotalTemps; i++)
	{
		...
		else
		{
			...
			count = psPhase->pui32SplitInfo[i].componentCount;
			rebase = psPhase->pui32SplitInfo[i].rebase;
			origReg = 0x10000 + i;
			newReg = psPhase->pui32SplitInfo[i].originalRegister;
      while (psPhase->pui32SplitInfo[newReg] != SplitInfo{})
				newReg = psPhase->pui32SplitInfo[newReg].originalRegister;
...
void Shader::PruneTempRegisters()
{
  ...
  for (auto& phase : asPhases)
  {
    CoalesceTemps(*this, &phase, maxOrigTemps);
      if (phase.psTempDeclaration != phase.psDecl.end())
        phase.psTempDeclaration->value.ui32NumTemps = maxOrigTemps * 4;
  }
...
{
	uint32_t i;
  RegisterSpace regSpace = psOperand->GetRegisterSpace(eShaderType, psPhase->ePhase);
	...
	case OPERAND_TYPE_INPUT_PATCH_CONSTANT:
		redirectTable = regSpace == RegisterSpace::per_vertex ? &psPhase->acInputNeedsRedirect[0] : &psPhase->acPatchConstantsNeedsRedirect[0];
		break;

	case OPERAND_TYPE_OUTPUT:
	case OPERAND_TYPE_OUTPUT_CONTROL_POINT:
		redirectTable = regSpace == RegisterSpace::per_vertex ? &psPhase->acOutputNeedsRedirect[0] : &psPhase->acPatchConstantsNeedsRedirect[0];
		break;
...
void Shader::ExpandSWAPCs()
{
	...
	for (size_t i = 0; i < asPhases.size(); i++)
	{
