...
dtStatus dtNavMesh::init(const dtNavMeshParams* params, bool init_free_list)
{
	...
	m_allowLoneTiles = init_free_list;
	...
	m_nextFree = 0;
	if (!init_free_list)//free list must be set later
	{
		for (int i = 0; i < m_maxTiles; ++i)
			m_tiles[i].salt = 1;
	}
...
/*
// moved to #include "detourNavMeshDagor.cpp" // dagor
							...
							dtTileRef lastRef, dtTileRef* result)
{
	...
	// Allocate a tile.
	dtAssert(lastRef || m_allowLoneTiles);
	dtMeshTile* tile = 0;
	if (!lastRef && m_allowLoneTiles)
	{
		if (m_nextFree)
		{
			tile = m_nextFree;
	...
	else if (!m_allowLoneTiles)
	{
		// Try to relocate the tile to specific index with same salt.
		int tileIndex = (int)decodePolyIdTile((dtPolyRef)lastRef);
		if (tileIndex >= m_maxTiles)
			return DT_FAILURE | DT_OUT_OF_MEMORY;
		// Assign tile from list a of tiles
...
static inline bool is_tile_inited(const dtMeshTile &tile)
{
	return tile.polys != nullptr && tile.verts != nullptr;
}

/// Reconstructs free list in m_nextFree after all addTile work is done
dtStatus dtNavMesh::reconstructFreeList()
{
	m_nextFree = 0;
	for (int i = m_maxTiles-1; i >= 0; --i)
	{
		if (is_tile_inited(m_tiles[i]))
			continue;
		m_tiles[i].next = m_nextFree;
		m_nextFree = &m_tiles[i];
	}
	m_allowLoneTiles = true;
	return DT_SUCCESS;
}
