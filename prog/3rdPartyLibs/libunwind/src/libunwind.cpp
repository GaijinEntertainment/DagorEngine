...
_LIBUNWIND_HIDDEN void __unw_set_stack_boundaries(unw_cursor_t *cursor, unw_word_t bottom, unw_word_t top) {
  _LIBUNWIND_TRACE_API("__unw_set_stack_boundaries(cursor=%p)", static_cast<void *>(cursor));
  AbstractUnwindCursor *co = (AbstractUnwindCursor *)cursor;
  co->setStackBoundaries(bottom, top);
}
_LIBUNWIND_WEAK_ALIAS(__unw_set_stack_boundaries, unw_set_stack_boundaries)
