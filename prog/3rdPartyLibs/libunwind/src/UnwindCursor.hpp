...
public:
  ...
  virtual void setStackBoundaries(unw_word_t, unw_word_t) {
    _LIBUNWIND_ABORT("setStackBoundaries not implemented");
  }
...
UnwindCursor<A, R>::UnwindCursor(unw_context_t *context, A &as)
    : _addressSpace(as), _unwindInfoMissing(false), _stackBottom(0), _stackTop(0) {
...
public:
  ...
  virtual void        setInfoBasedOnIPRegister(bool isReturnAddress = false);
  virtual const char *getRegisterName(int num);
  virtual void setStackBoundaries(unw_word_t bottom, unw_word_t top)
  {
    _stackBottom = bottom;
    _stackTop    = top;
  }
...
private:
  ...
  int stepWithDwarfFDE() {
    return DwarfInstructions<A, R>::stepWithDwarf(_addressSpace,
                                              ...
                                              (pint_t)_stackBottom, (pint_t)_stackTop,
...
UnwindCursor<A, R>::UnwindCursor(unw_context_t *context, A &as)
    : _addressSpace(as), _registers(context), _unwindInfoMissing(false),
      _isSignalFrame(false), _stackBottom(0), _stackTop(0) {
  static_assert((check_fit<UnwindCursor<A, R>, unw_cursor_t>::does_fit),
                "UnwindCursor<> does not fit in unw_cursor_t");
  static_assert((alignof(UnwindCursor<A, R>) <= alignof(unw_cursor_t)),
                "UnwindCursor<> requires more alignment than unw_cursor_t");
...
UnwindCursor<A, R>::UnwindCursor(A &as, void *)
    : _addressSpace(as), _unwindInfoMissing(false), _isSignalFrame(false), _stackBottom(0), _stackTop(0) {
  memset(&_info, 0, sizeof(_info));
  // FIXME
