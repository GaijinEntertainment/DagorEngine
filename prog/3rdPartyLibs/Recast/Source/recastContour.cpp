						   ...
						   bool& isBorderVertex)
{
	...
	for (int j = 0; j < 4; ++j)
	{
		...
		const bool twoSameInts = ((regs[c] | regs[d]) & RC_BORDER_REG) == 0 && regs[c] == regs[d];
		const bool intsSameArea = (regs[c]>>16) == (regs[d]>>16);
		const bool noZeros = regs[a] != 0 && regs[b] != 0 && regs[c] != 0 && regs[d] != 0;
		if (twoSameExts && twoSameInts && intsSameArea && noZeros)
		{
			isBorderVertex = true;
			break;
		}
...
static int __cdecl compareHoles(const void* va, const void* vb)
{
...
static int __cdecl compareDiagDist(const void* va, const void* vb)
{
