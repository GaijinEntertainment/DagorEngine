...
// DAGOR only
int rcGetHeightFieldSpanCountEx(rcContext* ctx, rcHeightfield& hf, bool with_null_areas)
{
	rcIgnoreUnused(ctx);
							   ...
							   rcHeightfield& hf, rcCompactHeightfield& chf)
{
	...
	const int w = hf.width;
	const int h = hf.height;
	const int spanCount = rcGetHeightFieldSpanCountEx(ctx, hf, walkableClimb < 0); // DAGOR
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			while (s)
			{
				if (s->area != RC_NULL_AREA || walkableClimb < 0) // DAGOR
				{
	...
	const int walkClimb = (walkableClimb < 0) ? MAX_HEIGHT : walkableClimb; // DAGOR
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				for (int dir = 0; dir < 4; ++dir)
				{
					...
					for (int k = (int)nc.index, nk = (int)(nc.index+nc.count); k < nk; ++k)
					{
						...
						if ((top - bot) >= walkableHeight && rcAbs((int)ns.y - (int)s.y) <= walkClimb)
						{
