...
dtQueryFilter::dtQueryFilter() :
	m_heuristicScale(0.999f),
	m_reopenTotalThreshold(0.0f),
...
// dagor: H_SCALE moved to dtQueryFilter::m_heuristicScale (default 0.999f)
...
dtStatus dtNavMeshQuery::findRandomPointsAroundCircle(dtPolyRef startRef, const float* centerPos, const float maxRadius,
										 const dtQueryFilter* filter, int (*rand)(), float (*frand)(),
										 dtPolyRef* randomRef, float* randomPt, int *resultCount, const int maxResult) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
	...
	*resultCount = 0;
	...
	if (!m_nav->isValidPolyRef(startRef) ||
		!centerPos || !dtVisfinite(centerPos) ||
		maxRadius < 0 || !dtMathIsfinite(maxRadius) ||
		!filter || !rand || !randomRef || !randomPt ||
    maxResult < 0)
	{
		return DT_FAILURE | DT_INVALID_PARAM;
	}
	...
	while (!m_openList->empty())
	{
		...
		if (bestPoly->getType() == DT_POLYTYPE_GROUND)
		{
			// Choose random polygon, using reservoir sampling.
      if (idx >= maxResult)
      {
        const int u = rand() % (idx + 1);
        if (u < maxResult)
          randomRef[u] = bestRef;
      }
      else
      {
        randomRef[idx] = bestRef;
        *resultCount = idx + 1;
      }
			idx++;
		}
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			const float total = bestNode->total + 1.f; //dtVdist(bestNode->pos, neighbourNode->pos);
  ...
  for (int i = 0; i < *resultCount; ++i)
  {
    // Randomly pick point on polygon.
		const dtMeshTile* tile = 0;
		const dtPoly* poly = 0;
		m_nav->getTileAndPolyByRefUnsafe(randomRef[i], &tile, &poly);
    const float* v = &tile->verts[poly->verts[0]*3];
    float verts[3*DT_VERTS_PER_POLYGON];
    float areas[DT_VERTS_PER_POLYGON];
    dtVcopy(&verts[0*3], v);
    ...
    float h = 0.0f;
    dtStatus stat = getPolyHeight(randomRef[i], pt, &h);
    if (dtStatusFailed(stat))
      return stat;
    pt[1] = h;
    
    dtVcopy(randomPt + 3 * i, pt);
  }
...
dtStatus dtNavMeshQuery::findPath(dtPolyRef startRef, dtPolyRef endRef,
								  const float* startPos, const float* endPos,
								  const dtQueryFilter* filter,
								  dtPolyRef* path, int* pathCount, const int maxPath, const float maxWeight) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
	...
	*pathCount = 0;
	...
	startNode->pidx = 0;
	startNode->cost = 0;
	const float hScale = filter->getHeuristicScale();
	startNode->total = dtVdist(startPos, endPos) * hScale;
	startNode->id = startRef;
	startNode->flags = DT_NODE_OPEN;
	m_openList->push(startNode);
	...
	const float filterReopenThreshold = filter->getReopenTotalThreshold();
	const float reopenTotalThreshold = hScale > 1.0f && filterReopenThreshold > 0.f ? filterReopenThreshold : 0.f;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			if (neighbourNode->flags == 0)
			{
				if (dtStatusFailed(getEdgeMidPoint(bestRef, bestPoly, bestTile,
			...
			else
			{
				...
				heuristic = dtVdist(neighbourNode->pos, endPos)*hScale;
			}

			const float total = cost + heuristic;
			
			// The node is already in open list and the new result is worse, skip.
			// Or the node is already visited and processed, and the new result is worse, skip.
			// For CLOSED nodes with inflated heuristic, require larger improvement.
			if (neighbourNode->flags & (DT_NODE_OPEN | DT_NODE_CLOSED))
			{
				if (neighbourNode->flags & DT_NODE_OPEN)
				{
					if (total >= nexttowardf(neighbourNode->total, 0.0f))
						continue;
				}
				else
				{
					if (total + reopenTotalThreshold >= nexttowardf(neighbourNode->total, 0.0f))
						continue;
				}
			}
	...
	dtStatus status = getPathToNode(lastBestNode, path, pathCount, maxPath, maxWeight);
...
/*
// moved to "detourNavMeshQueryDagor.cpp" // dagor
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int* doneIters)
{
	...
	const float hScale = m_query.filter->getHeuristicScale();
	...
	while (iter < maxIter && !m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			{
				heuristic = dtVdist(neighbourNode->pos, m_query.endPos)*hScale;
			}
	...
	*visitedCount = 0;
	...
	while (nstack)
	{
		...
		for (int i = 0, j = (int)curPoly->vertCount-1; i < (int)curPoly->vertCount; j = i++)
		{
			...
			if (curPoly->neis[j] & DT_EXT_LINK || curPoly->vertCount == 2) // DAGOR: handle offmesh connections
			{
				// Tile border.
								 ...
								 dtRaycastHit* hit, dtPolyRef prevRef) const
{
	...
	while (curRef)
	{
		...
		if (n < hit->maxPath)
			...
			return DT_FAILURE | DT_BUFFER_TOO_SMALL; // DAGOR
			//status |= DT_BUFFER_TOO_SMALL;
...
dtStatus dtNavMeshQuery::getPathFromDijkstraSearch(dtPolyRef endRef, dtPolyRef* path, int* pathCount, int maxPath, float maxWeight) const
{
	...
	*pathCount = 0;
	...
	return getPathToNode(endNode, path, pathCount, maxPath, maxWeight);
}
