                                                         ...
                                                         unordered_set<uint32_t> &processed_func_ids)
{
	...
	for (auto block : func.blocks)
	{
        ...
        auto &execution = get_entry_point();
        if (execution.output_primitives == 0)
          execution.output_primitives = std::stoi(to_unpacked_expression(ops[1]));
				break;
			}
...
void CompilerMSL::emit_resources()
{
    ...
    else if(builtin_mesh_primitive_indices_id != 0)
    {
      BuiltIn builtin = ir.meta[builtin_mesh_primitive_indices_id].decoration.builtin_type;
      if (builtin == BuiltInPrimitivePointIndicesEXT)
        topology = "topology::point";
        ...
        SPIRV_CROSS_THROW("can't determine mesh shader mesh primitive type");
    }
...
void CompilerMSL::emit_mesh_outputs()
{
	...
	if (mesh_out_per_primitive != 0 || builtin_mesh_primitive_indices_id != 0)
	{
		...
		if (builtin_mesh_primitive_indices_id != 0)
		{
			...
			else if (mode.flags.get(ExecutionModeOutputPoints))
			{
      ...
      else
      {
        ...
        else if (builtin == BuiltInPrimitiveTriangleIndicesEXT)
        {
          ...
          SPIRV_CROSS_THROW("can't determine mesh shader primitive type");
      }
		}
