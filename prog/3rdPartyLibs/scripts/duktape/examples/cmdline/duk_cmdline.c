...
static int handle_file(duk_context *ctx, const char *filename, const char *bytecode_filename) {
  ...
  if (filename)
  {
    ...
    if (filename[len - 3] != '.' || toupper(filename[len - 2]) != 'J' || toupper(filename[len - 1]) != 'S')
      return 0; // just ignore
  }
...
static duk_ret_t fileio_system(duk_context *ctx) {
  const char *s;

  s = duk_require_string(ctx, 0);
  system(s);
...
static duk_ret_t fileio_cmd_args(duk_context *ctx)
{
...
static duk_context *create_duktape_heap(int alloc_provider, int debugger, int lowmem_log) {
  ...
  duk_push_c_function(ctx, fileio_system, 1 /*nargs*/);
  duk_put_global_string(ctx, "systemExec");
  duk_push_c_function(ctx, fileio_cmd_args, 0 /*nargs*/);
  duk_put_global_string(ctx, "getCommandLineArgs");
#endif
