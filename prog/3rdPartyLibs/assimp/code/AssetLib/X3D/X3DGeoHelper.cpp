...
aiVector3D X3DGeoHelper::make_point2D(float angle, float radius) {
    return aiVector3D(radius * cos(angle), radius * sin(angle), 0);
}

void X3DGeoHelper::make_arc2D(float pStartAngle, float pEndAngle, float pRadius, size_t numSegments, eastl::list<aiVector3D> &pVertices) {
...
void X3DGeoHelper::extend_point_to_line(const eastl::list<aiVector3D> &pPoint, eastl::list<aiVector3D> &pLine) {
    eastl::list<aiVector3D>::const_iterator pit = pPoint.begin();
    eastl::list<aiVector3D>::const_iterator pit_last = pPoint.end();
...
void X3DGeoHelper::polylineIdx_to_lineIdx(const eastl::list<int32_t> &pPolylineCoordIdx, eastl::list<int32_t> &pLineCoordIdx) {
    eastl::list<int32_t>::const_iterator plit = pPolylineCoordIdx.begin();
    ...
    while (plit != pPolylineCoordIdx.end()) {
        ...
        while ((*plit != (-1)) && (plit != pPolylineCoordIdx.end())) {
            eastl::list<int32_t>::const_iterator plit_next;
...
void X3DGeoHelper::rect_parallel_epiped(const aiVector3D &pSize, eastl::list<aiVector3D> &pVertices) {
...
void X3DGeoHelper::coordIdx_str2faces_arr(const eastl::vector<int32_t> &pCoordIdx, eastl::vector<aiFace> &pFaces, unsigned int &pPrimitiveTypes) {
    eastl::vector<int32_t> f_data(pCoordIdx);
    eastl::vector<unsigned int> inds;
    ...
    for (eastl::vector<int32_t>::iterator it = f_data.begin(); it != f_data.end(); ++it) {
    ...
    } // for(eastl::list<int32_t>::iterator it = f_data.begin(); it != f_data.end(); it++)
...
void X3DGeoHelper::add_color(aiMesh &pMesh, const eastl::list<aiColor3D> &pColors, const bool pColorPerVertex) {
    eastl::list<aiColor4D> tcol;

    // create RGBA array from RGB.
    for (eastl::list<aiColor3D>::const_iterator it = pColors.begin(); it != pColors.end(); ++it)
...
void X3DGeoHelper::add_color(aiMesh &pMesh, const eastl::list<aiColor4D> &pColors, const bool pColorPerVertex) {
    eastl::list<aiColor4D>::const_iterator col_it = pColors.begin();
...
void X3DGeoHelper::add_color(aiMesh &pMesh, const eastl::vector<int32_t> &pCoordIdx, const eastl::vector<int32_t> &pColorIdx,
...
void X3DGeoHelper::add_color(aiMesh &pMesh, const eastl::vector<int32_t> &coordIdx, const eastl::vector<int32_t> &colorIdx,
        const eastl::list<aiColor4D> &colors, bool pColorPerVertex) {
    eastl::vector<aiColor4D> col_tgt_arr;
    eastl::list<aiColor4D> col_tgt_list;
    eastl::vector<aiColor4D> col_arr_copy;
    ...
    for (eastl::list<aiColor4D>::const_iterator it = colors.begin(); it != colors.end(); ++it) {
    ...
    if (pColorPerVertex) {
        if (colorIdx.size() > 0) {
            ...
            for (eastl::vector<int32_t>::const_iterator colidx_it = colorIdx.begin(), coordidx_it = coordIdx.begin(); colidx_it != colorIdx.end(); ++colidx_it, ++coordidx_it) {
                if (*colidx_it == (-1)) {
        ...
        if (colorIdx.size() > 0) {
            ...
            eastl::vector<int32_t>::const_iterator colidx_it = colorIdx.begin();
            for (size_t fi = 0; fi < pMesh.mNumFaces; fi++) {
    ...
    for (eastl::vector<aiColor4D>::const_iterator it = col_tgt_arr.begin(); it != col_tgt_arr.end(); ++it)
        col_tgt_list.push_back(*it);
    // add prepared colors list to mesh.
    add_color(pMesh, col_tgt_list, pColorPerVertex);
}

void X3DGeoHelper::add_normal(aiMesh &pMesh, const eastl::vector<int32_t> &pCoordIdx, const eastl::vector<int32_t> &pNormalIdx,
        const eastl::list<aiVector3D> &pNormals, const bool pNormalPerVertex) {
    eastl::vector<size_t> tind;
    eastl::vector<aiVector3D> norm_arr_copy;

    // copy list to array because we are need indexed access to normals.
    norm_arr_copy.reserve(pNormals.size());
    for (eastl::list<aiVector3D>::const_iterator it = pNormals.begin(); it != pNormals.end(); ++it) {
    ...
    if (pNormalPerVertex) {
        if (pNormalIdx.size() > 0) {
            ...
            for (eastl::vector<int32_t>::const_iterator it = pNormalIdx.begin(); it != pNormalIdx.end(); ++it) {
            ...
            // copy normals to mesh
            pMesh.mNormals = new aiVector3D[pMesh.mNumVertices];
            eastl::list<aiVector3D>::const_iterator norm_it = pNormals.begin();
            for (size_t i = 0; i < pMesh.mNumVertices; i++)
        ...
        if (pNormalIdx.size() > 0) {
            ...
            eastl::vector<int32_t>::const_iterator normidx_it = pNormalIdx.begin();
...
void X3DGeoHelper::add_normal(aiMesh &pMesh, const eastl::list<aiVector3D> &pNormals, const bool pNormalPerVertex) {
    eastl::list<aiVector3D>::const_iterator norm_it = pNormals.begin();
...
void X3DGeoHelper::add_tex_coord(aiMesh &pMesh, const eastl::vector<int32_t> &pCoordIdx, const eastl::vector<int32_t> &pTexCoordIdx,
        const eastl::list<aiVector2D> &pTexCoords) {
    eastl::vector<aiVector3D> texcoord_arr_copy;
    eastl::vector<aiFace> faces;
    unsigned int prim_type;

    // copy list to array because we are need indexed access to normals.
    texcoord_arr_copy.reserve(pTexCoords.size());
    for (eastl::list<aiVector2D>::const_iterator it = pTexCoords.begin(); it != pTexCoords.end(); ++it) {
...
void X3DGeoHelper::add_tex_coord(aiMesh &pMesh, const eastl::list<aiVector2D> &pTexCoords) {
    eastl::vector<aiVector3D> tc_arr_copy;
...
aiMesh *X3DGeoHelper::make_mesh(const eastl::vector<int32_t> &pCoordIdx, const eastl::list<aiVector3D> &pVertices) {
    eastl::vector<aiFace> faces;
    unsigned int prim_type = 0;
    ...
    // vertices
    eastl::list<aiVector3D>::const_iterator vit = pVertices.begin();
