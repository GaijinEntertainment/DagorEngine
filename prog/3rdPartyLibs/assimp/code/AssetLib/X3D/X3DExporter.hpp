...
class X3DExporter {
    ...
    struct SAttribute {
        ...
        SAttribute(const eastl::string &name, const eastl::string &value) :
                Name(name),
                Value(value) {
            // empty
        }
    ...
    IOStream *mOutFile;
    eastl::map<size_t, eastl::string> mDEF_Map_Mesh;
    eastl::map<size_t, eastl::string> mDEF_Map_Material;

private:
    eastl::string mIndentationString;
    ...
    /// \fn void XML_Write(const eastl::string& pData)
    /// Write data to XML-file.
    /// \param [in] pData - reference to string which must be written.
    void XML_Write(const eastl::string &pData);
    ...
    /// \fn void AttrHelper_CommaToPoint(eastl::string& pStringWithComma)
    /// Convert commas in string to points. That's needed because "eastl::to_string" result depends on locale (regional settings).
    /// \param [in, out] pStringWithComma - reference to string, which must be modified.
    void AttrHelper_CommaToPoint(eastl::string &pStringWithComma) {
        for (char &c : pStringWithComma) {
            if (c == ',') c = '.';
        }
    }

    /// \fn void AttrHelper_FloatToString(const float pValue, eastl::string& pTargetString)
    ...
    void AttrHelper_FloatToString(const float pValue, eastl::string &pTargetString);

    /// \fn void AttrHelper_Vec3DArrToString(const aiVector3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    ...
    /// \param [out] pTargetString - reference to string where result will be placed. Will be cleared before using.
    void AttrHelper_Vec3DArrToString(const aiVector3D *pArray, const size_t pArray_Size, eastl::string &pTargetString);

    /// \fn void AttrHelper_Vec2DArrToString(const aiVector2D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// \overload void AttrHelper_Vec3DArrToString(const aiVector3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    void AttrHelper_Vec2DArrToString(const aiVector2D *pArray, const size_t pArray_Size, eastl::string &pTargetString);

    /// \fn void AttrHelper_Vec3DAsVec2fArrToString(const aiVector3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// \overload void AttrHelper_Vec3DArrToString(const aiVector3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// Only x, y is used from aiVector3D.
    void AttrHelper_Vec3DAsVec2fArrToString(const aiVector3D *pArray, const size_t pArray_Size, eastl::string &pTargetString);

    /// \fn void AttrHelper_Col4DArrToString(const aiColor4D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// \overload void AttrHelper_Vec3DArrToString(const aiVector3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// Converts array of colors to string.
    void AttrHelper_Col4DArrToString(const aiColor4D *pArray, const size_t pArray_Size, eastl::string &pTargetString);

    /// \fn void AttrHelper_Col3DArrToString(const aiColor3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// \overload void AttrHelper_Col4DArrToString(const aiColor4D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    /// Converts array of colors to string.
    void AttrHelper_Col3DArrToString(const aiColor3D *pArray, const size_t pArray_Size, eastl::string &pTargetString);

    /// \fn void AttrHelper_FloatToAttrList(eastl::list<SAttribute> pList, const eastl::string& pName, const float pValue, const float pDefaultValue)
    /// \overload void AttrHelper_Col3DArrToString(const aiColor3D* pArray, const size_t pArray_Size, eastl::string& pTargetString)
    void AttrHelper_FloatToAttrList(eastl::list<SAttribute> &pList, const eastl::string &pName, const float pValue, const float pDefaultValue);

    /// \fn void AttrHelper_Color3ToAttrList(eastl::list<SAttribute> pList, const eastl::string& pName, const aiColor3D& pValue, const aiColor3D& pDefaultValue)
    ...
    void AttrHelper_Color3ToAttrList(eastl::list<SAttribute> &pList, const eastl::string &pName, const aiColor3D &pValue, const aiColor3D &pDefaultValue);

    /// \fn void NodeHelper_OpenNode(const eastl::string& pNodeName, const size_t pTabLevel, const bool pEmptyElement, const eastl::list<SAttribute>& pAttrList)
    ...
    void NodeHelper_OpenNode(const eastl::string &pNodeName, const size_t pTabLevel, const bool pEmptyElement, const eastl::list<SAttribute> &pAttrList);

    /// \fn void NodeHelper_OpenNode(const eastl::string& pNodeName, const size_t pTabLevel, const bool pEmptyElement = false)
    /// \overload void NodeHelper_OpenNode(const eastl::string& pNodeName, const size_t pTabLevel, const bool pEmptyElement, const eastl::list<SAttribute>& pAttrList)
    void NodeHelper_OpenNode(const eastl::string &pNodeName, const size_t pTabLevel, const bool pEmptyElement = false);

    /// \fn void NodeHelper_CloseNode(const eastl::string& pNodeName, const size_t pTabLevel)
    /// End XML-node element.
    /// \param [in] pNodeName - name of the element.
    /// \param [in] pTabLevel - indentation level.
    void NodeHelper_CloseNode(const eastl::string &pNodeName, const size_t pTabLevel);
    ...
    /// \fn void LogError(const eastl::string& pMessage)
    /// Short variant for calling \ref DefaultLogger::get()->error()
    void LogError(const eastl::string &pMessage) { DefaultLogger::get()->error(pMessage); }
