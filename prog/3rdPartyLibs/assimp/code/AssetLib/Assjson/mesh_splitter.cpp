...
void MeshSplitter::Execute( aiScene* pScene) {
	eastl::vector<eastl::pair<aiMesh*, unsigned int> > source_mesh_map;
...
// ------------------------------------------------------------------------------------------------
void MeshSplitter::UpdateNode(aiNode* pcNode, const eastl::vector<eastl::pair<aiMesh*, unsigned int> >& source_mesh_map) {
	// TODO: should better use eastl::(multi)set for source_mesh_map.

	// for every index in out list build a new entry
	eastl::vector<unsigned int> aiEntries;
...
// ------------------------------------------------------------------------------------------------
void MeshSplitter :: SplitMesh(unsigned int a, aiMesh* in_mesh, eastl::vector<eastl::pair<aiMesh*, unsigned int> >& source_mesh_map) {
	// TODO: should better use eastl::(multi)set for source_mesh_map.
	...
	// create a eastl::vector<unsigned int> to remember which vertices have already
	// been copied and to which position (i.e. output index)
	eastl::vector<unsigned int> was_copied_to;
	...
	while (true) {
		...
		typedef eastl::vector<aiVertexWeight> BoneWeightList;
		if (in_mesh->HasBones())	{
		...
		if (base)	{
			eastl::fill(was_copied_to.begin(), was_copied_to.end(), WAS_NOT_COPIED);
		}

		eastl::vector<aiFace> vFaces;
