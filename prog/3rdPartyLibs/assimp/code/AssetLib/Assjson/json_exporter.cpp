...
public:
    ...
    void Key(const eastl::string &name) {
        AddIndentation();
        Delimit();
        buff << '\"' + name + "\":" << space;
    }
    ...
    void SimpleValue(const void *buffer, size_t len) {
        ...
        char *const cur_out = new char[eastl::max(len * 2, static_cast<size_t>(16u))];
...
private:
    ...
    std::stringstream &LiteralToString(std::stringstream &stream, float f) {
        if (!eastl::numeric_limits<float>::is_iec559) {
        ...
        if (eastl::numeric_limits<float>::infinity() == fabs(f)) {
            if (flags & Flag_WriteSpecialFloats) {
                stream << (f < 0 ? "\"-" : "\"") + eastl::string("Infinity\"");
                return stream;
            }
            //  we should print this warning, but we can't - this is called from within a generic assimp exporter, we cannot use cerr
            //	std::cerr << "warning: cannot represent infinite number literal, substituting 0 instead (use -i flag to enforce Infinity/NaN)" << eastl::endl;
            stream << "0.0";
            return stream;
        }
...
void Write(JSONWriter &out, const aiMaterial &ai, bool is_elem = true) {
    ...
    for (unsigned int i = 0; i < ai.mNumProperties; ++i) {
        ...
        case aiPTI_Buffer: {
            // binary data is written as series of std::hex-encoded octets
...
void ExportAssimp2Json(const char *file, Assimp::IOSystem *io, const aiScene *scene, const Assimp::ExportProperties *pProperties) {
    eastl::unique_ptr<Assimp::IOStream> str(io->Open(file, "wt"));
    if (!str) {
