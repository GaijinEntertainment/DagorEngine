...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event)
{
    ...
    else
    {
       ...
       if (peer -> state == ENET_PEER_STATE_DISCONNECTED ||
           ...
           /*((host -> receivedAddress.host != peer -> address.host ||
             ...
             peer -> address.host != ENET_HOST_BROADCAST) ||*/ // dagor - allow peers to change addresses
           (peer -> outgoingPeerID < ENET_PROTOCOL_MAXIMUM_PEER_ID &&
            sessionID != peer -> incomingSessionID))
         return 0;
    }
 
    if ((flags & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED) || (peer != NULL && peer -> stickyCompressionFlag == 1))
    {
        if (peer != NULL && peer -> useStickyCompressionFlag == 1)
          peer -> stickyCompressionFlag |= ((flags & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED) != 0);

        size_t originalSize;
        if (host -> compressor.context == NULL || host -> compressor.decompress == NULL ||
            (flags & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED) == 0)
        {
          enet_logf("peer #%d (" HOSTPORT_FMT "|" HOSTPORT_FMT "): packet.compressed = %d != compressor.{ctx,decompress} = %d,%d",
        ...
        originalSize = host -> compressor.decompress (host -> compressor.context, peerID,
    ...
    if (peer != NULL)
    {
       if (peer -> state != ENET_PEER_STATE_DISCONNECTED &&
         ...
         enet_logf("peer #%d changed %s " HOSTPORT_FMT " -> " HOSTPORT_FMT,  (int)peerID,
                   (peer -> address.host != host -> receivedAddress.host) ? "address" : "port",
                   HOSTPORT(peer -> address.host, peer -> address.port),
                   HOSTPORT(host -> receivedAddress.host, host -> receivedAddress.port));
       peer -> address.host = host -> receivedAddress.host;
       peer -> address.port = host -> receivedAddress.port;
...
enet_protocol_send_acknowledgements (ENetHost * host, ENetPeer * peer)
{
    ...
    while (currentAcknowledgement != enet_list_end (& peer -> acknowledgements))
    {
       ...
       headerCommand = acknowledgement -> command.header.command & ENET_PROTOCOL_COMMAND_MASK;
       if (headerCommand == ENET_PROTOCOL_COMMAND_DISCONNECT)
         enet_protocol_dispatch_state (host, peer, ENET_PEER_STATE_ZOMBIE);

       //HACK to send ACKs twice
       //first we set the acknowledgement header command to none and only second remove it
       if (headerCommand != ENET_PROTOCOL_COMMAND_NONE)
         acknowledgement -> command.header.command = ENET_PROTOCOL_COMMAND_NONE;
       else
       {
...
enet_protocol_check_timeouts (ENetHost * host, ENetPeer * peer, ENetEvent * event)
{
    ...
    while (currentCommand != enet_list_end (& peer -> sentReliableCommands))
    {
       ...
       {
          enet_logf("disconnect peer #%d by MAX timeout %d ms, rtt %d+-%d ms, rtt timeout %d ms",
                    (int)(peer - host -> peers),
       ...
       {
          enet_logf("disconnect peer #%d by MIN timeout %d ms, rtt %d+-%d ms, rtt timeout %d ms",
                    (int)(peer - host -> peers),
                    ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout),
                    peer -> roundTripTime, peer -> roundTripTimeVariance,
                    outgoingCommand -> roundTripTimeout);
       ...
       outgoingCommand -> roundTripTimeout =
         ENET_MIN(outgoingCommand -> roundTripTimeout * 2, ENET_PEER_MAX_RTT_TIMEOUT);
...
enet_protocol_send_reliable_outgoing_commands (ENetHost * host, ENetPeer * peer)
{
    ...
    while (currentCommand != enet_list_end (& peer -> outgoingReliableCommands))
    {
       ...
       if (outgoingCommand -> roundTripTimeout == 0)
       {
          ...
          outgoingCommand -> roundTripTimeout = ENET_MAX (outgoingCommand -> roundTripTimeout, ENET_MIN_ROUND_TRIP_TIMEOUT);
...
enet_protocol_send_outgoing_commands (ENetHost * host, ENetEvent * event, int checkForTimeouts)
{
         ...
         ++ currentPeer)
    {
        ...
        if (ENET_TIME_DIFFERENCE (host -> serviceTime, currentPeer -> packetLossEpoch) >= ENET_PEER_PACKET_LOSS_INTERVAL &&
            currentPeer -> packetsDelivered + currentPeer -> packetsLost > 0)
        {
           int i = 0;
           enet_uint32 totalPacketsLost = 0;
           enet_uint32 totalPacketsDelivered = 0;
           enet_uint32 packetLoss = 0;

           currentPeer -> packetsLostHistory[currentPeer -> packetsHistoryWritePosition] = currentPeer -> packetsLost;
           currentPeer -> packetsDeliveredHistory[currentPeer -> packetsHistoryWritePosition] = currentPeer -> packetsDelivered;

           currentPeer -> packetsHistoryWritePosition++;
           if (currentPeer -> packetsHistoryWritePosition >= ENET_PEER_HISTORY_LENGTH)
              currentPeer -> packetsHistoryWritePosition = 0;

           for (i = 0; i < ENET_PEER_HISTORY_LENGTH; ++i)
           {
              totalPacketsLost += currentPeer -> packetsLostHistory[i];
              totalPacketsDelivered += currentPeer -> packetsDeliveredHistory[i];
           }
           
           packetLoss = totalPacketsLost * ENET_PEER_PACKET_LOSS_SCALE / (totalPacketsDelivered + totalPacketsLost);
           if (packetLoss > ENET_PEER_PACKET_LOSS_SCALE)
              packetLoss = ENET_PEER_PACKET_LOSS_SCALE;
           ...
           currentPeer -> packetsDelivered = 0;
