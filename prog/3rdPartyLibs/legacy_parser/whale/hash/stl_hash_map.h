...
template<class T>
struct __stl_select1st : public std::unary_function< T, typename T::first_type >
{
	T::first_type operator()(const T &p) const { return p.first; }
};
...
template <class Key, class T, class HashFcn = hash<Key>,
          ...
          class Alloc = allocator<__hashtable_node<pair<const Key, T>  >  >  >
#else
...
private:
  typedef hashtable<pair<const Key, T>, Key, HashFcn,
                    __stl_select1st<pair<const Key, T> >, EqualKey, Alloc> ht;
  ht rep;
...
public:
  ...
  friend bool
  operator== <> (const hash_multimap&, const hash_multimap&);
