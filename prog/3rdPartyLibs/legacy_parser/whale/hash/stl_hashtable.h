...
template <class Value, class Key, class HashFcn,
          ...
          class Alloc = allocator<__hashtable_node<Value> > >
class hashtable;
...
private:
  ...
  {
    node* n = (node *)allocator.allocate(sizeof(node));
    n->next = 0;
    try {
      ...
      return n;
    	...
    	n->val.~Value();
        allocator.deallocate(n, sizeof(node));
		throw;
    }
  }
...
template <class V, class K, class HF, class ExK, class EqK, class All>
inline typename hashtable<V, K, HF, ExK, EqK, All>::difference_type*
