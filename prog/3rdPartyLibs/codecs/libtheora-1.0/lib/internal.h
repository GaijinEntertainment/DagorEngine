 ...
 ********************************************************************
  ...
  function:
      internal.h 15469 2008-10-30 12:49:42Z tterribe
...
// Workaround for compiler bug in decode.c, do it here to make sure all the code is fixed.
#if _MSC_VER >= 1914            // 15.7 compilers (VS2017 Update 7), 15.8.2
# pragma optimize("s", on)
#endif

/*
   There is a bug in the optimizer that leads to the use of an uninitialized value.
   Reproduced on 15.7.3 and 15.7.5 compilers. Similar bug is promised to be fixed in
   upcoming 15.8 compiler (didn't happen). Until then, change the optimization setting to "Optimize
   for Size" that seems to solve the problem. Because the root of the problem is
   unknown, apply the workaround locally (in internal.h).
   Below follows the reference to check the fix in the future.

   Source:

   int th_decode_packetin(th_dec_ctx *_dec,const ogg_packet *_op,
   ...
         pipe.fragy0[pli]=stripe_fragy>>frag_shift;                         <-----------Write to pipe.fragy0[pli]
   ...
   frag=_dec->state.frags+fplane->froffset+(fragy0*fplane->nhfrags);        <-----------Read from _pipe->fragy0[_pli]
   ...
   Disassembly:
         ...
         pipe.fragy0[pli]+(pipe.mcu_nvfrags>>frag_shift));
         oc_dec_dc_unpredict_mcu_plane(_dec,&pipe,pli);
 0000010458F6FB1F 45 8B 07             mov         r8d,dword ptr [r15]      <-----------Read _pipe->fragy0[_pli] before write!
         pipe.fragy0[pli]=stripe_fragy>>frag_shift;
 0000010458F6FB22 8B C2                mov         eax,edx
         pipe.fragy0[pli]+(pipe.mcu_nvfrags>>frag_shift));
         oc_dec_dc_unpredict_mcu_plane(_dec,&pipe,pli);
 0000010458F6FB24 8B 16                mov         edx,dword ptr [rsi]
 0000010458F6FB26 45 8B F0             mov         r14d,r8d
         pipe.fragy0[pli]=stripe_fragy>>frag_shift;
 0000010458F6FB29 D3 F8                sar         eax,cl                   <-----------Shift and
 0000010458F6FB2B 41 89 07             mov         dword ptr [r15],eax      <-----------write to pipe.fragy0[pli]
         pipe.fragy_end[pli]=OC_MINI(fplane->nvfrags,
 0000010458F6FB2E 44 8B A5 04 0D 00 00 mov         r12d,dword ptr [rbp+0D04h]
 0000010458F6FB35 41 D3 FC             sar         r12d,cl
 0000010458F6FB38 44 03 E0             add         r12d,eax
 0000010458F6FB3B 89 4C 24 40          mov         dword ptr [frag_shift],ecx
 0000010458F6FB3F 44 39 66 04          cmp         dword ptr [rsi+4],r12d
         pipe.fragy0[pli]+(pipe.mcu_nvfrags>>frag_shift));
         oc_dec_dc_unpredict_mcu_plane(_dec,&pipe,pli);
 0000010458F6FB43 8B C2                mov         eax,edx
 0000010458F6FB45 44 89 44 24 5C       mov         dword ptr [rsp+5Ch],r8d
         pipe.fragy_end[pli]=OC_MINI(fplane->nvfrags,
 0000010458F6FB4A 44 0F 4E 66 04       cmovle      r12d,dword ptr [rsi+4]
         pipe.fragy0[pli]+(pipe.mcu_nvfrags>>frag_shift));
         oc_dec_dc_unpredict_mcu_plane(_dec,&pipe,pli);
 0000010458F6FB4F 41 0F AF C0          imul        eax,r8d                  <-----------Use the invalid fragy0 value
*/
...
#if defined(USE_ASM)
  #define THEORA_VCALL(ctx,func) ctx.func
  #define THEORA_VSET(ctx,func,impl) ctx.func = impl
#else
  #define THEORA_VCALL(ctx,func) oc_##func##_c
  #define THEORA_VSET(ctx,func,impl)
#endif
...
#define oc_frag_recon_intra(_state,_dst,_dst_ystride,_residue) \
  THEORA_VCALL((_state)->opt_vtable,frag_recon_intra)(_dst,_dst_ystride,_residue)
#define oc_frag_recon_inter(_state,_dst,_dst_ystride,_src,_src_ystride,_residue) \
  THEORA_VCALL((_state)->opt_vtable,frag_recon_inter)(_dst,_dst_ystride,_src,_src_ystride,_residue)
#define oc_frag_recon_inter2(_state,_dst,_dst_ystride,_src1,_src1_ystride,_src2,_src2_ystride,_residue) \
  THEORA_VCALL((_state)->opt_vtable,frag_recon_inter2)(_dst,_dst_ystride,_src1,_src1_ystride,_src2,_src2_ystride,_residue)
#define oc_state_frag_copy(_state,_fragis,_nfragis,_dst_frame,_src_frame,_pli) \
  THEORA_VCALL((_state)->opt_vtable,state_frag_copy)(_state,_fragis,_nfragis,_dst_frame,_src_frame,_pli)
#define oc_state_frag_recon(_state,_frag,_pli,_dct_coeffs,_last_zzi,_ncoefs,_dc_iquant,_ac_iquant) \
  THEORA_VCALL((_state)->opt_vtable,state_frag_recon)(_state,_frag,_pli,_dct_coeffs,_last_zzi,_ncoefs,_dc_iquant,_ac_iquant)
#define oc_state_loop_filter_frag_rows(_state,_bv,_refi,_pli,_fragy0,_fragy_end) \
  THEORA_VCALL((_state)->opt_vtable,state_loop_filter_frag_rows)(_state,_bv,_refi,_pli,_fragy0,_fragy_end)
#define oc_restore_fpu(_state) \
  THEORA_VCALL((_state)->opt_vtable,restore_fpu)()
...
#else
static __inline void oc_restore_fpu_c(void){}
#endif
