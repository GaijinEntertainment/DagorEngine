 ...
 ********************************************************************
  ...
  function:
    encoder_toplevel.c 15383 2008-10-10 14:33:46Z xiphmont
...
int theora_encode_packetout( theora_state *t, int last_p, ogg_packet *op){
  CP_INSTANCE *cpi=(CP_INSTANCE *)(t->internal_encode);
  ogg_int32_t bytes=oggpackB_bytes(cpi->oggbuffer);
...
static void _tp_writebuffer(oggpack_buffer *opb, const char *buf, const ogg_int32_t len)
{
  ogg_int32_t i;
...
static void _tp_writelsbint(oggpack_buffer *opb, ogg_int32_t value)
{
