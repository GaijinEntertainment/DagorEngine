 ...
 ********************************************************************
  ...
  function:
      decode.c 15403 2008-10-16 12:44:05Z tterribe
...
static int oc_dec_init(oc_dec_ctx *_dec,const th_info *_info,
 const th_setup_info *_setup){
  ...
  if(ret<0)return ret;
  _dec->huff_tables_storage = oc_huff_trees_copy(_dec->huff_tables,
...
static void oc_dec_clear(oc_dec_ctx *_dec){
  ...
  memset(_dec->huff_tables, 0, sizeof(_dec->huff_tables));
  _ogg_free(_dec->huff_tables_storage);
...
th_dec_ctx *th_decode_alloc(const th_info *_info,
 const th_setup_info *_setup){
  ...
  oc_huff_trees_clear((oc_huff_node**)_setup->huff_tables);
  return dec;
}
