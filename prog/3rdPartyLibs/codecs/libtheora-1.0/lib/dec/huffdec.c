 ...
 ********************************************************************
  ...
  function:
      huffdec.c 15431 2008-10-21 05:04:02Z giles
...
static size_t oc_huff_node_alloc_sz(int _nbits){
  size_t        size;
...
static oc_huff_node *oc_huff_node_alloc(int _nbits){
  oc_huff_node *ret;
  ret=_ogg_calloc(1,oc_huff_node_alloc_sz(_nbits));
...
static oc_huff_node *oc_huff_tree_copy(const oc_huff_node *_node, char **stor){
  oc_huff_node *ret = (oc_huff_node*)stor[0];
  stor[0] += oc_huff_node_alloc_sz(_node->nbits);

  ret->nbits=(unsigned char)_node->nbits;
  ...
  if(_node->nbits){
    ...
    nchildren=1<<_node->nbits;
    for(i=0;i<nchildren;){
      ret->nodes[i]=oc_huff_tree_copy(_node->nodes[i], stor);
...
static size_t oc_huff_tree_copy_calc_sz(const oc_huff_node *_node){
  size_t ret = 0;
  ret += oc_huff_node_alloc_sz(_node->nbits);
  if(_node->nbits){
    int nchildren=1<<_node->nbits, i;
    for(i=0;i<nchildren;i+=(1<<_node->nbits-_node->nodes[i]->depth))
      ret += oc_huff_tree_copy_calc_sz(_node->nodes[i]);
  }
  return ret;
}
...
void *oc_huff_trees_copy(oc_huff_node *_dst[TH_NHUFFMAN_TABLES],
 const oc_huff_node *const _src[TH_NHUFFMAN_TABLES]){
  ...
  char *storage, *sp;
  int i;
  for(i=0;i<TH_NHUFFMAN_TABLES;i++)sz+=oc_huff_tree_copy_calc_sz(_src[i]);

  sp = storage = (char*)_ogg_calloc(1, sz);
  for(i=0;i<TH_NHUFFMAN_TABLES;i++)_dst[i]=oc_huff_tree_copy(_src[i], &sp);
  return storage;
}
...
void oc_huff_trees_clear(oc_huff_node *_nodes[TH_NHUFFMAN_TABLES]){
  int i;
  for(i=0;i<TH_NHUFFMAN_TABLES;i++) { oc_huff_tree_free(_nodes[i]); _nodes[i] = NULL; }
}
