 ...
 ********************************************************************
  ...
  function:
      decinfo.c 15400 2008-10-15 12:10:58Z tterribe
...
static ogg_int32_t oc_unpack_length(oggpack_buffer *_opb){
  ogg_int32_t ret[4];
  int  i;
...
static int oc_comment_unpack(oggpack_buffer *_opb,th_comment *_tc){
  ...
  if(_tc->comments<0||_tc->comments>(INT_MAX>>2)||
   theorapackB_bytes(_opb)+((ogg_int32_t)_tc->comments<<2)>_opb->storage){
    return TH_EBADHEADER;
  }
