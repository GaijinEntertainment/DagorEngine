 ...
 ********************************************************************
  ...
  function:
      state.c 15469 2008-10-30 12:49:42Z tterribe
...
void oc_state_vtable_init_c(oc_theora_state *_state){
  THEORA_VSET(_state->opt_vtable,frag_recon_intra,oc_frag_recon_intra_c);
  THEORA_VSET(_state->opt_vtable,frag_recon_inter,oc_frag_recon_inter_c);
  THEORA_VSET(_state->opt_vtable,frag_recon_inter2,oc_frag_recon_inter2_c);
  THEORA_VSET(_state->opt_vtable,state_frag_copy,oc_state_frag_copy_c);
  THEORA_VSET(_state->opt_vtable,state_frag_recon,oc_state_frag_recon_c);
  THEORA_VSET(_state->opt_vtable,state_loop_filter_frag_rows,oc_state_loop_filter_frag_rows_c);
  THEORA_VSET(_state->opt_vtable,restore_fpu,oc_restore_fpu_c);
}
 ...
 ogg_uint16_t _dc_iquant, const ogg_uint16_t _ac_iquant[64]){
#if __GNUC__
  ogg_int16_t dct_buf[64] __attribute__((aligned(16)));
  ogg_int16_t res_buf[64] __attribute__((aligned(16)));
#else
  ogg_int16_t __declspec(align(16)) dct_buf[64];
  ...
  /*_last_zzi is subtly different from an actual count of the number of
    ...
    Although convoluted, this is arguably the correct behavior: it allows us to
     ...
     ends with a ogg_int32_t zero run instead of a normal EOB token.
  ...
  #if defined(USE_ASM_ALTIVEC)
    {
    vec_short8 rr = vec_splats8h(p);
    for(ci=0;ci<8;ci++)
      vec_stvlx(rr, ci*16, res_buf);
    }
  #else
    /*LOOP VECTORIZES.*/
