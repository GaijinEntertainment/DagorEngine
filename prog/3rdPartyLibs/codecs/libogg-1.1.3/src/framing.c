 ...
 ********************************************************************
 ...
 function: code raw [Vorbis] packets into framed OggSquish stream and
   ...
   framing.c 9601 2005-07-23 00:19:14Z giles
...
static ogg_uint32_t _ogg_crc_entry(ogg_uint32_t index){
  int           i;
  ogg_uint32_t r;
...
char *ogg_sync_buffer(ogg_sync_state *oy, ogg_int32_t size){
...
int ogg_sync_wrote(ogg_sync_state *oy, ogg_int32_t bytes){
...
ogg_int32_t ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
...
int ogg_sync_pageout(ogg_sync_state *oy, ogg_page *og){
  ...
  for(;;){
    ogg_int32_t ret=ogg_sync_pageseek(oy,og);
    if(ret>0){
      /* have a page */
      return(1);
    }
...
int ogg_stream_pagein(ogg_stream_state *os, ogg_page *og){
  ...
  ogg_int32_t pageno=ogg_page_pageno(og);
...
void checkpacket(ogg_packet *op,int len, int no, int pos){
  ...
  if(op->packetno!=sequence){
    fprintf(stderr,"incorrect packet sequence %ld != %d\n",
            (ogg_int32_t)(op->packetno),sequence);
    exit(1);
  }
...
void print_header(ogg_page *og){
  ...
  fprintf(stderr,"  granulepos: %d  serialno: %d  pageno: %ld\n",
          ...
          ((ogg_int32_t)(og->header[21])<<24)|(og->header[20]<<16)|
...
void test_pack(const int *pl, const int **headers, int byteskip,
  ...
  ogg_int32_t inptr=0;
  ogg_int32_t outptr=0;
  ogg_int32_t deptr=0;
  ogg_int32_t depacket=0;
  ogg_int32_t granule_pos=7,pageno=0;
          ...
          while(1){
            ...
            while(ogg_stream_packetpeek(&os_de,&op_de2)>0){
              ...
              if(op_de.granulepos!=-1){
                fprintf(stderr," granule:%ld ",(ogg_int32_t)op_de.granulepos);
              }
            }
          }
        }
