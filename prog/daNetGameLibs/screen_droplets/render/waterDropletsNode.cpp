...
dafg::NodeHandle makeWaterDropletsNode()
{
  ...
  return dafg::register_node("water_droplets_node", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
    ...
    auto screenDropletRTHndl =
      registry.createTexture2d("screen_droplets_tex", {TEXFMT_A16B16G16R16F | TEXCF_RTARGET, registry.getResolution<2>("droplets"), 2})
        ...
        .handle();
    {
      ...
      registry.create("screen_droplets_sampler").blob(d3d::request_sampler(smpInfo));
    }

    return [opaqueFinalTargetHndl, screenDropletRTHndl]() {
      get_screen_droplets_mgr()->render(screenDropletRTHndl.get(), opaqueFinalTargetHndl.get());
    };
  });
}
