...
inline void start_flash_blind_ecs_query(ecs::EntityManager &manager, Callable c);

template <typename Callable>
inline void stop_flash_blind_ecs_query(ecs::EntityManager &manager, Callable c);

template <typename Callable>
inline void flash_blind_finish_init_ecs_query(ecs::EntityManager &manager, Callable c);

static bool flash_blind_do_capture_query()
{
  bool doCapture = false;
  flash_blind_finish_init_ecs_query(*g_entity_mgr, [&doCapture](bool &flash_blind__just_captured) {
...
static void flash_blind_es(const ecs::Event &, ecs::EntityManager &manager, float flash_blind__intensity)
{
  start_flash_blind_ecs_query(manager, [flash_blind__intensity](float flash_blind__decay_factor,
                                         resource_slot::NodeHandleWithSlotsAccess &flashBlind, float &flash_blind__remaining_time,
                                         bool &flash_blind__just_captured) {
    ...
    flashBlind = resource_slot::register_access("flash_blind", DAFG_PP_NODE_SRC, {resource_slot::Read{"postfx_input_slot"}},
      [](resource_slot::State slotsState, dafg::Registry registry) {
        ...
        registry.requestRenderPass().color(
          {registry.createTexture2d("flash_blind_tex", {TEXFMT_R11G11B10F | TEXCF_RTARGET, registry.getResolution<2>("post_fx", 0.5f)})
...
static void flash_blind_stop_es(const ecs::Event &, ecs::EntityManager &manager)
{
  stop_flash_blind_ecs_query(manager, [](resource_slot::NodeHandleWithSlotsAccess &flashBlind, float &flash_blind__remaining_time) {
    flashBlind.reset();
    flash_blind__remaining_time = 0.0;
    ShaderGlobal::set_float(::get_shader_variable_id("flash_blind_remaining_time", true), flash_blind__remaining_time);
  });
}