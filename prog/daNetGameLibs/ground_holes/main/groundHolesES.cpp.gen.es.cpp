...
//start of 1 rw components at [0]
  {ECS_HASH("should_update_ground_holes_coll"), ecs::ComponentTypeInfo<bool>()}
};
static void ground_holes_update_coll_es_all(const ecs::UpdateStageInfo &__restrict info, const ecs::QueryView & __restrict components)
{
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE);
  do
    ground_holes_update_coll_es(*info.cast<ecs::UpdateStageInfoAct>()
    , ECS_RW_COMP(ground_holes_update_coll_es_comps, "should_update_ground_holes_coll", bool)
    );
  while (++comp != compE);
}
...
(
  ...
  ecs::EntitySystemOps(ground_holes_update_coll_es_all),
...
,nullptr,nullptr,"*");
static constexpr ecs::ComponentDesc ground_holes_physics_init_es_event_handler_comps[] =
{
...
static void ground_holes_physics_init_es_event_handler_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  G_UNUSED(components);
  ground_holes_physics_init_es_event_handler(evt
        );
}
static ecs::EntitySystemDesc ground_holes_physics_init_es_event_handler_es_desc
(
  "ground_holes_physics_init_es",
  "prog/daNetGameLibs/ground_holes/main/groundHolesES.cpp.inl",
  ecs::EntitySystemOps(nullptr, ground_holes_physics_init_es_event_handler_all_events),
  empty_span(),
  empty_span(),
  make_span(ground_holes_physics_init_es_event_handler_comps+0, 2)/*rq*/,
  empty_span(),
...
inline void set_holes_changed_ecs_query(Callable function)
{
  perform_query(g_entity_mgr, set_holes_changed_ecs_query_desc.getHandle(),
    [&function](const ecs::QueryView& __restrict components)
    {
        ...
        {
          function(
              ECS_RW_COMP(set_holes_changed_ecs_query_comps, "should_update_ground_holes_coll", bool)
