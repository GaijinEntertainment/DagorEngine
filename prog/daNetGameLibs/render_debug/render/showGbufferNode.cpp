...
void set_up_show_gbuffer_entity()
{
  ...
  init[ECS_HASH("showGbufferNode")] = dabfg::register_node("show_gbuffer_node", DABFG_PP_NODE_SRC, [](dabfg::Registry registry) {
    registry.orderMeAfter("after_post_fx_ecs_event_node");
    read_gbuffer(registry);
    registry.readTexture("motion_vecs_after_transparency").atStage(dabfg::Stage::PS).bindToShaderVar("motion_gbuf").optional();
    registry.readTexture("upscale_sampling_tex").atStage(dabfg::Stage::POST_RASTER).bindToShaderVar("upscale_sampling_tex").optional();
    registry.readTexture("far_downsampled_depth").atStage(dabfg::Stage::POST_RASTER).bindToShaderVar("downsampled_far_depth_tex");
    registry.readTexture("ssao_tex").atStage(dabfg::Stage::POST_RASTER).bindToShaderVar("ssao_tex").optional();
    registry.read("ssao_sampler").blob<d3d::SamplerHandle>().bindToShaderVar("ssao_tex_samplerstate").optional();
    registry.readTexture("ssr_target").atStage(dabfg::Stage::POST_RASTER).bindToShaderVar("ssr_target").optional();
    ...
    auto gdepth = registry.readTexture("depth_for_postfx").atStage(dabfg::Stage::POST_RASTER).bindToShaderVar("depth_gbuf").handle();
