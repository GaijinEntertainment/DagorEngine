...
#define SCOPE_VIEW_PROJ_MATRIX ScopeViewProjMatrix DAG_CONCAT(scopevpm, __LINE__)
#define SCOPE_VIEWPORT         ScopeViewport DAG_CONCAT(scopevp, __LINE__)
#define SCOPE_RENDER_TARGET \
  SCOPE_VIEWPORT;           \
  ScopeRenderTarget DAG_CONCAT(scopert, __LINE__)
...
#define STATE_GUARD(func_call, val, default_value)                                   \
  auto DAG_CONCAT(lambda, __LINE__) = [&](const decltype(val) VALUE) { func_call; }; \
  STATE_GUARD_IMPL(DAG_CONCAT(StateGuard, __LINE__), DAG_CONCAT(guard, __LINE__), DAG_CONCAT(lambda, __LINE__), val, default_value)
