...
template <class T>
struct is_type_relocatable<concurrent_element_pool_detail::FixedArray<T>> : eastl::true_type
{};
...
public:
  ...
  ConcurrentElementPool() { addInitialBucket(); }
  ...
  ConcurrentElementPool(ConcurrentElementPool &&other) : ConcurrentElementPool() { swap(other); }
  ...
  void ensureSpace(Id id)
  {
    ...
    while (bucket >= buckets.size() || (bucket + 1 == buckets.size() && firstFreeInLastBucket <= offset))
      allocate();
  }
  ...
  {
    const auto [bucket, offset] = break_index(id);
    return INITIAL_CAPACITY * ((1u << bucket) - 1) + offset;
  }
  ...
  static Id from_index(eastl::underlying_type_t<Id> idx)
  {
    ...
    const uint32_t offset = idx - ((1u << bucket) - 1) * INITIAL_CAPACITY;
...
private:
  ...
  void swap(ConcurrentElementPool &other)
  {
    buckets.swap(other.buckets);
    eastl::swap(firstFreeInLastBucket, other.firstFreeInLastBucket);
    eastl::swap(totalElems, other.totalElems);
  }
  ...
  constexpr static Id make_index(uint8_t bucket, uint32_t offset)
  {
  ...
  static eastl::tuple<uint8_t, uint32_t> break_index(Id id)
  {
    ...
    const uint32_t offset = raw & ((1u << offsetBitsCount) - 1);
