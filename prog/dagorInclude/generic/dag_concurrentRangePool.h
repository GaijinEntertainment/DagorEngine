...
template <class T>
struct is_type_relocatable<concurrent_range_pool_detail::FixedArray<T>> : eastl::true_type
{};
...
private:
  ...
  void swap(ConcurrentRangePool &other)
  {
  ...
  static Range make_range(uint8_t size, uint8_t bucket, uint32_t offset)
  {
  ...
  static eastl::tuple<uint8_t, uint8_t, uint32_t> break_range(Range r)
  {
    ...
    const uint32_t offset = rest & ((1u << offsetBitsCount) - 1);
    G_FAST_ASSERT(bucket < MAX_BUCKET_COUNT && offset + size <= capacity(bucket));
