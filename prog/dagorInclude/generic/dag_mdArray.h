...
inline constexpr bool all_types_are = (Checker<Ts, T>::value && ...);

template <class T, class... Ts>
inline constexpr bool all_types_are_convertible_to = all_types_are<T, eastl::is_convertible, Ts...>;

template <uint32_t Rank, class... Ts>
inline constexpr bool are_mdarray_indices = sizeof...(Ts) == Rank && all_types_are_convertible_to<uint32_t, Ts...>;
...
struct MdArrayDimensions<Rank> : carray<uint32_t, Rank>
{
  ...
  constexpr uint32_t rank() const { return BaseType::size(); }
  constexpr uint32_t totalSize() const { return BaseType::at(0); }
...
inline constexpr MdArrayDimensions<sizeof...(Dims)> dims_from_varargs{Dims...};

template <uint32_t Rank, class... Ids>
  requires(are_mdarray_indices<Rank, Ids...>)
MdArrayIndex<Rank> ids_from_varargs(Ids... ids)
{
...
inline constexpr void fill_mdarray_from_initializer(MdArray<T, Dims...> &arr,
  typename GetMdInitializerList<T, sizeof...(Dims)>::type initializer);
template <class T, uint32_t... Dims>
  requires(sizeof...(Dims) >= 1)
inline constexpr void fill_mdarray_from_initializer(MdArray<T, Dims...> &arr, std::initializer_list<T> initializer);

template <class TMdArray, size_t... Is, class F>
inline TMdArray mdarray_make_impl(eastl::index_sequence<Is...>, F &&make)
{
  return {((void)Is, make())...};
}
...
struct MdArray<T, Dim> : carray<T, Dim>
{
  static constexpr uint32_t RANK = 1;
  static constexpr detail::MdArrayDimensions<RANK> DIMS = detail::dims_from_varargs<Dim>;
  static constexpr uint32_t SIZE = Dim;

  using BaseType = carray<T, SIZE>;
  using SubElementType = T;
  using ElementType = T;
  using DimsType = detail::MdArrayDimensions<RANK>;
...
template <class T, uint32_t FDim, uint32_t... ODims>
  requires(sizeof...(ODims) > 0)
struct MdArray<T, FDim, ODims...> : carray<MdArray<T, ODims...>, FDim>
{
  static constexpr uint32_t RANK = sizeof...(ODims) + 1;
  static constexpr detail::MdArrayDimensions<RANK> DIMS = detail::dims_from_varargs<FDim, ODims...>;
  static constexpr uint32_t SIZE = DIMS.totalSize();

  using BaseType = carray<MdArray<T, ODims...>, FDim>;
  using SubElementType = MdArray<T, ODims...>;
  using ElementType = T;
  using DimsType = detail::MdArrayDimensions<RANK>;
  using IndexType = DimsType::IndexType;

  constexpr MdArray(typename detail::GetMdInitializerList<T, RANK>::type initializer)
    requires(eastl::is_default_constructible_v<T> && eastl::is_copy_assignable_v<T>)
  {
    detail::fill_mdarray_from_initializer(*this, initializer);
  }
  constexpr MdArray(std::initializer_list<T> initializer)
    requires(eastl::is_default_constructible_v<T> && eastl::is_copy_assignable_v<T>)
  {
  ...
  // template <class... Ids>
  //   requires(detail::are_mdarray_indices<RANK, Ids...>)
  // constexpr T &operator[](Ids... ids)
  // {
  //   return operator[](detail::ids_from_varargs<RANK>(ids...));
  // }
  //
  // template <class... Ids>
  //   requires(detail::are_mdarray_indices<RANK, Ids...>)
  ...
  {
    return at(detail::ids_from_varargs<RANK>(ids...));
  }
...
template <class T, uint32_t... Dims>
  requires(sizeof...(Dims) > 1)
inline constexpr void detail::fill_mdarray_from_initializer(MdArray<T, Dims...> &arr,
  typename GetMdInitializerList<T, sizeof...(Dims)>::type initializer)
{
...
template <class T, uint32_t... Dims>
  requires(sizeof...(Dims) >= 1)
inline constexpr void detail::fill_mdarray_from_initializer(MdArray<T, Dims...> &arr, std::initializer_list<T> initializer)
{
...
inline constexpr MdArray<eastl::invoke_result_t<F>, Dims...> mdarray_make(F &&make)
{
  return detail::mdarray_make_impl<MdArray<eastl::invoke_result_t<F>, Dims...>>(
