...
public:
  ...
  void addLeafDepthStats(depth_stats_t &stats)
  {
    for (const LinearGridSubCell<ObjectType> &cell : cellsData)
    {
      if (cell.rootLeaf == EMPTY_LEAF)
        continue;
      leaf_add_depth_stats(cell.rootLeaf, stats);
    }
  }
...
template <typename ObjectType>
VECTORCALL DAGOR_NOINLINE static void leaf_add_depth_stats(const LinearGrid<ObjectType> *__restrict grid, leaf_id_t leaf_idx,
  depth_stats_t &stats, uint32_t cur_depth = 0)
{
  cur_depth++;
  LG_VERIFY(leaf_idx != EMPTY_LEAF);
  if (grid->isBranch(leaf_idx))
  {
    UnpackedBranch branch = grid->unpackBranch(leaf_idx);
    leaf_add_depth_stats(grid, branch.leftIdx, stats, cur_depth);
    leaf_add_depth_stats(grid, branch.rightIdx, stats, cur_depth);
  }
  else // lnode
  {
    stats.depthSum += cur_depth;
    stats.count++;
    stats.maxDepth = max(cur_depth, stats.maxDepth);
  }
}
...
public:
  ...
  __forceinline UnpackedBranch unpackBranch(leaf_id_t leaf_idx) const
  {
    ...
    UnpackedBranch ret;
    v_bbox3_init_empty(ret.leftBox);
    v_bbox3_init_empty(ret.rightBox);
#if EXTRA_SMALL_BRANCHES_AND_64K_LEAFS_LIMIT
  ...
  struct OptimizationStats
  {
    ...
    uint32_t splitFails = 0;
    uint32_t splitFailMaxObj = 0;
  };
  ...
  DAGOR_NOINLINE void printStats(print_func_t print_func, const OptimizationStats &stats) const
  {
    ...
    print_func("Main ext: %.1f %.1f %.1f %.1f; Sub ext: %.1f; Max ov.rad: %.2f", V4D(maxMainExtension), maxSubExtension,
      maxOversizeRad);
    print_func("Max leaf depth: %u (avg. %.2f) Split fails: %i (max %i)", depthStats.maxDepth,
      double(depthStats.depthSum) / depthStats.count, stats.splitFails, stats.splitFailMaxObj);
  ...
  DAGOR_NOINLINE int findBadLeafCuts(leaf_id_t leaf, bbox3f parent_box, callback_t callback)
  {
    if (isBranch(leaf))
    {
      UnpackedBranch branch = unpackBranch(leaf, parent_box);
      int leftObjects = findBadLeafCuts(branch.leftIdx, branch.leftBox, callback);
      int rightObjects = findBadLeafCuts(branch.rightIdx, branch.rightBox, callback);
      unsigned configMaxLeafImbalanceRatioLog2 = 3;
      uint32_t allowSplitThreshold = (leftObjects + rightObjects) >> configMaxLeafImbalanceRatioLog2;
      if (leftObjects < allowSplitThreshold || rightObjects < allowSplitThreshold)
        callback(leftObjects, branch.leftBox, rightObjects, branch.rightBox);
      return leftObjects + rightObjects;
    }
    return getLeaf(leaf).objects.size();
  }

  template <typename callback_t>
  DAGOR_NOINLINE void findBadLeafCuts(callback_t callback)
  {
    for (LinearGridMainCell<ObjectType> &cell : cells)
    {
      if (cell.rootLeaf == EMPTY_LEAF)
        continue;
      findBadLeafCuts(cell.rootLeaf, cell.getBBox(), callback);
      if (const LinearSubGrid<ObjectType> *subGrid = getSubGrid(cell.subGridIdx))
      {
        for (const LinearGridSubCell<ObjectType> &subCell : subGrid->getCells())
        {
          if (subCell.rootLeaf == EMPTY_LEAF)
            continue;
          findBadLeafCuts(subCell.rootLeaf, subCell.getBBox(), callback);
        }
      }
    }
  }

  void addLeafDepthStats(depth_stats_t &stats) const
  {
    for (const LinearGridMainCell<ObjectType> &cell : cells)
    {
      if (cell.rootLeaf == EMPTY_LEAF)
        continue;
      leaf_add_depth_stats(this, cell.rootLeaf, stats);
    }
  }
...
private:
  ...
  VECTORCALL DAGOR_NOINLINE int findBestHalfCut(dag::ConstSpan<bbox3f> bboxes, Point3 &mids)
  {
    const int N = bboxes.size();
    constexpr int PARALLEL_THRESHOLD = 4000;

    dag::RelocatableFixedVector<float, 128, true /*overflow*/, framemem_allocator, uint32_t, false /*init*/> coords;
    coords.resize(N * 6);
    float *const amins[3] = {coords.data(), coords.data() + 2 * N, coords.data() + 4 * N};
    float *const amaxs[3] = {coords.data() + N, coords.data() + 3 * N, coords.data() + 5 * N};

    for (int i = 0; i < N; i++)
    {
      alignas(16) float bmin[4], bmax[4];
      v_st(bmin, bboxes[i].bmin);
      v_st(bmax, bboxes[i].bmax);
      amins[0][i] = bmin[0];
      amins[1][i] = bmin[1];
      amins[2][i] = bmin[2];
      amaxs[0][i] = bmax[0];
    ...
    auto processAxis = [](int N, float *amin, float *amax) -> std::pair<int, float> {
      constexpr float EPS = 0.05f;
      stlsort::sort_branchless(amin, amin + N);
      stlsort::sort_branchless(amax, amax + N);

      int bestVal = -1, pL = 0, pR = 0, mMin = 0, mMax = 0;
      float mid = 0.f;

      auto evaluateScore = [&](float x) -> bool {
        while (pL < N && amax[pL] < x)
          ++pL;
        while (pR < N && amin[pR] <= x)
          ++pR;
        const int left = pL;
        const int right = N - pR;
        const int val = left < right ? left : right;
        if (val > bestVal)
        {
          bestVal = val;
          mid = x;
      ...
      while (mMin < N && mMax < N)
      {
        const float xMin = amin[mMin] - EPS;
        const float xMax = amax[mMax] + EPS;
        float x;
        if (xMin <= xMax)
        {
          x = xMin;
      ...
      while (mMin < N)
      {
        if (!evaluateScore(amin[mMin] - EPS))
          break;
        ++mMin;
      }
      while (mMax < N)
      {
        if (!evaluateScore(amax[mMax] + EPS))
          break;
        ++mMax;
      }
      return {bestVal, mid};
    ...
    std::pair<int, float> results[3];
    if (N >= PARALLEL_THRESHOLD)
    {
      threadpool::JobPool pool(3);
      for (int axis = 0; axis < 3; axis++)
        pool.add([&, axis] { results[axis] = processAxis(N, amins[axis], amaxs[axis]); });
      pool.wait();
    }
    else
    {
      for (int axis = 0; axis < 3; axis++)
        results[axis] = processAxis(N, amins[axis], amaxs[axis]);
    }

    int bestIdx = 0, bestVal = -1;
    for (int axis = 0; axis < 3; axis++)
    {
      auto [val, mid] = results[axis];
      mids[axis] = mid;
      if (val > bestVal)
      {
        bestVal = val;
...
public:
  ...
  float configMaxSplitOverflow = 0.2f;
