...
struct RaiiOptionalCounter
{
  ...
  int prevCounter;
  void inc(int *__restrict c)
  {
    if (c)
    {
      prevCounter = *c;
      interlocked_increment(*c);
    }
  }
  void dec(int *__restrict c)
  {
    if (c)
    {
      interlocked_decrement(*c);
      if (prevCounter != *c)
        logerr("Unexpected nestedQuery value (%d != %d). Probably ECS systems/queries are called from multiple threads without "
               "constrainedMTMode",
          prevCounter, *c);
    }
  }
#else
  // Interlocked inc/dec are not necessary here, but we use them for safety
...
DAECS_RELEASE_INLINE const T *__restrict EntityManager::getNullable(EntityId eid, const HashedConstString name) const
{
  ...
  const void *__restrict val =
    get(eid, name, ComponentTypeInfo<T>::type, ComponentTypeInfo<T>::size, cidx, archetype DAECS_EXTENSIVE_CHECK_FOR_READ);
  return val ? &PtrComponentType<T>::cref(val) : nullptr;
}
...
__forceinline void EntityManager::emplaceUntypedEvent(Storage &storage, EntityId eid, Event &evt)
{
  ...
  storage.active.commitPush(len); // make event visible to readers only after data is written
}
...
inline bool EntityManager::destroyEntity(EntityId &eid) //-V659
{
