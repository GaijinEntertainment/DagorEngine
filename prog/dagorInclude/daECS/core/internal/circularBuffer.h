...
struct EventCircularBuffer
{
  ...
  bool canRead(uint32_t sz) const { return readAt + sz <= interlocked_acquire_load(writeAt); };
  ...
  {
    G_FAST_ASSERT(canWrite(sz + sizeof(EntityId)));
    G_UNREFERENCED(sz);
    uint8_t *ret = data.get() + writeIndex();
    new (ret, _NEW_INPLACE) EntityId(eid);
    // caller must call commitPush() after writing event data, otherwise a reader can see the new writeAt before the data in place
    return ret + sizeof(eid);
  }
  void commitPush(uint32_t event_sz) { interlocked_add(writeAt, sizeof(EntityId) + event_sz); }
...
#undef OFFSETOF
    ...
    interlocked_add(writeAt, excess);
    return true;
  }
};
...
struct DeferredEventsStorage
{
  ...
  void emplaceEvent(EntityId eid, T &&event)
  {
    ...
    active.commitPush(eventSize); // make event visible to readers only after data is written
  }
