...
bool save_png32(const char *fn, const TexPixel32 *ptr, int wd, int ht, int stride,
  unsigned char *text_data, // should be nullptr or valid C-string
  unsigned int text_data_len, bool save_alpha = true);

bool save_png32(const TexPixel32 *ptr, int wd, int ht, int stride, IGenSave &cwr, bool save_alpha = false,
  unsigned char *text_data = nullptr, unsigned int text_data_len = 0);
