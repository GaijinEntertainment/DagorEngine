...
inline T packun(float s, float _max, uint32_t n, const char *fu, const char *f, int l, const char *msg = "")
{
  ...
  if (s > _max)
    logerr("%s(%d) : pack overflow in %s max = %f, val = %f, msg = %s", f, l, fu, _max, s, msg);
  else if (s < 0.f)
...
{
  G_LOGERR_AND_DO(s != get_invalid_s(T()), return -_max, "%s(%d) : failed to unpack val=%d in %s() msg:'%s'", file, l, s, fn, msg);
  G_UNUSED(fn);
  G_UNUSED(file);
  G_UNUSED(l);
  G_UNUSED(msg);
