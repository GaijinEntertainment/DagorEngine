...
void android_app_pre_exec_cmd(struct android_app* android_app, int8_t cmd) {
    switch (cmd) {
        case APP_CMD_INPUT_CHANGED:
            ...
            pthread_mutex_lock(&android_app->mutex);
            if (android_app->externalInputProcessing != 1 && android_app->inputQueue != NULL) {
...
static void android_app_set_activity_state(struct android_app* android_app, int8_t cmd) {
    if (android_should_ignore_cmd(cmd)) {
        LOGV("Ignoring activity state %d", cmd);
        return;
    }
...
static void* onSaveInstanceState(ANativeActivity* activity, size_t* outLen) {
    ...
    if (android_should_ignore_cmd(APP_CMD_SAVE_STATE)) {
        LOGV("Ignoring saving state %d", APP_CMD_SAVE_STATE);
        return NULL;
    }
