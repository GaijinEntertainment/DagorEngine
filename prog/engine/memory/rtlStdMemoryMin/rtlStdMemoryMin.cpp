...
#else
inline size_t dagmem_malloc_usable_size(void *p) { return p ? _msize(p) : 0; }
#endif
...
public:
  ...
  size_t getSize(void *p) override { return dagmem_malloc_usable_size(p); }
  ...
  bool resizeInplace([[maybe_unused]] void *p, [[maybe_unused]] size_t sz) override { return false; }
...
char *str_dup(const char *s, [[maybe_unused]] IMemAlloc *a)
{
  if (!s)
    return NULL;
  size_t n = strlen(s) + 1;
...
extern "C" int memresizeinplace_default([[maybe_unused]] void *p, [[maybe_unused]] size_t sz) { return 0; }
