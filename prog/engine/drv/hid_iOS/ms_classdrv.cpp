...
{
  static float dagor_scr_dpi = 326;
  memset(&raw_state_pnt, 0, sizeof(raw_state_pnt));
  stg_pnt.touchScreen = true;
  stg_pnt.tapStillCircleRad = dagor_scr_dpi * 1.0f / 25.4f; // +-1 mm
  if (stg_pnt.tapStillCircleRad > 16)
    stg_pnt.tapStillCircleRad = 16;
  debug("tapStillCircleRad=%d (%.1fpix/mm)", stg_pnt.tapStillCircleRad, dagor_scr_dpi / 25.4f);
