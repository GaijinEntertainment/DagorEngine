...
BEGIN_BITFIELD_TYPE(ImageViewState, uint64_t)
  enum
  {
    ...
    IS_INPUT_ATTACHMENT_BITS = 1,
    IS_INPUT_ATTACHMENT_SHIFT = IS_ARRAY_BITS + IS_ARRAY_SHIFT,
    FORMAT_BITS = FormatStore::BITS_SZ,
    FORMAT_SHIFT = IS_INPUT_ATTACHMENT_SHIFT + IS_INPUT_ATTACHMENT_BITS,
  ...
  // A depth/stencil attachment read as an input attachment: attachment-usage view (like a render
  // target) but with a single (depth) aspect, which an input-attachment descriptor requires.
  ADD_BITFIELD_MEMBER(isInputAttachment, IS_INPUT_ATTACHMENT_SHIFT, IS_INPUT_ATTACHMENT_BITS)
