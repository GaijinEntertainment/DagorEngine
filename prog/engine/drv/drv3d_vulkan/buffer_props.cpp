...
VkDeviceSize BufferProps::getAlignForUsageAndFlags(VkBufferCreateFlags flags, VkBufferUsageFlags usage)
{
  ...
  G_UNUSED(flags);
  return perUsageFlags[usage].align;
}

uint32_t BufferProps::getMemoryTypeMaskForUsageAndFlags(VkBufferCreateFlags flags, VkBufferUsageFlags usage)
{
  G_ASSERTF(flags == 0, "vulkan: non zero buffer flags are not supported");
  G_ASSERTF(perUsageFlags.find(usage) != perUsageFlags.end(), "vulkan: unknown buffer usage flags %s", formatBufferUsageFlags(usage));
  G_UNUSED(flags);
  return perUsageFlags[usage].memoryTypeMask;
}

BufferProps::Prop BufferProps::calculateForUsageAndFlags(VkBufferCreateFlags flags, VkBufferUsageFlags usage)
{
  G_ASSERTF(flags == 0, "vulkan: non zero buffer flags are not supported");
  G_UNUSED(flags);
  ...
  if (resCode == VK_SUCCESS)
  {
    ...
    return {ret.requirements.alignment, ret.requirements.memoryTypeBits};
  }
  return {0, 0};
}

void BufferProps::init()
{
...
void BufferProps::shutdown() { perUsageFlags.clear(); }
