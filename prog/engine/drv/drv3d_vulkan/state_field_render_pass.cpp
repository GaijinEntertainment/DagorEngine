...
void StateFieldRenderPassSubpassIdx::applyTo(FrontRenderPassStateStorage &state, ExecutionState &target) const
{
  ...
  target.set<StateFieldGraphicsQueryScopeOpener, GraphicsQueryState::InvalidateTag, BackGraphicsState>({});
  target.set<StateFieldGraphicsQueryScopeCloser, GraphicsQueryState::InvalidateTag, BackScopeState>({});
  ...
  if (data == 0)
  {
    ...
    target.set<StateFieldGraphicsRenderPassEarlyScopeOpener, bool, BackGraphicsState>(true);
  }
  // closing the pass
