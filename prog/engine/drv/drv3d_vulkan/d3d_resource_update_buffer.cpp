  ...
  unsigned dest_slice, unsigned offset_x, unsigned offset_y, unsigned offset_z, unsigned width, unsigned height, unsigned depth)
{
  ...
  {
    logerr("vulkan: RUB region is out of bounds: asked (%u, %u, %u)-(%u, %u, %u) for tex mip %u size (%u, %u, %u) block (%u, %u) sz "
           "%u name %s",
      offset_x, offset_y, offset_z, offset_x + width, offset_y + height, offset_z + depth, dest_mip, mipW, mipH, mipD, bx, by, size,
      dest_tex->getDeviceImage(false)->getDebugName());
    return nullptr;
  }

  auto &device = drv3d_vulkan::get_device();
  ...
  Buffer *stagingBuffer =
    device.createBuffer(size, DeviceMemoryClass::HOST_RESIDENT_HOST_READ_WRITE_BUFFER, 1, BufferMemoryFlags::NONE);
  if (device.getDebugLevel())
    stagingBuffer->setDebugName(String(64, "RUB for %s", dest_tex->getDeviceImage(false)->getDebugName()));
...
d3d::ResUpdateBuffer *d3d::allocate_update_buffer_for_tex(BaseTexture *dest_base_texture, int dest_mip, int dest_slice)
{
  ...
  if ((oldSize > 0) && (oldSize + size > device.getTexUploadLimit()))
  {
    device.getContext().fillEmptySubresources(dest_tex->getDeviceImage(false));
    return nullptr;
  }

  Buffer *stagingBuffer = device.createBuffer(size, DeviceMemoryClass::HOST_RESIDENT_HOST_READ_WRITE_BUFFER, 1,
    is_main_thread() ? BufferMemoryFlags::TEMP : BufferMemoryFlags::NONE);
  if (device.getDebugLevel())
    stagingBuffer->setDebugName(String(64, "RUB for %s", dest_tex->getDeviceImage(false)->getDebugName()));
...
void d3d::release_update_buffer(d3d::ResUpdateBuffer *&rub)
{
  ...
  {
    drv3d_vulkan::get_device().getContext().destroyBuffer(rub_imp->stagingBuffer);
