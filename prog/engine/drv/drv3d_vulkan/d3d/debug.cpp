...
void d3d::beginEvent(const char *name)
{
  ...
  {
    CmdPushEvent cmd;
    OSSpinlockScopedLock frontLock(Frontend::State::pod.reorderTimestamps ? nullptr : &Globals::ctx.getFrontLock());
...
void d3d::endEvent()
{
  ...
  if (Frontend::State::pod.reorderTimestamps)
    Globals::ctx.dispatchCmdNoLock<CmdPopEvent>({});
  else
...
REGISTER_IMGUI_WINDOW("VULK", "Latency wait##VULK-latencywait", drv3d_vulkan::debug_ui_latency_wait);
...
REGISTER_IMGUI_WINDOW("VULK", "Swapchain##VULK-swapchain", drv3d_vulkan::debug_ui_swapchain);
