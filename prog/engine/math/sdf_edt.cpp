...
static void transpose(sdf::DistField &grid, int w)
{
  const int BLOCK = 64;
...
static void edt_2d(sdf::DistField &grid, int w)
{
  ...
  for (int y = 0; y < w; y++)
    sdf::dt_1d(grid.row(y), w, tmp_d, tmp_v, tmp_z);
  transpose(grid, w);
  for (int x = 0; x < w; x++)
    sdf::dt_1d(grid.row(x), w, tmp_d, tmp_v, tmp_z);
  transpose(grid, w);

  framemem_ptr()->free(tmp);
}

void sdf::build_edt(const uint32_t *mask, int w, DistField &out_sdf)
{
  ...
  for (int y = 0; y < w; y++)
  {
    float *r = out_sdf.row(y);
    for (int x = 0; x < w; x++)
    {
      int i = y * w + x;
      r[x] = ((mdata[i >> 5] >> (i & 31)) & 1) ? 1e20f : 0.0f;
    }
  }
