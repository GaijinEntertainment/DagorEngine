...
// this is straightforward adaptation
...
public:
  ...
  public:
    ...
    float toScalar() const { return sign * ((m_denominator == 0) ? SIMD_INFINITY : (float)m_numerator / m_denominator); }
  };
  ...
  public:
    ...
    float xvalue() const { return x.toScalar() / denominator.toScalar(); }

    float yvalue() const { return y.toScalar() / denominator.toScalar(); }

    float zvalue() const { return z.toScalar() / denominator.toScalar(); }
  };
  ...
  public:
    ...
    float xvalue() const { return (point.index >= 0) ? float(point.x) : point128.xvalue(); }

    float yvalue() const { return (point.index >= 0) ? float(point.y) : point128.yvalue(); }

    float zvalue() const { return (point.index >= 0) ? float(point.z) : point128.zvalue(); }
  ...
  public:
    ...
    void link(Edge *n)
    {
      G_ASSERT(reverse->target == n->reverse->target);
      next = n;
      n->prev = this;
    }
    ...
    void print()
    {
      printf("E%p : %d -> %d,  n=%p p=%p   (0 %d\t%d\t%d) -> (%d %d %d)", this, reverse->target->point.index, target->point.index,
        next, prev, reverse->target->point.x, reverse->target->point.y, reverse->target->point.z, target->point.x, target->point.y,
        target->point.z);
    }
#endif
  };
...
private:
  ...
  public:
    ...
    PoolArray(int size) : size(size), next(NULL)
    {
      ...
      array = (T *)memalloc(sizeof(T) * size, tmpmem);
    }

    ~PoolArray() { memfree(array, tmpmem); }
  ...
  public:
    ...
    void setArraySize(int array_size) { this->arraySize = array_size; }
    ...
    T *newObject()
    {
      ...
      if (!o)
      {
        ...
        {
          p = new (memalloc(sizeof(PoolArray<T>), tmpmem), _NEW_INPLACE) PoolArray<T>(arraySize);
          p->next = arrays;
          arrays = p;
        }
        o = p->init();
  ...
  vec4f toBtVector(const Point32 &v);

  vec4f getBtNormal(Face *face);

  bool shiftFace(Face *face, float amount, Tab<Vertex *> stack);
...
public:
  ...
  vec4f getCoordinates(const Vertex *v);
...
int ConvexHullInternal::Rational64::compare(const Rational64 &b) const
{
  if (sign != b.sign)
  {
    return sign - b.sign;
  ...
  //  return (numerator * b.denominator > b.numerator * denominator) ? sign : (numerator * b.denominator < b.numerator * denominator) ?
  //  -sign : 0;
  ...
  __asm__("mulq %[bn]\n\t"
          ...
          "decb %%bh\n\t"   // now bx=0x0000 if difference is zero, 0xff01 if it is negative, 0x0001 if it is positive (i.e., same sign
                            // as difference)
          "shll $16, %%ebx\n\t" // ebx has same sign as difference
          : "=&b"(result), [tmp] "=&r"(tmp), "=a"(dummy)
          : "a"(denominator), [bn] "g"(b.numerator), [tn] "g"(numerator), [bd] "g"(b.denominator)
          : "%rdx", "cc");
  return result ? result ^ sign // if sign is +1, only bit 0 of result is inverted, which does not change the sign of result (and
                                // cannot result in zero) if sign is -1, all bits of result are inverted, which changes the sign of
...
int ConvexHullInternal::Rational128::compare(const Rational128 &b) const
{
  if (sign != b.sign)
  {
    return sign - b.sign;
...
int ConvexHullInternal::Rational128::compare(int64_t b) const
{
  if (isInt64)
  {
...
bool ConvexHullInternal::mergeProjection(IntermediateHull &h0, IntermediateHull &h1, Vertex *&c0, Vertex *&c1)
{
  ...
  {
    G_ASSERT(v0->point.z < v1->point.z);
    Vertex *v1p = v1->prev;
    if (v1p == v1)
    {
      c0 = v0;
      if (v1->edges)
      {
        G_ASSERT(v1->edges->next == v1->edges);
        v1 = v1->edges->target;
        G_ASSERT(v1->edges->next == v1->edges);
      }
      c1 = v1;
...
void ConvexHullInternal::computeInternal(int start, int end, IntermediateHull &result)
{
  int n = end - start;
  switch (n)
  {
...
ConvexHullInternal::Orientation ConvexHullInternal::getOrientation(const Edge *prev, const Edge *next, const Point32 &s,
  const Point32 &t)
{
  G_ASSERT(prev->reverse->target == next->reverse->target);
  if (prev->next == next)
  {
    if (prev->prev == next)
    {
...
ConvexHullInternal::Edge *ConvexHullInternal::findMaxAngle(bool ccw, const Vertex *start, const Point32 &s, const Point64 &rxs,
  ...
  if (e)
  {
    do
    {
      if (e->copy > mergeStamp)
      {
        ...
        if (cot.isNaN())
        {
          G_ASSERT(ccw ? (t.dot(s) < 0) : (t.dot(s) > 0));
        }
        else
        {
          int cmp;
...
void ConvexHullInternal::findEdgeForCoplanarFaces(Vertex *c0, Vertex *c1, Edge *&e0, Edge *&e1, Vertex *stop0, Vertex *stop1)
{
  ...
  G_ASSERT(!start1 || (start1->target->point.dot(normal) == dist));
  Point64 perp = s.cross(normal);
  G_ASSERT(!perp.isZero());
  ...
  if (e0)
  {
    while (e0->target != stop0)
    {
      ...
      G_ASSERT(e->target->point.dot(normal) == dist);
      if (e->copy == mergeStamp)
      {
        break;
      }
    ...
    while (true)
    {
      ...
      if (e1 && (e1->target != stop1))
      {
        ...
        if (f1->copy > mergeStamp)
        {
          ...
          {
            G_ASSERT((e1 == start1) && (d1.dot(normal) < 0));
          }
        }
      }
    ...
    while (true)
    {
      ...
      if (e0 && (e0->target != stop0))
      {
        ...
        if (f0->copy > mergeStamp)
        {
          ...
          {
            G_ASSERT((e0 == start0) && (d0.dot(normal) < 0));
          }
        }
      }
...
void ConvexHullInternal::compute(const void *coords, bool, int stride, int count)
{
  vec4f min = v_make_vec4f(float(1e30), float(1e30), float(1e30), 0), max = v_make_vec4f(float(-1e30), float(-1e30), float(-1e30), 0);
  ...
  s = v_div(s, v_splats(float(10216)));
  if (((medAxis + 1) % 3) != maxAxis)
  {
    s = v_neg(s);
  }
  scaling = s;
  s = v_sel(v_rcp(s), s, v_cmp_eq(s, v_zero()));

  center = v_mul(v_add(min, max), v_splats(float(0.5)));
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    vec4f p = v_ldu(v);
    ptr += stride;
    p = v_mul(v_sub(p, center), s);
    alignas(16) int32_t piS[4];
    v_sti(piS, v_cvt_vec4i(p));
    points[i].x = piS[medAxis];
    points[i].y = piS[maxAxis];
    points[i].z = piS[minAxis];
    points[i].index = i;
  }
  stlsort::sort(points.data(), points.data() + points.size(), PointCmp());
...
vec4f ConvexHullInternal::toBtVector(const Point32 &v)
{
  alignas(16) float p[4];
  p[medAxis] = float(v.x);
  p[maxAxis] = float(v.y);
  p[minAxis] = float(v.z);
  return v_mul(v_ld(p), scaling);
}

vec4f ConvexHullInternal::getBtNormal(Face *face) { return v_norm3(v_cross3(toBtVector(face->dir0), toBtVector(face->dir1))); }

vec4f ConvexHullInternal::getCoordinates(const Vertex *v)
{
  alignas(16) float p[4];
  p[medAxis] = v->xvalue();
  p[maxAxis] = v->yvalue();
  p[minAxis] = v->zvalue();
  return v_add(v_mul(v_ld(p), scaling), center);
}
...
float ConvexHullInternal::shrink(float amount, float clampAmount)
{
  ...
  alignas(16) float hc[4];
  hc[medAxis] = hullCenterX.toScalar();
  hc[maxAxis] = hullCenterY.toScalar();
  hc[minAxis] = hullCenterZ.toScalar();
  vec4f hullCenter = v_ld(hc);
  hullCenter = v_div(hullCenter, v_splats(4 * volume.toScalar()));
  hullCenter = v_mul(hullCenter, scaling);

  int faceCount = faces.size();

  if (clampAmount > 0)
  {
    vec3f minDistV = v_splats(SIMD_INFINITY);
    for (int i = 0; i < faceCount; i++)
    {
      vec4f normal = getBtNormal(faces[i]);
      minDistV = v_min(minDistV, v_dot3_x(normal, v_sub(toBtVector(faces[i]->origin), hullCenter)));
    }
    float minDist = v_extract_x(minDistV);
    if (minDist <= 0)
    {
      return 0;
    }
  ...
  for (int i = 0; i < faceCount; i++, seed = 1664525 * seed + 1013904223)
    eastl::swap(faces[i], faces[seed % faceCount]);
...
bool ConvexHullInternal::shiftFace(Face *face, float amount, Tab<Vertex *> stack)
{
  vec4f origShiftV = v_mul(getBtNormal(face), v_splats(-amount));
  origShiftV = v_sel(v_div(origShiftV, scaling), origShiftV, v_cmp_eq(scaling, v_zero()));
  alignas(16) int origShiftI[4];
  v_sti(origShiftI, v_cvt_vec4i(origShiftV));
  Point32 shift(origShiftI[medAxis], origShiftI[maxAxis], origShiftI[minAxis]);
  if (shift.isZero())
  {
    return true;
  }
  ...
  printf("\nShrinking face (%d %d %d) (%d %d %d) (%d %d %d) by (%d %d %d)\n", face->origin.x, face->origin.y, face->origin.z,
    face->dir0.x, face->dir0.y, face->dir0.z, face->dir1.x, face->dir1.y, face->dir1.z, shift.x, shift.y, shift.z);
#endif
  int64_t origDot = face->origin.dot(normal);
  Point32 shiftedOrigin = face->origin + shift;
  int64_t shiftedDot = shiftedOrigin.dot(normal);
  G_ASSERT(shiftedDot <= origDot);
  ...
  if (cmp >= 0)
  {
    ...
    do
    {
      ...
      G_ASSERT(dot.compare(origDot) <= 0);
#ifdef DEBUG_CONVEX_HULL
  ...
  while (true)
  {
...
#endif
      e = e->reverse->prev;
      G_ASSERT(e != intersection->reverse);
    ...
    if (cmp > 0)
    {
      ...
      v->point128 = PointR128(Int128::mul(face->dir0.x * r0, m11) - Int128::mul(face->dir0.x * r1, m01) +
        ...
        Int128::mul(face->dir0.y * r0, m11) - Int128::mul(face->dir0.y * r1, m01) + Int128::mul(face->dir1.y * r1, m00) -
          Int128::mul(face->dir1.y * r0, m10) + det * shiftedOrigin.y,
        Int128::mul(face->dir0.z * r0, m11) - Int128::mul(face->dir0.z * r1, m01) + Int128::mul(face->dir1.z * r1, m00) -
...
static int getVertexCopy(ConvexHullInternal::Vertex *vertex, Tab<ConvexHullInternal::Vertex *> &vertices)
{
...
float ConvexHullComputer::compute(const float *__restrict coords, int stride, int count, float shrink, float shrinkClamp,
  bool only_vertex)
{
  vertices.resize(0);
  edges.resize(0);
  ...
  hull.compute(coords, false, stride, count);
  ...
  edges.resize(0);
  faces.resize(0);
  Tab<ConvexHullInternal::Vertex *> oldVertices;
