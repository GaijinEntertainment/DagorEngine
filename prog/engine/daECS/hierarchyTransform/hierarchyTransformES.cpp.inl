...
template <typename Callable>
static inline void collect_to_resolve_hierarchy_ecs_query(ecs::EntityManager &manager, Callable);

template <typename Callable>
static inline void resolve_hierarchy_ecs_query(ecs::EntityManager &manager, Callable);

template <typename Callable>
static inline void reresolve_hierarchy_ecs_query(ecs::EntityManager &manager, Callable);
  ...
  ecs::EntityManager &manager)
{
  ...
  collect_to_resolve_hierarchy_ecs_query(manager, [&](ecs::EntityId eid, int &hierarchy_unresolved_id) {
  ...
  resolve_hierarchy_ecs_query(manager, [&](ecs::EntityId eid, int &hierarchy_unresolved_parent_id, ecs::EntityId &hierarchy_parent) {
  ...
  reresolve_hierarchy_ecs_query(manager, [&](int &hierarchy_unresolved_parent_id, ecs::EntityId &hierarchy_parent) {
    if (hierarchy_unresolved_parent_id != 0)
    {
      auto it = hiearchy_unresolved_map.find(hierarchy_unresolved_parent_id);
      if (it != hiearchy_unresolved_map.end())
