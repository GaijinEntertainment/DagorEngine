...
struct QueryContainer
{
  ...
  {
    if (entitiesCnt >= entitiesCntEnd) [[unlikely]]
  ...
  {
    if (size_t(c) > static_cast<size_t>(componentsDataEnd - componentsData)) [[unlikely]]
...
inline void EntityManager::performQueryES(QueryId h, Cb fun, const CbArg &__restrict arg, void *user_data, int min_quant)
{
  ...
  if (DAGOR_UNLIKELY(min_quant && maxNumJobs && querySize > min_quant * 2)) // doesn't make sense to run multithreading if there is
    ...
    if (performMTQuery(query, [&fun, &arg](const QueryView &qv) { fun(arg, qv); }, user_data, min_quant))
      return;
  performSTQuery(query, user_data, [&fun, &arg](const QueryView &__restrict qv) { fun(arg, qv); });
}

template <typename Cb, class CbArg>
  requires eastl::is_invocable_v<Cb, const CbArg &, const QueryView &>
__forceinline void EntityManager::performQueryEmptyAllowed(QueryId h, Cb fun, const CbArg &arg, void *user_data, int min_quant)
{
  if (h)
    performQueryES(h, fun, arg, user_data, min_quant);
  else
    fun(arg, QueryView(*this, user_data));
}
