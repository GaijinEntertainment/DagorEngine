...
struct hash<Callstack>
{
  ...
  {
    return mem_hash_fnv1a((const char *)st.data(), sizeof(Callstack::value_type) * st.max_size());
  }
};
...
struct TrackAccessRecordRAIILock
{
  ...
  TrackAccessRecordRAIILock(std::recursive_mutex &_mtx, bool constrained_mt) : mtx(constrained_mt ? &_mtx : nullptr)
  {
...
{
  TrackAccessRecordRAIILock scopeLock(track_mutex, isConstrainedMTMode());
  ...
  ecs::EntityId eid) const
{
  ...
  rec.op = op;
  rec.details = details;
  rec.templateId = getEntityTemplateId(eid);
...
void ecs::EntityManager::stopTrackComponentsAndDump()
{
  ...
  for (const auto &kv : records_by_component)
  {
    ...
    debug("====[ %s ]====", getDataComponents().findComponentName(kv.first));
    ...
    for (const auto &kv : recordsByFrame)
    {
      ...
      for (const auto &r : curRecords)
      {
        ...
        if (r.templateId != ecs::INVALID_TEMPLATE_INDEX)
          report.append_sprintf(" (%s)", getTemplateName(r.templateId));
