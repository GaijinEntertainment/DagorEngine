...
void EntityManager::broadcastEventImmediate(Event &evt)
{
  ...
  if (esListIt != esEvents.end())
    for (es_index_type esListNo : esListIt->second)
    {
      ...
      {
        TIME_SCOPE_ES(es);
        performQueryEmptyAllowed(esListQueries[esListNo], es.ops.onEvent, evt, es.userData, es.quant);
      }
      else
        performQueryEmptyAllowed(esListQueries[esListNo], es.ops.onEvent, evt, es.userData, es.quant);
    }
...
#endif
  DAECS_EXT_ASSERTF(esUpdates.size() < sizeof(uint32_t) * 8, "esUpdates.size()=%d, 1<<size is UB for size>=32", (int)esUpdates.size());
...
void EntityManager::processAllEventsInStorage(T &storage, F &&f)
{
  for (auto bi = storage.buffers.begin(); !storage.buffers.empty(); bi = storage.buffers.erase(bi))
  {
    for (;;)
      if (!processEventInternal(*bi, f))
        break;
  }
  for (;;)
    if (!processEventInternal(storage.active, f))
      break;
}
...
{
  processAllEventsInStorage(storage, [](EntityId, Event &) {});
}
...
template <class T>
void EntityManager::destroyAndLogEvents(T &storage, uint32_t cnt)
{
  processAllEventsInStorage(storage, [&](EntityId eid, Event &evt) {
    if (cnt != 0)
    {
      const EventsDB::event_id_t evtId = getEventsDb().findEvent(evt.getType());
      debug_("<%s>%@<%s> ", evtId != EventsDB::invalid_event_id ? eventDb.getEventName(evtId) : "unknown", eid,
...
template void EntityManager::destroyAndLogEvents<decltype(EntityManager::LoadingEntityEvents::events)>(
  decltype(EntityManager::LoadingEntityEvents::events) &storage, uint32_t);
template void EntityManager::destroyAndLogEvents<decltype(EntityManager::eventsStorage)>(
  decltype(EntityManager::eventsStorage) &storage, uint32_t);
