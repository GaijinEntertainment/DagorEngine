...
public:
  ...
  {
    SystemIndex relay = getPeerIface().GetRelayConnection();
    if (relay != SystemIndex(0))
      getPeerIface().CloseConnection(relay);
  }

  virtual bool establishedRelayConnection() { return getPeerIface().HasRelayConnection(); }
  virtual void setRelayConnectionHandler(void (*relayConnectionHandler)(bool))
  {
    getPeerIface().SetRelayConnectionHandler(relayConnectionHandler);
  }
...
INetDriver *create_net_driver_startup()
{
  auto drv = create_net_driver_common();
