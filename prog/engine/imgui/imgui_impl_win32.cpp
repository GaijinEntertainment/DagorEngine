...
// Significantly modified to work with Dagor event processing for the main window.
...
extern void *imgui_main_window_override;
...
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead
// of multiple Dear ImGui contexts.
...
static ImGui_ImplWin32_Data *ImGui_ImplWin32_GetBackendData(ImGuiIO &io) { return (ImGui_ImplWin32_Data *)io.BackendPlatformUserData; }
...
class ImGuiMultiviewThread final : public DaThread
{
public:
  typedef eastl::fixed_function<sizeof(void *) * 4, void()> function_t;
  function_t codeToRun;
  os_event_t jobEvent, doneEvent;
  bool initialized = false;
  bool useThread = false;

  void init(bool use_thread)
  {
    if (initialized)
      return;
    os_event_create(&jobEvent);
    os_event_create(&doneEvent);
    initialized = true;
    useThread = use_thread;
  }

  ~ImGuiMultiviewThread()
  {
    if (initialized)
    {
      os_event_destroy(&jobEvent);
      os_event_destroy(&doneEvent);
    }
  }

  void runInThread(function_t &&code)
  {
    if (useThread)
    {
      codeToRun = eastl::move(code);
      os_event_set(&jobEvent);
      os_event_wait(&doneEvent, OS_WAIT_INFINITE);
    }
    else
    {
      code();
  ...
  void execute() override
  {
    ...
    {
      if (os_event_wait(&jobEvent, OS_WAIT_INFINITE) != OS_WAIT_OK)
        continue;
      ...
      if (codeToRun)
      {
        codeToRun();
        codeToRun = nullptr;
        os_event_set(&doneEvent);
      }
    }
  }
};
static ImGuiMultiviewThread multiview_thread;
...
bool ImGui_Multiview_Init()
{
  return ImGui_ImplWin32_InitEx(imgui_main_window_override ? imgui_main_window_override : win32_get_main_wnd(), false);
}

void ImGui_Multiview_Shutdown()
{
...
{
  // Left & right Shift keys: when both are pressed together, Windows tend to not generate the WM_KEYUP event for the first released
  // one.
...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiIO &io) { IM_UNUSED(io); }
...
void ImGui_Multiview_NewFrame()
{
  ...
  if (multiview_thread.useThread)
  {
    multiview_thread.runInThread([]() {
      static MSG msg;
...
// When implementing your own backend, you can read the io.WantCaptureMouse, io.WantCaptureKeyboard flags to tell if Dear ImGui wants
// to use your inputs.
// - When io.WantCaptureMouse is true, do not dispatch mouse input data to your main application, or clear/overwrite your copy of the
// mouse data.
// - When io.WantCaptureKeyboard is true, do not dispatch keyboard input data to your main application, or clear/overwrite your copy of
// the keyboard data. Generally you may always pass all inputs to Dear ImGui, and hide them from your application based on those two
// flags. PS: We treat DBLCLK messages as regular mouse down messages, so this code will work on windows classes that have the
...
typedef DPI_AWARENESS_CONTEXT(WINAPI *PFN_SetThreadDpiAwarenessContext)(DPI_AWARENESS_CONTEXT); // User32.lib + dll, Windows 10 v1607+
                                                                                                // (Creators Update)
...
// If you are new to dear imgui or creating a new binding for dear imgui, it is recommended that you completely ignore this section
// first..
//--------------------------------------------------------------------------------------------------------
...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport *viewport)
{
  ...
  {
    multiview_thread.runInThread([bd, vd]() {
...
static void ImGui_ImplWin32_SetWindowPos(ImGuiViewport *viewport, ImVec2 pos)
{
  ImGui_ImplWin32_ViewportData *vd = (ImGui_ImplWin32_ViewportData *)viewport->PlatformUserData;
  IM_ASSERT(vd->Hwnd != 0);
  multiview_thread.runInThread([viewport, vd, pos]() {
...
static void ImGui_ImplWin32_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
  ...
  multiview_thread.runInThread([viewport, vd, &rect]() {
    if (viewport->Flags & ImGuiViewportFlags_OwnedByApp)
      ImGui_ImplWin32_UpdateWin32StyleFromWindow(viewport);         // Not our window, poll style before using
    ::AdjustWindowRectEx(&rect, vd->DwStyle, FALSE, vd->DwExStyle); // Client to Screen
...
static void ImGui_ImplWin32_SetWindowFocus(ImGuiViewport *viewport)
{
  ImGui_ImplWin32_ViewportData *vd = (ImGui_ImplWin32_ViewportData *)viewport->PlatformUserData;
  IM_ASSERT(vd->Hwnd != 0);
  multiview_thread.runInThread([vd]() {
...
static void ImGui_ImplWin32_SetWindowTitle(ImGuiViewport *viewport, const char *title)
{
  ...
  multiview_thread.runInThread([vd, &title_w]() {
...
static void ImGui_ImplWin32_SetWindowAlpha(ImGuiViewport *viewport, float alpha)
{
  ImGui_ImplWin32_ViewportData *vd = (ImGui_ImplWin32_ViewportData *)viewport->PlatformUserData;
  IM_ASSERT(vd->Hwnd != 0);
  multiview_thread.runInThread([vd, alpha]() {
  ...
  else if (ImGuiViewport *viewport = ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io, hWnd))
  {
    switch (msg)
    {
      ...
      case WM_NCHITTEST:
        ...
        // optional). The ImGuiViewportFlags_NoInputs flag is set while dragging a viewport, as want to detect the window behind the
        // one we are dragging. If you cannot easily access those viewport flags from your windowing/event code: you may manually
...
static void ImGui_ImplWin32_InitMultiViewportSupport(bool platform_has_own_dc)
{
  ...
  // Use threaded window handling in games, use single-threaded mode in tools.
  multiview_thread.init(/*use_thread = */ imgui_main_window_override == nullptr);
  if (multiview_thread.useThread)
...
static void ImGui_ImplWin32_ShutdownMultiViewportSupport()
{
  ...
  multiview_thread.terminate(false);
  os_event_set(&multiview_thread.jobEvent);
}
