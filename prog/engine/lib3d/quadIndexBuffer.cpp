...
{
  box16bit = d3d::create_ib(36 * sizeof(uint16_t), 0, "ib_box_16bit", RESTAG_QUAD_INDEX_BUFFER);
  d3d_err(box16bit);
...
static void try_to_init_quads_16bit(const bool modify_counter = true)
{
  ...
  quads16bit = d3d::create_ib(INDICES_COUNT * sizeof(uint16_t), 0, "ib_quads_16bit", RESTAG_QUAD_INDEX_BUFFER);
  d3d_err(quads16bit);
...
static void try_to_init_quads_32bit(const bool modify_counter = true)
{
  if (quads32bitIndicesCount == 0)
    return;
  Ibuffer *biggerBuffer =
    d3d::create_ib(quads32bitIndicesCount * sizeof(uint32_t), SBCF_INDEX32, "ib_quads_32bit", RESTAG_QUAD_INDEX_BUFFER);
