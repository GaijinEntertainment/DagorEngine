...
bool ScriptedShaderMaterial::hasVariable(const int variable_id) const
{
  if (variable_id < 0 || variable_id >= ScriptedShadersBinDump::MAX_VARS)
    return false;
  int vid = props.shBinDump().varIdx[variable_id];
  return (vid < ScriptedShadersBinDump::VARIDX_ABSENT) ? props.sclass->localVars.findVar(vid) >= 0 : false;
}
...
const String ShaderMaterialProperties::getInfo() const
{
  ...
  for (j = 0; j < 3; j++)
  {
    info.aprintf(128, "%d='%s' ", textureId[j], get_managed_texture_name(textureId[j]));
  }

  info.aprintf(128, "f=%d ", (int)matflags);
  for (j = 0; j < stvar.size(); j++)
  {
    ...
    switch (sclass->localVars.v[j].type)
    {
      ...
      case SHVT_TEXTURE: info.aprintf(128, "(%d) ", stvar[j].texId); break;
