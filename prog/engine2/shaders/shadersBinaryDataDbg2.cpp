...
static void dumpVariantTable(const shaderbindump::VariantTable &vt, int indent, int /*code_num*/, ILocalDumpVariantData2 &vdump)
{
  using namespace shaderbindump;
...
static inline bool shader_passes_used(const shaderbindump::ShaderCode::Pass &p)
{
...
static inline bool shader_code_used(const shaderbindump::ShaderCode &code)
{
  ...
  for (int i = 0; i < code.passes.size(); i++)
    if (shader_passes_used(code.passes[i]))
      return true;
  return false;
}

static inline bool shader_passes_allused(const shaderbindump::ShaderCode::Pass &p)
{
  static constexpr int BAD = shaderbindump::ShaderCode::INVALID_FSH_VPR_ID;
    ...
    virtual void dump(int idx, const shaderbindump::VariantTable &vt, int vt_code)
    {
      ...
      public:
        ...
        virtual bool pretest(int idx)
        {
          ...
          if (idxShown.hasInt(idx))
            return true;
          return !shader_passes_allused(code.passes[idx]);
        }
        virtual void dump(int idx, const shaderbindump::VariantTable &vt, int vt_code)
        {
          if (!idxShown.addInt(idx))
      ...
      dumpVariantTable(code.dynVariants, 4, code.passes.size(), dynvardump);
    }
  };

  DumpStVar statvardump(cls);
  dumpVariantTable(cls.stVariants, 0, cls.code.size(), statvardump);
}
...
static shaderbindump::ShaderVariant dump_shader_stat(const shaderbindump::ShaderClass &cls, bool used_variants)
{
  using namespace shaderbindump;
  ...
  result.name = bindump::string(cls.name.data());
  if (!shader_used && !used_variants)
    return result;
  ...
  public:
    DumpStVar(const shaderbindump::ShaderClass &c, ShaderVariant &r) : cls(c), result(r) {}
    ...
    virtual void dump(int idx, const shaderbindump::VariantTable &vt, int vt_code)
    {
      ...
      if (!shader_code_used(code))
      {
        auto intervals = decodeVariantEx(vt, vt_code);
      ...
      public:
        DumpDynVar(const ShaderCode &c, ShaderVariant &r) : code(c), result(r) {}
      ...
      auto intervals = decodeVariantEx(vt, vt_code);
      result.intervals.insert(result.intervals.end(), intervals.begin(), intervals.end());
      DumpDynVar dynvardump(code, result);
      dumpVariantTable(code.dynVariants, 4, code.passes.size(), dynvardump);
    }
  };
...
{
  shaderbindump::ShaderStatistics stat;
  for (int i = 0; i < shBinDump().classes.size(); i++)
  {
    stat.usedVariants.emplace_back(dump_shader_stat(shBinDump().classes[i], true));
    stat.unusedVariants.emplace_back(dump_shader_stat(shBinDump().classes[i], false));
  }

  bindump::FileWriter writer(filename);
  bindump::streamWrite(stat, writer);
}
