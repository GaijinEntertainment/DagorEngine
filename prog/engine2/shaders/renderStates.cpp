...
static RenderState apply_override(RenderState state, const OverrideState &override_state)
{
  ...
  {
    state.ablend = 1;
    state.ablendSrc = override_state.sblend;
    state.ablendDst = override_state.dblend;
  }
  ...
  if (override_state.isOn(OverrideState::BLEND_OP))
    state.blendOp = override_state.blendOp;
  if (override_state.isOn(OverrideState::BLEND_OP_A))
    state.sepablendOp = override_state.blendOpA;
  if (override_state.isOn(OverrideState::BLEND_SRC_DEST_A))
  {
    state.sepablend = 1;
    state.sepablendSrc = override_state.sblenda;
    state.sepablendDst = override_state.dblenda;
  }
