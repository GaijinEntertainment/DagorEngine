  ...
  virtual bool processCommand(const char *argv[], int argc)
  {
    ...
    CONSOLE_CHECK_NAME_EX("render", "reload_shaders", 1, 3, "", "[use_fence] [shaders_bindump_name]")
    {
      ...
      for (int i = 0; i < sizeof(ver) / sizeof(ver[0]); i++)
        if ((dsc.fshver & ver[i].flg)) // && dd_file_exist(String(0, "%s.%s.shdump.bin", shname, ver[i].suf))
        {
          bool reloaded = useFence ? load_shaders_bindump_with_fence(shname ? shname : last_loaded_shaders_bindump(false), ver[i].ver)
                                   : load_shaders_bindump(shname, ver[i].ver);
          if (reloaded)
          {
            console::print_d("reloaded %s, ver=%s", last_loaded_shaders_bindump(false), ver[i].suf);
            return true;
          }
          console::print_d("failed to reload %s, ver=%d (%s)", last_loaded_shaders_bindump(false), ver[i].ver, ver[i].suf);
        }
