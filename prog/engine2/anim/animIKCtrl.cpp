...
void AnimV20::LegsIKCtrl::process(IPureAnimStateHolder &st, real wt, GeomNodeTree &tree, AnimPostBlendCtrl::Context &ctx)
{
  ...
  for (int i = 0; i < rec.size(); i++)
  {
    ...
    if (as_point3(&leg_wtm.col3).y > as_point3(&foot_wtm.col3).y)
    {
      float maxDist = rec[i].maxFootUp + 0.15;
      float footUpOfs = min(rec[i].maxFootUp, (as_point3(&leg_wtm.col3).y - as_point3(&foot_wtm.col3).y - 0.15f) * 0.9f);

      Point3_vec4 pt;
      v_st(&pt, v_add(v_mat44_mul_vec3p(tree.getRootWtmRel(), v_and(foot_p1, v_cast_vec4f(V_CI_MASK1010))), wofs));
      pt.y += footUpOfs;
      if (ctx.irq(GIRQT_TraceFootStepDown, (intptr_t)(void *)&pt, (intptr_t)(void *)&maxDist, (intptr_t)(void *)&trace_h1) ==
          GIRQR_TraceOK)
        trace_h1 -= footUpOfs;

      v_st(&pt, v_add(v_mat44_mul_vec3p(tree.getRootWtmRel(), v_and(foot_p2, v_cast_vec4f(V_CI_MASK1010))), wofs));
      pt.y += footUpOfs;
      if (ctx.irq(GIRQT_TraceFootStepDown, (intptr_t)(void *)&pt, (intptr_t)(void *)&maxDist, (intptr_t)(void *)&trace_h2) ==
          GIRQR_TraceOK)
        trace_h2 -= footUpOfs;
      else
        trace_h2 = trace_h1;
    ...
    if (alwaysSolve || nodes[i].dy || move_foot)
    {
      ...
      as_point4(&foot_wtm.col3).y += nodes[i].dy;
