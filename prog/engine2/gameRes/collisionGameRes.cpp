...
bool VECTORCALL CollisionResource::traceRayMeshNodeLocal(const CollisionNode &node, vec4f v_local_from, vec4f v_local_dir,
...
bool VECTORCALL CollisionResource::traceQuad(const CollisionNodeFilter &filter, vec3f a00, vec3f a01, vec3f a10, vec3f a11,
...
DAGOR_NOINLINE bool VECTORCALL CollisionResource::traceRayMeshNodeLocalCullCCW(const CollisionNode &node,
...
DAGOR_NOINLINE bool VECTORCALL CollisionResource::traceCapsuleMeshNodeLocalCullCCW(const CollisionNode &node, const vec4f v_local_from,
...
template <bool check_bounding>
DAGOR_NOINLINE bool VECTORCALL CollisionResource::traceRayMeshNodeLocalAllHits(const CollisionNode &node,
  const vec4f &v_local_from, // better to hold it in memory
  const vec4f &v_local_dir,  // it prevents inefficient loop optimizations
...
template <bool check_bounding>
DAGOR_NOINLINE bool VECTORCALL CollisionResource::rayHitMeshNodeLocalCullCCW(const CollisionNode &node, const vec3f &v_local_from,
...
DAGOR_NOINLINE bool VECTORCALL CollisionResource::capsuleHitMeshNodeLocalCullCCW(const CollisionNode &node, vec4f v_local_from,
...
bool VECTORCALL CollisionResource::rayHit(const mat44f &tm, vec3f v_from, vec3f v_dir, float in_t, int &out_mat_id) const
{
  uint8_t behaviorFilter = CollisionNode::TRACEABLE;
...
bool VECTORCALL CollisionResource::rayHit(const TMatrix &instance_tm, const GeomNodeTree *geom_node_tree, const Point3 &from,
  ...
  Point3 &out_norm, float &out_depth)
{
  ...
  while (cur < end)
  {
    ...
    {
      const Point3 cross = (corner2 - corner0) % (corner1 - corner0);
