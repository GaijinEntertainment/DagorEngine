...
bool VECTORCALL CollisionResource::traceRayMeshNodeLocalCullCCW_AVX256(const CollisionNode &node, const vec4f &v_local_from,
  const vec4f &v_local_dir, float &in_out_t, vec4f *v_out_norm)
{
  ...
  for (i = 0; EASTL_LIKELY(int(i) < int(indicesSize - (mainBatchSize * 3 - 1))); i += mainBatchSize * 3)
  {
    ...
    for (uint32_t j = 0; j < mainBatchSize; j++)
    {
      ...
      vert[j][2] = v_ld(&vertices[indices[i + j * 3 + 1]].x);
      vert[j][1] = v_ld(&vertices[indices[i + j * 3 + 2]].x);
    }
...
#pragma unroll(4)
    ...
    for (uint32_t j = i; j < indicesSize && count < finalizeBatchSize; j += 3)
    {
      ...
      vert[count][2] = v_ld(&vertices[indices[j + 1]].x);
      vert[count][1] = v_ld(&vertices[indices[j + 2]].x);
      count++;
    }
  ...
  if (resultIdx >= 0 && v_out_norm)
  {
    ...
    vec4f v2 = v_ld(&vertices[indices[resultIdx + 1]].x);
    vec4f v1 = v_ld(&vertices[indices[resultIdx + 2]].x);
...
template <bool check_bounding>
bool VECTORCALL CollisionResource::rayHitMeshNodeLocalCullCCW_AVX256(const CollisionNode &node, const vec4f &v_local_from,
...
template <bool check_bounding>
bool VECTORCALL CollisionResource::traceRayMeshNodeLocalAllHits_AVX256(const CollisionNode &node, const vec4f &v_local_from,
      ...
      for (uint32_t j = 0; j < mainBatchSize; j++, ret >>= 1)
      {
        if (EASTL_UNLIKELY(ret & 1u))
        {
          ...
          if (calc_normal)
          {
            ...
            vec4f v2 = v_ld(&vertices[indices[i + j * 3 + 1]].x);
            vec4f v1 = v_ld(&vertices[indices[i + j * 3 + 2]].x);
