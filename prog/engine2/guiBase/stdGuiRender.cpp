...
static float dynfont_compute_str_visbox_u_cached(const StdGuiFontContext &fctx, const wchar_t *str, int len, BBox2 &out_bb)
{
  // really we do not caching here because this func is very rarely called (never in WT & Enlisted)
  strboxcache::strVisBoxCnt++;
  return fctx.font->dynfont_compute_str_visbox_u(fctx.fontHt, str, len, 1.0f, fctx.spacing, out_bb);
}

static BBox2 get_char_vis_bbox(DagorFontBinDump *font, const DagorFontBinDump::GlyphData &g)
{
  BBox2 box;
...
BBox2 get_char_visible_bbox_u(wchar_t c, const StdGuiFontContext &fctx)
{
  ...
  BBox2 box;
  dynfont_compute_str_visbox_u_cached(fctx, &c, 1, box);
  return box;
}
...
// get string bounding box
BBox2 get_str_visible_bbox_u(const wchar_t *str, int len, const StdGuiFontContext &fctx)
{
  G_ASSERT(fctx.font);
  ...
  {
    if (fctx.monoW)
      fctx.font->dynfont_update_bbox_y(fctx.fontHt, box, true);
    else
      dynfont_compute_str_visbox_u_cached(fctx, str, len, box);
    return box;
  }
  ...
  while (*str && len)
  {
    ...
    b = get_char_vis_bbox(fctx.font, g);
    b[0] += pos;
    b[1] += pos;
    box += b;
...
BBox2 get_str_visible_bbox(const char *str, int len, const StdGuiFontContext &fctx)
{
  G_ASSERT(fctx.font);
  ...
  CVT_TO_UTF16_ON_STACK(tmpU16, str, len);
  replace_tabs_with_zwspaces(tmpU16);
  return get_str_visible_bbox_u(tmpU16, len, fctx);
}
...
static BBox2 get_str_real_visible_bbox_u(const wchar_t *str, int len, const StdGuiFontContext &fctx, float &dx)
{
  ...
  if (fctx.font->isFullyDynamicFont())
  {
    if (fctx.monoW)
      ...
      dx = dynfont_compute_str_visbox_u_cached(fctx, str, len, box);
    return box;
  }
...
void start_font_str(float s) { stdgui_context.start_font_str(s); }
}; // namespace StdGuiRender
