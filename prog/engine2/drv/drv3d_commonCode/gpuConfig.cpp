  ...
  const DagorDateTime &driver_date, GpuDriverConfig &out_cfg)
{
  ...
  debug("Mac nvidia check: model=%s web driver=%d code=%c%c%c%c", macModel.str(), macWebDriver, _DUMP4C(video.drvCode));
#endif
}
...
{
#if _TARGET_PC_WIN
  if (video.drvCode == _MAKE4C('DX12') || video.drvCode == _MAKE4C('VULK'))
    return;
#endif
  ...
  if (gpu_driver_config.integrated && activeVendor == D3D_VENDOR_INTEL && video.drvCode == _MAKE4C('DX11'))
  {
