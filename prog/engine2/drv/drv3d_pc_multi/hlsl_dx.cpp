...
static ID3D10Blob *compile_hlsl(const char *hlsl_text, const char *entry, const char *profile, String *out_err)
{
  ...
  if (!d3d_compile)
  {
    ...
    {
      d3d_compile = (pD3DCompile)GetProcAddress(d3dcompiler_module.get(), D3DCOMPILE_FUNC_NAME);
      if (!d3d_compile)
      {
        d3dcompiler_module.reset();
...
{
  unsigned code = d3d::get_driver_code();
  if (code == _MAKE4C('DX11'))
  {
    int max_const = 128;
