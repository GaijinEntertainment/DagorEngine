...
class PipelineVariant
{
  ...
  bool create(ID3D12Device2 *device, BasePipeline &base, PipelineCache &pipe_cache, const InputLayout &input_layout,
...
#if !_TARGET_XBOXONE
  bool createMesh(ID3D12Device2 *device, BasePipeline &base, PipelineCache &pipe_cache, bool is_wire_frame,
...
public:
  ...
  bool load(ID3D12Device2 *device, BasePipeline &base, PipelineCache &pipe_cache, const InputLayout &input_layout, bool is_wire_frame,
    const RenderStateSystem::StaticState &static_state, const FramebufferLayout &fb_layout, D3D12_PRIMITIVE_TOPOLOGY_TYPE top,
    RecoverablePipelineCompileBehavior on_error, bool give_name);
  bool loadMesh(ID3D12Device2 *device, BasePipeline &base, PipelineCache &pipe_cache, bool is_wire_frame,
    const RenderStateSystem::StaticState &static_state, const FramebufferLayout &fb_layout,
    RecoverablePipelineCompileBehavior on_error, bool give_name);
...
class BasePipeline
{
  ...
  const VertexShaderModule &vsModule;
  const PixelShaderModule &psModule;
  GraphicsProgramID program;
  GraphicsPipelineBaseCacheId cacheId;

public:
  BasePipeline(ID3D12Device2 *device, GraphicsProgramID p, GraphicsPipelineSignature &s, PipelineCache &cache,
    const eastl::vector<InputLayout> &input_layout_list, const eastl::vector<RenderStateSystem::StaticState> &static_state_list,
    FramebufferLayoutManager &framebuffer_layouts, const VertexShaderModule &vsm, const PixelShaderModule &psm,
    RecoverablePipelineCompileBehavior on_error, bool give_name) :
    program(p), signature(s), vsModule(vsm), psModule(psm)
  {
    PipelineCache::BasePipelineIdentifier cacheIdent = {};
    cacheIdent.vs = vsModule.ident.shaderHash;
    vsCombinedSRegisterCompareMask = vsModule.header.sRegisterCompareUseMask;
    eastl::copy(eastl::begin(vsModule.header.tRegisterTypes), eastl::end(vsModule.header.tRegisterTypes),
      eastl::begin(vsCombinedTRegisterTypes));
    eastl::copy(eastl::begin(vsModule.header.uRegisterTypes), eastl::end(vsModule.header.uRegisterTypes),
      eastl::begin(vsCombinedURegisterTypes));
    cacheIdent.ps = psModule.ident.shaderHash;
    if (auto gsModule = vsModule.geometryShader.get())
    {
      cacheIdent.gs = gsModule->ident.shaderHash;
      vsCombinedSRegisterCompareMask |= gsModule->header.sRegisterCompareUseMask;
      eastl::transform(eastl::begin(vsCombinedTRegisterTypes), eastl::end(vsCombinedTRegisterTypes),
        eastl::begin(gsModule->header.tRegisterTypes), eastl::begin(vsCombinedTRegisterTypes), [](auto a, auto b) { return a | b; });
      eastl::transform(eastl::begin(vsCombinedURegisterTypes), eastl::end(vsCombinedURegisterTypes),
        eastl::begin(gsModule->header.uRegisterTypes), eastl::begin(vsCombinedURegisterTypes), [](auto a, auto b) { return a | b; });
    }
    if (auto hsModule = vsModule.hullShader.get())
    {
      cacheIdent.hs = hsModule->ident.shaderHash;
      vsCombinedSRegisterCompareMask |= hsModule->header.sRegisterCompareUseMask;
      eastl::transform(eastl::begin(vsCombinedTRegisterTypes), eastl::end(vsCombinedTRegisterTypes),
        eastl::begin(hsModule->header.tRegisterTypes), eastl::begin(vsCombinedTRegisterTypes), [](auto a, auto b) { return a | b; });
      eastl::transform(eastl::begin(vsCombinedURegisterTypes), eastl::end(vsCombinedURegisterTypes),
        eastl::begin(hsModule->header.uRegisterTypes), eastl::begin(vsCombinedURegisterTypes), [](auto a, auto b) { return a | b; });
    }
    if (auto dsModule = vsModule.domainShader.get())
    {
      cacheIdent.ds = dsModule->ident.shaderHash;
      vsCombinedSRegisterCompareMask |= dsModule->header.sRegisterCompareUseMask;
      eastl::transform(eastl::begin(vsCombinedTRegisterTypes), eastl::end(vsCombinedTRegisterTypes),
        eastl::begin(dsModule->header.tRegisterTypes), eastl::begin(vsCombinedTRegisterTypes), [](auto a, auto b) { return a | b; });
      eastl::transform(eastl::begin(vsCombinedURegisterTypes), eastl::end(vsCombinedURegisterTypes),
        eastl::begin(dsModule->header.uRegisterTypes), eastl::begin(vsCombinedURegisterTypes), [](auto a, auto b) { return a | b; });
    }
    ...
    if (cnt > 0)
    {
      ...
      while (i < cnt)
      {
        ...
        if (!weAreMesh)
        {
          auto inputLayoutListEntry = eastl::find_if(begin(input_layout_list), end(input_layout_list),
            [&inputLayout](auto &l) //
            { return l == inputLayout; });
          if (inputLayoutListEntry == end(input_layout_list))
          {
            logerr("DX12: Graphics pipeline cache contains entry with input layout that is not in "
                   "the runtime input layout list, removing from cache and skipping");
            cnt = cache.removeGraphicsPipelineVariant(cacheId, i);
            continue;
          }

          auto &variant =
            getVariantFromConfiguration(InternalInputLayoutID{static_cast<int>(inputLayoutListEntry - begin(input_layout_list))},
          ...
          if (!variant.load(device, *this, cache, inputLayout, isWireFrame, staticState, framebufferLayout, top, on_error, give_name))
          {
            cnt = cache.removeGraphicsPipelineVariant(cacheId, i);
            continue;
          }
        ...
        else
        {
          ...
          if (!variant.loadMesh(device, *this, cache, isWireFrame, staticState, framebufferLayout, on_error, give_name))
          {
            cnt = cache.removeGraphicsPipelineVariant(cacheId, i);
            continue;
          }
  ...
  bool hasTessellationStage() const { return static_cast<bool>(vsModule.hullShader); }
  bool hasGeometryStage() const { return static_cast<bool>(vsModule.geometryShader); }
  bool isMesh() const { return is_mesh(vsModule); }
  D3D_PRIMITIVE getHullInputPrimitiveType() const { return static_cast<D3D_PRIMITIVE>(vsModule.hullShader->header.inputPrimitive); }
  const dxil::ShaderHeader &getPSHeader() const { return psModule.header; }
...
public:
  ...
  void registerInputLayout(InternalInputLayoutID ident, const InputLayout &layout)
  {
    if (ident.get() == inputLayouts.size())
    {
      inputLayouts.push_back(layout);
    }
    else
    {
      fatal("DX12: registerInputLayout was called with ident of %u but table has %u entries", ident.get(), inputLayouts.size());
    }
  }

  const InputLayout &getInputLayout(InternalInputLayoutID ident) { return inputLayouts[ident.get()]; }
};
