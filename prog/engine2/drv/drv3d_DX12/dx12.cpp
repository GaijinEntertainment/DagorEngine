...
struct ApiState
{
  ...
  void adjustCaps()
  {
    ...
    driverDesc.vprogver = FSHVER_66;
    driverDesc.maxprims = 0x7FFFFFFF;
    driverDesc.maxvertind = 0x7FFFFFFF;
    driverDesc.upixofs = 0.f;
    driverDesc.vpixofs = 0.f;
    driverDesc.fshver = DDFSH_4_0 | DDFSH_4_1 | DDFSH_5_0 | DDFSH_6_0 | DDFSH_6_6;
    driverDesc.cshver = DDCSH_4_0 | DDCSH_5_0 | DDCSH_6_0 | DDCSH_6_6;
  ...
  const char *title, Driver3dInitCallback *cb)
{
  ...
  auto init_device = [&](ComPtr<IDXGIAdapter1> adapter1, ComPtr<IDXGIOutput> output) {
    ...
    if (
      api_state.device.init(api_state.dxgi14.Get(), {eastl::move(adapter1), info}, featureLevel, api_state.d3d12Env, eastl::move(sci),
        api_state.debugState, api_state.shaderProgramDatabase, deviceCfg, dxCfg, cb ? cb->desiredStereoRender() : false))
    {
      char strBuffer[sizeof(DXGI_ADAPTER_DESC1::Description) * 2 + 1];
      const size_t size = wcstombs(strBuffer, info.Description, sizeof(strBuffer));
      strBuffer[size] = '\0';
...
unsigned d3d::get_driver_code() { return _MAKE4C('DX12'); }
...
int d3d::driver_command(int command, void *par1, void *par2, void *par3)
{
  ...
  switch (command)
  {
    ...
    case DRV3D_COMMAND_FLUSH_STATES:
    {
      ...
      api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::ALL);
    ...
    case DRV3D_COMMAND_REGISTER_SHADER_DUMP:
      api_state.shaderProgramDatabase.registerShaderBinDump(api_state.device, reinterpret_cast<ScriptedShadersBinDumpOwner *>(par1));
      return 1;
    ...
    case DRV3D_COMMAND_GET_VIDEO_MEMORY_BUDGET:
      return api_state.device.getGpuMemUsageStats(reinterpret_cast<uint32_t *>(par1), reinterpret_cast<uint32_t *>(par2),
...
bool d3d::set_program(PROGRAM prog_id)
{
  ...
  if (ProgramID::Null() != prog)
  {
    if (prog.isCompute())
    {
      api_state.state.setComputeProgram(prog, api_state.shaderProgramDatabase.getComputeProgramResourceUsage(prog));
    }
#if D3D_HAS_RAY_TRACING
    else if (prog.isRaytrace())
    {
      api_state.state.setRaytraceProgram(prog, api_state.shaderProgramDatabase.getRaytracingProgramResourceUsage(prog));
    }
#endif
    else
    {
...
bool d3d::set_depth(Texture *tex, bool read_only)
{
  CHECK_MAIN_THREAD();
  if (!tex)
  {
  ...
  else
  {
    ...
    api_state.state.setDepthStencilTarget(api_state.device.getContext(), texture, 0, read_only);
  }
  return true;
}
...
bool d3d::set_depth(BaseTexture *tex, int layer, bool read_only)
{
  ...
  else
  {
    ...
    api_state.state.setDepthStencilTarget(api_state.device.getContext(), texture, layer, read_only);
  }
  return true;
}
...
VDECL d3d::create_vdecl(VSDTYPE *vsd)
{
  ...
  return api_state.shaderProgramDatabase.registerInputLayout(layout).get();
}
...
bool d3d::draw_base(int type, int start, int numprim, uint32_t num_instances, uint32_t start_instance)
{
  ...
  ScopedCommitLock ctxLock{api_state.device.getContext()};
  if (!api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::DRAW))
  {
    return true;
  }
...
bool d3d::drawind_base(int type, int startind, int numprim, int base_vertex, uint32_t num_instances, uint32_t start_instance)
{
  ...
  ScopedCommitLock ctxLock{api_state.device.getContext()};
  if (!api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::DRAW_INDEXED))
  {
    return true;
  }
...
bool d3d::draw_up(int type, int numprim, const void *ptr, int stride_bytes)
{
  ...
  ScopedCommitLock ctxLock{api_state.device.getContext()};
  if (!api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::DRAW_UP))
  {
    return true;
  }
...
bool d3d::drawind_up(int type, int minvert, int numvert, int numprim, const uint16_t *ind, const void *ptr, int stride_bytes)
{
  ...
  ScopedCommitLock ctxLock{api_state.device.getContext()};
  if (!api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::DRAW_INDEXED_UP))
  {
    return true;
  }
...
void d3d::dispatch_mesh(uint32_t thread_group_x, uint32_t thread_group_y, uint32_t thread_group_z)
{
  ...
  if (!api_state.state.flushGraphics(api_state.device, api_state.shaderProgramDatabase, FrontendState::GraphicsMode::DISPATCH_MESH))
  {
    return;
  }
