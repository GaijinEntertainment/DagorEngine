...
void DeviceContext::bindVertexDecl(InternalInputLayoutID ident)
{
...
void DeviceContext::registerInputLayout(InternalInputLayoutID ident, const InputLayout &ptr)
{
...
void DeviceContext::ExecutionContext::setUniformBuffer(uint32_t stage, uint32_t unit, BufferResourceReferenceAndAddressRange buffer)
{
  ...
  if (buffer)
  {
    contextState.resourceStates.useBufferAsCBV(contextState.graphicsCommandListBarrierBatch, stage, buffer);
  ...
  bool as_const_ds, D3D12_CPU_DESCRIPTOR_HANDLE view)
{
  ...
  if (image && image->hasTrackedState())
  {
    contextState.resourceStates.useTextureAsSRV(contextState.graphicsCommandListBarrierBatch,
      contextState.graphicsCommandListSplitBarrierTracker, stage, image, view_state, as_const_ds);
  }
}
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE view)
{
  ...
  if (image)
  {
    contextState.resourceStates.useTextureAsUAV(contextState.graphicsCommandListBarrierBatch,
      contextState.graphicsCommandListSplitBarrierTracker, stage, image, view_state);
  }
}
...
void DeviceContext::ExecutionContext::setSRVBuffer(uint32_t stage, uint32_t unit, BufferResourceReferenceAndShaderResourceView buffer)
{
  ...
  if (buffer)
  {
    contextState.resourceStates.useBufferAsSRV(contextState.graphicsCommandListBarrierBatch, stage, buffer);
    contextState.bufferAccessTracker.updateLastFrameAccess(buffer.resourceId);
  }
}
  ...
  BufferResourceReferenceAndUnorderedResourceView buffer)
{
  ...
  if (buffer)
  {
    contextState.resourceStates.useBufferAsUAV(contextState.graphicsCommandListBarrierBatch, stage, buffer);
    contextState.bufferAccessTracker.updateLastFrameAccess(buffer.resourceId);
  }
}
#if D3D_HAS_RAY_TRACING
...
void DeviceContext::ExecutionContext::setInputLayout(InternalInputLayoutID ident)
{
...
void DeviceContext::ExecutionContext::flushGraphicsMeshState()
{
  ...
  if (!contextState.graphicsState.pipeline)
  {
    ...
    if (!contextState.graphicsState.pipeline->isReady())
    {
      ...
      contextState.graphicsState.pipeline->loadMesh(device.device.get(), *contextState.graphicsState.basePipeline,
...
void DeviceContext::ExecutionContext::flushGraphicsState(D3D12_PRIMITIVE_TOPOLOGY top)
{
  ...
  if (!contextState.graphicsState.pipeline || contextState.graphicsState.topology != topType)
  {
    ...
    contextState.graphicsState.pipeline =
      &contextState.graphicsState.basePipeline->getVariantFromConfiguration(contextState.graphicsState.inputLayoutIdent,
    ...
    if (!contextState.graphicsState.pipeline->isReady())
    {
      auto &inputLayout = device.pipeMan.getInputLayout(contextState.graphicsState.inputLayoutIdent);

      auto &staticRenderState = device.pipeMan.getStaticRenderState(contextState.graphicsState.staticRenderStateIdent);

      contextState.graphicsState.pipeline->load(device.device.get(), *contextState.graphicsState.basePipeline, device.pipelineCache,
...
{
  if (auto module = device.pipeMan.modifyVertexShader(shader))
  {
    module->debugName.assign(begin(name), end(name));
  }
}

void DeviceContext::ExecutionContext::updatePixelShaderName(ShaderID shader, StringIndexRef::RangeType name)
{
  if (auto module = device.pipeMan.modifyPixelShader(shader))
  {
    module->debugName.assign(begin(name), end(name));
  }
}
...
void DeviceContext::ExecutionContext::registerInputLayout(InternalInputLayoutID ident, const InputLayout &layout)
{
...
void DeviceContext::ExecutionContext::removeVertexShader(ShaderID shader)
{
  ...
  if (module)
  {
    frame.deletedVertexShaderModules.push_back(eastl::move(module));
  }
}
...
void DeviceContext::ExecutionContext::removePixelShader(ShaderID shader)
{
  ...
  if (module)
  {
    frame.deletedPixelShaderModules.push_back(eastl::move(module));
  }
}
