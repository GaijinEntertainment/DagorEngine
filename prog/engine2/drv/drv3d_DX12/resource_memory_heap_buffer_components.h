...
protected:
  ...
  struct Heap : BasicBuffer
  {
    ...
    eastl::vector<ValueRange<uint32_t>> freeRanges;

    bool free(ValueRange<uint32_t> range)
    {
    ...
    ValueRange<uint32_t> allocate(uint32_t size, uint32_t alignment)
    {
      if (freeRanges.empty())
      {
        return {};
      ...
      return make_value_range<uint32_t>(s, size);
    }
  };
  struct StandbyInfo
  {
  ...
  struct BufferHeapState
  {
    ...
    eastl::pair<Heap *, ValueRange<uint32_t>> tryAllocateFromReadyList(ResourceHeapProperties properties, uint32_t size,
      D3D12_RESOURCE_FLAGS flags, uint32_t offset_alignment, bool allow_offset)
    {
      Heap *selectedHeap = nullptr;
      ValueRange<uint32_t> allocationRange;
    ...
    eastl::pair<Heap *, ValueRange<uint32_t>> trySuballocateFromExistingHeaps(ResourceHeapProperties properties, uint32_t size,
    ...
    size_t freeBufferHeap(BufferHeap *manager, uint32_t index, ValueRange<uint32_t> range, const char *name)
    {
    ...
    BufferGlobalId createBufferHeap(BufferHeap *manager, DXGIAdapter *adapter, ID3D12Device *device, uint32_t allocation_size,
      ResourceHeapProperties properties, D3D12_RESOURCE_FLAGS flags, DeviceMemoryClass memory_class, const char *name)
    {
      ...
      newHeap.freeRanges.push_back(make_value_range<uint32_t>(0, allocation_size));
      newHeap.flags = flags;
...
public:
  ...
  BufferState allocateBuffer(DXGIAdapter *adapter, ID3D12Device *device, uint32_t size, uint32_t structure_size,
    ...
    bool disable_sub_alloc, bool name_objects)
  {
    ...
    if (canUseSubAlloc)
    {
      payloadSize = align_value<uint32_t>(payloadSize, offsetAlignment);
      ...
      if (!selectedHeap)
      {
        auto resIndex = bufferHeapStateAccess->createBufferHeap(this, adapter, device,
          align_value<uint32_t>(payloadSize * discard_count, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT), heapProperties, flags,
        ...
        if (heapIndex < bufferHeapStateAccess->bufferHeaps.size())
        {
          ...
          allocationRange = make_value_range<uint32_t>(0, payloadSize);
    ...
    else
    {
      if (discard_count > 1)
      {
        ...
        // possible
        discard_count = align_value<uint32_t>(payloadSize * discard_count, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT) / payloadSize;
      }
      else
      {
#if DX12_REPORT_BUFFER_PADDING
      ...
      uint32_t totalSize = payloadSize * discard_count;

      {
        ...
        {
          auto resIndex = bufferHeapStateAccess->createBufferHeap(this, adapter, device,
            align_value<uint32_t>(totalSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT), heapProperties, flags, memory_class, name);
          ...
          if (heapIndex < bufferHeapStateAccess->bufferHeaps.size())
          {
            selectedHeap = &bufferHeapStateAccess->bufferHeaps[heapIndex];
            memoryAllocatedSize = selectedHeap->bufferMemory.size();
            allocationRange = make_value_range<uint32_t>(0, totalSize);
