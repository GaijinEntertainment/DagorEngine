...
public:
  struct BasePipelineIdentifier
  {
    ...
    bool operator==(const BasePipelineIdentifier &o) const
    {
      return o.vs == vs && o.ps == ps && o.gs == gs && o.hs == hs && o.ds == ds;
    }
...
private:
  ...
  struct GraphicsPipelineVariantName
  {
    ...
    static constexpr size_t base_length = 1 + (stage_length - 1) * 5 + 1;
      ...
      bool is_wire_frame, uint32_t static_render_state_index, uint32_t frame_buffer_layout_index)
    {
      ...
      base.gs.convertToString(lbuf, sizeof(lbuf));
      ot = eastl::copy(eastl::begin(lbuf), eastl::end(lbuf) - 1, ot);

      base.hs.convertToString(lbuf, sizeof(lbuf));
      ot = eastl::copy(eastl::begin(lbuf), eastl::end(lbuf) - 1, ot);

      base.ds.convertToString(lbuf, sizeof(lbuf));
      ot = eastl::copy(eastl::begin(lbuf), eastl::end(lbuf) - 1, ot);
