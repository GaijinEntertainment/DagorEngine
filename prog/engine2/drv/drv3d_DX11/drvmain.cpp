...
int d3d::driver_command(int command, void *par1, void *par2, void *par3)
{
  ...
  switch (command)
  {
    ...
    case DRV3D_COMMAND_HDR_OUTPUT_MODE:
    {
      if (hdr_enabled)
      {
        return (int)(int10_hdr_buffer ? HdrOutputMode::HDR10_ONLY : HdrOutputMode::HDR_ONLY);
      }
    ...
    case DRV3D_COMMAND_GET_VIDEO_MEMORY_BUDGET:
    {
      ...
      if (SUCCEEDED(get_active_adapter(dx_device)->QueryInterface(COM_ARGS(&adapter3))))
      {
            ...
            (SUCCEEDED(adapter3->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_NON_LOCAL, &info)) && info.Budget > 0))
        {
          ...
          if (par2)
            *reinterpret_cast<uint32_t *>(par2) =
              info.Budget > info.CurrentUsage ? uint32_t((info.Budget - info.CurrentUsage) >> 10) : 0;
          if (par3)
            *reinterpret_cast<uint32_t *>(par3) = uint32_t(info.CurrentUsage >> 10);
          return 1;
        }
      }
      return 0;
...
bool d3d::update_screen(bool app_active)
{
  ...
  d3d::set_render_target(); // Render target is unbound on each present on Xbox, and with some presentation modes on PC as well.
...
{
#if _TARGET_PC_WIN
  if ((d3d::get_driver_desc().cshver & DDCSH_5_0) == 0)
  {
  ...
  if (SUCCEEDED(hr))
  {
    ...
    if (SUCCEEDED(hr) && gammaControlCapabilities.NumGammaControlPoints > 1)
    {
      if (power <= 0.f)
        power = 1.f;
      power = 1 / power;
      gamma_control.Offset.Red = gamma_control.Offset.Green = gamma_control.Offset.Blue = 0.f; // Must be 0 and 1 on Xbox.
