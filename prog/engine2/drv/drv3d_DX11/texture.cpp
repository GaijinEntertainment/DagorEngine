...
{
  if ((cflg & TEXFMT_MASK) == TEXFMT_BC7 && !(g_device_desc.fshver & DDFSH_5_0))
...
static bool fix_format_msaa(uint32_t &flg, DXGI_SAMPLE_DESC &sampleDesc)
{
  ...
  {
    sampleDesc.Quality = 0;
    sampleDesc.Count = max_aa_samples ? 1 : max_aa_samples;
  ...
  D3D11_SUBRESOURCE_DATA *initial_data, int array_size = 1, bool tmp_tex = false)
{
  ...
  {
    HRESULT hr = d3d::DagorCreateTexture2D(desc, initial_data, *bt_in, tex);
    if (FAILED(hr))
    {
  ...
  D3D11_SUBRESOURCE_DATA *initial_data)
{
  ...
  TEXQL_PRE_CLEAN(bt_in->ressize());
  {
    HRESULT hr = d3d::DagorCreateTexture3D(desc, initial_data, *bt_in, tex);
...
void d3d::get_texture_statistics(uint32_t *num_textures, uint64_t *total_mem, String *out_dump)
{
  ...
  uint64_t totalXboxTexAllocSize = 0;
...
void get_mem_stat(String *out_str)
{
  ...
  *out_str = String(0, "r:%d 2:%d c:%d v:%d a:%d s:%d x:%d v:%d", totalSizeRt >> 20, totalSize2d >> 20, totalSizeCube >> 20,
    totalSizeVol >> 20, totalSizeArr >> 20, stagingTexSz >> 20, totalXboxTexAllocSize >> 20, vbIbMem >> 20);
