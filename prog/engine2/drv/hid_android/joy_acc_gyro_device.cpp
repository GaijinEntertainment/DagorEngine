  ...
  intptr_t &result)
{
  ...
  {
    void *event = (void *)wParam;
    deviceId = android::motion_event::get_device_id(event);

    const int axis_scale = AXIS_SCALE_FACTOR;
    const int axis_scale_half = AXIS_SCALE_FACTOR / 2;

    float lx = android::motion_event::get_axis_value(event, 0, 0);
    float ly = -android::motion_event::get_axis_value(event, 1, 0);

    float rx = android::motion_event::get_axis_value(event, 11, 0);
    float ry = -android::motion_event::get_axis_value(event, 14, 0);
    ...
    nextState.slider[0] = clip_dead_zone(int(android::motion_event::get_axis_value(event, 23, 0) * axis_scale), 400);
    nextState.slider[1] = clip_dead_zone(int(android::motion_event::get_axis_value(event, 22, 0) * axis_scale), 400);

    int dpadx = int(android::motion_event::get_axis_value(event, 15, 0) * axis_scale);
    int dpady = int(android::motion_event::get_axis_value(event, 16, 0) * axis_scale);
