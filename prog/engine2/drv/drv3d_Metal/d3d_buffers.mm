...
{
  // Default ib and vb are not lockable in metal driver  (required for current raytraced shadow implementation)
  // SBCF_USAGE_READ_BACK is a workaround (sadly makes (staging + Private) -> Shared buffer)
  const int flags = flg | SBCF_BIND_VERTEX | (d3d::get_driver_desc().caps.hasRaytracingT11 ? SBCF_USAGE_READ_BACK : 0);
  return new Buffer(size, 0, flags, 0, name);
}
...
Sbuffer *d3d::create_ib(int size, int flg, const char *name)
{
  // Default ib and vb are not lockable in metal driver  (required for current raytraced shadow implementation)
  // SBCF_USAGE_READ_BACK is a workaround (sadly makes (staging + Private) -> Shared buffer)
  const int flags = flg | SBCF_BIND_INDEX | (d3d::get_driver_desc().caps.hasRaytracingT11 ? SBCF_USAGE_READ_BACK : 0);
  return new Buffer(size, 0, flags, 0, name);
}
