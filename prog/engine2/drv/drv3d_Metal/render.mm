...
namespace drv3d_metal
{
  ...
  void Render::StageBinding::apply(unsigned stage, Shader* shader)
  {
    for (int i = 0; i < GEOM_BUFFER_COUNT; ++i)
    {
      ...
      if (buffers[i])
        buffer = buffers[i]->getBuffer(), dynamic_offset = buffers[i]->getDynamicOffset();

      bindMetalBuffer(stage, buffer, buffers_metal[i], buffers_metal_offset[i], i, buffers_offset[i] + dynamic_offset);
    }
  ...
  bool Render::init()
  {
    ...
    cur_rstate.raster_state.ablend = false;
    cur_rstate.sample_count = 1;
    cur_rstate.raster_state.ablendOp = MTLBlendOperationAdd;
    cur_rstate.raster_state.ablendScr = MTLBlendFactorSourceAlpha;
    cur_rstate.raster_state.ablendDst = MTLBlendFactorOneMinusSourceAlpha;
    cur_rstate.raster_state.sepblend = false;
  ...
  void Render::doClearTexture(Texture* tex)
  {
    G_ASSERT(tex->apiTex);

    need_change_rt = 1;
    EndComputeEncoding(computeEncoder);
    EndRenderEncoding(renderEncoder);
    EndAccelerationEncoding(accelerationEncoder);

    if (!blitEncoder)
      blitEncoder = [commandBuffer blitCommandEncoder];

    int slices = 1, depth = 1;
    ...
    for (int slice = 0; slice < slices; ++slice)
      for (int level = 0; level < tex->mipLevels; ++level)
      {
        ...
        for (unsigned d = 0; d < local_depth; ++d)
        {
          ...
          Texture::getStride(tex->base_format, tex->width, tex->height, level, row_pitch, slice_pitch);

          unsigned width = max(tex->width >> level, 1);
          unsigned height = max(tex->height >> level, 1);
...

      {
        ...
        if (textures2upload.size() || buffers2copy.size())
        {
          ...
          for (int i = 0; i < buffers2copy.size(); ++i)
          {
            ...
            {
              render.computeEncoder = [render.commandBuffer computeCommandEncoder];
  ...
  struct StateSaver
  {
      ...
      , save_rt(save_rt)
    {
      ...
      render.cur_rstate.raster_state.ablend = 0;
    ...
    ~StateSaver()
    {
      ...
      render.cur_rstate.raster_state.ablend = old_rstate.raster_state.ablend;
  ...
  shaders::DriverRenderStateId Render::createRenderState(const shaders::RenderState & state)
  {
    ...
    rstate.raster_state.ablend = state.ablend;
    rstate.raster_state.ablendScr = convBlendArg(state.sepablendSrc);
    rstate.raster_state.ablendDst = convBlendArg(state.sepablendDst);
    rstate.raster_state.ablendOp = state.sepablendOp-1;
    rstate.raster_state.sepblend = state.sepablend;
    rstate.raster_state.rgbblendScr = convBlendArg(state.ablendSrc);
    rstate.raster_state.rgbblendDst = convBlendArg(state.ablendDst);
    rstate.raster_state.rgbblendOp = state.blendOp-1;
  ...
  void Render::deleteTLAS(RaytraceTopAccelerationStructure *tlas)
  {
    ...
    eastl::erase_if(topStructs, [&](const auto &as) { return as.get() == tlas; });
  }

  void Render::deleteBLAS(RaytraceBottomAccelerationStructure *botAccStruct)
  {
    std::lock_guard<std::mutex> scopedLock(blas_lock);
    auto search = eastl::find_if(bottomStructs.begin(), bottomStructs.end(), [&](const auto &blas) { return blas.get() == botAccStruct; });
    const bool isPresent = search != bottomStructs.end();
    if (!isPresent)
      return;

    const int deleteAtIndex = (*search)->index;
    for (auto it = bottomStructs.erase(search); it != bottomStructs.end(); it++)
      (*it)->index--;
  ...
  {
    if (bottomLevelAccelerationStructures[delete_index] != [NSNull null])
      resources2delete.acceleration_structures.push_back(bottomLevelAccelerationStructures[delete_index]);
  ...
  RaytraceTopAccelerationStructure *Render::createTopAccelerationStructure()
  {
    std::lock_guard<std::mutex> scopedLock(tlas_lock);
    return topStructs.emplace_back(new RaytraceTopAccelerationStructure{ nil }).get();
  }

  RaytraceBottomAccelerationStructure *Render::createBottomAccelerationStructure()
  {
    // Add Null struct to bottomLevelAccelerationStructures
    std::lock_guard<std::mutex> scopedLock(blas_lock);
    command_buffer->push<CommandAddBLASToList>();
    return bottomStructs.emplace_back(new RaytraceBottomAccelerationStructure{nil , bottomStructs.size()}).get();
  }

  id<MTLAccelerationStructure> Render::createAccelerationStructure(MTLAccelerationStructureDescriptor *desc)
  {
    ...
    id<MTLBuffer> scrathBuffer = [render.device newBufferWithLength: sizes.buildScratchBufferSize
    ...
    resources2delete.native_resources.push_back(scrathBuffer);
  ...
  void Render::scheduleTLASBuild(RaytraceTopAccelerationStructure *as, Sbuffer *instance_descriptor_buffer, uint32_t instance_count)
  {
    command_buffer->push<CommandBuildTLAS>(as, instance_descriptor_buffer, instance_count);
  }

  void Render::buildTLAS(RaytraceTopAccelerationStructure *tlas, Sbuffer *instance_descriptor_buffer, uint32_t instance_count)
  {
    ...
    accelDescriptor.instancedAccelerationStructures = bottomLevelAccelerationStructures;
    accelDescriptor.instanceCount = instance_count;
    accelDescriptor.instanceDescriptorBuffer = ((Buffer*)instance_descriptor_buffer)->getBuffer();

    if (tlas->instanceAccelerationStructure != nil)
      resources2delete.acceleration_structures.push_back(tlas->instanceAccelerationStructure);

    tlas->instanceAccelerationStructure = createAccelerationStructure(accelDescriptor);
  }

  void Render::scheduleBLASBuild(RaytraceBottomAccelerationStructure *blas, RaytraceGeometryDescription *desc)
  {
    command_buffer->push<CommandBuildBLAS>(blas, desc);
  }
  ...
  MTLAccelerationStructureGeometryDescriptor *getTriangleGeometryDesc(const RaytraceGeometryDescription::TrianglesInfo &triangles)
  {
    ...
    desc.vertexBuffer = ((Buffer*)triangles.vertexBuffer)->getBuffer();
    ...
    if (@available(macOS 13.0, iOS 16.0, *))
    {
      desc.vertexFormat = [&](){
        switch (triangles.vertexFormat) {
          case VSDT_FLOAT1: return MTLAttributeFormatFloat;
          case VSDT_FLOAT2: return MTLAttributeFormatFloat2;
          case VSDT_FLOAT3: return MTLAttributeFormatFloat3;
          case VSDT_FLOAT4: return MTLAttributeFormatFloat4;
          default: return MTLAttributeFormatFloat3;
        }
      }();
    }

    desc.indexBuffer = ((Buffer*)triangles.indexBuffer)->getBuffer();
  ...
  MTLAccelerationStructureGeometryDescriptor *getAABBGeometryDesc(const RaytraceGeometryDescription::AABBsInfo &aabbs)
  {
    ...
    desc.boundingBoxBuffer = ((Buffer*)aabbs.buffer)->getBuffer();
    desc.boundingBoxCount = aabbs.count;

    return desc;
  }

  void Render::buildBLAS(RaytraceBottomAccelerationStructure *blas, RaytraceGeometryDescription *desc)
  {
    // Was deleted on the same frame build was scheduled
    if (auto search = eastl::find_if(bottomStructs.begin(), bottomStructs.end(), [&](const auto &as) { return as.get() == blas; }); search == bottomStructs.end())
      return;

    MTLAccelerationStructureGeometryDescriptor *geometryDesc = [&]() {
      switch(desc->type)
      {
        case RaytraceGeometryDescription::Type::TRIANGLES:
          return getTriangleGeometryDesc(desc->data.triangles);
        case RaytraceGeometryDescription::Type::AABBS:
          return getAABBGeometryDesc(desc->data.aabbs);
      }
    }();

    MTLPrimitiveAccelerationStructureDescriptor *accDesc = [MTLPrimitiveAccelerationStructureDescriptor descriptor];
    accDesc.geometryDescriptors = @[ geometryDesc ];
    ...
    blas->primitiveAccelerationStructure = createAccelerationStructure(accDesc);
