...
struct CommandClearTexture
{
  ...
  CommandClearTexture(Texture *tex) : tex(tex) {}

  __forceinline void Process(Render &render) { render.doClearTexture(tex); }
};
...
struct CommandReadbackTex
{
  ...
  __forceinline void Process(Render &render)
  {
    ...
    render.EndComputeEncoding(render.computeEncoder);
    render.EndRenderEncoding(render.renderEncoder);
    render.EndAccelerationEncoding(render.accelerationEncoder);

    if (!render.blitEncoder)
      render.blitEncoder = [render.commandBuffer blitCommandEncoder];
...
struct CommandBuildTLAS
{
  ...
  RaytraceTopAccelerationStructure *accStruct;
  Sbuffer *descriptorBuffer;
  uint32_t instanceCount;

  CommandBuildTLAS(RaytraceTopAccelerationStructure *tlas, Sbuffer *instance_descriptor_buffer, uint32_t instance_count) :
    accStruct(tlas), descriptorBuffer(instance_descriptor_buffer), instanceCount(instance_count){};

  __forceinline void Process(Render &render) { render.buildTLAS(accStruct, descriptorBuffer, instanceCount); }
};
...
struct CommandBuildBLAS
{
  ...
  RaytraceGeometryDescription geomDesc;
  CommandBuildBLAS(RaytraceBottomAccelerationStructure *as, RaytraceGeometryDescription *desc) : accStruct(as), geomDesc(*desc){};

  __forceinline void Process(Render &render) { render.buildBLAS(accStruct, &geomDesc); }
};
    ...
    if (num_instances > 0 && render.applyStates())
    {
      ...
      else
      {
        if (base_vertex > 0)
        {
          ...
          Buffer::apply(vs_stage.buffers[0]->getBuffer(), STAGE_VS, 0,
            vs_stage.buffers_offset[0] + render.cur_rstate.vbuffer_stride[0] * base_vertex);
        }
...
struct CommandClearRW
{
  ...
  __forceinline void Process(Render &render)
  {
    ...
    {
      render.EndBlitEncoding(render.blitEncoder);

      if (!render.computeEncoder)
