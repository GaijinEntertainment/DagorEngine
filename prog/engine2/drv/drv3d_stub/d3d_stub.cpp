...
static Driver3dDesc stub_desc = {
  ...
  (0xFFFE0000 | (2 << 8) | (0)), // unsigned vprogver;

  65536 * 4, 65536 * 4, // int maxprims,maxvertind;
  0.5f, 0.5f,           // float upixofs,vpixofs;

  //! fragment shader version flags
  DDFSH_1_1 | DDFSH_1_3 | DDFSH_1_4 | DDFSH_2_0 | DDFSH_2_A | DDFSH_3_0 | DDFSH_4_0 | DDFSH_4_1 | DDFSH_5_0, // int fshver;
...
unsigned d3d::get_driver_code() { return DRV3D_CODE; }
...
bool d3d::set_depth(BaseTexture *tex, bool ro)
{
  if (!tex)
    currentRtState.removeDepth();
  else
    currentRtState.setDepth(tex, ro);
  return true;
}

bool d3d::set_depth(BaseTexture *tex, int face, bool ro)
{
  if (!tex)
    currentRtState.removeDepth();
  else
    currentRtState.setDepth(tex, face, ro);
  return true;
}
