...
bool isConstDepthStencilTargetForNativeRP(Image *img)
{
  ...
  return img->getState(0, 0) == SN_CONST_RENDER_TARGET;
}
...
TRegister::TRegister(Sbuffer *sb) : type(TYPE_NULL) //-V730
{
  ...
  buf = BufferRef{((GenericBufferInterface *)sb)->getDeviceBuffer(), (uint32_t)sb->ressize()};
  type = TYPE_BUF;
}
...
{
  if (sb)
    buffer = BufferRef{((GenericBufferInterface *)sb)->getDeviceBuffer(), (uint32_t)sb->ressize()};
}
