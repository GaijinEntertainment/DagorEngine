...
struct State
{
  ...
  uint32_t registerSpaceSizes[STAGE_MAX] = {MIN_COMPUTE_CONST_REGISTERS, FRAGMENT_SHADER_REGISTERS, MAX_VS_CONSTS_BONES};
  ...
  void flushGraphics(DeviceContext &ctx)
  {
    ...
    if (dirtyState.test(DirtyState::VERTEX_CONST_REGISTERS))
    {
      ...
      ctx.setConstRegisterBuffer(alloc.get().buffer->getHandle(), alloc.dataOffset(), alloc.get().size, STAGE_VS);
    }
    if (dirtyState.test(DirtyState::PIXEL_CONST_REGISTERS))
    {
      ...
      ctx.setConstRegisterBuffer(alloc.get().buffer->getHandle(), alloc.dataOffset(), alloc.get().size, STAGE_PS);
    }
  ...
  void flushCompute(DeviceContext &ctx)
  {
    ...
    if (dirtyState.test(DirtyState::COMPUTE_CONST_REGISTERS))
    {
      ...
      ctx.setConstRegisterBuffer(alloc.get().buffer->getHandle(), alloc.dataOffset(), alloc.get().size, STAGE_CS);
    }
