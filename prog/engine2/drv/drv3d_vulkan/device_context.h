...
public:
  ...
  void ensureBoundImageStates(const spirv::ShaderHeader &header, ShaderStage stageIndex);
  ...
  void beginPassInternal(RenderPassClass *pass_class, const FramebufferInfo &pass_images, VulkanFramebufferHandle fb, VkRect2D area,
    uint32_t clear_mode);
  void prepare();
  VulkanCommandBufferHandle getFrameCmdBuffer();
  String getCurrentCmdCaller();
  void reportMissingPipelineComponent(const char *component);
  bool setFrameImageState(Image *img, ImageState new_state, ValueRange<uint8_t> mip_range, ValueRange<uint16_t> array_range);
  void fixFrameImageState(Image *img, ImageState new_state, ValueRange<uint8_t> mip_range, ValueRange<uint16_t> array_range);

private:
  void writeExectionChekpoint(VulkanCommandBufferHandle cb, VkPipelineStageFlagBits stage);

  void registerImageState(Image *image, ValueRange<uint8_t> mip_range, ValueRange<uint16_t> array_range, ImageState initial_use);
  bool updateImageState(Image *img, ImageState new_state, ValueRange<uint8_t> mip_range, ValueRange<uint16_t> array_range);
  ...
  VulkanCommandBufferHandle flushBufferUploads(VulkanCommandBufferHandle cmd);
  VulkanCommandBufferHandle flushOrderedBufferUploads(VulkanCommandBufferHandle cmd);
...
public:
  ...
  void copyImageToBuffer(VulkanImageHandle src, VkImageLayout src_layout, VulkanBufferHandle dst, uint32_t region_count,
  ...
  void buildAccelerationStructure(VkAccelerationStructureTypeKHR type, uint32_t instance_count, Buffer *instance_buffer,
    ...
    VkBool32 update, VulkanAccelerationStructureKHR dst, VulkanAccelerationStructureKHR src, Buffer *scratch);
#endif
  ...
  void copyBuffer(VulkanBufferHandle src, VulkanBufferHandle dst, uint32_t src_offset, uint32_t dst_offset, uint32_t size);
  void copyBufferToImageOrdered(Image *dst, VulkanBufferHandle src, const VkBufferImageCopy &region);
  void fillBuffer(VulkanBufferHandle buffer, uint32_t offset, uint32_t size, uint32_t value);
  ...
  void copyQueryResult(VulkanQueryPoolHandle pool, uint32_t index, uint32_t count, VulkanBufferHandle buffer, uint32_t offset,
    uint32_t stride, VkQueryResultFlags flags);
