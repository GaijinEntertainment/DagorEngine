  ...
  Image *&target)
{
  ...
  for (auto &&fmt : (needCompare ? formatTableWithComparsionSampler : formatTableWithSimpleSampler))
  {
    ...
    {
      target = resources.alloc<Image>({ii, Image::MEM_NOT_EVICTABLE | Image::MEM_DEDICATE_ALLOC, fmt, SN_INITIAL}, true);
      break;
    }
  }
  if (!target)
...
void Device::setupDummyResources()
{
  ...
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_INDEX].image.sampler = defaultSampler->colorSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyImage2DwithCompare->getFormat());
  imageView = getImageView(dummyImage2DwithCompare, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_INDEX].image.sampler = defaultSampler->compareSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyStorage2D->getFormat());
  imageView = getImageView(dummyStorage2D, ivs);
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_2D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_2D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

  ivs.setArrayCount(dummy_array_count);
  ivs.isArray = 1;
  ivs.setFormat(dummyImage2D->getFormat());
  imageView = getImageView(dummyImage2D, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_ARRAY_INDEX].image.sampler = defaultSampler->colorSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_2D_ARRAY_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyImage2DwithCompare->getFormat());
  imageView = getImageView(dummyImage2DwithCompare, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_ARRAY_INDEX].image.sampler = defaultSampler->compareSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_2D_ARRAY_INDEX].image.imageLayout =
    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyStorage2D->getFormat());
  imageView = getImageView(dummyStorage2D, ivs);
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_2D_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_2D_ARRAY_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
  ...
  ivs.setFormat(dummyImage2D->getFormat());
  imageView = getImageView(dummyImage2D, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_INDEX].image.sampler = defaultSampler->colorSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyImage2DwithCompare->getFormat());
  imageView = getImageView(dummyImage2DwithCompare, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_INDEX].image.sampler = defaultSampler->compareSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_INDEX].image.imageLayout =
    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyStorage2D->getFormat());
  imageView = getImageView(dummyStorage2D, ivs);
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_CUBE_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_CUBE_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
  ...
  ivs.setFormat(dummyImage2D->getFormat());
  imageView = getImageView(dummyImage2D, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_ARRAY_INDEX].image.sampler = defaultSampler->colorSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_CUBE_ARRAY_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyImage2DwithCompare->getFormat());
  imageView = getImageView(dummyImage2DwithCompare, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_ARRAY_INDEX].image.sampler = defaultSampler->compareSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_CUBE_ARRAY_INDEX].image.imageLayout =
    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyStorage2D->getFormat());
  imageView = getImageView(dummyStorage2D, ivs);
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_CUBE_ARRAY_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_CUBE_ARRAY_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
  ...
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_3D_INDEX].image.sampler = defaultSampler->colorSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_3D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_3D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyImage3DwithCompare->getFormat());
  imageView = getImageView(dummyImage3DwithCompare, ivs);
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_3D_INDEX].image.sampler = defaultSampler->compareSampler();
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_3D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_SAMPLED_IMAGE_WITH_COMPARE_3D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  ivs.setFormat(dummyStorage3D->getFormat());
  imageView = getImageView(dummyStorage3D, ivs);
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_3D_INDEX].image.imageView = imageView;
  dummyResourceTable[spirv::MISSING_STORAGE_IMAGE_3D_INDEX].image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

  dummyResourceTable[spirv::MISSING_BUFFER_SAMPLED_IMAGE_INDEX].texelBuffer = dummyBuffer->getView();

  dummyResourceTable[spirv::MISSING_STORAGE_BUFFER_SAMPLED_IMAGE_INDEX].texelBuffer = dummyBuffer->getView();
  ...
  dummyResourceTable[spirv::MISSING_BUFFER_INDEX].buffer.buffer = dummyBuffer->getHandle();
  dummyResourceTable[spirv::MISSING_BUFFER_INDEX].buffer.offset = dummyBuffer->dataOffset(0);
  dummyResourceTable[spirv::MISSING_BUFFER_INDEX].buffer.range = dummyUniformSize;

  dummyResourceTable[spirv::MISSING_STORAGE_BUFFER_INDEX].buffer.buffer = dummyBuffer->getHandle();
  dummyResourceTable[spirv::MISSING_STORAGE_BUFFER_INDEX].buffer.offset = dummyBuffer->dataOffset(0);
  dummyResourceTable[spirv::MISSING_STORAGE_BUFFER_INDEX].buffer.range = dummyBuffer->dataSize();

  dummyResourceTable[spirv::MISSING_CONST_BUFFER_INDEX].buffer.buffer = dummyBuffer->getHandle();
  dummyResourceTable[spirv::MISSING_CONST_BUFFER_INDEX].buffer.offset = dummyBuffer->dataOffset(0);
  dummyResourceTable[spirv::MISSING_CONST_BUFFER_INDEX].buffer.range = dummyUniformSize;

  // dummy is special, should be treated as null to avoid clearing up after fallback replacement
  for (VkAnyDescriptorInfo &i : dummyResourceTable)
    i.type = VkAnyDescriptorInfo::TYPE_NULL;
...
void Device::adjustCaps(Driver3dDesc &caps)
{
...
#if _TARGET_PC_WIN || _TARGET_PC_LINUX || _TARGET_PC_MACOSX || _TARGET_ANDROID
  ...
  unsigned vertexShaderVersion = FSHVER_50;
  int computeShaderMask = DDCSH_4_0 | DDCSH_5_0 | DDCSH_6_0;
  if (!hasWaveOperations(VK_SHADER_STAGE_COMPUTE_BIT))
  {
    computeShaderMask ^= DDCSH_6_0;
  }

  int fragmentShaderMask = DDFSH_3_0 | DDFSH_4_0 | DDFSH_4_1 | DDFSH_5_0 | DDFSH_6_0;
  if (!hasWaveOperations(VK_SHADER_STAGE_FRAGMENT_BIT))
  {
    fragmentShaderMask ^= DDFSH_6_0;
  }
  if (!hasFragmentShaderUAV())
  {
    // not entirely correct
    fragmentShaderMask ^= DDFSH_5_0;
  }
  ...
  caps.vprogver = min(caps.vprogver, vertexShaderVersion);
  ...
  caps.fshver &= fragmentShaderMask;
  caps.cshver &= computeShaderMask;
...
// not supported for moltenvk
#if VK_KHR_maintenance1 && !_TARGET_PC_MACOSX
...
Image *Device::createImage(const ImageCreateInfo &ii)
{
  ...
  return resources.alloc<Image>({ici, memFlags, ii.format, SN_INITIAL}, true);
}
...
{
  ImageState currentVKState = ImageState::SN_INITIAL;
  if (current_state == ResourceBarrier::RB_RW_RENDER_TARGET)
    currentVKState = ImageState::SN_RENDER_TARGET;
  if ((current_state & ResourceBarrier::RB_RO_SRV) != 0)
    currentVKState = ImageState::SN_SAMPLED_FROM;
  ...
  SharedGuardedObjectAutoLock lock(resources);
  tex->tex.image = resources.alloc<Image>({ici, Image::MEM_NOT_EVICTABLE, dagorFormat, currentVKState}, false);
...
Device::Config drv3d_vulkan::get_device_config(const DataBlock *cfg)
{
  ...
  result.features.set(DeviceFeaturesConfig::STOP_PASS_EVERY_DRAW_DISPATCH, cfg->getBool("stopPassEveryDrawDispatch", false));
