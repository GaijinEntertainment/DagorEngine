...
{
  TIME_PROFILE(vulkan_push_space_wait);
...
{
  CmdClearColorTexture cmd{image, area, value, followup_usage};
...
void DeviceContext::copyImageToHostCopyBuffer(Image *image, Buffer *buffer, uint32_t region_count, VkBufferImageCopy *regions)
{
  ...
  info.image = image;
  if (image)
    info.img = image->getHandle();
  info.buf = buffer->getHandle();
...
void DeviceContext::copyHostCopyToImage(Buffer *src, Image *dst, uint32_t region_count, VkBufferImageCopy *regions)
{
  ImageCopyInfo info;
  info.image = dst;
  info.img = dst->getHandle();
  info.buf = src->getHandle();
  info.copyCount = region_count;
...
{
  {
    ...
    info.buf = src->getHandle();
    info.copyCount = region_count;
    // force source state for now, to avoid WAW hazards
    info.targetState = (dst->getUsage() & VK_IMAGE_USAGE_SAMPLED_BIT) ? SN_SAMPLED_FROM : SN_COPY_FROM;
    VULKAN_LOCK_FRONT();
    info.copyIndex = front.replayRecord->imageUploadCopies.size();
    front.replayRecord->imageUploads.push_back(info);
    front.replayRecord->imageUploadCopies.insert(end(front.replayRecord->imageUploadCopies), regions, regions + region_count);
...
{
  CmdCopyBufferToImageOrdered cmd{dst, src->getHandle(), regions[0]};
  VULKAN_LOCK_FRONT();
  VULKAN_DISPATCH_COMMAND_NO_LOCK(cmd);
...
{
  VULKAN_LOCK_FRONT();
  VULKAN_DISPATCH_COMMAND_NO_LOCK(device.surveys.start(name));
}
...
void DeviceContext::uploadBuffer(Buffer *src, Buffer *dst, uint32_t src_offset, uint32_t dst_offset, uint32_t size)
{
  ...
  copy.size = size;
  BufferCopyInfo info;
  info.src = src->getHandle();
  info.dst = dst->getHandle();
  info.copyCount = 1;
  VULKAN_LOCK_FRONT();
