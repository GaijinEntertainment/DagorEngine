...
typedef ContextedPipelineBarrier<BuiltinPipelineBarrierCache::SWAPCHAIN> SwapchainPipelineBarrier;
...
bool Swapchain::changeSwapchain(FrameInfo &frame)
{
  ...
  if (formatList.empty())
  {
    debug("vulkan: Output format list was empty, aborting...");
    return false;
  }
  ...
  if (modes.empty())
  {
    debug("vulkan: Present mode list was empty, aborting...");
    return false;
  }
  else
  {
  ...
  VULKAN_EXIT_ON_FAIL(vkDev.vkCreateSwapchainKHR(vkDev.get(), &sci, nullptr, ptr(handle)));
  ...
  return updateSwapchainImageList(sci);
}
  ...
  swapImages.reserve(imageList.size());
  {
    ...
    for (auto &&image : imageList)
    {
      Image::Description desc = {{ii.flags, VK_IMAGE_TYPE_2D, ii.size, 1, 1, ii.usage, VkSampleCountFlagBits::VK_SAMPLE_COUNT_1_BIT},
        Image::MEM_NOT_EVICTABLE, activeMode.colorFormat, SN_INITIAL};
...
void Swapchain::doBlit(VulkanCommandBufferHandle cmd, Image *from, Image *to)
{
  makeReadyForBlit(cmd, from, to);
  ...
  VULKAN_LOG_CALL(vkDev.vkCmdBlitImage(cmd, from->getHandle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, to->getHandle(),
    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR));
}

void Swapchain::makeReadyForBlit(VulkanCommandBufferHandle cmd, Image *from, Image *to)
{
  SwapchainPipelineBarrier barrier(vkDev);
  barrier.modifyImageTemplate(0, 1, 0, 1);

  barrier.modifyImageTemplate(from);
  barrier.addImageByTemplateFiltered((ImageState)from->getStateFromIndex(0), SN_COPY_FROM);
  barrier.modifyImageTemplate(to);
  barrier.addImageByTemplateFiltered((ImageState)to->getStateFromIndex(0), SN_COPY_TO);

  from->setStateForIndex(0, SN_COPY_FROM);
  to->setStateForIndex(0, SN_COPY_TO);

  if (!barrier.empty())
    barrier.submit(cmd);
}

void Swapchain::makeReadyForPresent(VulkanCommandBufferHandle cmd, Image *img)
{
  // handle case when we are not rendering, but presenting
  // fill screen with dummy black or last rendered image if it exists
  if (img->getState(0, 0) == SN_INITIAL)
  {
    bool useDummy = (lastRenderedImage == nullptr) || (lastRenderedImage->getState(0, 0) == SN_INITIAL);
    Image *fallbackImage = useDummy ? device.getDummy2DImage() : lastRenderedImage;

    doBlit(cmd, fallbackImage, img);

    if (useDummy)
    {
      // restore dummy state, to avoid barriers inside RP
      // also restores seal consistency
      SwapchainPipelineBarrier barrier(vkDev);
      barrier.modifyImageTemplate(0, 1, 0, 1);
      barrier.modifyImageTemplate(fallbackImage);
      barrier.addImageByTemplateFiltered((ImageState)fallbackImage->getStateFromIndex(0), SN_SAMPLED_FROM);
      fallbackImage->setStateForIndex(0, SN_SAMPLED_FROM);
      if (!barrier.empty())
        barrier.submit(cmd);
    }
  }
  else if (lastRenderedImage)
    doBlit(cmd, img, lastRenderedImage);

  SwapchainPipelineBarrier barrier(vkDev, 0, 0);
  barrier.modifyImageTemplate(0, 1, 0, 1);

  barrier.modifyImageTemplate(img);
  barrier.addImagePresent((ImageState)img->getStateFromIndex(0));
  img->resetState(0, 0, SN_INITIAL);
  barrier.submit(cmd);
}
...
void Swapchain::prePresent(VulkanCommandBufferHandle cmd, FrameInfo &frame)
{
  if (currentState == SWP_DELAYED_ACQUIRE)
  {
    ...
    if (acquireSwapImage(frame))
    {
      ...
      else
      {
        ...
        if (offscreenBuffer->getState(0, 0) != SN_INITIAL)
          doBlit(cmd, offscreenBuffer, swapImages[colorTargetIndex]);
      }
      // tell the device we do not need the contents any more and make layout transition fast
      offscreenBuffer->resetState(0, 0, SN_INITIAL);
  ...
  {
    makeReadyForPresent(cmd, swapImages[colorTargetIndex]);
    changeSwapState(SWP_PRESENT);
  }
}

void Swapchain::present(FrameInfo &frame)
{
...
void Swapchain::onFrameBegin(FrameInfo &frame)
{
  if (currentState == SWP_HEADLESS)
  {
...
void Swapchain::changeSwapchainMode(FrameInfo &frame, const SwapchainMode &new_mode)
{
