...
ImageState RenderPassResource::imageStateFromLayout(VkImageLayout layout)
{
  if (layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL)
    return SN_CONST_RENDER_TARGET;
  else if (layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
    return SN_SAMPLED_FROM;
  else
    return SN_RENDER_TARGET;
}
...
void RenderPassResource::updateImageState(ExecutionContext &ctx, uint32_t att_index, bool need_external_barrier, ImageState new_state)
{
  ...
  uint16_t arrayEnd = arrayBase + bakedAttachments->layerCounts[att_index];

  uint8_t mipBase = tgt.mipLevel;
  uint8_t mipEnd = mipBase + 1;

  if (need_external_barrier)
    ctx.setFrameImageState(tgt.image, new_state, {mipBase, mipEnd}, {arrayBase, arrayEnd});
  else
    ctx.fixFrameImageState(tgt.image, new_state, {mipBase, mipEnd}, {arrayBase, arrayEnd});
}

void RenderPassResource::updateImageStatesForCurrentSubpass(ExecutionContext &ctx)
{
  for (uint32_t i = 0; i < desc.targetCount; ++i)
    updateImageState(ctx, i, activeSubpass == 0 /*need_external_barrier*/, desc.attImageStates[activeSubpass][i]);
}
  ...
  uint32_t register_index, const VariatedGraphicsPipeline *pipeline)
{
  ...
  tgt.setTinputAttachment(register_index - spirv::T_INPUT_ATTACHMENT_OFFSET, state->targets.data[attIdx].image,
    ...
    desc.attImageStates[activeSubpass][attIdx] == SN_CONST_RENDER_TARGET, bakedAttachments->views[attIdx]);
}
...
void RenderPassResource::advanceSubpass(ExecutionContext &ctx)
{
  ...
  performSelfDepsForSubpass(activeSubpass, ctx.getFrameCmdBuffer());
  ++activeSubpass;
  ctx.popEventRaw();
...
void RenderPassResource::beginPass(ExecutionContext &ctx)
{
  ...
  ctx.vkDev.vkCmdBeginRenderPass(ctx.getFrameCmdBuffer(), &rpbi, VK_SUBPASS_CONTENTS_INLINE);
...
void RenderPassResource::nextSubpass(ExecutionContext &ctx)
{
  ...
  ctx.vkDev.vkCmdNextSubpass(ctx.getFrameCmdBuffer(), VK_SUBPASS_CONTENTS_INLINE);
...
void RenderPassResource::endPass(ExecutionContext &ctx)
{
  ...
  ctx.vkDev.vkCmdEndRenderPass(ctx.getFrameCmdBuffer());
