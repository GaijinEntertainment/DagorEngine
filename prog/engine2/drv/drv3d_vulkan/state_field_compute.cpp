...
void StateFieldComputeFlush::applyTo(BackComputeStateStorage &state, ExecutionContext &target) const
{
  ...
  target.ensureBoundImageStates(regRef.header, STAGE_CS);
  ...
  ctxState.stageState[STAGE_CS].apply(target.vkDev, target.device.getDummyResourceTable(), ctxState.frame->index, regRef, target,
    ...
    {
      VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_COMPUTE, layoutHandle,
