...
void StateFieldGraphicsScissor::applyTo(BackDynamicGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  ActiveExecutionStage stage = target.back.executionState.getRO<StateFieldActiveExecutionStage, ActiveExecutionStage>();
  bool drawExpected = target.back.executionState.getRO<StateFieldGraphicsFlush, bool, BackGraphicsState>();
  if (stage != ActiveExecutionStage::GRAPHICS || !drawExpected)
    return;

  VULKAN_LOG_CALL(target.vkDev.vkCmdSetScissor(target.getFrameCmdBuffer(), 0, 1, enabled ? &rect : &viewRect));
}
...
void StateFieldGraphicsDepthBias::applyTo(BackDynamicGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  VULKAN_LOG_CALL(
    target.vkDev.vkCmdSetDepthBias(target.getFrameCmdBuffer(), data.bias / MINIMUM_REPRESENTABLE_D16, 0.f, data.slopedBias));
}
...
void StateFieldGraphicsDepthBounds::applyTo(BackGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetDepthBounds(target.getFrameCmdBuffer(), data.min, data.max));
}
...
void StateFieldGraphicsStencilRef::applyTo(BackDynamicGraphicsStateStorage &state, ExecutionContext &target) const
{
  if (state.stencilRefOverride.data == STENCIL_REF_OVERRIDE_DISABLE)
    VULKAN_LOG_CALL(target.vkDev.vkCmdSetStencilReference(target.getFrameCmdBuffer(), VK_STENCIL_FACE_BOTH_BIT, data));
}
...
void StateFieldGraphicsStencilRefOverride::applyTo(BackDynamicGraphicsStateStorage &state, ExecutionContext &target) const
{
  ...
  if (data != STENCIL_REF_OVERRIDE_DISABLE)
    newStencilRef = data;
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetStencilReference(target.getFrameCmdBuffer(), VK_STENCIL_FACE_BOTH_BIT, newStencilRef));
}
...
{
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetStencilWriteMask(target.getFrameCmdBuffer(), VK_STENCIL_FACE_BOTH_BIT, data));
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetStencilCompareMask(target.getFrameCmdBuffer(), VK_STENCIL_FACE_BOTH_BIT, data));
}
...
void StateFieldGraphicsBlendConstantFactor::applyTo(BackGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetBlendConstants(target.getFrameCmdBuffer(), values));
}
...
void StateFieldGraphicsIndexBuffer::applyTo(BackGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  // false positives are triggered, as actual usage is not here
  // data.buffer->checkDead();
  VULKAN_LOG_CALL(target.vkDev.vkCmdBindIndexBuffer(target.getFrameCmdBuffer(), data.getHandle(), data.dataOffset(0), indexType));
}
...
void StateFieldGraphicsViewport::applyTo(BackGraphicsStateStorage &state, ExecutionContext &target) const
{
  // workaround for adreno drivers crash on viewport/scissor setted in command buffer before cs dispatch
  ActiveExecutionStage stage = target.back.executionState.getRO<StateFieldActiveExecutionStage, ActiveExecutionStage>();
  bool drawExpected = state.flush.needPipeline;
  if (stage != ActiveExecutionStage::GRAPHICS || !drawExpected)
    return;
  ...
  VULKAN_LOG_CALL(target.vkDev.vkCmdSetViewport(target.getFrameCmdBuffer(), 0, 1, &translatedViewport));
}
...
void StateFieldGraphicsVertexBuffersBindArray::applyTo(BackGraphicsStateStorage &, ExecutionContext &target) const
{
  ...
  VULKAN_LOG_CALL(target.vkDev.vkCmdBindVertexBuffers(target.getFrameCmdBuffer(), 0, cnt, ary(buffers), offsets));
}
...
void StateFieldGraphicsFlush::applyDescriptors(BackGraphicsStateStorage &state, ExecutionContext &target) const
{
  ...
  ctx.stageState[STAGE_VS].apply(vkDev, drvDev.getDummyResourceTable(), ctx.frame->index, regs.vs(), target,
    [&target, layoutHandle](VulkanDescriptorSetHandle set, const uint32_t *offsets, uint32_t offset_count) //
    {
      VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, layoutHandle,
  ...
  ctx.stageState[STAGE_PS].apply(vkDev, drvDev.getDummyResourceTable(), ctx.frame->index, regs.fs(), target,
    [&target, layoutHandle](VulkanDescriptorSetHandle set, const uint32_t *offsets, uint32_t offset_count) //
    {
      VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, layoutHandle,
  ...
  if (withGS)
  {
    ctx.stageState[STAGE_VS].apply(vkDev, drvDev.getDummyResourceTable(), ctx.frame->index, regs.gs(), target,
      [&target, layoutHandle](VulkanDescriptorSetHandle set, const uint32_t *offsets, uint32_t offset_count) //
      {
        VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, layoutHandle,
  ...
  if (withTC)
  {
    ctx.stageState[STAGE_VS].apply(vkDev, drvDev.getDummyResourceTable(), ctx.frame->index, regs.tc(), target,
      [&target, layoutHandle](VulkanDescriptorSetHandle set, const uint32_t *offsets, uint32_t offset_count) //
      {
        VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, layoutHandle,
  ...
  if (withTE)
  {
    ctx.stageState[STAGE_VS].apply(vkDev, drvDev.getDummyResourceTable(), ctx.frame->index, regs.te(), target,
      [&target, layoutHandle](VulkanDescriptorSetHandle set, const uint32_t *offsets, uint32_t offset_count) //
      {
        VULKAN_LOG_CALL(target.vkDev.vkCmdBindDescriptorSets(target.getFrameCmdBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, layoutHandle,
...
void StateFieldGraphicsFlush::applyBarriers(BackGraphicsStateStorage &state, ExecutionContext &target) const
{
  ...
  target.ensureBoundImageStates(regs.vs().header, STAGE_VS);
  target.ensureBoundImageStates(regs.fs().header, STAGE_PS);
  if (withGS)
    target.ensureBoundImageStates(regs.gs().header, STAGE_VS);
  if (withTC)
    target.ensureBoundImageStates(regs.tc().header, STAGE_VS);
  if (withTE)
    target.ensureBoundImageStates(regs.te().header, STAGE_VS);
}
