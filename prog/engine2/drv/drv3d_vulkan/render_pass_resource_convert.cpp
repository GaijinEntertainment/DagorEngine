...
void RenderPassResource::storeImageStates(const Tab<VkSubpassDescription> &subpasses, const Tab<VkAttachmentDescription> &attachments)
{
  ...
  clear_and_shrink(desc.attImageStates);
  for (uint32_t i = 0; i < subpasses.size(); ++i)
  {
    Tab<ImageState> imgStates;
    for (uint32_t j = 0; j < rpDesc.targetCount; ++j)
    {
      if (i == 0) // initial
        imgStates.push_back(imageStateFromLayout(attachments[j].initialLayout));
      else if (i == subpasses.size() - 1) // final
        imgStates.push_back(imageStateFromLayout(attachments[j].finalLayout));
      else
      {
        bool found = false;
        ...
        {
          found = true;
          imgStates.push_back(imageStateFromLayout(subpass.pDepthStencilAttachment->layout));
        }
        ...
        if (subpass.colorAttachmentCount && !found)
        {
          for (uint32_t k = 0; k < subpass.colorAttachmentCount; ++k)
          {
            ...
            {
              found = true;
              imgStates.push_back(imageStateFromLayout(subpass.pColorAttachments[k].layout));
              break;
            }
          }
        }
        ...
        if (subpass.inputAttachmentCount && !found)
        {
          for (uint32_t k = 0; k < subpass.inputAttachmentCount; ++k)
          {
            ...
            {
              found = true;
              imgStates.push_back(imageStateFromLayout(subpass.pInputAttachments[k].layout));
              break;
            }
          }
        }
        ...
        if (!found)
          imgStates.push_back(SN_RENDER_TARGET);
      }
    }
    desc.attImageStates.push_back(imgStates);
  }
}
