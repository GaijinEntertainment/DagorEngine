...
public:
  ...
  /// a power of 2. Note that at run-time we'll perform one more grid subdivision, so the effective block size is half of what is
  /// provided here.
  uint32 mBlockSize = 16;
...
private:
  ...
  uint32 mBlockSize = 16;                             ///< See HeightField16ShapeSettings::mBlockSize
  ...
  Array<RangeBlock> mRangeBlocks; ///< Hierarchical grid of range data describing the height variations within 1 block. The grid for
                                  ///< level <level> starts at offset sGridOffsets[<level>]
  const CompressedHeightmap *mHData = nullptr;
  const LandMeshHolesManager *holes = nullptr;
