...
void HeightField16Shape::CalculateActiveEdges()
{
  if (!sUseActiveEdges)
    return;
  ...
  for (uint y = 0; y < count_min_1; ++y)
  {
    for (uint x = 0; x < count_min_1; ++x)
    {
      ...
      bool edge1_active =
        y == count_min_1 - 1 || ActiveEdges::IsEdgeActive(normals[offset], normals[offset + 2 * count_min_1 + 1], x2y2 - x1y2);
...
void HeightField16Shape::StoreMaterialIndices(const Array<uint8> &inMaterialIndices)
{
  uint count_min_1 = mSampleCount - 1;

  mNumBitsPerMaterialIndex = 32 - CountLeadingZeros((uint32)mMaterials.size() - 1);
  mMaterialIndices.resize(((Square(count_min_1) * mNumBitsPerMaterialIndex + 7) >> 3) + 1); // Add 1 byte so we don't read out of
                                                                                            // bounds when reading an uint16
  ...
  mMaterials(inSettings.mMaterials)
{
  ...
  // Check block size
  if (mBlockSize < 2 || mBlockSize > 32)
  {
    outResult.SetError("HeightField16Shape: Block size must be in the range [2, 8]!");
    return;
  }
  ...
  // Reserve size for temporary range data + reserve 1 extra for a 1x1 grid that we won't store but use for calculating the bounding
  // box
  ...
  for (uint y = 0; y < n; ++y)
    for (uint x = 0; x < n; ++x)
    {
      ...
      range_dst->mMax = 0;
      uint max_bx = x == n - 1 ? mBlockSize : mBlockSize + 1; // for interior blocks take 1 more because the triangles connect to the
                                                              // next block so we must include their height too
      uint max_by = y == n - 1 ? mBlockSize : mBlockSize + 1;
      for (uint by = 0; by < max_by; ++by)
        for (uint bx = 0; bx < max_bx; ++bx)
        {
          uint16 h = mHData->decodePixelUnsafe(x * mBlockSize + bx, y * mBlockSize + by);
          if (h != cNoCollisionValue16)
          {
            range_dst->mMin = min(range_dst->mMin, h);
            range_dst->mMax = max(range_dst->mMax, uint16(h + 1)); // Add 1 to the max so we know the real value is between mMin and
                                                                   // mMax
          }
        }
      ++range_dst;
  ...
  bool inUseMaterialColors, bool inDrawWireframe) const
{
  ...
  if (sDrawTriangleOutlines)
  {
    struct Visitor
    {
      ...
      {
        UVec4 valid = UVec4::sOr(UVec4::sOr(Vec4::sLess(inBoundsMinX, inBoundsMaxX), Vec4::sLess(inBoundsMinY, inBoundsMaxY)),
...
public:
  ...
  JPH_INLINE void WalkHeightField(Visitor &ioVisitor)
  {
    ...
    uint max_level = HeightField16Shape::sGetMaxLevel(num_blocks);
    ...
    do
    {
      ...
      else
      {
        ...
        UVec4 block = UVec4::sLoadInt4Aligned(reinterpret_cast<const uint32 *>(&mShape->mRangeBlocks[offset])); //-V1032
...
{
  return Stats(sizeof(*this) + mMaterials.size() * sizeof(Ref<PhysicsMaterial>) + mRangeBlocks.size() * sizeof(RangeBlock) +
                 mActiveEdges.size() * sizeof(uint8) + mMaterialIndices.size() * sizeof(uint8),
