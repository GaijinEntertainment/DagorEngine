...
{
  api().SetPositionAndRotation(bodyId, to_jVec3(wtm.getcol(3)), to_jQuat(wtm),
...
JPH::RefConst<JPH::Shape> PhysBody::create_jolt_collision_shape(const PhysCollision *c)
{
  ...
  switch (c->collType)
  {
    ...
    case PhysCollision::TYPE_CAPSULE:
    {
      ...
      {
        case 0: return new JPH::RotatedTranslatedShape(JPH::Vec3::sZero(), JPH::Quat::sRotation(JPH::Vec3::sAxisZ(), M_PI / 2), shape);
        case 1: return shape;
        case 2: return new JPH::RotatedTranslatedShape(JPH::Vec3::sZero(), JPH::Quat::sRotation(JPH::Vec3::sAxisX(), M_PI / 2), shape);
    ...
    case PhysCollision::TYPE_CONVEXHULL:
    {
      ...
      if (convexColl->vstride == sizeof(JPH::Vec3))
        return JPH::ConvexHullShapeSettings((const JPH::Vec3 *)convexColl->vdata, convexColl->vnum).Create().Get();
      ...
      return JPH::ConvexHullShapeSettings(v.data(), v.size()).Create().Get();
    }
    break;
    ...
    case PhysCollision::TYPE_HEIGHTFIELD:
    {
      ...
      // hmapColl->hmStep ?
      return JPH::HeightField16ShapeSettings(&hmap->getCompressedData(), hmapColl->holes,
    ...
    case PhysCollision::TYPE_NATIVE_SHAPE:
    {
      ...
      if ((ns->localScale - JPH::Vec3::sReplicate(1.0f)).LengthSq() < 1e-6f)
        return ns->shape;
      return JPH::ScaledShapeSettings(ns->shape, ns->localScale).Create().Get();
    }
    break;
...
{
  auto shape = getShape();
  return shape ? new JoltPhysNativeShape(shape, to_jVec3(scale)) : nullptr;
}
void PhysBody::patchCollisionScaledCopy(const Point3 &s, PhysBody *)
{
  if (auto shape = getShape())
  {
    ...
    api().SetShape(bodyId, JPH::ScaledShapeSettings(shape, to_jVec3(s)).Create().Get(), false,
      isInWorld() ? JPH::EActivation::Activate : JPH::EActivation::DontActivate);
  }
}
...
struct JoltSimJob : public cpujobs::IJob
{
  int numSubSteps = 1;
  uint32_t qPos = 0;
  float dt = 0;
  void doJob() override
  {
    ...
    jolt_api::phys_sys().Update(dt, numSubSteps, tempAllocator, jobSystem);
    if (JoltJobSystemImpl2::self)
...
void PhysWorld::startSim(real dt, bool wake_up_thread)
{
  ...
  sim_job.numSubSteps = numSubSteps;
  sim_job.dt = dt;
