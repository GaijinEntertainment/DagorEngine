...
public:
  ...
  void DrawTriangle(JPH::Vec3Arg inV1, JPH::Vec3Arg inV2, JPH::Vec3Arg inV3, JPH::ColorArg inColor) override { debug_cp(); }
...
void physdbg::renderOneBody(PhysWorld *pw, const PhysBody *pb, const TMatrix &tm, RenderFlags rflg, unsigned col)
{
  ...
  pb->lockRO([&](const JPH::Body &body) {
    ...
    if (shape->GetSubType() != JPH::EShapeSubType::HeightField)
      shape->Draw(dbgRend, to_jMat44(tm).PostTranslated(shape->GetCenterOfMass()), JPH::Vec3::sReplicate(1.0f), color, false, true);
  });
