...
struct AddBodyToWorldAction : public AfterPhysUpdateAction
{
  PhysBody *body;
  unsigned short grp, mask;

  AddBodyToWorldAction(PhysBody *in_body, unsigned int in_grp, unsigned int in_mask) : body(in_body), grp(in_grp), mask(in_mask) {}

  virtual ~AddBodyToWorldAction() {}

  void doAction(bool) override { body->getPhysWorld()->getScene()->addRigidBody(body->getActor(), grp, mask); }
};
...
struct UpdateSingleAabbAction final : public AfterPhysUpdateAction
{
  ...
  UpdateSingleAabbAction(btCollisionObject *coll_obj, PhysWorld *in_world) : collObj(coll_obj), world(in_world) {}

  void doAction(bool) override { world->getScene()->updateSingleAabb(collObj); }
};
...
struct SetTmAction final : public AfterPhysUpdateAction
{
  ...
  SetTmAction(btRigidBody *b, PhysWorld *w, const btTransform &tm_, bool ums = true, bool ua = true) :
    body(b), world(w), tm(tm_), upMs(ums), upAabb(ua)
  {}
...
void PhysBody::setTm(const TMatrix &wtm)
{
  ...
  if (body->isInWorld())
    exec_or_add_after_phys_action<SetTmAction>(*world, body, world, finalTm);
  else
    SetTmAction(body, world, finalTm, /*upMs*/ true, /*upAabb*/ false).doAction(true);
}
...
void PhysBody::setTmInstant(const TMatrix &wtm, bool upd_aabb)
{
  btTransform finalTm = to_btTransform(multTmNonIdent ? (wtm * multTm) : wtm);
  if (body->isInWorld())
    exec_or_add_after_phys_action<SetTmAction>(*world, body, world, finalTm, /*upMs*/ false, upd_aabb);
  else
...
struct SetInteractionLayerAction : public AfterPhysUpdateAction
{
  PhysBody *body;
  unsigned int group;
  unsigned int mask;

  SetInteractionLayerAction(PhysBody *in_body, unsigned int gr, unsigned int msk) : body(in_body), group(gr), mask(msk) {}
  virtual ~SetInteractionLayerAction() {}


  void doAction(bool) override
  {
    btBroadphaseProxy *proxy = body->getActor()->getBroadphaseProxy();
    if (proxy != nullptr)
    {
    ...
    {
      body->getPhysWorld()->getScene()->removeCollisionObject(body->getActor());
      body->getPhysWorld()->getScene()->addRigidBody(body->getActor(), group, mask);
    }
  }
};
...
{
  exec_or_add_after_phys_action<SetInteractionLayerAction>(*world, this, groupMask = group, layerMask = mask);
}

struct ReAddToWorldAction : public AfterPhysUpdateAction
{
  PhysBody *body;
  unsigned short grp, mask;

  ReAddToWorldAction(PhysBody *in_body, unsigned int in_grp, unsigned int in_mask) : body(in_body), grp(in_grp), mask(in_mask) {}

  virtual ~ReAddToWorldAction() {}

  void doAction(bool) override
  {
    body->getPhysWorld()->getScene()->removeRigidBody(body->getActor());
    body->getPhysWorld()->getScene()->addRigidBody(body->getActor(), grp, mask);
  }
};
...
struct ActivateBodyAction final : public AfterPhysUpdateAction
{
  ...
  ActivateBodyAction(btRigidBody *body_, bool force_a = false) : body(body_), forceActivation(force_a) {}

  void doAction(bool) override { body->activate(forceActivation); }
};
...
struct SetCollisionAction : public AfterPhysUpdateAction
{
  ...
  SetCollisionAction(PhysBody *in_body, btCollisionShape *in_collider) : body(in_body), collider(in_collider) {}

  virtual ~SetCollisionAction() {}

  void doAction(bool) override
  {
    body->getPhysWorld()->getScene()->removeCollisionObject(body->getActor());
    if (!collider)
      delete body->getActor()->getCollisionShape();
    body->getActor()->setCollisionShape(collider);
    body->getPhysWorld()->getScene()->addRigidBody(body->getActor(), body->getGroupMask(), body->getInteractionLayer());
  }
};
...
struct SetStuffAndActivate<void (btRigidBody::*)(Args...), setter> final : public AfterPhysUpdateAction
{
  ...
  SetStuffAndActivate(btRigidBody *b, bool act, Args... args_) : body(b), activate(act), args(args_...) {}
...
struct AddImpulseAction final : public AfterPhysUpdateAction
{
  ...
  AddImpulseAction(btRigidBody *b, const Point3 &p, const Point3 &i, bool a) : body(b), pos(p), imp(i), activate(a) {}
...
struct DestroyJointAction final : public AfterPhysUpdateAction
{
  ...
  DestroyJointAction(btDiscreteDynamicsWorld *w, btTypedConstraint *j) : world(w), joint(j) {}
...
PhysJoint::~PhysJoint()
{
  ...
  exec_or_add_after_phys_action<DestroyJointAction>(*w, w->getScene(), joint);
}
...
bool PhysWorld::fetchSimRes(bool wait)
{
  if (wait)
  {
#if DAGOR_DBGLEVEL > 0
    ...
    if (multithreadedUpdate)
      threadpool::wait(this);
    if (!delayedActions.empty() && isControledByCurThread())
    {
...
void PhysWorld::updateAabb(btCollisionObject *obj) { exec_or_add_after_phys_action<UpdateSingleAabbAction>(*this, obj, this); }
...
struct AddConstraintToWorldAction final : public AfterPhysUpdateAction
{
  ...
  AddConstraintToWorldAction(PhysJoint *in_joint, PhysWorld *in_world) : joint(in_joint->getJoint()), world(in_world->getScene()) {}

  void doAction(bool) override { world->addConstraint(joint, /*disableCollisionsBetweenLinkedBodies*/ true); }
};

PhysJoint *PhysWorld::regJoint(PhysJoint *j)
{
  exec_or_add_after_phys_action<AddConstraintToWorldAction>(*this, j, this);
  return j;
}
