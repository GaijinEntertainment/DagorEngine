...
PhysSystemInstance::PhysSystemInstance(PhysicsResource *res, PhysWorld *world, void *userData, real sleep_threshold) :
  resource(res), bodies(midmem), joints(midmem), scaleTm(TMatrix::IDENT)
{
  G_ASSERT(res);
  G_ASSERT(world);

  bodies.resize(resource->bodies.size());
  ...
  for (int i = 0; i < bodies.size(); ++i)
    bodies[i].init(resource->bodies[i], world, userData, jcnt > 0);
  ...
  for (int i = 0; i < resource->rdBallJoints.size(); ++i)
  {
    ...
    PhysJoint *j = world->createRagdollBallJoint(bodies[jnt.body1].body, bodies[jnt.body2].body, jnt.tm, jnt.minLimit, jnt.maxLimit,
      jnt.damping, jnt.twistDamping, jnt.stiffness, sleep_threshold);
  ...
  for (int i = 0; i < resource->rdHingeJoints.size(); ++i)
  {
    ...
    PhysJoint *j = world->createRagdollHingeJoint(bodies[jnt.body1].body, bodies[jnt.body2].body, jnt.pos, jnt.axis, jnt.midAxis,
      jnt.xAxis, jnt.angleLimit, jnt.damping, jnt.stiffness, sleep_threshold);
  ...
  for (int i = 0; i < resource->revoluteJoints.size(); ++i)
  {
    ...
    PhysJoint *j = world->createRevoluteJoint(bodies[jnt.body1].body, bodies[jnt.body2].body, jnt.pos, jnt.axis, jnt.minAngle,
      jnt.maxAngle, jnt.minRestitution, jnt.maxRestitution, jnt.spring, jnt.projType, jnt.projAngle, jnt.projDistance, jnt.damping,
      jnt.flags, sleep_threshold);
  ...
  for (int i = 0; i < resource->sphericalJoints.size(); ++i)
  {
    ...
    PhysJoint *j = world->createSphericalJoint(bodies[jnt.body1].body, bodies[jnt.body2].body, jnt.pos, jnt.dir, jnt.axis,
...
PhysSystemInstance::Body::Body() : body(NULL), tmHelpers(midmem) {}


PhysSystemInstance::Body::~Body() { del_it(body); }


void PhysSystemInstance::Body::init(PhysicsResource::Body &res_body, PhysWorld *world, void *userData, bool has_joints)
{
  //  debug("----------- body '%s' mass=%g  " FMT_P3 "", (const char*)res_body.name, res_body.mass, P3D(res_body.momj));
  //  debug("-----------  wtm=" FMT_TM "", TMD(res_body.tm));
  ...
  body = new PhysBody(world, res_body.mass, &coll, res_body.tm, pbcd);
