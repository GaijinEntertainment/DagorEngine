...
template <>
void DynamicPhysObject::init(const DynamicPhysObjectData *phys_obj_data, PhysWorld *world)
{
  if (phys_obj_data->physRes && world)
    physSys = new PhysSystemInstance(phys_obj_data->physRes, world, &ud, 0.1);
...
template <>
DynamicPhysObject *DynamicPhysObject::create(const DynamicPhysObjectData *data, PhysWorld *world, const TMatrix *tm)
{
  if (!data || !world)
    return NULL;
...
void DynamicPhysObject::beforeRender(const Point3 &cam_pos)
{
  if (physSys && modelEntries.size() == 1 && physSys->getBodyCount() == 1)
  {
    ...
    {
      modelEntries[0]->model->beforeRender();
      return;
    }
  }
