...
real AnimcharRendComponent::getSqDistanceToViewerLegacy(const mat44f &root_wtm) const
{
  vec4f rootPos = v_mat43_mul_vec3p(ownerTm, root_wtm.col3);
  return v_extract_x(v_length3_sq_x(v_sub(v_ldu(&::grs_cur_view.pos.x), rootPos)));
}
...
void AnimcharRendComponent::render(real transp)
{
...
void AnimcharRendComponent::renderTrans(real transp)
{
