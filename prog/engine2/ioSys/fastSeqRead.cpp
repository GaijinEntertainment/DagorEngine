...
void FastSeqReader::assignFile(void *handle, const char *fname, int size, int min_chunk_size, int max_back_seek)
{
  ...
  if (unusedMask && readAheadPos < file.size)
    for (int i = 0, bit = 1; i < BUF_CNT; i++, bit <<= 1)
      ...
      again:
        ...
        G_VERIFY(dfa_read_async(file.handle, buf[i].handle, buf[i].sa, buf[i].data, buf[i].ea - buf[i].sa));
...
bool FastSeqReadCB::open(const char *fname, int max_back_seek, const char *base_path)
{
  ...
  const char *real_name = df_get_real_name(fname);
  (void)base_path;
  if (!real_name)
    return false;

  void *handle = dfa_open_for_read(real_name, true);
  if (!handle)
    return false;

  int size = dfa_file_length(handle);
  if (size <= 0)
  {
    dfa_close(handle);
    return false;
  ...
  assignFile(handle, fname, size, dfa_chunk_size(real_name), max_back_seek);
  return true;
}
