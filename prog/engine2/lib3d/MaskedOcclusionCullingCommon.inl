...
// Tile dimensions are 32xN pixels. These values are not tweakable and the code must also be
// modified to support different tile sizes as it is tightly coupled with the SSE/AVX register size
#define TILE_WIDTH_SHIFT 5
#define TILE_WIDTH       (1 << TILE_WIDTH_SHIFT)
#define TILE_HEIGHT      (1 << TILE_HEIGHT_SHIFT)

// Sub-tiles (used for updating the masked HiZ buffer) are 8x4 tiles, so there are 4x2 sub-tiles in
// a tile
...
#else
// Note that too low precision, without precise coverage, may cause overshoots / false coverage
// during rasterization. This is configured for 14 bits for AVX512 and 16 bits for SSE. Max tile
// slope delta is roughly (screenWidth + 2*(GUARD_BAND_PIXEL_SIZE + 1)) * (2^FP_BITS * (TILE_HEIGHT
// + GUARD_BAND_PIXEL_SIZE + 1)) and must fit in 31 bits. With this config, max image resolution
// (width) is ~3272, so stay well clear of this limit.
...
// Maximum number of triangles that may be generated during clipping. We process SIMD_LANES
// triangles at a time and clip against 5 planes, so the max should be 5*8 = 40 (we immediately draw
// the first clipped triangle). This number must be a power of two.
#define MAX_CLIPPED      (8 * SIMD_LANES)
#define MAX_CLIPPED_WRAP (MAX_CLIPPED - 1)

// Size of guard band in pixels. Clipping doesn't seem to be very expensive so we use a small guard
// band to improve rasterization performance. It's not recommended to set the guard band to zero, as
// this may cause leakage along the screen border due to precision/rounding.
#define GUARD_BAND_PIXEL_SIZE 1.0f

// We classify triangles as big if the bounding box is wider than this given threshold and use a
// tighter but slightly more expensive traversal algorithm. This improves performance greatly for
// sliver triangles
...
class MaskedOcclusionCullingPrivate final : public MaskedOcclusionCulling
{
public:
  struct ZTile
  {
  ...
  ~MaskedOcclusionCullingPrivate() MOC_OVERRIDE
  {
  ...
  FORCE_INLINE void SetResolution(unsigned int width, unsigned int height) MOC_OVERRIDE
  {
    // Resolution must be a multiple of the subtile size
    assert(width % SUB_TILE_WIDTH == 0 && height % SUB_TILE_HEIGHT == 0);
#if PRECISE_COVERAGE == 0
    // Test if combination of resolution & SLOPE_FP_BITS bits may cause 32-bit overflow. Note that
    // the maximum resolution estimate is only an estimate (not conservative). It's advicable to
    // stay well below the limit.
    assert(width < ((1U << 31) - 1U) / ((1U << FP_BITS) * (TILE_HEIGHT + (unsigned int)(GUARD_BAND_PIXEL_SIZE + 1.0f))) -
                     (2U * (unsigned int)(GUARD_BAND_PIXEL_SIZE + 1.0f)));
#endif
    ...
    float guardBandWidth = mWidth != 0 ? (2.0f / (float)mWidth) * GUARD_BAND_PIXEL_SIZE : 0;
    float guardBandHeight = mHeight != 0 ? (2.0f / (float)mHeight) * GUARD_BAND_PIXEL_SIZE : 0;
  ...
  FORCE_INLINE void GetResolution(unsigned int &width, unsigned int &height) const MOC_OVERRIDE
  {
    width = mWidth;
    height = mHeight;
  }

  FORCE_INLINE void ComputeBinWidthHeight(unsigned int nBinsW, unsigned int nBinsH, unsigned int &outBinWidth,
    unsigned int &outBinHeight) MOC_OVERRIDE
  {
    outBinWidth = (mWidth / nBinsW) - ((mWidth / nBinsW) % TILE_WIDTH);
    outBinHeight = (mHeight / nBinsH) - ((mHeight / nBinsH) % TILE_HEIGHT);
  }

  FORCE_INLINE void SetNearClipPlane(float nearDist) MOC_OVERRIDE
  {
  ...
  FORCE_INLINE float GetNearClipPlane() const MOC_OVERRIDE { return mNearDist; }

  FORCE_INLINE void ClearBuffer() MOC_OVERRIDE
  {
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  FORCE_INLINE void MergeBuffer(MaskedOcclusionCulling *BufferB) MOC_OVERRIDE
  {
    assert(mMaskedHiZBuffer != nullptr);
    ...
    for (int i = 0; i < mTilesWidth * mTilesHeight; i++)
    {
      ...
      // Only merge tiles that have data in zMinB[0], use the sign bit to determine if they are
      // still in a clear state
      ...
      else
      {
        ...
        if (!_mmw_testz_epi32(sign0, sign0))
        {
          // build a mask for Zmin[0], full if the layer has been completed, or partial if tile is
          // still partly filled. cant just use the completement of the mask, as tiles might not get
          // updated by merge
  ...
  {
    __m128 p0 = (n > 0) ? inVtx[n - 1] : _mm_setzero_ps();
  ...
  FORCE_INLINE void ClipTriangleAndAddToBuffer(__mw *vtxX, __mw *vtxY, __mw *vtxW, __m128 *clippedTrisBuffer, int &clipWriteIdx,
    unsigned int &triMask, unsigned int triClipMask, ClipPlanes clipPlaneMask)
  {
    if (!triClipMask)
      return;
    ...
    unsigned int clipAndStraddleMask =
      (straddleMask[0] | straddleMask[1] | straddleMask[2] | straddleMask[3] | straddleMask[4]) & clipMask;
    ...
    while (clipMask)
    {
      ...
      for (int i = 0; i < 5; ++i)
      {
        if ((straddleMask[i] & triBit) && (clipPlaneMask & (1 << i))) // <- second part maybe not
                                                                      // needed?
        {
    ...
    unsigned int clipMask =
      (straddleMask[0] | straddleMask[1] | straddleMask[2] | straddleMask[3] | straddleMask[4]) & (triClipMask & triMask);
    while (clipMask)
    {
      // Find and setup next triangle to clip
      unsigned int triIdx = find_clear_lsb(&clipMask);
  ...
  FORCE_INLINE void TransformVerts(__mw *vtxX, __mw *vtxY, __mw *vtxW, const float *modelToClipMatrix)
  {
    if (modelToClipMatrix != nullptr)
    {
      ...
      {
        __mw tmpX, tmpY, tmpW;
        tmpX = _mmw_fmadd_ps(vtxX[i], _mmw_set1_ps(modelToClipMatrix[0]),
          _mmw_fmadd_ps(vtxY[i], _mmw_set1_ps(modelToClipMatrix[4]),
            _mmw_fmadd_ps(vtxW[i], _mmw_set1_ps(modelToClipMatrix[8]), _mmw_set1_ps(modelToClipMatrix[12]))));
        tmpY = _mmw_fmadd_ps(vtxX[i], _mmw_set1_ps(modelToClipMatrix[1]),
          _mmw_fmadd_ps(vtxY[i], _mmw_set1_ps(modelToClipMatrix[5]),
            _mmw_fmadd_ps(vtxW[i], _mmw_set1_ps(modelToClipMatrix[9]), _mmw_set1_ps(modelToClipMatrix[13]))));
        tmpW = _mmw_fmadd_ps(vtxX[i], _mmw_set1_ps(modelToClipMatrix[3]),
          _mmw_fmadd_ps(vtxY[i], _mmw_set1_ps(modelToClipMatrix[7]),
            _mmw_fmadd_ps(vtxW[i], _mmw_set1_ps(modelToClipMatrix[11]), _mmw_set1_ps(modelToClipMatrix[15]))));
        vtxX[i] = tmpX;
        vtxY[i] = tmpY;
        vtxW[i] = tmpW;
      }
...
#if PRECISE_COVERAGE != 0
  FORCE_INLINE void ProjectVertices(__mwi *ipVtxX, __mwi *ipVtxY, __mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw *vtxX,
    const __mw *vtxY, const __mw *vtxW)
  {
#if USE_D3D != 0
    ...
    // Project vertices and transform to screen space. Snap to sub-pixel coordinates with FP_BITS
    // precision.
    for (int i = 0; i < 3; i++)
    {
  ...
  FORCE_INLINE void ProjectVertices(__mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw *vtxX, const __mw *vtxY, const __mw *vtxW)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      // The rounding modes are set to match HW rasterization with OpenGL. In practice our samples
      // are placed in the (1,0) corner of each pixel, while HW rasterizer uses (0.5, 0.5). We get
      // (1,0) because of the floor used when interpolating along triangle edges. The rounding modes
      // match an offset of (0.5, -0.5)
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Common SSE/AVX input assembly functions, note that there are specialized gathers for the
  // general case in the SSE/AVX specific files
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  FORCE_INLINE void GatherVerticesFast(__mw *vtxX, __mw *vtxY, __mw *vtxW, const float *inVtx, const unsigned short *inTrisPtr,
    int numLanes)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      // Load 4 (x,y,z,w) vectors per SSE part of the SIMD register (so 4 vectors for SSE, 8 vectors
      // for AVX) this fetch uses templates to unroll the loop
      ...
      vtxW[i] = _mmw_shuffle_ps_3131(swz[2], swz[3]);
    }
    (void)swz;
  }

  FORCE_INLINE void GatherVerticesFastZ(__mw *vtxX, __mw *vtxY, __mw *vtxW, const float *inVtx, const unsigned short *inTrisPtr,
    int numLanes)
  {
    // This function assumes that the vertex layout is four packed x, y, z, w-values.
    // Since the layout is known we can get some additional performance by using a
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      vtxX[i] = _mmw_shuffle_ps_2020(swz[0], swz[1]);
      vtxY[i] = _mmw_shuffle_ps_3131(swz[0], swz[1]);
      vtxW[i] = _mmw_shuffle_ps_2020(swz[2], swz[3]);
    }
    (void)swz;
  }
  ...
  FORCE_INLINE void ComputeBoundingBox(__mwi &bbminX, __mwi &bbminY, __mwi &bbmaxX, __mwi &bbmaxY, const __mw *vX, const __mw *vY,
  ...
  FORCE_INLINE int CullBackfaces(__mwi *ipVtxX, __mwi *ipVtxY, __mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw &ccwMask,
    BackfaceWinding bfWinding)
  {
    // Reverse vertex order if non cw faces are considered front facing (rasterizer code requires
    // CCW order)
  ...
  FORCE_INLINE void UpdateTileQuick(int tileIdx, const __mwi &coverage, const __mw &zTriv)
  {
    ...
    // Use distance heuristic to discard layer 1 if incoming triangle is significantly nearer to
    // observer than the buffer contents. See Section 3.2 in "Masked Software Occlusion Culling"
    ...
    // Compute new value for zMin[1]. This has one of four outcomes: zMin[1] = min(zMin[1], zTriv),
    // zMin[1] = zTriv, zMin[1] = FLT_MAX or unchanged, depending on if the layer is updated,
    // discarded, fully covered, or not updated
  ...
  FORCE_INLINE int TraverseScanline(int leftOffset, int rightOffset, int tileIdx, int rightEvent, int leftEvent, const __mwi *events,
    const __mw &zTriMin, const __mw &zTriMax, const __mw &iz0, float zx)
  {
    ...
    for (int i = 0; i < NRIGHT; ++i)
      right[i] =
        _mmw_max_epi32(_mmw_sub_epi32(_mmw_srai_epi32(events[rightEvent + i], FP_BITS), _mmw_set1_epi32(eventOffset)), SIMD_BITS_ZERO);
    for (int i = 0; i < NLEFT; ++i)
      left[i] =
        _mmw_max_epi32(_mmw_sub_epi32(_mmw_srai_epi32(events[leftEvent - i], FP_BITS), _mmw_set1_epi32(eventOffset)), SIMD_BITS_ZERO);
    ...
    for (;;)
    {
      ...
      __mwi mask = mMaskedHiZBuffer[tileIdx].mMask;
      __mw zMin0 = _mmw_blendv_ps(mMaskedHiZBuffer[tileIdx].mZMin[0], mMaskedHiZBuffer[tileIdx].mZMin[1],
        simd_cast<__mw>(_mmw_cmpeq_epi32(mask, _mmw_set1_epi32(~0))));
      __mw zMin1 = _mmw_blendv_ps(mMaskedHiZBuffer[tileIdx].mZMin[1], mMaskedHiZBuffer[tileIdx].mZMin[0],
        simd_cast<__mw>(_mmw_cmpeq_epi32(mask, _mmw_setzero_epi32())));
      ...
      if (_mmw_movemask_ps(dist0) != SIMD_ALL_LANES_MASK)
      {
        ...
        if (TEST_Z)
        {
          // Perform a conservative visibility test (test zMax against buffer for each covered 8x4
          // subtile)
        ...
        {
          // Compute interpolated min for each 8x4 subtile and update the masked hierarchical z
          // buffer entry
      ...
      for (int i = 0; i < NRIGHT; ++i)
        right[i] = _mmw_subs_epu16(right[i], SIMD_TILE_WIDTH); // Trick, use sub saturated to avoid
                                                               // checking against < 0 for shift
                                                               // (values should fit in 16 bits)
      for (int i = 0; i < NLEFT; ++i)
  ...
  FORCE_INLINE int RasterizeTriangle(unsigned int triIdx, int bbWidth, int tileRowIdx, int tileMidRowIdx, int tileEndRowIdx,
    const __mwi *eventStart, const __mw *slope, const __mwi *slopeTileDelta, const __mw &zTriMin, const __mw &zTriMax, __mw &z0,
    float zx, float zy, const __mwi *edgeY, const __mwi *absEdgeX, const __mwi *slopeSign, const __mwi *eventStartRemainder,
    const __mwi *slopeTileRemainder)
#else
  FORCE_INLINE int RasterizeTriangle(unsigned int triIdx, int bbWidth, int tileRowIdx, int tileMidRowIdx, int tileEndRowIdx,
    const __mwi *eventStart, const __mwi *slope, const __mwi *slopeTileDelta, const __mw &zTriMin, const __mw &zTriMax, __mw &z0,
    float zx, float zy)
#endif
  {
    if (TEST_Z)
...
#define UPDATE_TILE_EVENTS_Y(i)                                                                          \
  ...
  triEvent[i] = _mmw_add_epi32(triEvent[i], _mmw_add_epi32(triSlopeTileDelta[i], _mmw_and_epi32(overflow##i, triSlopeSign[i])))
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      __mwi scanlineSlopeRemainder = _mmw_sub_epi32(_mmw_mullo_epi32(triAbsEdgeX, SIMD_LANE_YCOORD_I),
        _mmw_mullo_epi32(_mmw_abs_epi32(scanlineDelta), triEdgeY[i]));
...
#define UPDATE_TILE_EVENTS_Y(i) triEvent[i] = _mmw_add_epi32(triEvent[i], triSlopeTileDelta[i]);
    ...
    __mwi triEvent[3];
    triEvent[0] = _mmw_add_epi32(_mmw_set1_epi32(simd_i32(eventStart[0])[triIdx]),
      _mmw_mullo_epi32(SIMD_LANE_IDX, _mmw_set1_epi32(simd_i32(slope[0])[triIdx])));
    triEvent[1] = _mmw_add_epi32(_mmw_set1_epi32(simd_i32(eventStart[1])[triIdx]),
      _mmw_mullo_epi32(SIMD_LANE_IDX, _mmw_set1_epi32(simd_i32(slope[1])[triIdx])));
    triEvent[2] = _mmw_add_epi32(_mmw_set1_epi32(simd_i32(eventStart[2])[triIdx]),
      _mmw_mullo_epi32(SIMD_LANE_IDX, _mmw_set1_epi32(simd_i32(slope[2])[triIdx])));
#endif
    ...
    if (tileRowIdx <= tileMidRowIdx)
    {
      ...
      while (tileRowIdx < tileStopIdx)
      {
        ...
        if (TEST_Z && cullResult == CullingResult::VISIBLE) // Early out if performing occlusion
                                                            // query
          return CullingResult::VISIBLE;
      ...
      if (tileRowIdx < tileEndRowIdx)
      {
        ...
        for (;;)
        {
          ...
          // Traverse the scanline and update the masked hierarchical z buffer
          cullResult = TraverseScanline<TEST_Z, 1, 1>(start, end, tileRowIdx, MID_VTX_RIGHT + 0, MID_VTX_RIGHT + 1, triEvent, zTriMin,
            zTriMax, z0, zx);
...
#if PRECISE_COVERAGE != 0
  FORCE_INLINE int RasterizeTriangleBatch(__mwi ipVtxX[3], __mwi ipVtxY[3], __mw pVtxX[3], __mw pVtxY[3], __mw pVtxZ[3],
    unsigned int triMask, const ScissorRect *scissor)
#else
  FORCE_INLINE int RasterizeTriangleBatch(__mw pVtxX[3], __mw pVtxY[3], __mw pVtxZ[3], unsigned int triMask,
    const ScissorRect *scissor)
#endif
  {
    ...
    // Compute z value at min corner of bounding box. Offset to make sure z is conservative for all
    // 8x4 subtiles
    ...
    // Compute edges
    __mwi edgeX[3] = {
      _mmw_sub_epi32(ipVtxX[1], ipVtxX[0]), _mmw_sub_epi32(ipVtxX[2], ipVtxX[1]), _mmw_sub_epi32(ipVtxX[2], ipVtxX[0])};
    __mwi edgeY[3] = {
      _mmw_sub_epi32(ipVtxY[1], ipVtxY[0]), _mmw_sub_epi32(ipVtxY[2], ipVtxY[1]), _mmw_sub_epi32(ipVtxY[2], ipVtxY[0])};
    ...
    // Modify slope of horizontal edges to make sure they mask out pixels above/below the edge. The
    // slope is set to screen width to mask out all pixels above or below the horizontal edge. We
    // must also add a small bias to acount for that vertices may end up off screen due to clipping.
    // We're assuming that the round off error is no bigger than 1.0
    ...
    __mwi offset0 =
      _mmw_and_epi32(_mmw_add_epi32(yDiffi[0], _mmw_set1_epi32(FP_HALF_PIXEL - 1)), _mmw_set1_epi32((int)((~0u) << FP_BITS)));
    __mwi offset1 =
      _mmw_and_epi32(_mmw_add_epi32(yDiffi[1], _mmw_set1_epi32(FP_HALF_PIXEL - 1)), _mmw_set1_epi32((int)((~0u) << FP_BITS)));
    ...
    for (int i = 0; i < 3; i++)
    {
      // Common, compute slope sign (used to propagate the remainder term when overflowing) is
      // postive or negative x-direction
      slopeSign[i] = _mmw_blendv_epi32(_mmw_set1_epi32(1), _mmw_set1_epi32(-1), edgeX[i]);
      absEdgeX[i] = _mmw_abs_epi32(edgeX[i]);

      // Delta and error term for one vertical tile step. The exact delta is exactDelta = edgeX /
      // edgeY, due to limited precision we repersent the delta as delta = qoutient + remainder /
      // edgeY, where quotient = int(edgeX / edgeY). In this case, since we step one tile of
      // scanlines at a time, the slope is computed for a tile-sized step.
      slopeTileDelta[i] = _mmw_cvttps_epi32(_mmw_mul_ps(slope[i], _mmw_set1_ps(FP_TILE_HEIGHT)));
      slopeTileRemainder[i] = _mmw_sub_epi32(_mmw_slli_epi32(absEdgeX[i], FP_TILE_HEIGHT_SHIFT),
        _mmw_mullo_epi32(_mmw_abs_epi32(slopeTileDelta[i]), edgeY[i]));

      // Jump to bottom scanline of tile row, this is the bottom of the bounding box, or the middle
      // vertex of the triangle. The jump can be in both positive and negative y-direction due to
      // clipping / offscreen vertices.
      ...
      __mwi tileStartRemainder = _mmw_sub_epi32(_mmw_mullo_epi32(absEdgeX[i], _mmw_abs_epi32(vy[i])),
        _mmw_mullo_epi32(_mmw_abs_epi32(tileStartSlope), edgeY[i]));
      ...
      eventStartRemainder[i] = _mmw_blendv_epi32(eventStartRemainder[i],
        _mmw_sub_epi32(_mmw_sub_epi32(edgeY[i], eventStartRemainder[i]), _mmw_set1_epi32(1)), vy[i]);

      // eventStart[i] = xDiffi[i & 1] + tileStartSlope + (overflow & tileStartDir) +
      // _mmw_set1_epi32(FP_HALF_PIXEL - 1) + tieBreaker;
    ...
    __mwi bbTopIdx =
      _mmw_add_epi32(bbTileMinX, _mmw_mullo_epi32(_mmw_add_epi32(bbTileMinY, bbTileSizeY), _mmw_set1_epi32(mTilesWidth)));
    ...
    while (triMask)
    {
      ...
      __mw z0 = _mmw_fmadd_ps(_mmw_set1_ps(simd_f32(zPixelDx)[triIdx]), SIMD_SUB_TILE_COL_OFFSET_F,
        _mmw_fmadd_ps(_mmw_set1_ps(simd_f32(zPixelDy)[triIdx]), SIMD_SUB_TILE_ROW_OFFSET_F,
          _mmw_set1_ps(simd_f32(zPlaneOffset)[triIdx])));
      ...
      // do not allow out of bound tiles
      tileRowIdx = min(tileRowIdx, mTilesHeight * mTilesWidth);
      tileMidRowIdx = min(tileMidRowIdx, mTilesHeight * mTilesWidth);
      tileEndRowIdx = min(tileEndRowIdx, mTilesHeight * mTilesWidth);

      if (bbWidth > BIG_TRIANGLE && bbHeight > BIG_TRIANGLE) // For big triangles we use a more
                                                             // expensive but tighter traversal
                                                             // algorithm
      {
#if PRECISE_COVERAGE != 0
        if (triMidVtxRight)
          cullResult &= RasterizeTriangle<TEST_Z, 1, 1>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slope,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy, edgeY, absEdgeX, slopeSign, eventStartRemainder, slopeTileRemainder);
        else
          cullResult &= RasterizeTriangle<TEST_Z, 1, 0>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slope,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy, edgeY, absEdgeX, slopeSign, eventStartRemainder, slopeTileRemainder);
#else
        if (triMidVtxRight)
          cullResult &= RasterizeTriangle<TEST_Z, 1, 1>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slopeFP,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy);
        else
          cullResult &= RasterizeTriangle<TEST_Z, 1, 0>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slopeFP,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy);
#endif
      }
      else
...
#if PRECISE_COVERAGE != 0
        if (triMidVtxRight)
          cullResult &= RasterizeTriangle<TEST_Z, 0, 1>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slope,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy, edgeY, absEdgeX, slopeSign, eventStartRemainder, slopeTileRemainder);
        else
          cullResult &= RasterizeTriangle<TEST_Z, 0, 0>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slope,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy, edgeY, absEdgeX, slopeSign, eventStartRemainder, slopeTileRemainder);
#else
        if (triMidVtxRight)
          cullResult &= RasterizeTriangle<TEST_Z, 0, 1>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slopeFP,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy);
        else
          cullResult &= RasterizeTriangle<TEST_Z, 0, 0>(triIdx, bbWidth, tileRowIdx, tileMidRowIdx, tileEndRowIdx, eventStart, slopeFP,
            slopeTileDelta, zTriMin, zTriMax, z0, zx, zy);
#endif
      }
  ...
  template <int TEST_Z>
  FORCE_INLINE CullingResult RenderTriangles(const float *inVtx, const unsigned short *inTris, int nTris,
    const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask)
  {
    assert(mMaskedHiZBuffer != nullptr);
    ...
    const unsigned short *inTrisPtr = inTris;
    ...
    while (triIndex < nTris || clipHead != clipTail)
    {
      ...
      GatherTransformClip(clipHead, clipTail, numLanes, nTris, triIndex, vtxX, vtxY, vtxW, inVtx, inTrisPtr, modelToClipMatrix,
        clipTriBuffer, triMask, clipPlaneMask);
  ...
  FORCE_INLINE
  CullingResult RenderTriangles(const float *inVtx, const unsigned short *inTris, int nTris, const float *modelToClipMatrix,
    BackfaceWinding bfWinding, ClipPlanes clipPlaneMask) MOC_OVERRIDE
  {
    CullingResult retVal;

    retVal = (CullingResult)RenderTriangles<0>(inVtx, inTris, nTris, modelToClipMatrix, bfWinding, clipPlaneMask);
  ...
  FORCE_INLINE
  CullingResult TestTriangles(const float *inVtx, const unsigned short *inTris, int nTris, const float *modelToClipMatrix,
    BackfaceWinding bfWinding, ClipPlanes clipPlaneMask) MOC_OVERRIDE
  {
    CullingResult retVal;

    retVal = (CullingResult)RenderTriangles<1>(inVtx, inTris, nTris, modelToClipMatrix, bfWinding, clipPlaneMask);
  ...
  FORCE_INLINE
  CullingResult TestRect(float xmin, float ymin, float xmax, float ymax, float wmin) const MOC_OVERRIDE
  {
    ...
    static const __m128i SIMD_TILE_PAD_MASK =
      _mm_setr_epi32(~(TILE_WIDTH - 1), ~(TILE_WIDTH - 1), ~(TILE_HEIGHT - 1), ~(TILE_HEIGHT - 1));
    static const __m128i SIMD_SUB_TILE_PAD = _mm_setr_epi32(0, SUB_TILE_WIDTH, 0, SUB_TILE_HEIGHT);
    static const __m128i SIMD_SUB_TILE_PAD_MASK =
      _mm_setr_epi32(~(SUB_TILE_WIDTH - 1), ~(SUB_TILE_WIDTH - 1), ~(SUB_TILE_HEIGHT - 1), ~(SUB_TILE_HEIGHT - 1));
...
#endif
        // Perform conservative greater than test against hierarchical Z buffer (zMax >= zBuf means
        // the subtile is visible)
  ...
  FORCE_INLINE
  void BinTriangles(const float *inVtx, const unsigned short *inTris, int nTris, TriList *triLists, unsigned int nBinsW,
    unsigned int nBinsH, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask) MOC_OVERRIDE
  {
    assert(mMaskedHiZBuffer != nullptr);
  ...
  void GatherTransformClip(int &clipHead, int &clipTail, int &numLanes, int nTris, int &triIndex, __mw *vtxX, __mw *vtxY, __mw *vtxW,
    const float *inVtx, const unsigned short *&inTrisPtr, const float *modelToClipMatrix, __m128 *clipTriBuffer, unsigned int &triMask,
    ClipPlanes clipPlaneMask)
  {
    ...
    if (clipHead != clipTail)
    {
      ...
      if (numLanes > 0)
      {
        ...
        {
          GatherVerticesFastZ(vtxX, vtxY, vtxW, inVtx, inTrisPtr, numLanes);
  ...
  FORCE_INLINE void RenderTrilist(const TriList &triList, const ScissorRect *scissor) MOC_OVERRIDE
  {
    assert(mMaskedHiZBuffer != nullptr);
  ...
  FORCE_INLINE MaskedOcclusionCulling::Implementation GetImplementation() MOC_OVERRIDE { return gInstructionSet; }

  FORCE_INLINE void ComputePixelDepthBuffer(float *depthData, bool flipY) MOC_OVERRIDE
  {
    assert(mMaskedHiZBuffer != nullptr);
  ...
  FORCE_INLINE void CombinePixelDepthBuffer2W(float *depthData, int w, int h) MOC_OVERRIDE
  {
    G_UNUSED(w);
    G_UNUSED(h);
    OCCLUSION_ASSERT(w == mWidth / SUB_TILE_WIDTH * 2 && h == mHeight / SUB_TILE_HEIGHT);
    OCCLUSION_ASSERT(w % SUB_TILE_WIDTH == 0 && w % TILE_WIDTH == 0);
    OCCLUSION_ASSERT(h % SUB_TILE_HEIGHT == 0 && w % TILE_HEIGHT == 0);
    OCCLUSION_ASSERT(SUB_TILE_WIDTH == 8 && SUB_TILE_HEIGHT == 4);
    OCCLUSION_ASSERT(mMaskedHiZBuffer != nullptr);
#if TILE_HEIGHT == SUB_TILE_HEIGHT
    __mw *vDepth = (__mw *)ASSUME_ALIGNED(depthData, alignof(__mw));
    for (int tileIdx = 0; tileIdx < (mHeight / SUB_TILE_HEIGHT) * (mWidth / TILE_WIDTH) / 2; tileIdx++, vDepth += 2)
    {
      __mw zmin = CONVERT_MASKED_TO_DEPTH(mMaskedHiZBuffer[tileIdx].mZMin[0]);
      vDepth[0] = DEPTH_VMIN(vDepth[0], _mmw_shuffle_ps_1100(zmin, zmin));
      vDepth[1] = DEPTH_VMIN(vDepth[1], _mmw_shuffle_ps_3322(zmin, zmin));
    }
#elif TILE_HEIGHT == 2 * SUB_TILE_HEIGHT
    int tileIdx = 0;
    for (int tileY = 0; tileY < (mHeight / SUB_TILE_HEIGHT); tileY += 2)
    {
      __mw *vDepth0 = (__mw *)ASSUME_ALIGNED(depthData, alignof(__mw)) + tileY * (mWidth / TILE_WIDTH);
      __mw *vDepth1 = vDepth0 + (mWidth / TILE_WIDTH);
      for (int tileX = 0; tileX < (mWidth / TILE_WIDTH); ++tileX)
      {
        __mw zmin = CONVERT_MASKED_TO_DEPTH(mMaskedHiZBuffer[tileIdx].mZMin[0]);

        __m256 tmp0 = _mm256_shuffle_ps(zmin, zmin, _MM_SHUFFLE(1, 1, 0, 0));
        __m256 tmp1 = _mm256_shuffle_ps(zmin, zmin, _MM_SHUFFLE(3, 3, 2, 2));

        vDepth0[0] = DEPTH_VMIN(vDepth0[0], _mm256_permute2f128_ps(tmp0, tmp1, 0b00100000));
        vDepth1[0] = DEPTH_VMIN(vDepth1[0], _mm256_permute2f128_ps(tmp0, tmp1, 0b00110001));
...
#else
#error "Unsupported TILE_HEIGHT"
#endif
  }
  FORCE_INLINE void DecodePixelDepthBuffer2W(float *depthData, int w, int h) MOC_OVERRIDE
  {
    G_UNUSED(w);
    G_UNUSED(h);
    OCCLUSION_ASSERT(w == mWidth / SUB_TILE_WIDTH * 2 && h == mHeight / SUB_TILE_HEIGHT);
    OCCLUSION_ASSERT(w % SUB_TILE_WIDTH == 0 && w % TILE_WIDTH == 0);
    OCCLUSION_ASSERT(h % SUB_TILE_HEIGHT == 0 && w % TILE_HEIGHT == 0);
    // OCCLUSION_ASSERT(TILE_HEIGHT == 4 && TILE_WIDTH == 32);
    OCCLUSION_ASSERT(SUB_TILE_WIDTH == 8 && SUB_TILE_HEIGHT == 4);
    OCCLUSION_ASSERT(mMaskedHiZBuffer != nullptr);
#if TILE_HEIGHT == SUB_TILE_HEIGHT
    __mw *vDepth = (__mw *)ASSUME_ALIGNED(depthData, alignof(__mw));
    for (int tileIdx = 0; tileIdx < (mHeight / TILE_HEIGHT) * (mWidth / TILE_WIDTH); tileIdx++, vDepth += 2)
    {
      __mw zmin = CONVERT_MASKED_TO_DEPTH(mMaskedHiZBuffer[tileIdx].mZMin[0]);
      vDepth[0] = _mmw_shuffle_ps_1100(zmin, zmin);
      vDepth[1] = _mmw_shuffle_ps_3322(zmin, zmin);
    }
#elif TILE_HEIGHT == 2 * SUB_TILE_HEIGHT
    int tileIdx = 0;
    for (int tileY = 0; tileY < (mHeight / SUB_TILE_HEIGHT); tileY += 2)
    {
      ...
      for (int tileX = 0; tileX < (mWidth / TILE_WIDTH); ++tileX)
      {
        ...
        vDepth0[0] = _mm256_permute2f128_ps(tmp0, tmp1, 0b00100000);
        vDepth1[0] = _mm256_permute2f128_ps(tmp0, tmp1, 0b00110001);
...
#error "Unsupported TILE_HEIGHT"
  ...
  FORCE_INLINE void mergeOcclusions(MaskedOcclusionCulling **_another_occl, uint32_t occl_count, uint32_t first_tile,
    uint32_t last_tile) MOC_OVERRIDE
  {
    auto **another_occl = (MaskedOcclusionCullingPrivate **)_another_occl;
    for (int i = first_tile; i < last_tile; ++i)
      for (int j = 0; j < occl_count; ++j)
        UpdateTileByAnotherTile(i, another_occl[j]->mMaskedHiZBuffer[i].mMask, another_occl[j]->mMaskedHiZBuffer[i].mZMin[0],
          another_occl[j]->mMaskedHiZBuffer[i].mZMin[1]);
  }

  FORCE_INLINE void UpdateTileByAnotherTile(int tileIdx, const __mwi &mask, const __mw &zmin0, const __mw &zmin1)
  {
    // UpdateTileQuick(tileIdx, v_xori(mask, V_CI_MASK1111), zmin0);
    UpdateTileQuick(tileIdx, _mmw_xor_epi32(mask, _mmw_set1_epi32(0xffffffff)), zmin0);
    UpdateTileQuick(tileIdx, mask, zmin1);
  }

  FORCE_INLINE
  void mergeOcclusionsZmin(MaskedOcclusionCulling **_another_occl, uint32_t occl_count, uint32_t first_tile,
    uint32_t last_tile) MOC_OVERRIDE
  {
    auto **another_occl = (MaskedOcclusionCullingPrivate **)_another_occl;
    for (int j = 0; j < occl_count; ++j)
    {
      for (int i = first_tile; i < last_tile; ++i) // still cache ineffiecient, but at least 1/3 of
                                                   // cache is available
        mMaskedHiZBuffer[i].mZMin[0] = _mmw_max_ps(mMaskedHiZBuffer[i].mZMin[0], another_occl[j]->mMaskedHiZBuffer[i].mZMin[0]);
    }
  }

  FORCE_INLINE uint32_t getTilesCount() const MOC_OVERRIDE { return mTilesWidth * mTilesHeight; }

  FORCE_INLINE OcclusionCullingStatistics GetStatistics() MOC_OVERRIDE { return mStats; }
};
