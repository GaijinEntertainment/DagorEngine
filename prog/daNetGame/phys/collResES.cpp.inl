...
{
  bool ret = false;
  get_animchar_collision_transform_ecs_query(*g_entity_mgr, eid,
    [&](const CollisionResource &collres, const AnimV20::AnimcharBaseComponent &animchar, const TMatrix &transform = TMatrix::IDENT) {
      ...
      logerr("%s: %d<%s> collnode=%d(%s) nodeTree[%d(%s)][3]=%@ rtm[3]=%@ etm[3]=%@", fnname, (ecs::entity_id_t)eid,
        g_entity_mgr->getEntityTemplateName(eid), coll_node_id, collres.getNodeName(coll_node_id), cnode ? (int)cnode->geomNodeId : -2,
        cnode ? tree.getNodeName(cnode->geomNodeId) : nullptr, tm.getcol(3), tree.getRootWposScalar(), transform.getcol(3));
      debug_flush(false);
      loggedOnce = true;
    });
  return ret;
