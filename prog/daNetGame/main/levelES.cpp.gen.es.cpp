...
static constexpr ecs::ComponentDesc level_es_event_handler_comps[] =
{
//start of 1 rw components at [0]
  {ECS_HASH("level__loaded"), ecs::ComponentTypeInfo<bool>()}
};
static void level_es_event_handler_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  G_FAST_ASSERT(evt.is<EventLevelLoaded>());
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
    level_es_event_handler(static_cast<const EventLevelLoaded&>(evt)
        , ECS_RW_COMP(level_es_event_handler_comps, "level__loaded", bool)
    );
  while (++comp != compE);
}
static ecs::EntitySystemDesc level_es_event_handler_es_desc
(
  "level_es",
  "prog/daNetGame/main/levelES.cpp.inl",
  ecs::EntitySystemOps(nullptr, level_es_event_handler_all_events),
  make_span(level_es_event_handler_comps+0, 1)/*rw*/,
  empty_span(),
  empty_span(),
  empty_span(),
