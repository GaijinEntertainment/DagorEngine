...
eastl::fixed_vector<dafg::NodeHandle, 3> makeControlOpaqueDynamicsNodes(const char *prev_region_ns)
{
  ...
  result.push_back(ns.registerNode("end", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
    registry.create("acesfx_start_update_token").blob<OrderingToken>();
...
{
  eastl::fixed_vector<dafg::NodeHandle, 3> res;
  auto decoNs = dafg::root() / "opaque" / "decorations";

  res.push_back(decoNs.registerNode("begin", DAFG_PP_NODE_SRC, [previous_opaque_ns](dafg::Registry registry) {
    auto ns = registry.root() / "opaque" / previous_opaque_ns;
    start_gbuffer_rendering_region(registry, ns, true);
  }));

  res.push_back(decoNs.registerNode("complete_prepass", DAFG_PP_NODE_SRC,
    [](dafg::Registry registry) { complete_prepass_of_gbuffer_rendering_region(registry); }));

  res.push_back(decoNs.registerNode("end", DAFG_PP_NODE_SRC, [](dafg::Registry registry) { end_gbuffer_rendering_region(registry); }));
