...
dafg::NodeHandle makeDownsampleDepthWithWaterNode()
{
  return dafg::register_node("downsample_depth_with_water_node", DAFG_PP_NODE_SRC, [](dafg::Registry registry) {
    ...
    auto downsampledDepthHdnl = registry.create(depthName.c_str())
    ...
    return [downsampledDepthHdnl, depthHndl, waterModeHndl, makeCheckerboardDepth, mainViewResolution] {
      if (waterModeHndl.ref() == WaterRenderMode::LATE)
        return;
      BaseTexture *downsampledDepthWithWaterTex = downsampledDepthHdnl.get();
      BaseTexture *depth = depthHndl.get();
...
static void create_downsample_depth_with_water_node_es(const OnCameraNodeConstruction &evt)
{
  if (WRDispatcher::hasHighResFx())
    return;
  evt.nodes->push_back(makeDownsampleDepthWithWaterNode());
}
