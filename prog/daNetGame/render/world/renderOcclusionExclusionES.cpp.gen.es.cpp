...
inline void process_animchar_eid_ecs_query(ecs::EntityId eid, Callable function)
{
  perform_query(g_entity_mgr, eid, process_animchar_eid_ecs_query_desc.getHandle(),
    [&function](const ecs::QueryView& __restrict components)
    {
        ...
        {
          function(
              ECS_RO_COMP(process_animchar_eid_ecs_query_comps, "animchar_visbits", uint8_t)
    ...
    [&function](const ecs::QueryView& __restrict components)
    {
        auto comp = components.begin(), compE = components.end(); G_ASSERT(comp != compE); do
        {
          function(
            ...
            , ECS_RO_COMP(render_hero_ecs_query_comps, "animchar_visbits", uint8_t)
    ...
    [&function](const ecs::QueryView& __restrict components)
    {
        auto comp = components.begin(), compE = components.end(); G_ASSERT(comp != compE); do
        {
          function(
            ...
            , ECS_RO_COMP(render_hero_vehicle_ecs_query_comps, "animchar_visbits", uint8_t)
