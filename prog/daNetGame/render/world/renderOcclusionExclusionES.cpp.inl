...
{
  const Point4 zero(0, 0, 0, 0);
  const bool hasAdditionalData = additional_data && !additional_data->empty();
  const Point4 *additionalData = hasAdditionalData ? additional_data->data() : &zero;
  const uint32_t additionalDataSize = hasAdditionalData ? additional_data->size() : 1;

  state.process_animchar(ShaderMesh::STG_opaque, ShaderMesh::STG_opaque, scene_instance, additionalData, additionalDataSize, false);
}
...
static void process_animchar_eid(DynModelRenderingState &state, ecs::EntityId animchar_eid)
{
  process_animchar_eid_ecs_query(animchar_eid,
    [&](uint8_t animchar_visbits, AnimV20::AnimcharRendComponent &animchar_render, const ecs::Point4List *additional_data) {
...
static void render_occlusion_exclusion_es_event_handler(const OcclusionExclusion &stg)
{
  ...
  render_hero_ecs_query([&](ECS_REQUIRE(ecs::auto_type watchedByPlr) AnimV20::AnimcharRendComponent &animchar_render,
                          const uint8_t &animchar_visbits, const ecs::EidList *attaches_list, const ecs::Point4List *additional_data) {
    // Check visibility
    if (!(animchar_visbits & VISFLG_MAIN_AND_SHADOW_VISIBLE))
      return;
  ...
  render_hero_vehicle_ecs_query([&](ECS_REQUIRE(ecs::auto_type vehicleWithWatched) AnimV20::AnimcharRendComponent &animchar_render,
                                  const uint8_t &animchar_visbits, const ecs::Point4List *additional_data) {
    // Check visibility
    if (!(animchar_visbits & VISFLG_MAIN_AND_SHADOW_VISIBLE))
      return;
    DynamicRenderableSceneInstance *scene = animchar_render.getSceneInstance();
