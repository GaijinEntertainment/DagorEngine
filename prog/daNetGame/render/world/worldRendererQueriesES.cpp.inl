...
static void get_fpv_shadow_dist_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);

float get_fpv_shadow_dist()
{
  float shadowDist = 0.f;
  get_fpv_shadow_dist_ecs_query(*g_entity_mgr, get_cur_cam_entity(),
...
template <typename Callable>
static void get_camera_fov_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);

float get_camera_fov()
{
  float FoV = 0.f;
  get_camera_fov_ecs_query(*g_entity_mgr, get_cur_cam_entity(), [&FoV](float fov) { FoV = fov; });
  return FoV;
}
...
static void gather_closest_occluders_ecs_query(ecs::EntityManager &manager, Callable c);
  ...
  mat44f_cref globtm, const vec3f &origin, eastl::vector<ParallelOcclusionRasterizer::RasterizationTaskData> &tasks)
{
  ...
  gather_closest_occluders_ecs_query(*g_entity_mgr,
  ...
  for (uint32_t i = 0; i < objectsToRasterize; ++i)
  {
    ...
    CollisionGeometryFeeder::addRasterizationTasks(*collisions[i], worldviewproj, tasks, (uint32_t)sw_raster_partition_size.get(),
      /*allow_convex*/ false);
  }
...
static ecs::QueryCbResult try_render_custom_sky_ecs_query(ecs::EntityManager &manager, Callable c);

bool try_render_custom_sky(const TMatrix &view_tm, const TMatrix4 &proj_tm, const Driver3dPerspective &persp)
{
  return try_render_custom_sky_ecs_query(*g_entity_mgr, [&view_tm, &proj_tm, &persp](ecs::EntityId eid) {
...
template <typename Callable>
ECS_REQUIRE(ecs::Tag customSkyRenderer)
static ecs::QueryCbResult find_custom_sky_ecs_query(ecs::EntityManager &manager, Callable c);

bool has_custom_sky()
{
  return find_custom_sky_ecs_query(*g_entity_mgr, []() { return ecs::QueryCbResult::Stop; }) == ecs::QueryCbResult::Stop;
}

template <typename Callable>
static void get_envi_probe_render_flags_ecs_query(ecs::EntityManager &manager, Callable c);

uint32_t WorldRenderer::getEnviProbeRenderFlags() const
{
  uint32_t flags = 0;
  get_envi_probe_render_flags_ecs_query(*g_entity_mgr, [&flags](bool envi_probe_use_geometry, bool envi_probe_sun_enabled) {
...
static ecs::QueryCbResult find_custom_envi_probe_renderer_ecs_query(ecs::EntityManager &manager, Callable c);

bool render_custom_envi_probe(const ManagedTex *cubeTarget, int face_num)
{
  return find_custom_envi_probe_renderer_ecs_query(*g_entity_mgr, [&](ecs::EntityId eid) {
...
template <typename Callable>
static inline void gather_occlusion_data_ecs_query(ecs::EntityManager &manager, Callable function);
OcclusionData gather_occlusion_data()
{
  OcclusionData occlusionData{};
  gather_occlusion_data_ecs_query(*g_entity_mgr, [&](const mat44f &close_geometry_prev_to_curr_frame_transform,
...
template <typename Callable>
static inline void gather_underground_zones_ecs_query(ecs::EntityManager &manager, Callable function);

void get_underground_zones_data(Tab<Point3_vec4> &bboxes)
{
  gather_underground_zones_ecs_query(*g_entity_mgr, [&](ECS_REQUIRE(ecs::Tag underground_zone) const TMatrix &transform) {
...
template <typename Callable>
static void needs_water_heightmap_ecs_query(ecs::EntityManager &manager, Callable c);
bool needs_water_heightmap(Color4 underwaterFade)
{
  ...
  needs_water_heightmap_ecs_query(*g_entity_mgr,
...
template <typename Callable>
static void use_foam_tex_ecs_query(ecs::EntityManager &manager, Callable c);
bool use_foam_tex()
{
  bool useFoamTex = false;
  use_foam_tex_ecs_query(*g_entity_mgr, [&useFoamTex](bool use_foam_tex) { useFoamTex = useFoamTex || use_foam_tex; });
  return useFoamTex;
}


template <typename Callable>
static void use_wfx_textures_ecs_query(ecs::EntityManager &manager, Callable c);
bool use_wfx_textures()
{
  bool useWfxTextures = false;
  use_wfx_textures_ecs_query(*g_entity_mgr,
    [&useWfxTextures](bool should_use_wfx_textures) { useWfxTextures = useWfxTextures || should_use_wfx_textures; });
  return useWfxTextures;
}

template <typename Callable>
static void get_sky_polarization_filter_term_ecs_query(ecs::EntityManager &, Callable cb);

float get_sky_polarization_filter_term()
{
  float term = 1.0f;
  get_sky_polarization_filter_term_ecs_query(*g_entity_mgr,
    [&term](float sky_polarization_filter__value) { term = sky_polarization_filter__value; });
  return term;
}