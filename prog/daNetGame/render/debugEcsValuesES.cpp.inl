...
static void items_with_debug_values_ecs_query(ecs::EntityManager &manager, Callable c);

ECS_TAG(render, dev)
ECS_NO_ORDER
static void debug_draw_ecs_values_es(const UpdateStageInfoRenderDebug &, ecs::EntityManager &manager)
{
  if (!ecs_debug_values.get())
    return;

  items_with_debug_values_ecs_query(manager,
    [&manager](ecs::EntityId eid, const TMatrix &transform, const ecs::Array &ecs_debug__debugValues,
      ...
      for (auto &value : ecs_debug__debugValues)
      {
        ...
        auto &&component = manager.getComponentRef(eid, ECS_HASH_SLOW(value.getStr()));
        ...
        {
          const char *typeInfo = manager.getComponentTypes().getTypeNameById(component.getTypeId());
