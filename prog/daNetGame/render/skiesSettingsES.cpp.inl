...
#define INIT_SEED() uint32_t weatherSeed = get_skies_seed();
...
static void skies_settings_weather_seed_created_es_event_handler(const ecs::Event &, int &skies_settings__weatherSeed)
{
  ...
  set_skies_seed(skies_settings__weatherSeed);
}
...
static void invalidate_skies(DngSkies *skies)
{
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void clouds_rendering_es_event_handler(const ecs::Event &,
  float clouds_rendering__forward_eccentricity,
  float clouds_rendering__back_eccentricity,
  float clouds_rendering__forward_eccentricity_weight,
  float clouds_rendering__erosion_noise_size,
  float clouds_rendering__ambient_desaturation,
  float clouds_rendering__ms_contribution,
  float clouds_rendering__ms_attenuation,
  float clouds_rendering__ms_ecc_attenuation,
  float clouds_rendering__erosionWindSpeed)
{
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void strata_clouds_es_event_handler(
  const ecs::Event &, const Point2 &strata_clouds__amount, const Point2 &strata_clouds__altitude)
{
  DngSkies *skies = get_daskies();
  if (!skies)
    return;
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void clouds_form_es_event_handler(const ecs::Event &,
  const ecs::Array &clouds_form__layers,
  const Point2 &clouds_form__extinction,
  const Point2 &clouds_form__turbulenceStrength,
  int clouds_form__shapeNoiseScale,
  int clouds_form__cumulonimbusShapeScale,
  int clouds_form__turbulenceFreq)
{
  DngSkies *skies = get_daskies();
  if (!skies)
    return;
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void clouds_settings_es_event_handler(const ecs::Event &,
  float clouds_settings__maximum_averaging_ratio,
  int clouds_settings__quality,
  int clouds_settings__target_quality,
  bool clouds_settings__competitive_advantage,
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void clouds_weather_gen_es_event_handler(const ecs::Event &,
  const ecs::Array &clouds_weather_gen__layers,
  float clouds_weather_gen__epicness,
  const Point2 &clouds_weather_gen__cumulonimbusCoverage,
  const Point2 &clouds_weather_gen__cumulonimbusSeed,
  float clouds_weather_gen__worldSize)
{
  DngSkies *skies = get_daskies();
  if (!skies)
    return;
...
ECS_TRACK(*)
ECS_ON_EVENT(on_appear, SkiesLoaded)
static __forceinline void sky_atmosphere_es_event_handler(const ecs::Event &,
  float sky_atmosphere__average_ground_albedo,
  float sky_atmosphere__min_ground_offset,
  float sky_settings__haze_strength,
  float sky_settings__haze_min_angle,
  float sky_settings__haze_peak_hour_offset,
  const Point3 &sky_atmosphere__ground_color,
  const Point2 &sky_atmosphere__mie2_thickness,
  const Point2 &sky_atmosphere__mie2_altitude,
  const Point2 &sky_atmosphere__mie2_scale,
  const Point2 &sky_atmosphere__mie_height,
  const Point2 &sky_atmosphere__mie_scale,
  const Point2 &sky_atmosphere__mie_absorption_scale,
  const Point3 &sky_atmosphere__mie_scattering_color,
  const Point3 &sky_atmosphere__mie_absorption_color,
  const Point2 &sky_atmosphere__mie_assymetry,
  const Point2 &sky_atmosphere__mie_back_assymetry,
  float sky_atmosphere__planet_scale,
  float sky_atmosphere__atmosphere_scale,
  const Point2 &sky_atmosphere__rayleigh_scale,
  const Point2 &sky_atmosphere__rayleigh_alt_scale,
  const Point3 &sky_atmosphere__rayleigh_color,
  float sky_atmosphere__multiple_scattering_factor,
  float sky_atmosphere__ozone_alt_dist,
  float sky_atmosphere__ozone_max_alt,
  const Point2 &sky_atmosphere__ozone_scale,
  float sky_atmosphere__sun_brightness,
  float sky_atmosphere__moon_brightness,
  const Point3 &sky_atmosphere__moon_color,
  bool sky_disable_sky_influence = false)
{
  DngSkies *skies = get_daskies();
  if (!skies)
    return;
...
{
  if (DngSkies *skies = get_daskies(); skies)
...
static __forceinline void clouds_hole_es_event_handler(const ecs::Event &, const TMatrix &transform, const float &density)
{
  ...
  if (!enabled)
    ...
    skies->resetCloudsHole(transform.getcol(3), density);
...
ECS_REQUIRE(ecs::Tag skies_settings_tag)
static void randomize_skies_setting_seed_es_event_handler(
  const ecs::Event &, bool randomize_seed_button, int &skies_settings__weatherSeed)
{
