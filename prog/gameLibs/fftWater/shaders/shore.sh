...
float4 shore_waves_dist = (600, 1000, 0, 50);
float shore_gerstner_speed = 1.0;
float shore_depth_scale = 1;
...
(stage) {
  ...
  shore_gspeed__depth_scale@f4 = (shore_gerstner_speed, shore_depth_scale, shore_waves_on, 1.0 / world_to_heightmap.x);
  ...
  shore_waves_dist@f4 = (
    1.0 / (shore_waves_dist.y - shore_waves_dist.x), -shore_waves_dist.x / (shore_waves_dist.y - shore_waves_dist.x),
    1.0 / (shore_waves_dist.w - shore_waves_dist.z), -shore_waves_dist.z / (shore_waves_dist.w - shore_waves_dist.z));
}
...
hlsl(stage) {
  ...
  #define shore_waves_on (shore_gspeed__depth_scale.z == 1)
  #define world_heightmap_size shore_gspeed__depth_scale.w
...
#endif
{
  ...
  float g_BaseGerstnerSpeed = sqrt(9.81f*g_BaseGerstnerWavelength*(1/TWOPI)) * shore_gspeed__depth_scale.x;
  ...
  float distanceMultiplier = inLandMul
    * (1.0 - saturate(shoreDist * shore_waves_dist.x + shore_waves_dist.y))
    * (1.0 - saturate(-shoreDist * shore_waves_dist.z + shore_waves_dist.w))
  ...
  depth *= shore_gspeed__depth_scale.y;
  float depthMultiplier = saturate((g_BaseGerstnerWavelength*0.25 + depth)*0.5); // Shore waves fade in when depth is less than quarter the wave length, we use 0.5 as this parameter also allows shore waves to heighten as the depth decreases
