...
macro USE_WATER_COMMON(stage)
  ...
  hlsl(stage) {
    ...
    void get_blend_factor(float3 worldPos, float nvsf_distance, out float4 nvsf_tex_coord_cascade01, out float4 nvsf_tex_coord_cascade23,
                          out float4 nvsf_tex_coord_cascade45, out float4 nvsf_blendfactors0123, out float4 nvsf_blendfactors4567)
    {
      // UVs
      nvsf_tex_coord_cascade01 = worldPos.xyxy * UVScaleCascade0123.xxyy;
      nvsf_tex_coord_cascade23 = worldPos.xyxy * UVScaleCascade0123.zzww;
      ##if water_cascades == five
        nvsf_tex_coord_cascade45 = worldPos.xyxy * UVScaleCascade4567.xxyy;
      ##else
      ...
      nvsf_blendfactors0123 = 1;
      nvsf_blendfactors4567 = 1;
    }
  }
endmacro
...
macro USE_WATER_DISPLACEMENT(stage, surf)
  ...
  hlsl(stage) {
    ...
    {
      float4 nvsf_blendfactors0123, nvsf_blendfactors4567;
      get_blend_factor(worldPos, worldPosDistance, nvsf_tex_coord_cascade01, nvsf_tex_coord_cascade23, nvsf_tex_coord_cascade45,
...
##if compatibility_mode == compatibility_mode_off
      ...
      {
        float4 nvsf_blendfactors0123, nvsf_blendfactors4567;
        get_blend_factor(world_pos, view_dist, tex_coord_cascade01, tex_coord_cascade23, tex_coord_cascade45,
                        nvsf_blendfactors0123, nvsf_blendfactors4567);
      }
##endif
...
macro USE_WATER_GRADIENTS(surf)
  USE_WATER_COMMON(ps)
  INIT_SHORE(ps)
  USE_SHORE(ps, 1, surf)
  ...
  hlsl(ps) {
    ...
    GradientsData getWaterGradients(float3 world_pos, float view_dist, float perlin, float3 weights, ShoreData shore_data)
    {
      float4 nvsf_blendfactors0123, nvsf_blendfactors4567;
      float4 nvsf_tex_coord_cascade01, nvsf_tex_coord_cascade23, nvsf_tex_coord_cascade45;
      get_blend_factor(world_pos.xzy, view_dist, nvsf_tex_coord_cascade01, nvsf_tex_coord_cascade23,
      ...
      perlinWaves.x = 1.0;
      float nearDetailsWeight = saturate(view_dist * details_weight.x + details_weight.y);
      nvsf_blendfactors0123 *= lerp(perlinWaves, float4(1, 1, 1, 1), nearDetailsWeight);
      nvsf_blendfactors4567 *= lerp(perlinWaves, float4(1, 1, 1, 1), nearDetailsWeight);
