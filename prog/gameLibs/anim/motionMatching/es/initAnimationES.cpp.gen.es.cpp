...
(
  ...
  "prog/gameLibs/anim/motionMatching/es/initAnimationES.cpp.inl",
...
inline void load_data_base_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable function)
{
  perform_query(&manager, eid, load_data_base_ecs_query_desc.getHandle(),
...
(
  ...
  empty_span());
template<typename Callable>
inline void get_data_base_ecs_query(ecs::EntityManager &manager, Callable function)
{
  perform_query(&manager, get_data_base_ecs_query_desc.getHandle(),
    [&function](const ecs::QueryView& __restrict components)
    {
        auto comp = components.begin(), compE = components.end(); G_ASSERT(comp != compE); do
        {
