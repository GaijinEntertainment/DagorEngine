...
static void mm_update_root_orientation_es_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  ...
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
    mm_update_root_orientation_es(static_cast<const ParallelUpdateFrameDelayed&>(evt)
        , ECS_RO_COMP(mm_update_root_orientation_es_comps, "transform", TMatrix)
    ...
    , ECS_RO_COMP(mm_update_root_orientation_es_comps, "mm_trajectory__featureDirections", ecs::Point3List)
    , ECS_RW_COMP(mm_update_root_orientation_es_comps, "motion_matching__controller", MotionMatchingController)
    , ECS_RO_COMP_OR(mm_update_root_orientation_es_comps, "animchar__turnDir", bool(false))
    );
  while (++comp != compE);
}
...
(
  ...
  make_span(mm_update_root_orientation_es_comps+1, 5)/*ro*/,
  empty_span(),
  empty_span(),
...
{
  G_FAST_ASSERT(evt.is<MotionMatchingStartSearchJob>());
  motion_matching_job_es(static_cast<const MotionMatchingStartSearchJob&>(evt)
        , components.manager()
    );
}
...
(
  ...
  empty_span(),
  empty_span(),
  ecs::EventSetBuilder<MotionMatchingStartSearchJob>::build(),
  0
,"render");
...
(
  ...
  ecs::EventSetBuilder<ParallelUpdateFrameDelayed>::build(),
  0
,"render",nullptr,"before_animchar_update_sync","after_net_phys_sync");
...
(
  ...
  make_span(motion_matching_ecs_query_comps+5, 7)/*ro*/,
  empty_span(),
  empty_span()
  , 1);
template<typename Callable>
inline void motion_matching_ecs_query(ecs::EntityManager &manager, Callable function)
{
  perform_query(&manager, motion_matching_ecs_query_desc.getHandle(),
    [&function](const ecs::QueryView& __restrict components)
    {
        auto comp = components.begin(), compE = components.end(); G_ASSERT(comp != compE); do
        {
          ...
          function(
            ...
            , ECS_RO_COMP(motion_matching_ecs_query_comps, "mm_trajectory__linearVelocity", Point3)
            , ECS_RO_COMP(motion_matching_ecs_query_comps, "mm_trajectory__angularVelocity", Point3)
            );
...
(
  ...
  empty_span());
template<typename Callable>
inline void motion_matching_per_frame_limit_ecs_query(ecs::EntityManager &manager, Callable function)
{
  perform_query(&manager, motion_matching_per_frame_limit_ecs_query_desc.getHandle(),
    [&function](const ecs::QueryView& __restrict components)
    {
        auto comp = components.begin(), compE = components.end(); G_ASSERT(comp != compE); do
        {
