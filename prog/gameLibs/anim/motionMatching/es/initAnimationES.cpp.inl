...
AnimationRootMotionConfig load_root_motion_config(const ecs::string &root_node, const ecs::StringList &direction_nodes,
  const ecs::FloatList &direction_weights, const ecs::StringList &center_of_mass_nodes, const ecs::Point4List &center_of_mass_params);

void load_animations(AnimationDataBase &dataBase, const AnimationRootMotionConfig &rootMotionConfig, const DataBlock &node_masks,
...
static void load_post_blend_controller_weight_overrides(AnimationDataBase &dataBase, const ecs::Object &pbc_overrides,
  const AnimV20::AnimationGraph *anim_graph)
{
  if (!anim_graph)
    return;
...
template <class T>
inline void get_data_base_ecs_query(ecs::EntityManager &manager, T);

template <class T>
inline void load_data_base_ecs_query(ecs::EntityManager &manager, ecs::EntityId, T);

static void load_animation_data_base(ecs::EntityManager &manager, ecs::EntityId database_eid, const GeomNodeTree *skeleton,
  AnimV20::AnimationGraph *anim_graph)
{
  G_ASSERT(skeleton && anim_graph);
  load_data_base_ecs_query(manager, database_eid,
    [skeleton, anim_graph, database_eid, &manager](bool &main_database__loaded, const ecs::StringList &data_bases_paths,
      ...
      if (!main_database__footLockerCtrlName.empty())
      {
        ...
        if (dataBase.footLockerParamId == -1)
        {
          ...
          get_game_resource_name(anim_graph->resId, animGraphName);
          logerr("Motion Matching: (%d)<%s> can't find main_database__footLockerCtrlName = '%s' in graph '%s'",
            (ecs::entity_id_t)database_eid, manager.getEntityTemplateName(database_eid), main_database__footLockerCtrlName.c_str(),
            animGraphName.c_str());
        }
        else
        {
          ...
          if (footLockerNumLegs != dataBase.footLockerNodes.size())
          {
            logerr("Motion Matching: (%d)<%s> FootLockerIKCtrl has '%d' legs specified while main_database__footLockerNodes has '%s'. "
              ...
              (ecs::entity_id_t)database_eid, manager.getEntityTemplateName(database_eid), footLockerNumLegs,
      ...
      manager.sendEventImmediate(database_eid, AnimationDataBaseLoaded());
    });
}

ECS_ON_EVENT(on_appear, InvalidateAnimationDataBase)
static void load_animation_data_base_es(const ecs::Event &, ecs::EntityManager &manager, ecs::EntityId eid,
  const ecs::string &main_database__skeletonRes, const ecs::string &main_database__animGraphRes)
{
  ...
  skeletonRes.reset(get_game_resource_ex(main_database__skeletonRes.c_str(), GeomNodeTreeGameResClassId));
  animGraphRes.reset(get_game_resource_ex(main_database__animGraphRes.c_str(), AnimGraphGameResClassId));
  load_animation_data_base(manager, eid, reinterpret_cast<const GeomNodeTree *>(skeletonRes.get()),
    reinterpret_cast<AnimGraphResData *>(animGraphRes.get())->graph);
}
...
ECS_TAG(render)
ECS_ON_EVENT(on_appear, InvalidateAnimationDataBase)
static void init_animations_es(const ecs::Event &, ecs::EntityManager &manager, ecs::EntityId eid,
  AnimV20::AnimcharBaseComponent &animchar, MotionMatchingController &motion_matching__controller, bool &motion_matching__enabled,
  int &motion_matching__presetIdx, ecs::EntityId &motion_matching__dataBaseEid,
  const ecs::string &motion_matching__dataBaseTemplateName, ecs::IntList &motion_matching__goalNodesIdx,
  FrameFeatures &motion_matching__goalFeature)
{
  ...
  if (!manager.getSingletonEntity(ECS_HASH_SLOW(dbTemplateName)) && !animation_data_base_entity_is_creating)
  {
    animation_data_base_entity_is_creating = true;
    manager.createEntityAsync(dbTemplateName, {}, {}, [&manager](ecs::EntityId) {
      manager.broadcastEventImmediate(InvalidateAnimationDataBase());
      animation_data_base_entity_is_creating = false;
    });
  }

  get_data_base_ecs_query(manager, [&](ecs::EntityId eid, bool main_database__loaded, const AnimationDataBase &dataBase) {
    // current data base entity can be with daeditor_selected subtemplate
    if (!find_sub_template_name(manager.getEntityTemplateName(eid), dbTemplateName))
      return;
    if (!main_database__loaded)
    {
      load_animation_data_base(manager, eid, animchar.getOriginalNodeTree(), animchar.getAnimGraph());
    }
    ...
    {
      String refSkeletonName, charSkeletonName;
      resolve_game_resource_name(refSkeletonName, dataBase.getReferenceSkeleton());
      resolve_game_resource_name(charSkeletonName, animchar.getOriginalNodeTree());
      logerr("Motion Matching: dataBase '%s' skeleton '%s' differs from char '%s' skeleton '%s'", manager.getEntityTemplateName(eid),
        refSkeletonName, animchar.getResName(), charSkeletonName);
      return;
    }
    if (dataBase.referenceAnimGraph != animchar.getAnimGraph())
    {
      ...
      logerr("Motion Matching: single animation database cannot be used with different AnimTrees/AnimGraphs. "
             "'%s' != '%s' (used by '%s')",
        animcharAnimGraphName, dataBaseAnimGraphName, manager.getEntityTemplateName(eid));
      return;
    }
  ...
  manager.sendEventImmediate(eid, AnimationDataBaseAssigned());
}
