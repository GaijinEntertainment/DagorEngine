...
public:
  ...
  bool rootAdjustmentPos = false;
  bool rootAdjustmentRot = false;
  ...
  void updateRoot(float dt, mat44f_cref animchar_tm, const Point3 &animchar_velocity, const Point3 &animchar_angular_velocity,
  ...
  int getClosestFrame() const
  {
    if (!hasActiveAnimation())
      return -1;
    int frame = getCurrentFrame();
    return getCurrentFrameProgress() > 0.5f ? frame + 1 : frame;
  }
  ...
  bool isAnimationTagsMatch() const
  {
    const AnimationClip &clip = dataBase->clips[currentClipInfo.clip];
    return currentTags.isMatch(clip.intervals[clip.getInterval(currentClipInfo.frame)].tags);
  }
};
