...
int AnimationDataBase::getPresetIdxFromTags(const AnimationFilterTags &filter_tags) const
{
  for (int i = 0; i < tagsPresets.size(); ++i)
  {
    int tagIdx = tagsPresets[i].requiredTagIdx;
    if (tagIdx >= 0 && filter_tags.isTagRequired(tagIdx))
      return i;
  }
  return 0;
}
...
void AnimationDataBase::requestResources(const char *, const ecs::resource_request_cb_t &res_cb)
{
  for (const ecs::string &path : res_cb.get<ecs::StringList>(ECS_HASH("data_bases_paths")))
  {
    ...
    for (int i = 0, n = clipsBlock->blockCount(); i < n; i++)
    {
      ...
      if (get_game_resource_pack_id_by_resource(clip_name) < 0)
        continue;
#endif
  ...
  if (!animGraphRes.empty())
    res_cb(animGraphRes.c_str(), AnimGraphGameResClassId);
}
