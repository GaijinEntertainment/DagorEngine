...
void MotionMatchingController::updateRoot(float dt, mat44f_cref animchar_tm, const Point3 &animchar_velocity,
  const Point3 &animchar_angular_velocity, const Point3 &wish_root_direction)
{
  ...
  else
  {
    ...
    if (rootAdjustmentRot)
    {
      ...
      if (rootAdjustmentAngVelocityRatio > 0)
      {
        ...
        float adjustmentAngle = v_extract_w(v_mul(V_C_TWO, v_safe_acos(adjustmentRot)));
    ...
    if (rootClamping)
    {
      ...
      float clampingAngle = v_extract_w(v_mul(V_C_TWO, v_safe_acos(clampingRot)));
...
void MotionMatchingController::copyPoseFeaturesFromActiveAnimation(FrameFeatures &goal_features) const
{
  ...
  const AnimationClip &clip = dataBase->clips[getCurrentClip()];
  int frame = getClosestFrame();
  ConstNodeFeature srcFeatures = clip.features.get_node_feature(frame);
  ...
  int nodeFeaturesOffset = clip.featuresNormalizationGroup * dataBase->featuresSize + goal_features.trajectorySizeInVec4f;
