...
static inline void inertialize_transition_v3(vec3f &off_x, vec3f &off_v, const vec3f src_x, const vec3f src_v, const vec3f dst_x,
  const vec3f dst_v)
{
...
static inline void inertialize_transition_q(quat4f &off_x, vec3f &off_v, const quat4f src_x, const vec3f src_v, const quat4f dst_x,
  const vec3f dst_v)
{
...
void inertialize_pose_transition(BoneInertialInfo &offset, const BoneInertialInfo &current, const BoneInertialInfo &goal,
  const dag::Vector<float> &node_weights, const dag::Vector<float> &goal_node_mask);
...
void inertialize_pose_update(BoneInertialInfo &result, BoneInertialInfo &offset, const BoneInertialInfo &current,
  const dag::Vector<float> &node_weights, const float halflife, const float dt);
