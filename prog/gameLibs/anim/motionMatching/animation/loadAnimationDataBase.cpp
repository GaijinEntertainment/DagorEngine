...
static void load_animation_tags(const AnimationDataBase &dataBase, const AnimV20::AnimData &anim_data, AnimationClip &clip,
  const DataBlock *clip_block, const Tags &default_tags)
{
...
static void load_animation_features(const AnimationDataBase &dataBase, AnimationClip &clip, const DataBlock &clip_block)
{
  ...
  clip.features.init(clip.tickDuration + 1, dataBase.nodeCount, dataBase.trajectorySize);
  dag::Vector<RootMotion, framemem_allocator> invRootMotions(clip.rootMotion.size());
  ...
  {
    NodeFeature lastFeature = clip.features.get_node_feature(clip.tickDuration);
    NodeFeature prevFeature = clip.features.get_node_feature(clip.tickDuration - 1);
    for (int i = 0, n = dataBase.nodesName.size(); i < n; i++)
    {
...
void validate_clips_duplication(const AnimationDataBase &data_base, const char *clip_name)
{
  for (const AnimationClip &clip : data_base.clips)
    if (clip.name == clip_name && !clip.duplicate)
    {
...
static void load_animations(AnimationDataBase &dataBase, const DataBlock &node_masks, const eastl::string &path)
{
  ...
  for (int i = 0, n = clipsBlock->blockCount(); i < n; i++)
  {
    ...
    bool duplicate = clipBlock->getBool("duplicate", false);
#if DAGOR_DBGLEVEL > 0
    if (!duplicate)
      validate_clips_duplication(dataBase, clip_name);
#endif
    AnimV20::AnimData *animData = (AnimV20::AnimData *)get_game_resource_ex(clip_name, Anim2DataGameResClassId);
    if (animData)
    {
      ...
      {
#if DAGOR_DBGLEVEL > 0
        if (a2dTicks == animData->getLabelTime(clipBlock->getStr("endLabel")))
          logwarn("MM: endLabel:t=\"%s\" is redundant for '%s' clip", clipBlock->getStr("endLabel"), clip_name);
#endif
...
static void updade_trajectory_features_from_next_clip(AnimationClip &clip, const AnimationClip &next_clip,
  const AnimationDataBase &dataBase)
{
...
static void resolve_next_clips(dag::Vector<AnimationClip> &clips, const AnimationDataBase &dataBase)
{
  for (int i = 0, n = clips.size(); i < n; i++)
  {
    ...
    {
#if DAGOR_DBGLEVEL > 0
      validate_next_clip_pose(clip, clip, dataBase);
#endif
      clip.nextClip = i;
      continue;
    }
