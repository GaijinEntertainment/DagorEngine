...
struct AnimationDataBase
{
  ...
  {
    if (skeleton == getReferenceSkeleton())
      return;
    referenceSkeleton.reset(reinterpret_cast<GameResource *>(const_cast<GeomNodeTree *>(skeleton)));
    ::game_resource_add_ref_ex(referenceSkeleton.get(), GeomNodeTreeGameResClassId);
  }
  ...
  int getPresetIdxFromTags(const AnimationFilterTags &tags) const;
