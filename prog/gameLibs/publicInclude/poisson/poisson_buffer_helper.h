...
// NOTE: Resulting 2d points are padded to be aligned to float4 values!
//
inline bool generate_poisson_points(UniqueBufWithShaderVar &buf, uint32_t seed, int num_samples, const char *name,
  const char *var_name, const Point2 &offset_mul, const Point2 &offset_add)
{
  if (!VariableMap::isVariablePresent(get_shader_variable_id(var_name, true)))
    return false;

  buf.close();
  buf = UniqueBufWithShaderVar(generate_poisson_points_buffer(seed, num_samples, name, offset_mul, offset_add), var_name);
