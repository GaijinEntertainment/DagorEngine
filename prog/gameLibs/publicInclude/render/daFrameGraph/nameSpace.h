...
/// \cond DETAIL
#define DAFG_PP_STRINGIZE2(x) #x
#define DAFG_PP_STRINGIZE(x)  DAFG_PP_STRINGIZE2(x)
/// \endcond

/**
 * \ingroup DaFgCore
 * \brief A macro that expands to the current source location. Should always
 * be provided to dafg::register_node as the `source_location` parameter.
 */
#if DAGOR_DBGLEVEL > 0
#define DAFG_PP_NODE_SRC (__FILE__ ":" DAFG_PP_STRINGIZE(__LINE__))
#else
#define DAFG_PP_NODE_SRC ""
#endif
...
public:
   ...
   * \param source_location Should always be the DAFG_PP_NODE_SRC macro
   ...
   *   accept a dafg::multiplexing::Index object (or may accept nothing).
   ...
   * See dafg::updateExternallyConsumedResourceSet(eastl::span<const char *const> res_names)
   *
  ...
  friend bool operator==(const NameSpace &, const NameSpace &) = default;
  friend bool operator<(const NameSpace &fst, const NameSpace &snd) { return fst.nameId < snd.nameId; }

  inline static NameSpace _make_namespace(dafg::NameSpaceNameId nid) { return {nid}; }
...
template <>
struct hash<dafg::NameSpace>
{
  size_t operator()(dafg::NameSpace ns) const
  {
    return eastl::hash<eastl::underlying_type_t<dafg::NameSpaceNameId>>{}(eastl::to_underlying(ns.nameId));
  }
};
