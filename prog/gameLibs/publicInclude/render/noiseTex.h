...
extern const SharedTexWithShaderVar &init_and_get_argb8_64_noise(); // increment counter and init if not inited
extern const SharedTexWithShaderVar &init_and_get_l8_64_noise();    // increment counter and init if not inited
// increment counter and init if not inited
extern const SharedTexWithShaderVar &init_and_get_perlin_noise_3d(Point3 &min_r, Point3 &max_r);

// noise LUT texture by @Dave_Hoskins, www.shadertoy.com/view/4sfGzS
// size is 128x128
extern const SharedTexWithShaderVar &init_and_get_hash_128_noise(); // increment counter and init if not inited

// 128x128 rg8 blue noise texture
extern const SharedTexWithShaderVar &init_and_get_blue_noise();
