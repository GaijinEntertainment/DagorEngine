...
#include <daEditor4/de4_objCreator.h>
      ...
      if (const char *entTempl = g_entity_mgr->getEntityTemplateName(o->getEid()))
        if (const ecs::Template *templ = templates.getTemplateByName(entTempl))
          for (const uint32_t p : templ->getParents())
            if (auto *cc = g_entity_mgr->getTemplateDB().getTemplateById(p)->getComponentsMap().find(EDITABLE_TEMPLATE_COMP))
              if (!cc->getOr(true))
                subTemplsToRemove.push_back(g_entity_mgr->getTemplateDB().getTemplateById(p)->getName());
...
bool EntityObjEditor::processCommand(const char *argv[], int argc)
{
  if (argc < 1)
    return false;
  int found = 0;
  CONSOLE_CHECK_NAME("daEd4", "saveObjects", 1, 2) { saveObjects(argc > 1 ? argv[1] : NULL); }
