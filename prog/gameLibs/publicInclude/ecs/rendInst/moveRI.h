...
inline bool move_ri_extra_tm_ex(rendinst::riex_handle_t handle, const TMatrix &prev_transform, const TMatrix &transform,
  const Point3 &vel, const Point3 &omega, bool do_not_lock, ecs::EntityId eid)
{
  mat44f m;
...
struct MoveRiExtraDA final : public DelayedAction
{
  ...
  MoveRiExtraDA(rendinst::riex_handle_t h, const TMatrix &ot, const TMatrix &t, const Point3 &v, const Point3 &o,
    ...
    handle(h), prevTm(ot), targetTm(t), vel(v), omega(o), eid(e)
  {}
  ...
  void performAction() override
  {
    if (rendinst::isRiGenExtraValid(handle)) // rendinst might been destroyed since addition of this DA
      move_ri_extra_tm_ex(handle, prevTm, targetTm, vel, omega, /*do_not_lock*/ false, eid);
  }
};

inline void move_ri_extra_tm_with_motion(rendinst::riex_handle_t handle, const TMatrix &transform, const Point3 &vel,
  const Point3 &omega, const ecs::EntityId eid = ecs::INVALID_ENTITY_ID)
{
  ...
  if (!rendinst::isRiExtraLoaded() || !move_ri_extra_tm_ex(handle, origTm, transform, vel, omega, /*do_not_lock*/ true, eid))
    add_delayed_action(new MoveRiExtraDA(handle, origTm, transform, vel, omega, eid)); // ...try again in delayed action
}
