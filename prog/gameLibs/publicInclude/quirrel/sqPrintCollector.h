...
struct SQPrintCollector
{
  ...
  void setToVm(HSQUIRRELVM vm)
  {
    ...
    sq_pushstring(vm, registry_key, -1);
  ...
  static void sq_printf(HSQUIRRELVM vm, const char *s, ...)
  {
    va_list vl;
    va_start(vl, s);
    char tempBuf[8192];
  ...
  static void sq_compile_error_handler(HSQUIRRELVM vm, SQMessageSeverity severity, const char *desc, const char *source,
  ...
  static inline const char registry_key[] = "dagor:print_collector";
};
