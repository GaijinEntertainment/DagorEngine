...
inline void iterate_dag_textures(const char *path_to_dag, const das::TBlock<void, const char *> &block, das::Context *context,
  das::LineInfoArg *at)
{
  DagData data;
...
inline void get_filtered_assets(const DagorAssetMgr &asset__manager, int asset_type_id,
  const das::TBlock<void, const das::TArray<int>> &block, das::Context *context, das::LineInfoArg *at)
{
  auto filteredAssets = asset__manager.getFilteredAssets(dag::ConstSpan<int>(&asset_type_id, 1));

  das::Array arr;
  arr.flags = 0;
  das::array_mark_locked(arr, (char *)filteredAssets.data(), filteredAssets.size());

  vec4f arg = das::cast<das::Array *>::from(&arr);
  context->invoke(block, &arg, nullptr, at);
}

inline void get_asset_source_files(const DagorAsset &asset, const das::TBlock<void, const das::TArray<char *>> &block,
  das::Context *context, das::LineInfoArg *at)
{
  if (auto assetMgr = get_asset_manager())
  {
    if (IDagorAssetExporter *exporter = assetMgr->getAssetExporter(asset.getType()))
    {
      ...
      das::Array arr;
      arr.flags = 0;
      das::array_mark_locked(arr, (char *)filesSrc.data(), filesSrc.size());
...
inline void get_asset_dependencies(DagorAsset &asset, const das::TBlock<void, const das::TArray<char *>> &block, das::Context *context,
  das::LineInfoArg *at)
{
  ...
  das::Array arr;
  arr.flags = 0;
  das::array_mark_locked(arr, (char *)assetNames.data(), assetNames.size());
  vec4f arg = das::cast<das::Array *>::from(&arr);
  context->invoke(block, &arg, nullptr, at);
}
