...
static void init_asset_manager_es(const ecs::Event &, DagorAssetMgr &asset__manager, const ecs::string &asset__applicationBlkPath,
  const ecs::string &asset__baseFolder, const ecs::string &asset__pluginsFolder, const ecs::StringList &asset__loadAdditionalFolders,
  ecs::string &asset__baseFolderAbsPath, int &asset__rendInstType)
{
...
AssetLoadingStatus get_asset_status(DagorAsset &asset, bool update_status)
{
  ...
  else
  {
    ...
    {
      bool gameResLoaded = is_game_resource_loaded_nolock(assetName, exp->getGameResClassId());
...
static void track_a2d_es(const AssetChangedEvent &evt, ecs::EntityManager &manager)
{
  const DagorAsset *changedAsset = evt.get<0>();
  if (strcmp("a2d", changedAsset->getTypeStr()) != 0)
    return;
  auto assetManager = get_asset_manager();
  int animTreeId = assetManager->getAssetTypeId("animTree");
  auto animTrees = assetManager->getFilteredAssets(make_span_const(&animTreeId, 1));
  for (int treeId : animTrees)
  {
    DagorAsset &animTree = assetManager->getAsset(treeId);
    if (is_asset_depends_on(*changedAsset, animTree))
    {
      dabuildcache::invalidate_asset(animTree, false);
      manager.broadcastEvent(AssetChangedEvent(&animTree));
    }
  }
}

template <class T>
inline void reload_animchar_resources_ecs_query(ecs::EntityManager &manager, ecs::EntityId, T);

void reload_animchar_resources(ecs::EntityId eid, bool update_base_component, bool update_render_component)
{
  reload_animchar_resources_ecs_query(*g_entity_mgr, eid,
    [&](AnimV20::AnimcharBaseComponent &animchar, AnimV20::AnimcharRendComponent &animchar_render, const ecs::string &animchar__res,
      ...
      if (!animCharSource)
      {
        logerr("Failed to get_game_resource_ex for '%s'. Resource will not be reloaded.", animchar__res.c_str());
        return;
      }

      if (update_base_component)
        animCharSource->baseComp().cloneTo(&animchar, true);
      if (update_render_component)
...
template <class T>
inline void remove_animchar_resource_ref_holder_ecs_query(ecs::EntityManager &manager, T);

ECS_BEFORE(track_animchar_assets_es)
static void unload_active_animchar_resources_es(const AssetChangedEvent &evt, ecs::EntityManager &manager,
  const DagorAssetMgr &asset__manager)
{
  const DagorAsset *changedAsset = evt.get<0>();
  int typeId = changedAsset->getType();
  if (typeId != asset__manager.getAssetTypeId("animTree") && typeId != asset__manager.getAssetTypeId("dynModel"))
    return;

  // We need to remove all references for animchar resources to unload them as unused, otherwise we can't load modified
  // animTree or dynModel later because `get_game_resource_ex` returns already loaded animchar with old animTree and dynModel
  bool hadReferencedAnimchars = false;
  remove_animchar_resource_ref_holder_ecs_query(manager, [&](AnimcharResourceReferenceHolder &animchar__resRefHolder) {
    if (!animchar__resRefHolder.animcharRes)
      return;
    hadReferencedAnimchars = true;
    ::release_game_resource_ex(animchar__resRefHolder.animcharRes, CharacterGameResClassId);
    animchar__resRefHolder.animcharRes = nullptr;
  });
  if (hadReferencedAnimchars)
    free_unused_game_resources();
}

static void track_animchar_assets_es(const AssetChangedEvent &evt, ecs::EntityId eid, const AnimV20::AnimcharBaseComponent &animchar,
  const ecs::string &animchar__res)
{
  ...
  reload_animchar_resources(eid, animTreeChanged, dynModelChanged);
}
