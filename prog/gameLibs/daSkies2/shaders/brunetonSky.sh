...
include_optional "skiesSettings.sh"
...
float4 skies_froxels_resolution;
...
macro INIT_BRUNETON_FOG(stage)
  ...
  (stage) {
    ...
    skies_panoramic_scattering__inv_distance@f4 = (
      ...
      skies_frustum_scattering_last_tz/skies_froxels_resolution.w,
      ...
      //4*skies_froxels_resolution.z*skies_froxels_resolution.z/skies_froxels_resolution.w,//we use clamp addressing, so our zero slice is always in center (0.5) of slice. And so it is never no scattering, even in 0.01 meters from camera
...
macro BASE_USE_BRUNETON_FOG(code)
  ...
  hlsl(code) {
    ...
    {
      float3 lossf, addf;
      get_bruneton_fog(-point_to_eye, dist, lossf, addf);
      loss = (half3)lossf;
      add  = (half3)addf;
    }
