...
shader skies_generate_transmittance_ps//very first call
{
  ...
  TRANSMITTANCE(ps)
  hlsl(ps) {
    float3 skies_gen_ps(VsOutput input HW_USE_SCREEN_POS):SV_Target0
    {
      float4 sp = GET_SCREEN_POS(input.pos);
      return saturate(ComputeTransmittanceToTopAtmosphereBoundaryTexture2(theAtmosphere, sp.xy));
    }
  }
...
shader skies_generate_transmittance_cs//very first call
{
  ...
  hlsl(cs) {
    ...
    {
      OutputTexture[dtId] = saturate(ComputeTransmittanceToTopAtmosphereBoundaryTexture2(theAtmosphere, dtId + 0.5));
    }
  }
...
shader skies_approximate_ms_lut_cs
{
  ...
  hlsl(cs) {
    ...
    void skies_gen_cs(uint3 dtId : SV_DispatchThreadID)
    {
      ...
      OutputTexture[dtId.xy] = theAtmosphere.multiple_scattering_factor * MS;
    }
...
shader skies_approximate_ms_lut_ps
{
  ...
  hlsl(ps) {
    half3 skies_gen_ps(VsOutput input HW_USE_SCREEN_POS): SV_Target0
    {
      ...
      return theAtmosphere.multiple_scattering_factor * MS;
    }
...
shader indirect_irradiance_ms
{
  ...
  hlsl(ps){
    float3 indirect_irradiance_ms(VsOutput input HW_USE_SCREEN_POS): SV_Target0
    {
      float4 sp = GET_SCREEN_POS(input.pos); 
      return ComputeIndirectIrradianceTextureMS(theAtmosphere, SamplerTexture2DFromName(skies_transmittance_texture),
...
shader indirect_irradiance_ms_cs
{
  ...
  hlsl(cs) {
    ...
    void skies_gen_cs(uint threadId : SV_GroupIndex, uint3 gId:SV_GroupID)
    {
      ...
      if (threadId == 0)
      {
        OutputTexture[gId.xy] = ray[0] + ray[1];
      }
    }
