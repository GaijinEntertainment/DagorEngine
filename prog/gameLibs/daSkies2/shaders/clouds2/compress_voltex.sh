...
macro COMPRESS_VOLTEX_BC4_MAIN(stage)
  hlsl(stage)
  {
    uint2 compress_voltex_bc4_main(uint3 tid)
    {
      ...
      {\
        bc4_type v = source[srcId + uint3(x,y,0)].r;\
...
shader compress_voltex_bc4_cs
{
  ...
  hlsl(cs) {
    ...
    void cs_main(uint3 tid : SV_DispatchThreadID) {
      output[tid] = compress_voltex_bc4_main(tid);
    }
  }
  ...
  } else
  {
    ...
    hlsl(cs) {
      ...
      void cs_main(uint2 tid : SV_DispatchThreadID) {
        uint2 inp = tid*2;
        output[tid] = (source[inp + uint2(0,0)].x + source[inp + uint2(1,0)].x + source[inp + uint2(0,1)].x + source[inp + uint2(1,1)].x)*0.25;
      }
    }
  }
  compile("cs_5_0", "cs_main")
