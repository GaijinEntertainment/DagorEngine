...
macro SAMPLE_CLOUDS_NUKE_INVERSION(code)
  ...
  hlsl(code) {
    float sampleNukeCloudsInversion(float3 p, float density, float heightFraction)
    {
      ...
      float inversion = 1-saturate(nuke_wave.y*abs(waveDistance-length(p - nuke_pos.xyz)));
      float waveFade = nuke_wave.z;
      inversion*=waveFade;
      if (inversion>0)
      {
        float noise2 = tex3Dlod(perlin_noise3d, float4(p*0.0002f, 0)).x*
                       tex3Dlod(perlin_noise3d, float4(p*0.00003f, 0)).y;
        cloudDensity2 = saturate(1 - 2*abs(heightFraction-0.5))*0.05*saturate(10*noise2-2.5);
        cloudDensity2*=pow2(waveFade);
      }
      float extinctionDistance = nuke_wave.w;
      float extinction = saturate(nuke_wave.y*(length(p - nuke_pos.xyz)-extinctionDistance));

      return lerp(density*extinction*saturate(1-3*inversion), cloudDensity2, saturate(inversion*2.4f));
    }
  }
endmacro
