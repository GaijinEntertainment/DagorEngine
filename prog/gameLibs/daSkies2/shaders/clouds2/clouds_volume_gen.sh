...
macro WRITE_BUILD_VOLUME(code)
  ...
  (code) {
    ...
    cloud_volume_res@f4=cloud_volume_res;
  ...
  hlsl(code) {
    float iterateTransmittance(float2 texcoord, int2 scri, int slice_end)
    {
      ...
      {
        float p = (i+0.5)/(cloud_volume_res.z-0.5);
        float cDist = pow2(p)*cloud_volume_res.w;
...
shader build_dacloud_volume_cs
{
  hlsl(cs) {
    ...
    #define EACH_ITERATION(i, t) cloudsVolumeTransmittance[int3(scri, i)] = t;
  }
...
##endif
    ...
    {
      cloudsVolumeTransmittance[int3(dtid.xy,0)] = 1;
      if (skies_has_clouds)
        iterateTransmittance(dtid*invres.xy + invres.zw, dtid, cloud_volume_res.z);
      else
      {
        for (int i = 1; i < cloud_volume_res.z; ++i)
          cloudsVolumeTransmittance[int3(dtid.xy,i)] = 1;
      }
    }
  }
