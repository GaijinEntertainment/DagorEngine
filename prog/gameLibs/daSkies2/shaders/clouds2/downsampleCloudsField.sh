...
include "globtm.sh"
include "clouds_hole_pos_tex.sh"
  ...
  hlsl(code) {
    ...
    ##if nuke_in_atmosphere == on
      bool checkNuke(float3 p)
      {
        ...
        float inversion = 1-saturate(0.2*nuke_wave.y*abs(waveDistance-length(p - nuke_pos.xyz)));
...
shader clouds_tile_dist_prepare_cs
{
  (cs) {
    ...
    clouds_tiled_res@f4 = (1./clouds_tiled_res.x, 1./clouds_tiled_res.y, 0.5/clouds_tiled_res.x, 0.5/clouds_tiled_res.y);
  }
  ...
  hlsl(cs) {
    ...
    {
      float2 texcoord = tid*clouds_tiled_res.xy + clouds_tiled_res.zw;
      float3 viewVect = view_vecLT + (view_vecRT_minus_view_vecLT)*texcoord.x + (view_vecLB_minus_view_vecLT)*texcoord.y;
      tiledDist[tid] = getTiledDistance(normalize(viewVect));
    }
  }
...
shader clouds_tile_dist_min_cs
{
  ...
  hlsl(cs) {
    ...
    void cs_main(uint2 tid : SV_DispatchThreadID)
    {
      tiledDist[tid] = filter_tiled_dist(tid.xy);
    }
  }
  compile("cs_5_0", "cs_main");
}
...
shader clouds_tile_dist_count_cs
{
  ...
  ENABLE_ASSERT(cs)
  (cs) { clouds_tiled_res@f2 = clouds_tiled_res;}
  ...
  hlsl(cs) {
    ...
    {
      uint2 p = dtid;
      uint ret = (clouds_tile_distance[p].x == 1 || any(p >= uint2(clouds_tiled_res.xy))) ? 0 : 1;//todo: downsample 4 times
      thread_result[flatIdx] = ret;
      GroupMemoryBarrierWithGroupSync();
      const int WARP_SIZE = 32;
      UNROLL
...
macro USE_REFINE_ALTITUDES(stage)
  (stage) {
    cloud_layers_non_empty@smp = cloud_layers_non_empty hlsl { Texture2D<float> cloud_layers_non_empty@smp; }
...
shader clouds_refine_altitudes
{
  ...
  hlsl (cs) {
    ...
    void cs_main() {
      ...
      output[uint2(0, 0)] = float4(firstRadius-0.001, lastRadius+0.001, -firstRadius*firstRadius, -lastRadius*lastRadius);
      output[uint2(1, 0)] = float4(first, last, firstRadius, lastRadius);
    }
  }
  compile("cs_5_0", "cs_main")
}
