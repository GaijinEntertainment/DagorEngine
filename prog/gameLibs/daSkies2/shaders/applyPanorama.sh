...
include "lightningFlashPanorama.sh"
...
shader applyCloudsPanorama
{
  ...
  hlsl(ps) {
    //TODO: there is similar function in panorama.sh but it does not return modified view
  ...
  hlsl(ps) {
    ...
    half4 envi_ps(VsOutput input HW_USE_SCREEN_POS) : SV_Target
    {
      ...
      ##if sky_panorama_patch == on
        ...
        if (patch_vignette < PANORAMA_PATCH_THRESHOLD && viewDirection.y>0)
        {
          ...
          const float LERP_AMOUNT = 0.99;
          panorama = lerp(panorama, ptch, saturate((-1.f/(1-LERP_AMOUNT))*patch_vignette + PANORAMA_PATCH_THRESHOLD*LERP_AMOUNT/(1-LERP_AMOUNT) ));
        }
      ##endif
      ...
      apply_lightning_flash(sky_uv, originalViewDirection, panorama.rgb);
...
shader clouds_panorama_mip
{
  ...
  hlsl(ps) {
    half4 samplePanorama(float2 uv)
    {
    ...
    half3 downsample_panorama_ps(VsOutput IN) : SV_Target
    {
...
shader clouds_panorama_blend
{
  ...
  hlsl(ps) {
    half4 update_panorama_ps(VsOutput IN) : SV_Target
    {
