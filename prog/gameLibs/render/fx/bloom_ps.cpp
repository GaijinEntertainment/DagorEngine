...
void BloomPS::init(int w, int h)
{
  ...
  bloomMips = dag::create_tex(NULL, width / 4, height / 4, fmtflags, mipsCount, "bloom_tex", RESTAG_POSTFX);
...
static void ensure_mip(int mip, UniqueTex &bloom_last_mip, int &bloom_last_mip_start_mip, int mips_count, int width, int height)
{
  ...
  bloom_last_mip = dag::create_tex(NULL, w, h, TEXCF_RTARGET | TEXFMT_R11G11B10F, mips_count - bloom_last_mip_start_mip,
    "gauss_bloom_mip", RESTAG_POSTFX);
...
void BloomPS::perform(ManagedTexView downsampled_frame)
{
  ...
  if (!isWorking)
  {
    ShaderGlobal::set_float(bloom_mulVarId, 0);
  ...
  TIME_D3D_PROFILE(downsample_mips);
  {
    ...
    ShaderGlobal::set_float4(blur_src_tex_sizeVarId, 1.0f / (width / 2), 1.0f / (height / 2), 0, 0);
  ...
  bloomMips.setVar();
  ShaderGlobal::set_float(bloom_mulVarId, settings.mul);
