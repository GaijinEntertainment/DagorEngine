...
void ParallelOcclusionRasterizer::rasterizeMeshes(eastl::vector<RasterizationTaskData> &&tasks, float z_near,
  MaskedOcclusionCulling::BackfaceWinding backface_winding, eastl::fixed_function<sizeof(void *) * 3, void()> &&done_cb)
{
  if (tasks.empty())
  {
    if (done_cb)
  ...
  rasterizersContext.remainingJobs.store(usedOcclusions, dag::mo::relaxed);
  rasterizersContext.backfaceWinding = backface_winding;
  rasterizersContext.allDoneCb = eastl::move(done_cb);
...
uint32_t ParallelOcclusionRasterizer::mergeRasterizationResults(Occlusion &occlusion)
{
  ...
  if (!nonEmptyOcclusions.empty())
  {
    ...
    mergedTiles.store(0u, dag::memory_order_relaxed);
