...
{
template <>
class hash<dabfg::intermediate::RenderPass>
{
public:
  size_t operator()(const dabfg::intermediate::RenderPass &pass) const
  {
    size_t result = 0;
    for (const auto &attachment : pass.colorAttachments)
      if (attachment)
        ...
        hashPack(result, dabfg::intermediate::RESOURCE_NOT_MAPPED);
...
{
static bool operator==(const SubresourceRef &fst, const SubresourceRef &snd)
{
  return fst.resource == snd.resource && fst.layer == snd.layer && fst.mipLevel == snd.mipLevel;
}

static bool operator==(const RenderPass &fst, const RenderPass &snd)
{
  if (fst.depthAttachment != snd.depthAttachment)
    return false;
  if (fst.depthReadOnly != snd.depthReadOnly)
    return false;
  if (fst.colorAttachments.size() != snd.colorAttachments.size())
    return false;
  for (uint32_t i = 0; i < fst.colorAttachments.size(); ++i)
    if (fst.colorAttachments[i] != snd.colorAttachments[i])
      return false;
  return true;
}
...
static GpuRequestMapFmem make_gpu_requests_map(const intermediate::Graph &graph)
{
  ...
  for (auto idx : ReverseView(graph.nodes.keys()))
  ...
  const GpuRequestMapFmem &gpu_requests)
{
  ...
  for (auto nodeId : ReverseView(graph.nodes.keys()))
