...
float4 puddles_params = float4(160.0, 200.0, 2.0, 7.5);
// x = 0..1 - dist puddles min
// y = 0..1 - dist puddles max
// z = 0..1 - height factor down
// w = 0..1 - height factor up

float4 wetness_noise_weight = float4(0.012, 0.02, 0.1, 0.2);
// x = 0..1 - world space tile for hi spread noise
// y = 0..1 - world space tile for low spread noise
// z = 0..1 - min level for low spread mask
// w = 0..1 - max level for low spread mask
...
macro INIT_WETNESS_BASE()
  INIT_WETNESS_POWER(ps)
  (ps) {
    wf0_pseed@f4 = (wetness_f0.x, wetness_f0.y, wetness_f0.z, wetness_puddles_seed);
  }
endmacro
...
texture water_ripples_info_texture;
...
macro USE_WATER_RIPPLES_DISPL(code)
  ...
  hlsl(code) {
    ...
    ##if compatibility_mode == compatibility_mode_off
      ...
      return clamp(tex2Dlod(water_ripples_info_texture, float4(wUV, 0.0, 0.0)).r, -water_ripples_origin_size_dmax.w, water_ripples_origin_size_dmax.w);
    ##else
      return 0.0;
    ##endif
    }
...
macro USE_WATER_RIPPLES()
  ...
  hlsl(ps) {
    ...
    ##if compatibility_mode == compatibility_mode_off
      ...
      float2 rippleNormalPacked = (half2)tex2Dlod(water_ripples_normal, float4(wUV, 0.0, 0.0)).rg;
      rippleNormalPacked = lerp(rippleNormalPacked.rg, float2(0.5, 0.5), max(borderFade.x, borderFade.y));
      return restore_normal((half2)rippleNormalPacked).xzy;
    ##else
      return half3(0.h, 1.h, 0.h);
    ##endif
    }
...
macro INIT_WRITE_GBUFFER_WETNESS()
  (ps) { hpuddles_gfade@f2 = (heightmap_puddles, rain_gbuf_fade, 0, 0); }
endmacro
...
macro USE_ENV_WETNESS()
  ...
  hlsl(ps) {
    void apply_env_wetness(float3 world_pos, float3 vertical_normal, float3 view, float cloud_shadow, inout UnpackedGbuffer g_buf)
    {
      float waterBorder;
      float wetness = get_base_wetness(world_pos, vertical_normal, waterBorder);
      // Apply the wetness for planes (or when an object is high above ground) only in clouds
      wetness *= 1.0 - cloud_shadow * saturate((world_pos.y - waterBorder - 100) * 0.01);
      if (wetness > 0)
      {
        BRANCH
        ...
        if (check_rain_ripples_condition() && g_buf.normal.y > 0)
        {
          ...
          get_rain_ripples(ripplesTc, world_pos, vertical_normal, g_buf.smoothness, g_buf.material == SHADING_SUBSURFACE ? 1.0 : hpuddles_gfade.y, ripplesNorm, ripplesPower, ripplesTcDx, ripplesTcDy);
