...
shader render_big_light_shadows
{
  ...
  (ps) {
    ...
    downsampled_far_depth_tex@smp2d = downsampled_far_depth_tex;//downsampled_close_depth_tex;
  ...
  hlsl(ps) {
    ...
    float get_big_light_shadow(float2 scrpos, float3 viewVect, float w, float3 cameraToPoint, float3 worldPos, float3 worldNormal, float3 lightPos, float light_radius)
    {
      ...
      float2 hitUV;
      float dither = interleavedGradientNoiseFramed(scrpos.xy, floor(shadow_params.y));//if we have temporal aa
      float shadow = 1;
      ...
      if (lightDir.y > 0 && shadow>0.01)
      {
        ...
        for (uint i = 0; i < hsteps; ++i, dist+=stepSize, stepSize*=1.15)
        {
          ...
          float2 tc = getWorldBlurredDepthTC(rayPos, vignette_effect);
          float rayHeightmap = decode_depth_above(tex2Dlod(blurred_depth, float4(tc,0,0) ).x);
      ...
      if (shadow>0.01)
        contactShadow = contactShadowRayCast(
            ...
            cameraToPoint,
            lightDir, w*0.3, 20, dither-0.5, projectionMatrix, w, viewProjectionMatrixNoOfs, hitUV,
            float2(3,2));
    ...
    float4 big_lights_render(VsOutput input HW_USE_SCREEN_POS) : SV_Target0
    {
      ...
      float rawDepth = tex2Dlod(downsampled_far_depth_tex, float4(tc, 0, 0)).x;
      if (rawDepth == 0)
        return 1;
...
shader temporal_big_light_shadows
{
  ...
  SSAO_REPROJECTION(ps, big_shadows_prev_tex)
  ...
  hlsl(ps) {
    ...
    float lengthSq(float3 a) {return dot(a,a);}
    ...
    float4 big_lights_blur_and_temporal(VsOutput input HW_USE_SCREEN_POS) : SV_Target0
    {
      ...
      float rawDepth = tex2Dlod(downsampled_far_depth_tex, float4(input.texcoord.xy,0,0));
      float w = linearize_z(rawDepth, zn_zfar.zw);
      /*/
      float2 dim;
      ...
      float4 depthLT = downsampled_far_depth_tex.GatherRed(downsampled_far_depth_tex_samplerstate, ltc).wzxy;//.wzxy;
      ...
      float2 tc = ltc;
      float4 depthRT = downsampled_far_depth_tex.GatherRed(downsampled_far_depth_tex_samplerstate, toptc.zw).wzxy;
      float4 depthLB = downsampled_far_depth_tex.GatherRed(downsampled_far_depth_tex_samplerstate, bottc.xy).wzxy;
      float4 depthRB = downsampled_far_depth_tex.GatherRed(downsampled_far_depth_tex_samplerstate, bottc.zw).wzxy;
      ...
      float2 motionVector = get_motion_vector(screenTC, input.viewVect * w);
      ...
      #if VALIDATE_HISTORY_UV
      {
        ...
        float4 rawHistoryDepth4 = prev_downsampled_far_depth_tex.GatherRed(prev_downsampled_far_depth_tex_samplerstate, histTCBilinear).wzxy;
