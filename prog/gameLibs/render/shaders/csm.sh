...
macro USE_ESM_SHADOW(code)
  hlsl(code) {
    ...
    {
      return shadow2D(downsampled_shadows, uv_z);
    }
    ##else
...
macro USE_CSM_SHADOW_INTERNAL(code, num_cascades)
  hlsl(code) {
    ...
    half get_csm_shadow_8_tap(float3 depthShadowTC, float dither)
    {
      float2 rotation;
    ...
    half2 get_csm_shadow(float3 pointToEye, float w)
    {
      uint cascade_id;
    ...
    half2 get_csm_shadow_dithered(float3 pointToEye, float dither, out uint cascade_id, out float3 shadow_tc)
    {
      float csmEffect = 1;
      shadow_tc = get_csm_shadow_tc_scaled(pointToEye, cascade_id, csmEffect, 1.0 + dither*0.1);
      FLATTEN
      if (shadow_tc.z<1)
        return half2(get_csm_shadow_8_tap(shadow_tc, dither), saturate(csmEffect*10-9));//saturate(csmEffect*10-9));//saturate(csmEffect*10-9));
      return half2( 1, 1 );
    }
