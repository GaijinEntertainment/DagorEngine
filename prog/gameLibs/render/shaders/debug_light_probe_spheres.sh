...
shader debug_mirror_sphere
{
  ...
  hlsl(ps)
  {
    half3 debug_light_probe_spheres_ps(VsOutput input):SV_Target0
    {
      float3 normal = normalize(input.normal);
      float4 indoorRefl__localWeight = use_indoor_probes(input.worldPos.xyz, normal, 0 /* roughMip */, 0 /* viewDist */);
