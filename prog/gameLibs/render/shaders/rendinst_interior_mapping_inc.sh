...
define_macro_if_not_defined INIT_NUCLEAR_DESTRUCTION()
endmacro

define_macro_if_not_defined USE_NUCLEAR_DESTRUCTION()
endmacro

macro USE_RENDINST_INTERIOR_MAPPING()
  RENDINST_BLOCK()

  INIT_NUCLEAR_DESTRUCTION()
  ...
  {
    hlsl(ps) {
      half4 rendinst_render_depth_black_ps(VsOutput input HW_USE_SCREEN_POS) : SV_Target
      {
  ...
  else
  {
    hlsl(ps) {
      ...
      )
      {
        ...
        return encode_gbuffer3(gbuffer, input.pointToEye, GET_SCREEN_POS(input.pos));
      }
    }
    compile("target_ps", "simple_ps");
  }

  USE_NUCLEAR_DESTRUCTION()
endmacro