int bc6h_compression_mode = 1;
interval bc6h_compression_mode : bc6h_compression_fast < 1, bc6h_compression_quality;

macro USE_BC6H_COMPRESSION(code)

hlsl {
  #define HALF_MAX 65504.0f
  #define BC6H_PATTERN_NUM 32
  ...
  uint4 pack_bc6h_block(float4 texels[16])
  {
    ...
    float3 rgb_texels[16];
#define COPY_ITERATION(n) rgb_texels[n] = texels[n].rgb;
  COPY_ITERATION(0);
  COPY_ITERATION(1);
  COPY_ITERATION(2);
  COPY_ITERATION(3);
  ...
  COPY_ITERATION(14);
  COPY_ITERATION(15);
#undef COPY_ITERATION

    uint4 block = uint4(0, 0, 0, 0);
    float blockMSLE = 0.0f;

    EncodeP1(block, blockMSLE, rgb_texels);
##if bc6h_compression_mode == bc6h_compression_quality && !hardware.vulkan//HLSLcc or Vulkan generates shit
    for (uint i = 0; i < 32; ++i)
    {
      EncodeP2Pattern(block, blockMSLE, i, rgb_texels);
    }
##endif
