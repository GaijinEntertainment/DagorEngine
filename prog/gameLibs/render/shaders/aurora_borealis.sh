...
shader aurora_borealis
{
  ...
  hlsl(ps) {
    ...
    half3 aurora_borealis(VsOutput input HW_USE_SCREEN_POS):SV_Target0
    {
...
shader apply_aurora_borealis
{
  ...
  {
    // In compatibility mode, INIT_HDR_STCODE() is in <global_frame> block
    USE_HDR()
  }
  ...
  hlsl(ps) {
    half3 apply_aurora_borealis(VsOutput input HW_USE_SCREEN_POS):SV_Target0
    {
