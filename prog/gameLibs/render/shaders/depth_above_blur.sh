...
shader single_pass_blur11
{
  ...
  (ps) {
    tex0@smp2d = depth_ao_tex_to_blur;
  ...
  hlsl(ps) {
    ...
    out_type GaussianBlur( float2 centreUV, float2 pixelOffset, float4 clampTc )
    {
        ...
        for( int i = 0; i < stepCount; i++ )
        {
          ...
          out_type col = tex2Dlod(tex0, float4(clamp(centreUV + texCoordOffset, clampTc.xz, clampTc.yw), 0,0)).x +
            tex2Dlod(tex0, float4(clamp(centreUV - texCoordOffset, clampTc.xz, clampTc.yw),0,0)).x;
        ...
        {
          float2 texCoordOffset = gOffsets[i] * pixelOffset;
          out_type col = tex2Dlod(tex0, float4(centreUV + texCoordOffset,0,0)).x + tex2Dlod(tex0, float4(centreUV - texCoordOffset,0,0)).x;
          colOut += gWeights[i] * col;
        }
        #elif  stepCount == 9
