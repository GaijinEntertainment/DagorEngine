...
macro USE_DEFERRED_SNOW_COVER(code)
  hlsl(code) {
    float apply_snow_cover(float3 worldPos, float3 normal, float eclipse_progress_value, inout half3 diffuse_color, inout half3 specular_color,
...
#if !IS_SNOW_COVER_COMPATIBILITY
        computeSnowSparkle(worldPos.xyz, world_view_pos.xyz, min(0.99, 1 - eclipse_progress_value), normal, smoothness);
#endif
...
macro DEFERRED_SNOW_COVER_COMPATIBILITY_WITH_PARAMS(use_noise, use_intensity_map)
  if (game != game_wtm && compatibility_mode == compatibility_mode_on)
  {
    if (use_noise)
    {
