...
int baked_impostor_preshadows = 1;
...
macro INIT_BAKED_IMPOSTOR_VARS(shader_var_type) // shader_var_type: dynamic or static
  ...
  hlsl(vs)
  {
    ...
    {
      return impostor_data_buffer[impostor_data_offset+IMPOSTOR_DATA_BSP_Y__PRESHADOW].x;
    }
...
##elif baked_impostor_preshadows == preshadow_dynamic
      return impostor_data_buffer[impostor_data_offset+IMPOSTOR_DATA_BSP_Y__PRESHADOW].y > 0;
##else
      return true;
##endif
    }
...
macro INIT_BAKED_IMPOSTORS()
  ...
  ##if rendinst_render_pass == rendinst_render_pass_depth || rendinst_render_pass == rendinst_render_pass_normal
    ...
    ##else
      #define OVERWRITE_IMP_HALF_SIZE 1
    ##endif
  ##endif
  }
endmacro
...
macro USE_IMPOSTORS_COMMON2()
  hlsl
  {
    ...
    float2 get_impostor_half_size( float4 slices, float slices_det, float4 s01, float4 s23, float4 s45, float4 s67, float2 s7 )
    {
      float widths[9] =  { s01.x, s01.z, s23.x, s23.z, s45.x, s45.z, s67.x, s67.z, s7.x };
      float heights[9] = { s01.y, s01.w, s23.y, s23.w, s45.y, s45.w, s67.y, s67.w, s7.y };

      uint i0 = min(uint(slices.x + 0.5), 8);
      uint i1 = min(uint(slices.y + 0.5), 8);

      float2 wh0 = float2( widths[i0], heights[i0] );
      float2 wh1 = float2( widths[i1], heights[i1] );

      return lerp( wh0, wh1, slices_det );
    }
