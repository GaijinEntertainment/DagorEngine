...
RW_PREFIX RWTexture2D<unorm float4>  NBSGbuf0 : register(u3);
RW_PREFIX RWTexture2D<unorm float4>  NBSGbuf1 : register(u4);
RW_PREFIX RWTexture2D<unorm float4>  NBSGbuf2 : register(u5);
RW_PREFIX RWTexture2D<float4>  NBSGbuf3 : register(u6);
#if GBUFFER_NORMALS_PACKED
  RW_PREFIX RWTexture2D<unorm float4>  NBSGbufExtraBits : register(u7);
#endif
...
#if GBUFFER_NORMALS_PACKED
  void writeGbuffNormal(uint2 tc, half4 normal_mat) { NBSGbuf1[tc] = normal_mat; }
  void writeGbuffExtraBits(uint2 tc, half4 extra_bits) { NBSGbufExtraBits[tc] = extra_bits; }
  float4 readGbuffNormal(uint2 tc) { return NBSGbuf1[tc]; }
  float4 readGbuffExtraBits(uint2 tc) { return NBSGbufExtraBits[tc]; }
#else
