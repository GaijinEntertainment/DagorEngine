    ...
    VsOutput impostor_vs(VsInput input, uint vertexId : SV_VertexID HW_USE_INSTANCE_ID)
    {
      ...
      ##else
        ...
        forward /= distToInstance;
         ##if rendinst_dissolve == rendinst_dissolve_inout
          opacity = saturate(distToInstance * rendinst_opacity.z + rendinst_opacity.w);
        ##endif
  ...
  hlsl(ps) {
    ...
    ##elif rendinst_render_pass == rendinst_render_pass_normal
      ...
      ##endif
      {
        ...
        return encode_gbuffer(result, screenpos.xyz);
      }
    ##endif
  }
