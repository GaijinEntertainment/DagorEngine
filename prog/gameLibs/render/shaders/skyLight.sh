...
half3 getSkyReflectionMip(float mip, float linearRoughness, float3 roughReflection, float NoV, float3 specularColor)
{
  return half3(texCUBElod(envi_probe_specular, float4(roughReflection, mip)).rgb * EnvBRDFApprox(half3(specularColor), half(linearRoughness), half(NoV)));
}

half3 getSkyReflection(float linearRoughness, float3 roughReflection, float NoV, float3 specularColor)
{
  float mip = ComputeReflectionCaptureMipFromRoughness((half)linearRoughness);
