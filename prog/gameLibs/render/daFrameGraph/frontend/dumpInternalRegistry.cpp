...
void dafg::dump_internal_registry(const InternalRegistry &registry)
{
  dump_internal_registry(registry, [](auto) { return true; }, [](auto) { return true; });
}

void dafg::dump_internal_registry(const InternalRegistry &registry, NodeValidCb nodeValid, ResValidCb resourceValid)
{
  logwarn("daFG: Framegraph full user graph state dump:");
  for (auto [nodeId, nodeData] : registry.nodes.enumerate())
  {
    auto logNode = [&](NodeNameId id) {
      logwarn("daFG: \t\t'%s'%s", registry.knownNames.getName(id), !nodeValid(id) ? " (BROKEN)" : "");
    };

    auto logRes = [&, &nodeData = nodeData](ResNameId id) {
      const auto &req = nodeData.resourceRequests.find(id)->second;
      logwarn("daFG: \t\t%s'%s'%s", req.optional ? "optional " : "", registry.knownNames.getName(id),
        !resourceValid(id) ? " (BROKEN)" : "");
    };

    auto dumpHelper = [](const auto &data, const char *heading, const auto &f) {
      if (data.empty())
        return;
      logwarn("daFG: \t%s", heading);
      for (const auto &d : data)
        f(d);
    };

    const bool broken = !nodeValid(nodeId);
    logwarn("daFG: Node '%s' (%spriority %d)", registry.knownNames.getName(nodeId), broken ? "BROKEN, " : "", nodeData.priority);
    ...
    dumpHelper(nodeData.historyResourceReadRequests, "History read resources:", [&](const auto &pair) {
      logwarn("daFG: \t\t%s'%s'%s", pair.second.optional ? "optional " : "", registry.knownNames.getName(pair.first),
    ...
    dumpHelper(nodeData.renamedResources, "Renamed resources:", [&](auto pair) {
      ...
      logwarn("daFG: \t\t'%s'%s -> '%s'%s", registry.knownNames.getName(pair.second), secondBroken ? " (BROKEN)" : "",
        registry.knownNames.getName(pair.first), firstBroken ? " (BROKEN)" : "");
    });
  }
  logwarn("daFG: Finished dumping framegraph state.");
}
