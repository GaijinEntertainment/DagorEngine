...
VirtualResourceSemiRequest<NameSpaceRequest::NewRoRequestPolicy> NameSpaceRequest::read(const char *name) const
{
  ...
  registry->nodes[nodeId].resourceRequests.emplace(resId, ResourceRequest{ResourceUsage{Usage::UNKNOWN, Access::READ_ONLY}});
...
VirtualResourceSemiRequest<NameSpaceRequest::NewRoRequestPolicy> NameSpaceRequest::read(NamedSlot slot_name) const
{
  ...
  registry->nodes[nodeId].resourceRequests.emplace(slotResId, ResourceRequest{ResourceUsage{Usage::UNKNOWN, Access::READ_ONLY}, true});
...
{
  const auto resId = registry->knownNames.addNameId<ResNameId>(nameSpaceId, name);
  registry->nodes[nodeId].historyResourceReadRequests.emplace(resId,
    ResourceRequest{ResourceUsage{Usage::UNKNOWN, Access::READ_ONLY}});
...
VirtualResourceSemiRequest<NameSpaceRequest::NewRwRequestPolicy> NameSpaceRequest::modify(NamedSlot slot_name) const
{
  ...
  registry->nodes[nodeId].resourceRequests.emplace(slotResId,
    ResourceRequest{ResourceUsage{Usage::UNKNOWN, Access::READ_WRITE}, true});
