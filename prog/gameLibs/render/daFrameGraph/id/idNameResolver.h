...
public:
  ...
  void rebuild(const IdHierarchicalNameMap<FolderT, Ts...> &name_map, const IdIndexedFlags<Ts, A> &...validity_flags_pack)
  {
    ...
    (
      [&]() {
        ...
        {
          const auto targetShortNameId = name_map.getShortNameId(from);
          ...
          {
            const auto candidate = name_map.template getNameId<T>(currFolder, targetShortNameId);
          ...
          if (stepUpCounter == INFINITE_LOOP_THRESHOLD)
            logerr("daFG: Went up by %d levels of nested namespaces, probably an infinite loop in dafg backend!", stepUpCounter);
        }
      }(),
      ...);
  }
  ...
  auto size() const
  {
    return detail::PerTypeResolver<T>::resolved.size();
  }
};
