...
#define GLOBAL_VARS_LIST       \
  ...
  VAR(beam_stride)             \
  VAR(beam_command_count)      \
  VAR(beam_trace_cull_current_time)
...
#undef VAR
  ...
  tracerBuffer = dag::buffers::create_ua_sr_structured(sizeof(GPUBeamTracer), MAX_BEAM_TRACERS, "beamTracers", d3d::buffers::Init::No,
    RESTAG_TRACER);
  tracerVertsBuffer = dag::buffers::create_ua_sr_structured(sizeof(GPUBeamTracerVertices), TRACER_SEGMENTS_COUNT * MAX_BEAM_TRACERS,
    "beamTracerVerts", d3d::buffers::Init::No, RESTAG_TRACER);
  tracerBufferDynamic = dag::buffers::create_ua_sr_structured(sizeof(GPUBeamTracerDynamic), MAX_BEAM_TRACERS, "beamTracersDynamic",
    d3d::buffers::Init::No, RESTAG_TRACER);
  culledTracerTails = dag::buffers::create_ua_sr_structured(sizeof(GPUBeamTracerTailRender), MAX_BEAM_TRACERS, //*2
    "culledTracerTails", d3d::buffers::Init::No, RESTAG_TRACER);
  culledTracerHeads = dag::buffers::create_ua_sr_structured(sizeof(GPUBeamTracerHeadRender), MAX_BEAM_TRACERS, "culledTracerHeads",
    d3d::buffers::Init::No, RESTAG_TRACER);
  drawIndirectBuffer = dag::buffers::create_ua_indirect(dag::buffers::Indirect::Draw, 2, "culledTracersIndirect", RESTAG_TRACER);

  tracerIdsBuffer.set(d3d::buffers::create_persistent_cb(dag::buffers::cb_array_reg_count<uint4>(TRACER_CONST_BUF_MAX_SIZE),
                        "tracerIdsBuffer", RESTAG_TRACER),
    "tracerIdsBuffer");
  createCommandsBuffer.set(
    d3d::buffers::create_persistent_cb(dag::buffers::cb_array_reg_count<BeamTracerCreateCommand>(BEAM_TRACER_BEAM_MAX_CREATE_COMMANDS),
      "beamTracerCreateCommandsBuffer", RESTAG_TRACER),
    "beamTracerCreateCommandsBuffer");
  updateCommandsBuffer.set(
    d3d::buffers::create_persistent_cb(dag::buffers::cb_array_reg_count<BeamTracerUpdateCommand>(BEAM_TRACER_BEAM_MAX_UPDATE_COMMANDS),
      "beamTracerUpdateCommandsBuffer", RESTAG_TRACER),
    "beamTracerUpdateCommandsBuffer");
...
void BeamTracerManager::performGPUCommands()
{
  ...
  Point4 time = {cTime, lastDt, 0, 0};
  ShaderGlobal::set_float4(beam_trace_cull_current_timeVarId, time);
  ...
  if (createCommands.size())
  {
    ...
    createCommandsBuffer.getBuf()->updateDataWithLock(0, sizeof(BeamTracerCreateCommand) * createCommands.size(),
      createCommands.data(), VBLOCK_WRITEONLY | VBLOCK_DISCARD);
    createCommandsBuffer.setVar();
    ...
    G_STATIC_ASSERT(sizeof(createCommands[0]) == BEAM_TRACER_BEAM_CREATE_COMMAND_SIZE * sizeof(float4));

    for (const auto &createCommand : createCommands)
      usedTracers[currentUsed].push_back(createCommand.id);

    for (int i = 0; i < createCommands.size(); i += TRACER_COMMAND_WARP_SIZE)
    {
      int batch_size = createCommands.size() - i;
      // debug("batch_size = %d", batch_size);

      ShaderGlobal::set_int(beam_command_countVarId, batch_size);
      ShaderGlobal::set_int(beam_strideVarId, i);
  ...
  if (updateCommands.size())
  {
    ...
    updateCommandsBuffer.getBuf()->updateDataWithLock(0, sizeof(BeamTracerUpdateCommand) * updateCommands.size(),
      updateCommands.data(), VBLOCK_WRITEONLY | VBLOCK_DISCARD);
    updateCommandsBuffer.setVar();

    // debug("%d updateCommands", updateCommands.size());
    G_STATIC_ASSERT(sizeof(updateCommands[0]) % sizeof(float4) == 0);
    G_STATIC_ASSERT(sizeof(updateCommands[0]) == BEAM_TRACER_BEAM_UPDATE_COMMAND_SIZE * sizeof(float4));

    for (int i = 0; i < updateCommands.size(); i += TRACER_COMMAND_WARP_SIZE)
    {
      int batch_size = updateCommands.size() - i;
...
void BeamTracerManager::beforeRender(const Frustum &frustum, float exposure_time, float pixel_scale)
{
  ...
  Point4 time = {cTime, lastDt, exposure_time, pixel_scale};
  ShaderGlobal::set_float4(beam_trace_cull_current_timeVarId, time);
  G_ASSERT(usedTracers[0].static_size % 16 == 0);

  tracerIdsBuffer.getBuf()->updateDataWithLock(0, sizeof(uint4) * usedTracers[currentUsed].size(), usedTracers[currentUsed].data(),
    VBLOCK_WRITEONLY | VBLOCK_DISCARD);
  tracerIdsBuffer.setVar();

  for (int i = 0; i < usedTracers[currentUsed].size(); i += TRACER_CULL_WARP_SIZE)
  {
    int batch_size = usedTracers[currentUsed].size() - i;
    ShaderGlobal::set_int(beam_command_countVarId, batch_size);
    ShaderGlobal::set_int(beam_strideVarId, i);
...
void BeamTracerManager::renderTrans()
{
  ...
  ShaderGlobal::set_float(beam_max_intensity_mulVarId, tacLaserRenderSettings.maxIntensityMul);
  ShaderGlobal::set_float(beam_fog_intensity_mulVarId, tacLaserRenderSettings.fogIntensityMul);
...
{
  freeTracers.resize(MAX_BEAM_TRACERS);
  for (int i = 0; i < MAX_BEAM_TRACERS; ++i)
  {
    freeTracers[i] = MAX_BEAM_TRACERS - 1 - i;
