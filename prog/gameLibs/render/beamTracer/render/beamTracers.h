...
protected:
  ...
  SbufferIDHolderWithVar tracerIdsBuffer, createCommandsBuffer, updateCommandsBuffer;
  eastl::unique_ptr<ComputeShaderElement> updateCommands_cs, createCommands_cs, cullTracers_cs, clearIndirectBuffers;

  carray<BeamTracer, MAX_BEAM_TRACERS> tracers;
  carray<float, MAX_BEAM_TRACERS> tracersVaporTime;
#if DAGOR_DBGLEVEL > 0
  FixedBitArray<MAX_BEAM_TRACERS> tracersLeft;
#endif
  // carray<bbox3f, MAX_BEAM_TRACERS> tracersBox;
  StaticTab<int16_t, MAX_BEAM_TRACERS> freeTracers;
  StaticTab<int16_t, MAX_BEAM_TRACERS> usedTracers[2];
  int currentUsed;
