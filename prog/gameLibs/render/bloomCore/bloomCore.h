...
void regenerate_downsample_chain(
  dafg::NameSpace ns, dag::Vector<dafg::NodeHandle> &nodes, uint32_t mips_count, bool use_esram = true,
  RenderCallback cb = [](RenderFunc render_func) { render_func(); });
void regenerate_upsample_chain(
  dafg::NameSpace ns, dag::Vector<dafg::NodeHandle> &nodes, uint32_t mips_count, float upsample_factor, const Color3 &halation_color,
