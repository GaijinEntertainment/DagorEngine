#ifndef VOLFOG_COMMON_INCLUDED
#define VOLFOG_COMMON_INCLUDED 1
...
half volfog_get_mieG0() { return 0.55h; }
half volfog_get_mieG1() { return -0.4h; }

half calc_sun_phase(half3 view_dir_norm, half3 from_sun_direction)
{
  //for high mie we need self shadow from media. Instead we change mie extrincity
  return half(phaseSchlickTwoLobe(dot(-from_sun_direction, view_dir_norm), 1.5h*volfog_get_mieG0(), 1.5h*volfog_get_mieG1()));
}
...
#define is_voxel_id_shadow_occluded(dtId) is_voxel_id_occluded_impl(volfog_shadow_occlusion, dtId)
