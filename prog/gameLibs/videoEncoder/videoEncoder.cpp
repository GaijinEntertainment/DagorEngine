...
bool get_specific_device(LPCWSTR szLongName, ComPtr<IMMDevice> &ppMMDevice)
{
  ...
  for (UINT i = 0; i < count; i++)
  {
    ...
    if (0 == _wcsicmp(pv.pwszVal, szLongName))
    {
      ...
      {
        PropVariantClear(&pv);
        logerr("Found (at least) two devices named %s", szLongName);
        return false;
      }
    }
    PropVariantClear(&pv);
  ...
  if (!ppMMDevice)
  {
    logerr("Could not find a device named %s", szLongName);
    return false;
  }
...
bool VideoEncoder::start(const VideoSettings &settings)
{
  ...
  stretchedTex = dag::create_tex(nullptr, w, h, TEXCF_RTARGET, 1, "stretchedVideoTex", RESTAG_VIDEO);
