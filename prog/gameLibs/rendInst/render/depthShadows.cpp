...
struct ColorDepthTexturePair
{
  ...
  {
    colorTex = d3d::create_tex(nullptr, size, size, TEXCF_RTARGET, 1, "rtTempTex");
    d3d_err(colorTex);
    createDepth(size);
  }
  ...
  {
    depthTex = d3d::create_tex(nullptr, size, size, TEXFMT_DEPTH24 | TEXCF_RTARGET, 1, "rtTempTexDepth");
    d3d_err(depthTex);
  }
};
  ...
  if (pool.shadowImpostorUpdatePhase == PHASE_LOW_PASS_RENDER || pool.shadowImpostorUpdatePhase == PHASE_HIGH_PASS_RENDER)
  {
    ...
    if (pool.hasImpostor())
      cb.setInstancing(3, 1,
        rendinst::gen::get_rotation_palette_manager()->getImpostorDataBufferOffset({layerIdx, poolNo}, pool.impostorDataOffsetCache));
    else
    ...
    d3d::set_render_target(RenderTarget{targetPair.depthTex, 0, 0}, DepthAccess::RW, {{targetPair.colorTex, 0, 0}});
