...
#define CLIPMAP_SHADOW_DELTA_MUL (1.f / 16.f)
...
void ClipmapShadow::init(int shadowSize)
{
  ...
  clipmapShadowTex.set(d3d::create_tex(NULL, clipmapShadowSize * NUM_CLIPMAP_SHADOW_CASCADES, clipmapShadowSize,
...
void ClipmapShadow::switchOff()
{
  ...
  img[0].w = img[0].h = 1;
  img[1].w = img[1].h = -1;
  clipmapShadowTex.set(d3d::create_tex(img, 1, 1, TEXFMT_A8R8G8B8, 1, "clipmapShadowWhiteTex"));
  d3d_err(clipmapShadowTex.getTex2D());

  clipmapShadowTexVarId = get_shader_variable_id("clipmap_shadow_tex", true);
  ShaderGlobal::set_texture(clipmapShadowTexVarId, clipmapShadowTex.getId());
  ...
  else
  {
    ...
    for (int cascadeNo = NUM_CLIPMAP_SHADOW_CASCADES - 1; cascadeNo >= 0; cascadeNo--)
    {
      ...
      for (int i = 0; i < quadRegions[cascadeNo].size(); ++i)
      {
        ...
        rendinst::renderRIGenShadowsToClipmap(boxReg, cascadeNo);
      }
