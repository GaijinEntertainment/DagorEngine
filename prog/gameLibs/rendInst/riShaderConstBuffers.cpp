...
void endRenderInstancing()
{
  ...
  d3d::set_const_buffer(STAGE_VS, perinstBuffNo, NULL);
  d3d::set_const_buffer(STAGE_VS, instanceBuffNo, NULL);
...
void startRenderInstancing()
{
  ...
  d3d::set_const_buffer(STAGE_VS, perinstBuffNo, rendinstgenrender::perDrawCB); // fixme: set only once
#endif
}
...
void init_instances_tb()
{
  for (int i = 0; i < instancesCB.size(); ++i)
    instancesCB[i] = d3d_buffers::create_one_frame_cb(MIN_INST_COUNT << i, "perInstanceData");
...
void RiShaderConstBuffers::flushPerDraw()
{
  ...
  if (perDrawCB)
  {
    bool res = rendinstgenrender::perDrawCB->updateDataWithLock(0, sizeof(consts), consts, VBLOCK_DISCARD);
    /*G_ASSERT(res);*/ (void)res;
    d3d::set_const_buffer(STAGE_VS, perinstBuffNo, rendinstgenrender::perDrawCB);
  }
  else
  {
...
void RiShaderConstBuffers::setInstancePositions(const float *data, int vec4_count)
{
  ...
  const int bin = (d3d::get_driver_code() == _MAKE4C('MTL'))
                    ? rendinstgenrender::instancesCB.size() - 1 // because of metal validator has to select cb size that matches the
                                                                // shader
                    : min<int>(get_bigger_log2(1 + (vec4_count - 1) / MIN_INST_COUNT), rendinstgenrender::instancesCB.size() - 1);

  rendinstgenrender::instancesCB[bin]->updateDataWithLock(0, vec4_count * sizeof(vec4f), data, VBLOCK_WRITEONLY | VBLOCK_DISCARD);
  // d3d::set_buffer(STAGE_VS, INSTANCING_TEXREG, rendinstgenrender::instancesCB[bin]);
  // on my 1070 ConstBuffer is proven to work faster than TB. We can actually handle a lot of instances in one CB. It may be worth
  // switching to CB only instancing... however, in synthetic tests both TB and SB were faster than CB when fetch 3 float4. I guess,
  // the difference is that we sample just ONE float4 for trees, and cache prefetching doesn't help us (like it does in TB/SB)
  d3d::set_const_buffer(STAGE_VS, instanceBuffNo, rendinstgenrender::instancesCB[bin]);
#endif
}
