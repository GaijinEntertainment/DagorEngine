...
{
static int sourceTexVarId[rendinstgenrender::IMP_COUNT] = {-1, -1, -1};
static int texelSizeVarId = -1;
static int texIndVid = -1;
static int baked_impostor_multisampleVarId = -1;
static int vertical_impostor_slicesVarId = -1;
...
static int impostor_tex_count = rendinstgenrender::IMP_COUNT;
static uint32_t impostor_compressed_texformats[rendinstgenrender::IMP_COUNT] = {
  TEXFMT_DXT5 | TEXCF_SRGBREAD, TEXFMT_DXT5, TEXFMT_DXT5};
static uint32_t impostor_texformats[rendinstgenrender::IMP_COUNT] = {TEXCF_SRGBREAD | TEXCF_SRGBWRITE, 0, 0};
static E3DCOLOR impostor_clear_color[rendinstgenrender::IMP_COUNT] = {IMPOSTORS_CLEAR_COL, 0x00FFFFFF, 0xFFFFFFFF};
UniqueTex impostorColorTexture[rendinstgenrender::IMP_COUNT];
UniqueTex impostorCompressionBuffers[rendinstgenrender::IMP_COUNT];
BcCompressor *bcCompressors[rendinstgenrender::IMP_COUNT] = {nullptr, nullptr, nullptr};
...
static __forceinline void get_up_left(Point3 &up, Point3 &left, const TMatrix &itm)
{
...
static __forceinline void set_up_left_to_shader(const TMatrix &itm)
{
  Point3 up, left;

  get_up_left(up, left, itm);

  ShaderGlobal::set_color4_fast(rendinstgenrender::dynamicImpostorViewXVarId, Color4(left.x, left.y, left.z, 0));
  ShaderGlobal::set_color4_fast(rendinstgenrender::dynamicImpostorViewYVarId, Color4(up.x, up.y, up.z, 0));
}

void rendinst::set_billboards_vertical(bool is_vertical) { rendinstgenrender::vertical_billboards = is_vertical; }

bool rendinst::enable_impostors_compression(bool enabled)
{
  if (!rendinstgenrender::bcCompressors[0])
    enabled = false;

  bool ov = rendinstgenrender::use_impostors_compression;
  rendinstgenrender::use_impostors_compression = enabled;
...
{
  if (rendinstgenrender::maxAlbedoRTSizeMult != value)
  {
    rendinstgenrender::maxAlbedoRTSizeMult = value;
    rendinstgenrender::is_render_target_size_changed = true;
  }
}

void initImpostorsTempTextures()
{
  int texWidth = rendinstgenrender::use_impostors_compression
                   ? rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE * rendinstgenrender::maxAlbedoRTSizeMult
                   : rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE;
  int texHeight = texWidth * IMPOSTOR_MAX_ASPECT_RATIO;
  int numMips = get_log2i(max(texWidth, texHeight)) + 1;

  if (rendinstgenrender::use_color_padding && !rendinstgenrender::impostorColorTexture[0].getTex2D())
  {
    for (int i = 0; i < rendinstgenrender::impostor_tex_count; ++i)
    {
      rendinstgenrender::impostorColorTexture[i] = dag::create_tex(NULL, texWidth, texHeight,
        TEXCF_RTARGET | rendinstgenrender::impostor_texformats[i], 1, String(0, "colorImpostorTexture%i", i).str());
      rendinstgenrender::impostorColorTexture[i]->texfilter(TEXFILTER_POINT);
      rendinstgenrender::impostorColorTexture[i]->texaddru(TEXADDR_CLAMP);
      rendinstgenrender::impostorColorTexture[i]->texaddrv(TEXADDR_CLAMP);
    }
  }

  if (rendinstgenrender::use_impostors_compression && !rendinstgenrender::impostorCompressionBuffers[0].getTex2D())
  {
    for (int i = 0, e = rendinstgenrender::impostor_tex_count; i < e; ++i)
    {
      int flags = TEXCF_RTARGET | rendinstgenrender::impostor_texformats[i];
      rendinstgenrender::impostorCompressionBuffers[i] =
        dag::create_tex(NULL, texWidth, texHeight, flags, numMips, String(0, "impostorCompressionBuffer%i", i).str());
      rendinstgenrender::impostorCompressionBuffers[i]->texfilter(TEXFILTER_POINT);
      rendinstgenrender::impostorCompressionBuffers[i]->texaddru(TEXADDR_CLAMP);
      rendinstgenrender::impostorCompressionBuffers[i]->texaddrv(TEXADDR_CLAMP);
    }
  }
}
...
{
  const uint32_t fmt = rendinstgenrender::impostor_compressed_texformats[i];
...
void initImpostorsGlobals()
{
  ...
  rendinstgenrender::impostor_tex_count = graphics->getInt("impostorTexCount", compatibilityMode ? 1 : rendinstgenrender::IMP_COUNT);

  rendinstgenrender::impostor_texformats[0] = get_format(graphics->getStr("rendinstImpostorTex0", "argb8_srgb"));
  rendinstgenrender::impostor_compressed_texformats[0] = get_format(graphics->getStr("rendinstImpostorCompressedTex0", "dxt5_srgb"));
  for (int i = 1; i < rendinstgenrender::impostor_tex_count; ++i)
  {
    rendinstgenrender::impostor_texformats[i] = get_format(graphics->getStr(String(0, "rendinstImpostorTex%d", i).c_str(), "argb8"));
    rendinstgenrender::impostor_compressed_texformats[i] =
      get_format(graphics->getStr(String(0, "rendinstImpostorCompressedTex%d", i).c_str(), "dxt5"));
  }
  for (int i = 0; i < rendinstgenrender::impostor_tex_count; ++i)
  {
    rendinstgenrender::impostor_clear_color[i] =
      graphics->getE3dcolor(String(0, "rendinstImpostorClearColor%d", i).c_str(), rendinstgenrender::impostor_clear_color[i]);
  }

  rendinstgenrender::sourceTexVarId[0] = ::get_shader_variable_id("source_tex0");
  rendinstgenrender::sourceTexVarId[1] = ::get_shader_variable_id("source_tex1", true);
  rendinstgenrender::sourceTexVarId[2] = ::get_shader_variable_id("source_tex2", true);
  rendinstgenrender::texelSizeVarId = ::get_shader_variable_id("texel_size");
  rendinstgenrender::texIndVid = ::get_shader_variable_id("rendinst_impostor_tex_index", true);
  rendinstgenrender::dynamicImpostorViewXVarId = ::get_shader_glob_var_id("impostor_view_x");
  rendinstgenrender::dynamicImpostorViewYVarId = ::get_shader_glob_var_id("impostor_view_y");

  rendinstgenrender::postfxBuildMip = create_postfx_renderer("generate_imp_mipmaps", false);
  ...
  state.colorWr = 0;
  rendinstgenrender::impostorShadowOverride.reset(shaders::overrides::create(state));

  if (!rendinstgenrender::postfxBuildMip)
    rendinstgenrender::use_color_padding = false;

  bool compressionAvailable = false;
  if (BcCompressor::isAvailable(BcCompressor::COMPRESSION_BC3))
  {
    rendinstgenrender::bcCompressors[0] = new BcCompressor(get_compressor(0), 0, 0, 0, 1, get_compressor_shader(0));
    compressionAvailable = rendinstgenrender::bcCompressors[0]->getCompressionType() != BcCompressor::COMPRESSION_ERR;
    for (int i = 1; i < rendinstgenrender::impostor_tex_count && compressionAvailable; ++i)
    {
      rendinstgenrender::bcCompressors[i] = new BcCompressor(get_compressor(i), 0, 0, 0, 1, get_compressor_shader(i));
      if (rendinstgenrender::bcCompressors[i]->getCompressionType() == BcCompressor::COMPRESSION_ERR)
      {
  ...
  {
    for (int i = 0; i < rendinstgenrender::IMP_COUNT; ++i)
      del_it(rendinstgenrender::bcCompressors[i]);

    rendinstgenrender::use_impostors_compression = false;
  }
...
void termImpostorsGlobals()
{
  ...
  shaders::overrides::destroy(rendinstgenrender::impostorShadowOverride);
  del_it(rendinstgenrender::postfxBuildMip);
  for (int i = 0; i < rendinstgenrender::impostor_tex_count; ++i)
  {
    rendinstgenrender::impostorColorTexture[i].close();
    rendinstgenrender::impostorCompressionBuffers[i].close();
    del_it(rendinstgenrender::bcCompressors[i]);
  }
  rendinstgenrender::impostorDepthTextures.clear();
}
...
void RendInstGenData::RtData::initImpostors()
{
  ...
  rendinstgenrender::MIN_DYNAMIC_IMPOSTOR_TEX_SIZE = graphics->getInt("impostorMinTexSize", impMinTexSize);
  rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE = get_impostor_max_tex_size(*graphics);

  numImpostorsCount = 0;
  if (rendinstgenrender::dynamic_impostor_texture_const_no < 0)
    return;
  ...
  for (unsigned int poolNo = 0; poolNo < rtPoolData.size(); poolNo++)
  {
    ...
    if (riRes[poolNo]->hasImpostor())
    {
      ...
      pool.impostor.tex.resize(rendinstgenrender::impostor_tex_count);
      ...
      d3d::get_render_target_size(screenWidth, screenHeight, NULL, 0);

      int rtSize = (int)(screenWidth * impostorScreenMul * radius / max(1.f, distance));

      rtSize = get_bigger_pow2(rtSize);

      int maximumRTSize = rendinstgenrender::use_impostors_compression
                            ? rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE * rendinstgenrender::maxAlbedoRTSizeMult
                            : rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE;
      rtSize = clamp(rtSize, (int)rendinstgenrender::MIN_DYNAMIC_IMPOSTOR_TEX_SIZE, maximumRTSize);
      int rtSizeY = rtSize;
      if (bboxAspectRatio > 1.75)
        rtSizeY = rtSize * IMPOSTOR_MAX_ASPECT_RATIO; // trees are more likely to be taller, then wider

      pool.flags &= rendinstgenrender::RtPoolData::HAS_TRANSITION_LOD;
      ...
      if (rendinstgenrender::use_impostors_compression)
        texflags = TEXCF_CLEAR_ON_CREATE;

      const unsigned int texflagsColor =
        texflags | (rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostor_compressed_texformats[0]
                                                                 : rendinstgenrender::impostor_texformats[0]);
      ...
      pool.impostor.tex[0] = dag::create_tex(NULL, rtSize, rtSizeY, texflagsColor, pool.impostor.renderMips, name);
      ...
      rtSize =
        clamp(rtSize, (int)rendinstgenrender::MIN_DYNAMIC_IMPOSTOR_TEX_SIZE, (int)rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE);
      rtSizeY = rtSize;
      if (bboxAspectRatio > 1.75)
        rtSizeY = rtSize * IMPOSTOR_MAX_ASPECT_RATIO; // trees are more likely to be taller, then wider
      ...
      for (int i = 1; i < pool.impostor.tex.size(); ++i)
      {
        ...
        const uint32_t fmt = (rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostor_compressed_texformats[i]
                                                                           : rendinstgenrender::impostor_texformats[i]);
        pool.impostor.tex[i] = dag::create_tex(NULL, rtSize, rtSizeY, texflags | fmt, pool.impostor.renderMips, name);
...
static bool fill_palette_vb(int rotation_count, const rendinstgen::RotationPaletteManager::PaletteEntry *rotations)
{
  if (rendinstgenrender::rotationPaletteTmVb)
  {
    if (auto lockedBuf = lock_sbuffer<float>(rendinstgenrender::rotationPaletteTmVb, 0, 0, VBLOCK_WRITEONLY | VBLOCK_DISCARD))
    {
      for (int i = 0; i < rotation_count + 1; ++i)
      {
        TMatrix rot;
        if (i > 0)
          rot = rendinstgen::RotationPaletteManager::get_tm(rotations[i - 1]);
        else
          rot.identity();
        for (int j = 0; j < 12; ++j)
...
bool RendInstGenData::RtData::updateImpostorsPreshadow(int poolNo, const Point3 &sunDir0, int paletteId, const UniqueTex &depth_atlas)
{
  ...
  if (paletteId != -1)
  {
    const rendinstgen::RotationPaletteManager::Palette palette =
      rendinstgen::get_rotation_palette_manager()->getPalette({layerIdx, poolNo});
    // paletteOffset doesn't include the offset obtained from the palette rotation manager,
    // because it's used to index into rendinstgenrender::rotationPaletteTmVb,
    // which only stores the current palette + the identity rotation at the beginning
    paletteOffset = (1 + paletteId) * rendinstgenrender::ROTATION_PALETTE_TM_VB_STRIDE;
    instanceRotation = rendinstgen::RotationPaletteManager::get_tm(palette, paletteId);
  }
  ...
  if (!pool.shadowImpostorTexture)
  {
    ...
    pool.shadowImpostorTexture =
      d3d::create_tex(NULL, IMPOSTOR_SHADOW_SIZE, IMPOSTOR_SHADOW_SIZE, TEXCF_RTARGET | TEXFMT_DEPTH16, 1, name.str());
    //= d3d::create_tex(NULL, IMPOSTOR_SHADOW_SIZE, IMPOSTOR_SHADOW_SIZE, TEXCF_RTARGET, 1, "shadowImpostorTexture");
  ...
  // d3d::set_render_target(0, (Texture*)rendinstgenrender::shadowImpostorTexture, 0, true);
  // d3d::clearview(CLEAR_TARGET | CLEAR_ZBUFFER, 0, 1.f, 0);
  d3d::set_render_target(0, (Texture *)NULL, 0);
  d3d::set_depth(pool.shadowImpostorTexture, false);
  ...
  shaders::overrides::set(rendinstgenrender::impostorShadowOverride);
...
bool RendInstGenData::RtData::updateImpostorsPreshadow(int poolNo, const Point3 &sunDir0)
{
  ...
  if (riRes[poolNo]->isBakedImpostor() &&
    ...
    return false;
  if (!riPaletteRotation[poolNo] || !rendinstgen::get_rotation_palette_manager()->hasPalette({layerIdx, poolNo}))
  ...
  d3d::set_buffer(STAGE_VS, rendinstgenrender::INSTANCING_TEXREG, nullptr);
  bool updated = fill_palette_vb(palette.count, palette.rotations);
  d3d::set_buffer(STAGE_VS, rendinstgenrender::INSTANCING_TEXREG, rendinstgenrender::rotationPaletteTmVb);
  if (!updated)
    return false;
...
static void impostorMipSRVBarrier(rendinstgenrender::RtPoolData &pool, int mip)
{
  for (int j = 0; j < pool.impostor.tex.size(); ++j)
  {
    UniqueTex &tex =
      rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostorCompressionBuffers[j] : pool.impostor.tex[j];
    BaseTexture *pTex = tex.getBaseTex();
    d3d::resource_barrier({pTex, RB_RO_SRV | RB_STAGE_PIXEL, (unsigned)(mip), 1});
  }
}

void renderImpostorMips(rendinstgenrender::RtPoolData &pool, int currentRenderMips)
{
  PostFxRenderer *pFx = rendinstgenrender::postfxBuildMip;
  if (!pFx)
  {
    if (rendinstgenrender::use_impostors_compression)
    {
      for (int j = 0; j < rendinstgenrender::impostor_tex_count; ++j)
        if (rendinstgenrender::impostorCompressionBuffers[j].getTex2D())
          rendinstgenrender::impostorCompressionBuffers[j].getTex2D()->generateMips();
    }
    else
    {
      for (int j = 0; j < pool.impostor.tex.size(); ++j)
  ...
  ShaderGlobal::set_int(rendinstPaddingVarId, rendinstgenrender::use_color_padding);
  ...
  for (int mip = 1; mip < currentRenderMips; ++mip)
  {
    ...
    for (int j = 0; j < pool.impostor.tex.size(); ++j)
    {
      ...
      if (pTex)
      {
        ...
        ShaderGlobal::set_texture(rendinstgenrender::sourceTexVarId[j], tex.getTexId());
        pTex->texmiplevel(src_mip, src_mip);
      }
    }

    UniqueTex &tex =
      rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostorCompressionBuffers[0] : pool.impostor.tex[0];
    TextureInfo ti;
    tex.getBaseTex()->getinfo(ti, src_mip);
    ShaderGlobal::set_color4(rendinstgenrender::texelSizeVarId, 1.f, 1.f, 1.f / ti.w, 1.f / ti.h);
    ShaderGlobal::set_int(rendinstgenrender::texIndVid, -1);
  ...
  for (int j = 0; j < pool.impostor.tex.size(); ++j)
  {
    ...
    if (rendinstgenrender::use_impostors_compression)
      rendinstgenrender::impostorCompressionBuffers[j].getTex2D()->texmiplevel(pool.impostor.baseMip, lastMip);
  }
}

void compressImpostor(rendinstgenrender::RtPoolData &pool)
{
  for (int i = 0, e = pool.impostor.tex.size(); i < e; ++i)
  {
    int texWidth = (i == 0) ? rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE * rendinstgenrender::maxAlbedoRTSizeMult
                            : rendinstgenrender::MAX_DYNAMIC_IMPOSTOR_TEX_SIZE;
    int texHeight = texWidth * IMPOSTOR_MAX_ASPECT_RATIO;
    int bufMips = (get_log2i(max(texWidth / 4, texHeight / 4)) + 1);
    BcCompressor *compr = rendinstgenrender::bcCompressors[i];
    ...
    for (int j = pool.impostor.baseMip; j < pool.impostor.renderMips; ++j)
    {
      ...
      dstTex->getinfo(ti, j);
      int scrMipToAdd = (i != 0 && rendinstgenrender::maxAlbedoRTSizeMult > 1) ? 1 : 0;
      compr->updateFromMip(rendinstgenrender::impostorCompressionBuffers[i].getTexId(), j + scrMipToAdd, j);
...
{
  rendinstgenrender::dir_from_sun = sunDir0;
  if (rendinstgenrender::dynamic_impostor_texture_const_no < 0 || !rtPoolData.size())
    return;
  ...
  mat44f savedViewMatrix, savedProjMatrix;
  ...
  if (rendinstgenrender::vertical_billboards)
  ...
  mat44f renderProjMatrix;
  d3d::gettm(TM_PROJ, renderProjMatrix);
  int numUpdated = 0;
       ...
       updatePoolNo++, numCheckedPools++)
  {
    ...
    if (currentRenderMips > 1)
    {
      vec4f screenImpostorHalfDiag =
        v_mat44_mul_vec4(renderProjMatrix, v_make_vec4f(pool.impostor.currentHalfWidth, pool.impostor.currentHalfHeight, range, 1.f));
      ...
      int screenW, screenH;
      d3d::get_render_target_size(screenW, screenH, NULL);
    ...
    d3d::set_render_target(rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostorCompressionBuffers[0].getTex2D()
    ...
    RenderableInstanceResource *sourceScene = riResLodScene(poolNo, sourceLodNo);
    {
      UniqueTex *dstRT[rendinstgenrender::IMP_COUNT];

      for (int j = 0; j < pool.impostor.tex.size(); ++j)
      {
        if (rendinstgenrender::use_color_padding)
          dstRT[j] = &rendinstgenrender::impostorColorTexture[j];
        else if (rendinstgenrender::use_impostors_compression)
          dstRT[j] = &rendinstgenrender::impostorCompressionBuffers[j];
        else
          dstRT[j] = &pool.impostor.tex[j];
      }
      int baseMip = rendinstgenrender::use_color_padding ? 0 : pool.impostor.baseMip;
      ...
      d3d::driver_command(DRV3D_COMMAND_BEGIN_MRT_CLEAR_SEQUENCE, ((uint8_t *)1), NULL, NULL);
      for (int j = 1; j < pool.impostor.tex.size(); ++j)
      {
        ...
        if (pTex)
        {
          ...
          d3d::clearview(CLEAR_TARGET, ::grs_draw_wire ? (E3DCOLOR)0xFF000000 : rendinstgenrender::impostor_clear_color[j], 1.f, 0);
        }
      }
      // reset index of mrt clear sequence to 0, to complete the mrt block
      d3d::driver_command(DRV3D_COMMAND_BEGIN_MRT_CLEAR_SEQUENCE, ((uint8_t *)0), NULL, NULL);

      // required to be set before block, soopengl driver use correct axis flipping
      d3d::set_render_target(dstRT[0]->getBaseTex(), baseMip);

      auto depthTexIt = rendinstgenrender::impostorDepthTextures.find(IPoint2(tinfo0.w >> baseMip, tinfo0.h >> baseMip));
      if (depthTexIt == rendinstgenrender::impostorDepthTextures.end())
      {
        depthTexIt =
          rendinstgenrender::impostorDepthTextures.emplace(IPoint2(tinfo0.w >> baseMip, tinfo0.h >> baseMip), UniqueTex()).first;
        String name(0, "impostorDepth%dx%d", tinfo0.w >> baseMip, tinfo0.h >> baseMip);
        depthTexIt->second =
          dag::create_tex(NULL, tinfo0.w >> baseMip, tinfo0.h >> baseMip, TEXCF_RTARGET | TEXFMT_DEPTH16, 1, name.str());
      }
      d3d::set_depth(depthTexIt->second.getTex2D(), d3d::get_driver_code() == _MAKE4C('MTL'));

      unsigned impostor_clear_color = (::grs_draw_wire ? 0xFF000000 : 0) | rendinstgenrender::impostor_clear_color[0];
      if (d3d::get_driver_code() != _MAKE4C('MTL')) // clear color/depth for non-METAL here or it is not cleared at all!
      ...
      d3d::driver_command(DRV3D_COMMAND_END_MRT_CLEAR_SEQUENCE, (uint8_t *)1, NULL, NULL);
#if _TARGET_C1 | _TARGET_C2
      ...
      if (rendinstgenrender::use_color_padding || rendinstgenrender::use_impostors_compression)
      {
        TextureInfo tinfo;
        ...
        UniqueTex &tex = rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostorCompressionBuffers[0]
                                                                      : rendinstgenrender::impostorColorTexture[0];
      ...
      if (rendinstgenrender::use_color_padding)
      {
        ...
        for (int j = 0; j < pool.impostor.tex.size(); ++j)
        {
          ...
          if (pTex)
          {
            ...
            ShaderGlobal::set_texture(rendinstgenrender::sourceTexVarId[j], rendinstgenrender::impostorColorTexture[j].getTexId());
          }
        }
        TextureInfo ti;
        UniqueTex &tex = rendinstgenrender::use_impostors_compression ? rendinstgenrender::impostorCompressionBuffers[0]
        ...
        ShaderGlobal::set_color4(rendinstgenrender::texelSizeVarId, viewportPartX, viewportPartY, 1.f / ti.w, 1.f / ti.h);
  ...
  d3d::settm(TM_VIEW, savedViewMatrix);
  ...
  d3d::setview(vx, vy, vw, vh, vzn, vzf);
  ShaderGlobal::set_int_fast(rendinstgenrender::rendinstRenderPassVarId,
...
{
  rendinstgenrender::impostorPreshadowNeedUpdate = true;
...
void rendinst::updateRIGenImpostors(float shadowDistance, const Point3 &sunDir0, const TMatrix &view_itm)
{
  ...
  if (rendinstgenrender::use_impostors_compression != rendinstgenrender::was_impostors_compression ||
      rendinstgenrender::is_render_target_size_changed || rendinstgenrender::impostorPreshadowNeedUpdate)
  {
    if (rendinstgenrender::is_render_target_size_changed)
    {
      for (int i = 0; i < rendinstgenrender::impostor_tex_count; ++i)
      {
        rendinstgenrender::impostorColorTexture[i].close();
      ...
      rendinstgenrender::impostorDepthTextures.clear();
      rendinstgenrender::is_render_target_size_changed = false;
    }
  ...
  rendinstgenrender::was_impostors_compression = rendinstgenrender::use_impostors_compression;

  FOR_EACH_RG_LAYER_DO (rgl)
  {
    ScopedLockRead lock(rgl->rtData->riRwCs);
    float shadowDistOfs = (rendinstgenrender::per_instance_visibility ? 0 : (0.5f * rgl->grid2world * rgl->cellSz * subCellDistMul));
    ...
    if (rgl->rtData->dynamicImpostorToUpdateNo < rgl->rtData->rtPoolData.size())
      rgl->rtData->updateImpostors(shadowDistance - shadowDistOfs, sunDir0, view_itm);
  }

  rendinstgenrender::impostorPreshadowNeedUpdate = false;
}

static void render_impostors_ofs(int count, int start_ofs, int vectors_cnt)
{
#if D3D_HAS_QUADS
  (void)start_ofs;
  (void)vectors_cnt;
...
#else
  if (rendinstgenrender::impostor_vb_replication) //-V547
  {
    for (int startInst = 0; startInst < count; startInst += rendinstgenrender::impostor_vb_replication_count)
    {
      int renderCount = min(count - startInst, rendinstgenrender::impostor_vb_replication_count);
