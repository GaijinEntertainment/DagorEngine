...
static Vbuffer *rendinstShadowsToClipmapVb = NULL;
static Ibuffer *rendinstShadowsToClipmapIb = NULL;
...
{
  rendinstShadowsToClipmapShaderElem = NULL;
  rendinstShadowsToClipmapShaderMaterial = NULL;
...
void initClipmapShadows()
{
  ...
  unsigned ibflags = 0;
  rendinstShadowsToClipmapIb = d3d::create_ib(6 * sizeof(uint16_t) * count, ibflags);

  uint16_t *indices;
  rendinstShadowsToClipmapIb->lock(0, 6 * sizeof(uint16_t) * count, &indices, VBLOCK_WRITEONLY);

  for (int i = 0; i < count; ++i, indices += 6)
  {
    indices[0] = 0 + i * 4;
    indices[1] = 1 + i * 4;
    indices[2] = 2 + i * 4;
...
void allocate_clipmap_shadows()
{
  ...
  blurTemp.close();
  blurTemp = dag::create_tex(NULL, rendinstClipmapShadowTexSize, rendinstClipmapShadowTexSize, shadow_rt_texFmt | TEXCF_RTARGET, 1,
    "clipmapshadow_blurTempTex");
  d3d_err(blurTemp.getTex2D());
  blurTemp.getTex2D()->texaddr(TEXADDR_CLAMP);
  rtTemp.close();
  rtTemp = dag::create_tex(NULL, rendinstClipmapShadowTexSize, rendinstClipmapShadowTexSize, shadow_rt_texFmt | TEXCF_RTARGET, 1,
...
bool render_clipmap_shadow_pool(rendinstgenrender::RtPoolData &pool, RenderableInstanceResource *sourceScene, bool for_sli,
  bool keep_texture, uint32_t impostor_buffer_offset)
{
  if (!for_sli)
  {
    ...
    if (!pool.rendinstClipmapShadowTex)
    {
      ...
      pool.rendinstClipmapShadowTex =
        d3d::create_tex(NULL, rendinstClipmapShadowTexSize, rendinstClipmapShadowTexSize, shadow_texFmt, numMips, name);
  ...
  rendinstgenrender::setCoordType(pool.hasImpostor() ? rendinstgenrender::COORD_TYPE_POS : rendinstgenrender::COORD_TYPE_TM);
...
void end_clipmap_shadows()
{
  ...
  rendinstgenrender::offsetSunDistMax = Point2(max(offsetSunDist.x, 0.f), max(offsetSunDist.z, 0.f));
  rendinstgenrender::offsetSunDistMin = Point2(min(offsetSunDist.x, 0.f), min(offsetSunDist.z, 0.f));
}
...
bool RendInstGenData::RtData::renderRendinstClipmapShadowsToTextures(const Point3 &sunDir0, bool for_sli, bool force_update)
{
  ...
  rendinstgenrender::set_sun_dir(sunDir0);
  rendinstgenrender::start_clipmap_shadows();
  ...
  for (; nextPoolForClipmapShadows < rtPoolData.size(); nextPoolForClipmapShadows++)
  {
    ...
    uint32_t impostorBufferOffset = rtPoolData[poolNo]->hasImpostor()
                                      ? rendinstgen::get_rotation_palette_manager()->getImpostorDataBufferOffset({layerIdx, poolNo},
                                          rtPoolData[poolNo]->impostorDataOffsetCache)
                                      : 0;

    rendinstgenrender::render_clipmap_shadow_pool(*rtPoolData[poolNo], riResLodScene(poolNo, 0), for_sli, !force_update,
...
{
  bbox3f regionBBox;
  regionBBox.bmin = v_make_vec4f(region[0].x + rendinstgenrender::offsetSunDistMin.x, MIN_REAL,
    region[0].y + rendinstgenrender::offsetSunDistMin.y, 0);
  regionBBox.bmax = v_make_vec4f(region[1].x + rendinstgenrender::offsetSunDistMax.x, MAX_REAL,
    region[1].y + rendinstgenrender::offsetSunDistMax.y, 0);
  ...
  for (int z = regions[1], cellI = regions[1] * cellNumW + regions[0]; z <= regions[3]; z++, cellI += cellXStride)
    for (int x = regions[0]; x <= regions[2]; x++, cellI++)
    {
      ...
      for (unsigned int ri_idx = 0; ri_idx < rtData->riRes.size(); ri_idx++)
      {
        ...
        ShaderGlobal::set_texture_fast(rendinstgenrender::rendinstShadowTexVarId, pool.rendinstClipmapShadowTexId);
        // ShaderGlobal::set_color4_fast(rendinstgenrender::boundingSphereVarId, 0.f, 0.f, 0.f, pool.sphereRadius *
        // rendinstShadowScale);
        ShaderGlobal::set_color4_fast(rendinstgenrender::clipmapShadowScaleVarId, pool.clipShadowWk, pool.clipShadowHk,
        ...
        for (int rangeI = 0; rangeI < resultRangesCount; ++rangeI)
        {
          ...
          uint32_t impostorOffset = pool.hasImpostor() ? rendinstgen::get_rotation_palette_manager()->getImpostorDataBufferOffset(
...
bool rendinst::renderRIGenClipmapShadowsToTextures(const Point3 &sunDir0, bool for_sli, bool force_update)
{
  if (!rendinstClipmapShadows || !rendinstShadowsToClipmapShaderMaterial)
    return true;
...
bool rendinst::notRenderedClipmapShadowsBBox(BBox2 &box, int cascadeNo)
{
  if (!rendinstClipmapShadows || !rendinstShadowsToClipmapShaderMaterial)
    return false;
...
void rendinst::setClipmapShadowsRendered(int cascadeNo)
{
  if (!rendinstClipmapShadows || !rendinstShadowsToClipmapShaderMaterial)
    return;
  FOR_EACH_RG_LAYER_RENDER (rgl, rgRenderMaskCMS)
...
void rendinst::renderRIGenShadowsToClipmap(const BBox2 &region, int newForCascadeNo)
{
  if (!rendinstClipmapShadows || !rendinstShadowsToClipmapShaderMaterial)
    return;
  FOR_EACH_RG_LAYER_RENDER (rgl, rgRenderMaskCMS)
