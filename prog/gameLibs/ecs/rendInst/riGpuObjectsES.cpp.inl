...
gpu_objects::PlacingParameters gpu_objects::prepare_gpu_object_parameters(int ri_gpu_object__seed,
  ...
  const bool ri_gpu_object__face_coast, const Point2 &ri_gpu_object__random_y_ofs, const ecs::List<int> *dagdp__biomes)
{
  ...
  parameters.randomYOfs = ri_gpu_object__random_y_ofs;

  if (dagdp__biomes)
  {
    parameters.grass = !dagdp__biomes->empty();
    for (const auto id : *dagdp__biomes)
      parameters.biomes.push_back(Point4(id, 1, 0, 0));
  }

  if (parameters.biomes.size() == 0 && ri_gpu_object__biome_params)
    for (const auto &params : *ri_gpu_object__biome_params)
      parameters.biomes.push_back(params.get<Point4>());

  if (parameters.biomes.size() == 0 && ri_gpu_object__biom_indexes)
  {
...
static __forceinline void ri_gpu_object_create_es_event_handler(const ecs::Event &, ecs::EntityManager &manager, ecs::EntityId eid,
  ...
  const bool ri_gpu_object__face_coast, const Point2 &ri_gpu_object__random_y_ofs, const ecs::List<int> *dagdp__biomes)
{
  ...
  {
    if (!manager.has(eid, ECS_HASH("disabled_update")))
  ...
  gpu_objects::PlacingParameters parameters = gpu_objects::prepare_gpu_object_parameters(ri_gpu_object__seed, ri_gpu_object__up_vector,
    ...
    ri_gpu_object__render_into_shadows, ri_gpu_object__coast_range, ri_gpu_object__face_coast, ri_gpu_object__random_y_ofs,
    dagdp__biomes);
