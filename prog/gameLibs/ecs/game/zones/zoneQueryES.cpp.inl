...
inline void active_sphere_capzones_ecs_query(ecs::EntityManager &manager, Callable c);
template <typename Callable>
inline void active_box_capzones_ecs_query(ecs::EntityManager &manager, Callable c);
template <typename Callable>
inline void active_relative_poly_capzones_ecs_query(ecs::EntityManager &manager, Callable c);
...
void game::get_active_capzones_on_pos(const Point3 &pos, const char *tag_to_have_not_null, Tab<ecs::EntityId> &zones_in)
{
  ...
  active_sphere_capzones_ecs_query(*g_entity_mgr,
    [&](ECS_REQUIRE(eastl::true_type active = true) ECS_REQUIRE(ecs::Tag capzone) ecs::EntityId eid, const TMatrix &transform,
      float sphere_zone__radius, bool sphere_zone__inverted = false) {
  ...
  active_box_capzones_ecs_query(*g_entity_mgr,
  ...
  active_relative_poly_capzones_ecs_query(*g_entity_mgr,
...
template <typename Callable>
inline bool sphere_zone_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);
template <typename Callable>
inline bool poly_battle_area_zone_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);
template <typename Callable>
inline bool poly_capzone_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);
template <typename Callable>
inline bool box_zone_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);
template <typename Callable>
inline bool relative_box_zone_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);


bool game::is_point_in_capzone(const Point3 &pos, ecs::EntityId zone_eid, float zone_scale)
{
  bool res = false;
  if (box_zone_ecs_query(*g_entity_mgr, zone_eid,
        [&](ECS_REQUIRE(ecs::Tag box_zone) const TMatrix &transform, bool box_zone__inverted = false) {
  ...
  if (sphere_zone_ecs_query(*g_entity_mgr, zone_eid,
  ...
  if (poly_capzone_ecs_query(*g_entity_mgr, zone_eid,
        [&](const ecs::Point2List &capzone__areaPoints, const float capzone__minHeight, const float capzone__maxHeight,
          bool poly_zone__inverted = false) {
          const bool isInPolyZone = (pos.y > capzone__minHeight && pos.y < capzone__maxHeight) &&
                                    is_point_in_poly(Point2::xz(pos), capzone__areaPoints.data(), capzone__areaPoints.size());
...
{
  bool res = false;
  poly_battle_area_zone_ecs_query(*g_entity_mgr, zone_eid,
    [&](const ecs::Point2List &battleAreaPoints, bool poly_zone__inverted = false) {
...
template <typename Callable>
inline void get_transform_ecs_query(ecs::EntityManager &manager, ecs::EntityId eid, Callable c);

bool game::is_entity_in_capzone(const ecs::EntityId eid, const ecs::EntityId zone_eid)
{
  bool res = false;
  get_transform_ecs_query(*g_entity_mgr, eid,
    [&](const TMatrix &transform) { res = game::is_point_in_capzone(transform.getcol(3), zone_eid); });
  return res;
}
