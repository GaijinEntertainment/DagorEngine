...
static constexpr ecs::ComponentDesc animchar_effectors_apply_immediate_es_comps[] =
{
//start of 1 rw components at [0]
  {ECS_HASH("animchar"), ecs::ComponentTypeInfo<AnimV20::AnimcharBaseComponent>()},
//start of 4 ro components at [1]
...
static void animchar_effectors_apply_immediate_es_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  G_FAST_ASSERT(evt.is<CmdApplyAnimcharEffectorsImmediate>());
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
  {
    if ( !(ECS_RO_COMP(animchar_effectors_apply_immediate_es_comps, "animchar__updatable", bool) && ECS_RO_COMP_OR(animchar_effectors_apply_immediate_es_comps, "animchar__visible", bool( true))) )
      continue;
    animchar_effectors_apply_immediate_es(static_cast<const CmdApplyAnimcharEffectorsImmediate&>(evt)
          , ECS_RW_COMP(animchar_effectors_apply_immediate_es_comps, "animchar", AnimV20::AnimcharBaseComponent)
      , ECS_RO_COMP(animchar_effectors_apply_immediate_es_comps, "transform", TMatrix)
      , ECS_RO_COMP(animchar_effectors_apply_immediate_es_comps, "animchar_effectors__effectorsState", ecs::Object)
      );
  } while (++comp != compE);
}
static ecs::EntitySystemDesc animchar_effectors_apply_immediate_es_es_desc
(
  "animchar_effectors_apply_immediate_es",
  "prog/gameLibs/ecs/anim/animCharEffectorsES.cpp.inl",
  ecs::EntitySystemOps(nullptr, animchar_effectors_apply_immediate_es_all_events),
  make_span(animchar_effectors_apply_immediate_es_comps+0, 1)/*rw*/,
  make_span(animchar_effectors_apply_immediate_es_comps+1, 4)/*ro*/,
  empty_span(),
  make_span(animchar_effectors_apply_immediate_es_comps+5, 1)/*no*/,
  ecs::EventSetBuilder<CmdApplyAnimcharEffectorsImmediate>::build(),
  0
,"render");
