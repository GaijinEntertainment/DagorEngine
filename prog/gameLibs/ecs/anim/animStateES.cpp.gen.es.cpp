...
static constexpr ecs::ComponentDesc human_anim_state_change_full_es_event_handler_comps[] =
{
//start of 2 rw components at [0]
  {ECS_HASH("animchar__animState"), ecs::ComponentTypeInfo<ecs::Object>()},
  {ECS_HASH("animchar__animStateDirty"), ecs::ComponentTypeInfo<bool>()},
...
static void human_anim_state_change_full_es_event_handler_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  G_FAST_ASSERT(evt.is<EventChangeAnimStateFull>());
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
    human_anim_state_change_full_es_event_handler(static_cast<const EventChangeAnimStateFull&>(evt)
        , ECS_RW_COMP(human_anim_state_change_full_es_event_handler_comps, "animchar__animState", ecs::Object)
    , ECS_RO_COMP(human_anim_state_change_full_es_event_handler_comps, "human_anim_restriction_ids__prohibited", ecs::Array)
    , ECS_RW_COMP(human_anim_state_change_full_es_event_handler_comps, "animchar__animStateDirty", bool)
    );
  while (++comp != compE);
}
static ecs::EntitySystemDesc human_anim_state_change_full_es_event_handler_es_desc
(
  "human_anim_state_change_full_es",
  "prog/gameLibs/ecs/anim/animStateES.cpp.inl",
  ecs::EntitySystemOps(nullptr, human_anim_state_change_full_es_event_handler_all_events),
  make_span(human_anim_state_change_full_es_event_handler_comps+0, 2)/*rw*/,
  make_span(human_anim_state_change_full_es_event_handler_comps+2, 1)/*ro*/,
  empty_span(),
  make_span(human_anim_state_change_full_es_event_handler_comps+3, 1)/*no*/,
  ecs::EventSetBuilder<EventChangeAnimStateFull>::build(),
  0
);
static constexpr ecs::ComponentDesc anim_state_change_full_es_event_handler_comps[] =
{
//start of 2 rw components at [0]
  {ECS_HASH("animchar__animState"), ecs::ComponentTypeInfo<ecs::Object>()},
  {ECS_HASH("animchar__animStateDirty"), ecs::ComponentTypeInfo<bool>()},
...
static void anim_state_change_full_es_event_handler_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  G_FAST_ASSERT(evt.is<EventChangeAnimStateFull>());
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
    anim_state_change_full_es_event_handler(static_cast<const EventChangeAnimStateFull&>(evt)
        , ECS_RW_COMP(anim_state_change_full_es_event_handler_comps, "animchar__animState", ecs::Object)
    , ECS_RW_COMP(anim_state_change_full_es_event_handler_comps, "animchar__animStateDirty", bool)
    );
  while (++comp != compE);
}
static ecs::EntitySystemDesc anim_state_change_full_es_event_handler_es_desc
(
  "anim_state_change_full_es",
  "prog/gameLibs/ecs/anim/animStateES.cpp.inl",
  ecs::EntitySystemOps(nullptr, anim_state_change_full_es_event_handler_all_events),
  make_span(anim_state_change_full_es_event_handler_comps+0, 2)/*rw*/,
  empty_span(),
  empty_span(),
  make_span(anim_state_change_full_es_event_handler_comps+2, 1)/*no*/,
  ecs::EventSetBuilder<EventChangeAnimStateFull>::build(),
  0
);
