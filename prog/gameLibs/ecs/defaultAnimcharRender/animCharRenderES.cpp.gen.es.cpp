...
static void animchar_render_opaque_es_all(const ecs::UpdateStageInfo &__restrict info, const ecs::QueryView & __restrict components)
{
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE);
  do
    animchar_render_opaque_es(*info.cast<ecs::UpdateStageInfoRender>()
...
static void animchar_render_trans_es_all(const ecs::UpdateStageInfo &__restrict info, const ecs::QueryView & __restrict components)
{
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE);
  do
    animchar_render_trans_es(*info.cast<ecs::UpdateStageInfoRenderTrans>()
...
static void animchar_render_es_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  ...
  auto comp = components.begin(), compE = components.end(); G_ASSERT(comp!=compE); do
    animchar_render_es(static_cast<const UpdateStageInfoBeforeRender&>(evt)
        , ECS_RW_COMP(animchar_render_es_comps, "animchar_render", AnimV20::AnimcharRendComponent)
    , ECS_RO_COMP(animchar_render_es_comps, "animchar_node_wtm", AnimcharNodesMat44)
    , ECS_RW_COMP(animchar_render_es_comps, "animchar_bsph", vec4f)
    , ECS_RW_COMP(animchar_render_es_comps, "animchar_bbox", bbox3f)
    , ECS_RW_COMP(animchar_render_es_comps, "animchar_visbits", uint8_t)
    , ECS_RO_COMP_OR(animchar_render_es_comps, "animchar_render__enabled", bool(true))
    );
  while (++comp != compE);
}
...
(
  ...
  make_span(animchar_render_es_comps+0, 4)/*rw*/,
  make_span(animchar_render_es_comps+4, 2)/*ro*/,
  empty_span(),
  empty_span(),
