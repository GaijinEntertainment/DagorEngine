...
public:
  ...
  void setWheelsModel(const char *model_res_name) override
  {
    DynamicRenderableSceneLodsResource *model =
      (DynamicRenderableSceneLodsResource *)::get_game_resource_ex(model_res_name, DynModelGameResClassId);
    ...
    release_game_resource_ex(model, DynModelGameResClassId);
  }
  ...
  void render(int parts_flags) override
  {
    ...
    if (parts_flags & RP_BODY)
    {
      ...
      ShaderGlobal::set_float4(planar_norm_var_id, planarNorm);
