...
struct TraversalStackState
{
    ...
    void WorkAfterTraversalIntrinsicIssued()
    {
        ...
        const uint node1 = m_stackCache.x;

        if (node1 != XdxrInvalidNodePtr)
        {
            const uint node2 = m_stackCache.y;

            if (node2 != XdxrInvalidNodePtr)
            {
                const uint node3 = m_stackCache.z;
        ...
        uint4 rayTest)
    {
        ...
        m_stackCache.x = rayTest.y;
        m_stackCache.y = rayTest.z;
        m_stackCache.z = rayTest.w;

        uint retNode = rayTest.x;
        ...
        m_lastValidNode = retNode == XdxrInvalidNodePtr ? parentNode : retNode;
        return retNode;
    }
