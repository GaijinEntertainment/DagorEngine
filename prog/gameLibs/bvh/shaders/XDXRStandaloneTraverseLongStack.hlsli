...
static const uint32_t DIV_STACK_DEPTH = (32);
static const uint32_t DIV_STACK_SPACE = (DIV_STACK_DEPTH * XDXR_TRAVERSAL_WAVE_SIZE);
static const uint32_t DIV_MAX_STACK_PTR = (DIV_STACK_SPACE * 4);
static const uint32_t DIV_STACK_INCREMENT = (XDXR_TRAVERSAL_WAVE_SIZE * 4);
...
struct TraversalStackState
{
    ...
    void StackPop(
        ...
        const uint )
    {
        int stackPtr = g_stackPtr & 0xFFFF;

        stackPtr -= DIV_STACK_INCREMENT;
        ...
        {
            node = g_divergentStack[stackPtr / 4];

            uint tlasLevel = g_stackPtr >> 16;

            if (stackPtr < tlasLevel)
            {
                loopCond = TRAVERSE_STEP_RESULT_ITERATE_BACK_TO_TLAS;
                tlasLevel = 0;
            }

            g_stackPtr = (tlasLevel << 16) | stackPtr;
        }
    }
    ...
    void StackPush(
        uint node)
    {
        uint stackPtr = g_stackPtr & 0xFFFF;
        if (stackPtr < DIV_MAX_STACK_PTR)
        {
            g_divergentStack[stackPtr / 4] = node;
            g_stackPtr += DIV_STACK_INCREMENT;
        }
    }
        ...
        uint4 tests)
    {
        ...
        if (XdxrIsValidNodePointer(tests.y))
        {
            if (XdxrIsValidNodePointer(tests.z))
            {
                if (XdxrIsValidNodePointer(tests.w))
                {
                    StackPush(tests.w);
                }
                StackPush(tests.z);
