...
groupshared uint g_tlasStack[(256)];

struct HybridUniformStackState
{

    int32_t m_stackIdx;
    int32_t m_numCachedNodes;
    ...
    void StackInit()
    {
        ...
        (m_nodeCache = __XB_WriteLane(m_nodeCache, 12, 0));
        (m_nodeCache = __XB_WriteLane(m_nodeCache, 13, 0));
        (m_nodeCache = __XB_WriteLane(m_nodeCache, 14, 0));
    ...
    void StackPop(out uint nodePtr, out uint laneMask)
    {
        laneMask = g_tlasStack[--m_stackIdx];
        nodePtr = g_tlasStack[--m_stackIdx];
    }

    void WriteCache(uint cacheIdx, uint nodePtr, uint laneMask)
    {
        (m_nodeCache = __XB_WriteLane(m_nodeCache, cacheIdx, nodePtr));
        (m_nodeCache = __XB_WriteLane(m_nodeCache, cacheIdx+1, laneMask));
    }



    void PushCachedEntry(uint laneIdx)
    {
        if (m_stackIdx + 2 <= (256))
        {
            g_tlasStack[m_stackIdx++] = (WaveReadLaneAt(m_nodeCache, laneIdx));
            g_tlasStack[m_stackIdx++] = (WaveReadLaneAt(m_nodeCache, laneIdx+1));
        }
    }

    void WorkAfterTraversalIntrinsicIssued(uint laneID)
    {
        const uint prioA = (WaveReadLaneAt(m_nodeCache, 12));
        const uint prioB = (WaveReadLaneAt(m_nodeCache, 13));
        const uint prioC = (WaveReadLaneAt(m_nodeCache, 14));


        const uint max3 = __XB_Max3_U32(prioA, prioB, prioC);
        if (max3 > 7)
        {
            const uint med3 = __XB_Med3_U32(prioA, prioB, prioC);
            if (med3 > 7)
            {
                const uint min3 = __XB_Min3_U32(prioA, prioB, prioC);
                if (min3 > 7)
                {
                    PushCachedEntry(min3 & 7);
                }
                PushCachedEntry(med3 & 7);
            }
            PushCachedEntry(max3 & 7);
        }
    ...
    uint ComputePriority(uint childPtr, uint4 sortedHits, out uint laneMask)
    {
        uint4 ballots;
        ballots.x = (WaveActiveBallot(childPtr == sortedHits.x).x);
        ballots.y = (WaveActiveBallot(childPtr == sortedHits.y).x);
        ballots.z = (WaveActiveBallot(childPtr == sortedHits.z).x);
        ballots.w = (WaveActiveBallot(childPtr == sortedHits.w).x);

        laneMask = (ballots.x | ballots.y | ballots.z | ballots.w);


        uint priority = countbits(ballots.x);
        priority = __XB_MadU24(priority, 32, countbits(ballots.y));
        priority = __XB_MadU24(priority, 32, countbits(ballots.z));
        priority = __XB_MadU24(priority, 32, countbits(ballots.w));
        return priority << 3;
    }
        ...
        uint4 children)
    {
        ...
        children = max((int4)0, (int4)children);

        uint4 laneMasks, priorities;
        priorities[0] = ComputePriority(children[0], sortedHits, laneMasks[0]) | 6;
        priorities[1] = ComputePriority(children[1], sortedHits, laneMasks[1]) | 4;
        priorities[2] = ComputePriority(children[2], sortedHits, laneMasks[2]) | 2;
        priorities[3] = ComputePriority(children[3], sortedHits, laneMasks[3]) | 0;

        WriteCache(6, children[0], laneMasks[0]);
        WriteCache(4, children[1], laneMasks[1]);
        WriteCache(2, children[2], laneMasks[2]);
        WriteCache(0, children[3], laneMasks[3]);

        uint min3 = __XB_Min3_U32(priorities[1], priorities[2], priorities[3]);
        uint med3 = __XB_Med3_U32(priorities[1], priorities[2], priorities[3]);
        uint max3 = __XB_Max3_U32(priorities[1], priorities[2], priorities[3]);
        uint best = max(priorities[0], max3);
        max3 = min(priorities[0], max3);

        (m_nodeCache = __XB_WriteLane(m_nodeCache, 12, max3));
        (m_nodeCache = __XB_WriteLane(m_nodeCache, 13, med3));
        (m_nodeCache = __XB_WriteLane(m_nodeCache, 14, min3));

        const uint childIdx = best & 7;
        node = (WaveReadLaneAt(m_nodeCache, childIdx));
        return (WaveReadLaneAt(m_nodeCache, childIdx+1));
    }
};

groupshared uint g_blasStack[((1 << (3)) * XDXR_TRAVERSAL_WAVE_SIZE)];





struct HybridDivergentStackState
{
    uint3 m_stackCache;
    ...
    {
        m_stackPtr += (XDXR_TRAVERSAL_WAVE_SIZE << 2);
        g_blasStack[(m_stackPtr & ((((1 << (3)) * XDXR_TRAVERSAL_WAVE_SIZE) - 1) << 2)) / 4] = nodePtr;
    }




    uint FindParentNode(uint childPtr, uint64_t vaBVH)
    {
                                                ;
                                                                     ;
        ...
        return XboxBvh4Node_LoadParentPointer(vaBVH, childPtr, m_parentTableOffset);
    }
    ...
    void WorkAfterTraversalIntrinsicIssued()
    {
        ...
        if (node1 != XdxrInvalidNodePtr)
        {
            ...
            if (node2 != XdxrInvalidNodePtr)
            {
                ...
                {
                    StackPush(__XB_UBFE(29, 0, node3));
                }
                StackPush(__XB_UBFE(29, 0, node2));
            }
            StackPush(__XB_UBFE(29, 0, node1));
        }
    ...
    inline uint CleanPointer(uint dirtyPtr)
    {
    ...
    void StackPop(
        inout uint node,
        const uint64_t vaBLAS)
    {
        if (m_stackPtr < 0)
            return;


        InterlockedExchange(g_blasStack[(m_stackPtr & ((((1 << (3)) * XDXR_TRAVERSAL_WAVE_SIZE) - 1) << 2)) / 4], XdxrInvalidNodePtr, node);
        ...
        else
        {
            ...
            node = FindParentNode(m_lastValidNode, vaBLAS);
        }
    }
    ...
    void StackInit(
        uint laneID,
        uint blasRootPtr,
        int parentTableOffset = 0)
    {

        m_stackPtr = laneID * 4 - (XDXR_TRAVERSAL_WAVE_SIZE << 2);
        ...
        m_parentTableOffset = parentTableOffset;
    }
...
#error This shader is optimized for and requires Wave32
#endif
...
#endif
struct TraversalState_Hybrid
{



    uint4 m_sclBvhDesc;
    uint m_sclTlasNodePtr;


    uint32_t m_sclLiveLaneMask;


    uint32_t m_sclHitLaneMask;




    uint m_vecThisLaneIndex;
    uint m_vecBlasNodePtr;
    RayState m_rayState;
    float3 m_vecInverseDirection;
    HybridUniformStackState m_uniformStack;
    HybridDivergentStackState m_divergentStack;

    uint4 BvhIntersectRay(uint nodePtr)
    {
        ...
        return __XB_BvhIntersectRay(
            ...
            nodePtr,
            XDXR_RayTCurrent(),
            m_rayState.m_rayData.Origin,
            m_rayState.m_rayData.Direction,



            m_vecInverseDirection);
    ...
    void ResetRayData()
    {
        ...
        m_vecInverseDirection = 1.f / m_rayState.m_rayData.Direction;
    }
    ...
    void TerminateRay(bool condition)
    {
        m_sclLiveLaneMask &= ~(WaveActiveBallot(condition).x);
        m_sclHitLaneMask &= m_sclLiveLaneMask;
    }

    bool IsAnyLaneAlive()
    {
        return m_sclLiveLaneMask != 0;
    }

    bool IsAnyLaneActive()
    {
        return m_sclHitLaneMask != 0;
    }

    bool IsCurrentLaneActive()
    {
        return (m_sclHitLaneMask >> m_vecThisLaneIndex) & 1;
    }

    void SetBvhStartAddress(uint vaBVH_256BAligned, bool sortBoxes)
    {
        m_sclBvhDesc.x = vaBVH_256BAligned;
    ...
    uint64_t GetBvhStartAddress()
    {
        return uint64_t(m_sclBvhDesc.x) << uint64_t(8);
    }

    void ResetToTLAS()
    {
        const uint vaTLAS_256BAligned = (WaveReadLaneFirst(uint(XDXR_GetTLASPtr() >> 8)));
        SetBvhStartAddress(vaTLAS_256BAligned, (bool)1);
    }

    void Initialize()
    {

        m_sclBvhDesc = __XB_BvhMakeDescriptor(0, 0, 0, true, true, true);
        ResetToTLAS();

        m_sclTlasNodePtr = __XB_s_load_dword(GetBvhStartAddress() + XboxBvh4Header_OffsetToRootNodePtr);
        m_sclHitLaneMask = m_sclLiveLaneMask = (WaveActiveBallot(XdxrIsValidNodePointer(m_sclTlasNodePtr)).x);
        m_uniformStack.StackInit();


        m_rayState.InitState();
        m_vecInverseDirection = 1.f / m_rayState.m_rayData.Direction;
        m_vecBlasNodePtr = XdxrInvalidNodePtr;
        m_vecThisLaneIndex = (WaveGetLaneIndex());
    }
        ...
        inout TLambda lambda)
    {
        ...
        while (XdxrIsValidNodePointer(m_vecBlasNodePtr) && loopCond == TRAVERSE_STEP_RESULT_ITERATE)
        {
            g_xdxrCountIterations++;;

            const uint nodeType = XdxrTypeOfNodePointer(m_vecBlasNodePtr);
            const uint4 rayTest = BvhIntersectRay(m_vecBlasNodePtr);

            uint4 metadata1Temp;


            if (nodeType != XDXR_NODE_TYPE_BOX16 && nodeType != XDXR_NODE_TYPE_BOX32)
            {
                XDXR_SetPendingBlasLeafNodePtr(m_vecBlasNodePtr);
                uint64_t loadAddr = XdxrNodePointerToVA(GetBvhStartAddress(), m_vecBlasNodePtr);
                metadata1Temp = GpuVALoad4Offset(loadAddr, XboxBvh4Leaf_OffsetToGeometryIndexAndFlags);
            }


            m_divergentStack.WorkAfterTraversalIntrinsicIssued();
                ...
                nodeType == XDXR_NODE_TYPE_BOX32)
            {
                ...
                m_vecBlasNodePtr = m_divergentStack.HandleBoxTests(m_vecBlasNodePtr, rayTest);
            }
            else if (disableIS && nodeType == XDXR_NODE_TYPE_AABB_GEOMETRY)
            {
                m_vecBlasNodePtr = XdxrInvalidNodePtr;
            }
            else
            {
                ...
                m_vecBlasNodePtr |= (isQuad & isTri0 & !culled) ? 1 : -1;
            ...
            if (!XdxrIsValidNodePointer(m_vecBlasNodePtr))
            {
                m_divergentStack.StackPop(m_vecBlasNodePtr, GetBvhStartAddress());
            }
        ...
        inout TLambda lambda)
    {
        do
        {
            ...
            uint4 rayTest = XdxrInvalidNodePtr;

            const uint nodeType = XdxrTypeOfNodePointer(m_sclTlasNodePtr);

            if (IsCurrentLaneActive() && nodeType != XDXR_NODE_TYPE_INSTANCE)
            {
                rayTest = BvhIntersectRay(m_sclTlasNodePtr);
            }
            ...
            {
                uint64_t loadAddr = XdxrNodePointerToVA(GetBvhStartAddress(), m_sclTlasNodePtr);
                ...
                {
                    loadAddr += XboxBvh4Instance_OffsetToInstanceMetaData1;
                }

                metadata1Temp = __XB_s_load_dwordx4(loadAddr);
            }


            m_uniformStack.WorkAfterTraversalIntrinsicIssued(m_vecThisLaneIndex);
            ...
            if (nodeType == XDXR_NODE_TYPE_INSTANCE)
            {
                ...
                m_sclHitLaneMask &= (WaveActiveBallot(included).x);



                if (IsAnyLaneActive())
                {
                    XDXR_SetPendingInstanceNodePtr(m_sclTlasNodePtr);

                    uint64_t vaInstance = XdxrNodePointerToVA(GetBvhStartAddress(), m_sclTlasNodePtr);

                    const uint instanceOffsetAndFlags = metadata1Temp.y;


                    const uint blasRootNodePtr = XdxrUnpackRootNodePtr(metadata1Temp.z);


                    SetBvhStartAddress(metadata1Temp.w, true);

                    const int parentTableOffset = XboxBvh4Node_GetParentTableOffset(GetBvhStartAddress());
                    ...
                    uint4 instanceTransform[3];
                    instanceTransform[0] = __XB_s_load_dwordx4(instanceTransformVA);

                    if (instanceRotationNeeded)
                    {
                        instanceTransform[1] = __XB_s_load_dwordx4(instanceTransformVA + 16);
                        instanceTransform[2] = __XB_s_load_dwordx4(instanceTransformVA + 32);

                        XdxrMatrix worldToObject;
                        XdxrMatrix_UnpackInstanceWorldToObject(worldToObject, instanceTransform[0], instanceTransform[1], instanceTransform[2]);

                        m_rayState.m_rayData.Origin = worldToObject.Mul3x4(XDXR_WorldRayOriginV());
                    ...
                    {
                        m_rayState.m_rayData.Origin += asfloat(instanceTransform[0].xyz);
                    }

                    m_rayState.UpdateFlagsForInstance(instanceOffsetAndFlags);

                    m_divergentStack.StackInit(m_vecThisLaneIndex, blasRootNodePtr, parentTableOffset);


                    m_vecBlasNodePtr = IsCurrentLaneActive() ? blasRootNodePtr : XdxrInvalidNodePtr;


                    if (!allowBreakout)
                    {
                        uint loopCond = Traverse_BLAS(disableAHS, disableIS, lambda);
                        TerminateRay(loopCond == TRAVERSE_STEP_RESULT_BREAK);
                        ResetToTLAS();
                        ResetRayData();
                    }
                }
            ...
            else
            {
                ...
                m_sclHitLaneMask = m_uniformStack.HandleBoxTests(m_sclTlasNodePtr, rayTest, metadata1Temp);
            ...
            while (m_sclHitLaneMask == 0)
            {
                if (m_uniformStack.IsStackEmpty())
                {

                    m_sclTlasNodePtr = XdxrInvalidNodePtr;
                    break;
                }

                uint nodeMask;
                m_uniformStack.StackPop(m_sclTlasNodePtr, nodeMask);


                m_sclHitLaneMask = nodeMask & m_sclLiveLaneMask;
            }


            if (allowBreakout && WaveActiveAnyTrue(XdxrIsValidNodePointer(m_vecBlasNodePtr)))
            {
                break;
            }
        }
        while (XdxrIsValidNodePointer(m_sclTlasNodePtr));
    }
        ...
        inout TLambda lambda)
    {
        ...
        if (!WaveActiveAnyTrue(XdxrIsValidNodePointer(m_vecBlasNodePtr)))
        {
            if (XdxrIsValidNodePointer(m_sclTlasNodePtr))
            {
                ResetToTLAS();
                ResetRayData();


                Traverse_TLAS(disableAHS, disableIS, true, lambda);
            }
            else
            {

                m_sclLiveLaneMask = 0;
                return TRAVERSE_STEP_RESULT_BREAK;
            }
        }


        return Traverse_BLAS(disableAHS, disableIS, lambda);
    }
        ...
        inout TLambda lambda)
    {
        if (IsAnyLaneAlive())
        {
            Traverse_TLAS(disableAHS, disableIS, false, lambda);
            m_sclLiveLaneMask = 0;
        }
    }
...
#if XDXR_USE_HLSL_2021
TraversalState_Hybrid<TLambda> state;
#else
static const uint TFlags = 0;
TraversalState_Hybrid state;
#endif
    ...
    bool Proceed(inout TLambda lambda)
    {
        ...
        const bool isIterable = (!disableAHS || !disableIS) && !lambda.EnableInlineAHS() && !lambda.EnableInlineIS();

        if (isIterable)
        {
            state.TerminateRay(rayQueryProxy.loopCond == TRAVERSE_STEP_RESULT_BREAK);

            if (!state.IsAnyLaneAlive())
            {
        ...
        {
            state.Traverse_Loop(disableAHS, disableIS, lambda);
            EndInlineRaylogEntry();;
            return false;
        }
    }
