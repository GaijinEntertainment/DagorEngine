#ifndef SCREENPROBES_ENCODING_INCLUDED
#define SCREENPROBES_ENCODING_INCLUDED 1
...
uint sp_is_moving_probe(uint encoded_probe) {return encoded_probe&(SP_TAG_MOVING_PROBE<<SP_TAG_BITS_SHIFT);}
uint sp_is_newborn_probe(uint encoded_probe) {return (encoded_probe&(SP_TAG_AGE_MASK<<SP_TAG_BITS_SHIFT)) == (SP_TAG_AGE_BORN<<SP_TAG_BITS_SHIFT);}
...
uint encodeProbeColor(float3 color)
{
  return PackRGBE(color);
}
float3 decodeProbeColor(uint color)
{
  return UnpackRGBE(color);
}
