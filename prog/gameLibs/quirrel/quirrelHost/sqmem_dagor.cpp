...
enum
{
  ALLOC_ALIGNMENT_SHIFT = 3,
  ALLOC_ALIGNMENT_MASK = (1 << ALLOC_ALIGNMENT_SHIFT) - 1
};
...
void *sq_vm_malloc(SQAllocContext ctx, SQUnsignedInteger size)
{
  ...
  if (ctx)
  {
    ...
    {
      void *n = ctx->allocatedBins[bin][--ctx->allocatedBinsCount[bin]];
#if ENABLE_CONTEXT_CHECK
...
#endif
#if MEM_TRACE_ENABLED == 1
      sqmemtrace::on_alloc(ctx, ctx->vm, ret, size);
#endif
      return ret;
    }
  }
