...
#include "osApiWrappers/dag_files.h" // for sq_memtrace.dump_all
#endif
  ...
  switch (sq_type(obj))
  {
    ...
    case OT_CLOSURE:
    {
      ...
      size_t size = sizeof(SQClosure) + _FUNC_SIZE(func->_ninstructions, func->_nliterals, func->_nparameters, func->_nfunctions,
                                          func->_noutervalues, func->_nlineinfos, func->_lineinfos->_is_compressed,
                                          func->_nlocalvarinfos, func->_ndefaultparams, func->_nstaticmemos);
    ...
    case OT_FUNCPROTO:
    {
      ...
      size_t size = _FUNC_SIZE(func->_ninstructions, func->_nliterals, func->_nparameters, func->_nfunctions, func->_noutervalues,
        func->_nlineinfos, func->_lineinfos->_is_compressed, func->_nlocalvarinfos, func->_ndefaultparams, func->_nstaticmemos);
...
static void dump_sq_allocations_internal(ScriptAllocRecordsMap &alloc_map, file_ptr_t out_file, int n_top_records)
{
  ...
  debug(buf.c_str());
  if (out_file)
    df_write(out_file, buf.c_str(), buf.length());
}
...
void remove_ctx(SQAllocContext ctx)
{
  ...
  if (!mem_trace[idx].empty())
  {
    ...
    dump_sq_allocations_internal(mem_trace[idx], nullptr, 128);
  }
...
void dump_statistics_for_current_vm(HSQUIRRELVM vm, int n_top_records)
{
  SQAllocContext ctx = vm->_sharedstate->_alloc_ctx;
  int idx = get_memtrace_index(ctx);
  dump_sq_allocations_internal(mem_trace[idx], nullptr, n_top_records);
}


void dump_all(const char *out_file_name, int n_top_records)
{
  file_ptr_t f = out_file_name ? df_open(out_file_name, DF_CREATE | DF_WRITE) : nullptr;
  ...
  for (int i = 0; i < MEM_TRACE_MAX_VM; i++)
    if (!mem_trace[i].empty())
    {
      ...
      debug_(buf.c_str());
      if (f)
        df_write(f, buf.c_str(), buf.length());

      dump_sq_allocations_internal(mem_trace[i], f, n_top_records);
    }
...
void reset_all() { logerr("compile exe with SqVarTrace=yes to call sqmemtrace::reset_all"); }
...
void dump_all(const char *, int) { logerr("compile exe with SqVarTrace=yes to call sqmemtrace::dump_all"); }
