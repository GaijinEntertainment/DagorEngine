...
shader random_point_occlusion_ambient_voxels_cs
{
  ...
  hlsl(cs) {
    ...
    StructuredBuffer<float> prev_probability: register(t15);//todo: use it to better bin handling
    StructuredBuffer<uint> frustum_visible_ambient_voxels_count: register(t13);

    RWStructuredBuffer<VisibleAmbientVoxelPoint> visible_ambient_voxels: register(u0);
    RWStructuredBuffer<uint> voxelsWeightHistogram: register(u1);
...
shader ssgi_change_probability_cs
{
  ...
  hlsl(cs) {
    ...
    RWStructuredBuffer<float> next_probability: register(u0);
    RWByteAddressBuffer dispatchIndirectBuffer : register( u1 );
    RWStructuredBuffer<uint> voxelsWeightHistogram: register(u2);
    ...
    void select_bins_cs( uint tid: SV_GroupIndex )
    {
      ...
      if (bin<MAX_BINS)
      {
        ...
        next_probability[bin] = probability;
      }
      {
        uint groups = 0;
        if (tid == 0)
...
shader temporal_ambient_voxels_cs
{
  ...
  ENABLE_ASSERT(cs)
  hlsl(cs) {
    StructuredBuffer<uint> voxelsWeightHistogram: register(t14);
    RWStructuredBuffer<VisibleAmbientVoxel> visible_ambient_voxels: register(u0);
    RWStructuredBuffer<AmbientVoxelsPlanes> visible_ambient_voxels_walls_planes: register(u1);
