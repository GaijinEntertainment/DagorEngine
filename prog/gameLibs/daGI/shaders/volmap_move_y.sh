  ...
  hlsl(cs) {
    ...
    void ssgi_copy_volmap_cs( uint3 gId : SV_GroupID, uint3 dtId : SV_DispatchThreadID )//uint3 gId : SV_GroupId,
    {
      ...
      ##if (shader == move_y_from_cs)
        ...
        UNROLL
        for (uint i = 0; i < 6; ++i)
          voxelColors[texel*7+i] = Pack_R11G11B10_FLOAT_LOG(col[i]);
        voxelColors[texel*7+6] = asint(ssgi_ambient_volmap_temporal[int3(dtId.xy, temporalZFrom)].x);
      ##elif (shader == move_y_to_cs)
        ...
        UNROLL
        for (uint i = 0; i < 6; ++i)
          col[i] = Unpack_R11G11B10_FLOAT_LOG(voxelColors[texel*7+i]);
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[0]; colorZTo+=dimZ;
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[1]; colorZTo+=dimZ;
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[2]; colorZTo+=dimZ;
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[3]; colorZTo+=dimZ;
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[4]; colorZTo+=dimZ;
        gi_ambient_volmap[int3(dtId.xy, colorZTo)] = col[5];
        ssgi_ambient_volmap_temporal[int3(dtId.xy, temporalZTo)] = asfloat(voxelColors[texel*7+6]);
     ##endif
    }
  }
