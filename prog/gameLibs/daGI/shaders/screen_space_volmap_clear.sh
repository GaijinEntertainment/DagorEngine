...
shader ssgi_clear_volmap_cs
{
  ...
  hlsl(cs) {
    ...
    void ssgi_clear_volmap_cs( uint3 dtId : SV_DispatchThreadID )//uint3 gId : SV_GroupId,
    {
      ...
      ssgi_ambient_volmap_temporal[uint3(coord.xy, coord.z + ssgi_cascade_z_crd_ofs(cascadeId))] = copied ? SSGI_TEMPORAL_COPIED_VALUE : 0;
      coord.z += ssgi_cascade_z_crd_ofs(cascadeId)*6;
      uint z_ofs = volmap_y_dim(cascadeId);
##if gi_quality != only_ao
      gi_ambient_volmap[coord] = col0;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = col1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = col2;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = col3;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = col4;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = col5;
##else
      gi_ambient_volmap[coord] = 1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = 1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = 1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = 1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = 1;coord.z+=z_ofs;
      gi_ambient_volmap[coord] = 1;
##endif
    }
  }
