...
shader calc_initial_ambient_walls_dist_cs
{
  ...
  hlsl(cs) {
    RWStructuredBuffer<AmbientVoxelsPlanes> visible_ambient_voxels_walls_planes: register(u0);
...
int visible_ambient_voxels_walls_planes_no = 14;
int ssgi_ambient_volmap_temporal_no = 7;
int gi_ambient_volmap_init_no = 6;
...
shader light_initial_ambient_voxels_cs
{
  ...
  (cs) {
    ...
    gi_ambient_volmap_init@uav: register(gi_ambient_volmap_init_no) hlsl {
      RWTexture3D<float3> gi_ambient_volmap_init@uav;
    }
    visible_ambient_voxels_walls_planes@buf: register(visible_ambient_voxels_walls_planes_no) hlsl {
      #include "dagi_common_types.hlsli"
  ...
  hlsl(cs) {
    ...
    void first_light_voxels_cs( uint gId : SV_GroupID, uint tid: SV_GroupIndex, uint dtId : SV_DispatchThreadID )//uint3 gId : SV_GroupId,
    {
      ...
      if (tid == 0)
      {
        ...
        ssgi_ambient_volmap_temporal[coord] = SSGI_TEMPORAL_INITED_VALUE;
        coord.z += ssgi_cascade_z_crd_ofs(cascade_id)*5;
        uint z_ofs = volmap_y_dim(cascade_id);
        gi_ambient_volmap_init[coord] = col0;coord.z+=z_ofs;
        gi_ambient_volmap_init[coord] = col1;coord.z+=z_ofs;
        gi_ambient_volmap_init[coord] = col2;coord.z+=z_ofs;
        gi_ambient_volmap_init[coord] = col3;coord.z+=z_ofs;
        gi_ambient_volmap_init[coord] = col4;coord.z+=z_ofs;
        gi_ambient_volmap_init[coord] = col5;
      }
    }
  }
