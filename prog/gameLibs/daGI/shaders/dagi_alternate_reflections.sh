...
macro DAGI_ALTERNATE_REFLECTIONS(code)
  if (gi_quality >= colored)
  {
    ...
    hlsl(code) {
      ...
      half4 get_gi_dithered_reflections(uint2 pixelPos, float3 cameraToPoint, float3 normal)
      {
        uint frameRandom = uint(SSRParams.z);
        float3 wdir = reflect(normalize(cameraToPoint), normal);
      ...
      void dagi_alternate_reflections(inout half4 newTarget, uint2 pixelPos, float3 cameraToPoint, float3 normal, bool hit, bool hit_not_zfar)
      {
        if (!hit)
        {
          newTarget = get_gi_dithered_reflections(pixelPos, cameraToPoint, normal);
