...
hlsl {
  ...
  cbuffer BufferCounter : register( b1 )
  {
    uint numVoxels;
  };
}
...
shader create_debug_render_cs
{
  ...
  hlsl(cs) {
    StructuredBuffer<uint> frustum_visible_ambient_voxels_count: register(t1);
    RWByteAddressBuffer drawIndirectBuffer : register( u0 );
    [numthreads(1, 1, 1)]
...
shader debug_render_all_volmap
{
  ...
  hlsl(vs) {
    StructuredBuffer<VisibleAmbientVoxel> visible_voxels:register(t8);
