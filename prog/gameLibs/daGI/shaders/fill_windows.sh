...
shader remove_windows_cs2
{
  ...
  ENABLE_ASSERT(cs)
  hlsl(cs) {
    RWTexture3D<float>  voxelsAlpha : register(u0);

    [numthreads(4, 4, 4)]
    void remove_windows_cs(uint3 dId : SV_DispatchThreadID)
    {
      if (any(dId >= VOXEL_RESOLUTION))
        return;
      int3 coord = dId;
      uint cascade = 0;
      float3 center = sceneCoordToWorldPos(coord, cascade);
      if (inWindow(center))
        voxelsAlpha[wrapSceneVoxelCoord(coord, cascade) + getCascadeShift(cascade)] = 0;
    }
  }
  compile("cs_5_0", "remove_windows_cs");
}
float4 windowsBoxLt;
float4 windowsBoxRb;
