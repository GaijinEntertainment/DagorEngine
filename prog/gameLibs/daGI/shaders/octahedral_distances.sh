...
shader octahedral_distances_cs
{
  ...
  hlsl(cs) {
    ...
    void main_cs( uint3 gId : SV_GroupID, uint3 gTId : SV_GroupThreadID, uint tid: SV_GroupIndex, uint3 dtId : SV_DispatchThreadID)
    {
      ...
      {
        i = 0; j = 0; k = 0;
        dead_probes[uint3(gId.x, gId.y, gId.z + ssgi_cascade_z_crd_ofs(cascadeId))] = 0;
      }
      ...
      if (j == OCTAHEDRAL_TILE_SIDE_LENGTH * OCTAHEDRAL_TILE_SIDE_LENGTH)
      {
        ...
        if (i > (k >> 1) && k > (j >> 1))
          dead_probes[uint3(gId.x, gId.y, gId.z + ssgi_cascade_z_crd_ofs(cascadeId))] = 1.0;
      }
      float2 tc = octEncode(worldDir);
      tc.xy = tc.xy * 0.5 + 0.5;
      octahedral_distances[uint3(cornerTexelCoord + tc.x * OCTAHEDRAL_TILE_SIDE_LENGTH,
