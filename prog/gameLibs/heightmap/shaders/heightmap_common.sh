...
hlsl {
  ##if in_editor == yes
  #define has_high_texture 1
  ##else
  #define has_high_texture 0
  ##endif
}

float4 world_to_hmap_high = (1/2048, 1/2048, 0, 0);
...
//int heightmap_vs_texture_no = 0;
int heightmap_vs_high_texture_no = 3;
...
float4 hmap_holes_scale_step_offset;
buffer heightmap_holes_zones_cb;
int heightmap_holes_zones_num;
int heightmap_holes_support_assume = 1;
interval heightmap_holes_support_assume: off < 1, on;

bool heightmap_holes = heightmap_holes_tex != NULL;
  ...
  hlsl(vs) {
    ...
    float getHeight(float2 worldPosXZ)
    {
      ...
      if (tc_high.x == tc_high_sat.x && tc_high.y == tc_high_sat.y)
      {
        ...
        height = tex2Dlod(hmap_hdetail, float4(tc_high,0,0));
      }
      #endif
...
macro USE_HEIGHTMAP_COMMON_BASE(code)
  ...
  ##endif
    ...
    half3 getNormalLow(float2 pos)
    {
      ...
      ##endif
      float3 offset = float3(tex_hmap_inv_sizes.x, 0, tex_hmap_inv_sizes.y);
      half W = tex2D(tex_hmap_low, float2(pos.xy - offset.xy)).x;
      half E = tex2D(tex_hmap_low, float2(pos.xy + offset.xy)).x;
      half N = tex2D(tex_hmap_low, float2(pos.xy - offset.yz)).x;
      half S = tex2D(tex_hmap_low, float2(pos.xy + offset.yz)).x;
      return normalize(half3(W-E, sizeInMeters, N-S));
      #endif
    }
    #if has_high_texture
    half3 getNormalHigh(float2 pos)
    {
      ...
      half W = tex2D(tex_hmap_high, float2(pos.xy - offset.xy)).x;
      half E = tex2D(tex_hmap_high, float2(pos.xy + offset.xy)).x;
      half N = tex2D(tex_hmap_high, float2(pos.xy - offset.yz)).x;
      half S = tex2D(tex_hmap_high, float2(pos.xy + offset.yz)).x;
      return normalize(half3(W-E, sizeInMeters, N-S));
      #endif
    }
    #endif
...
macro HEIGHTMAP_CALCULATIONS()
  ...
  hlsl(ps) {
    ...
    void calculateHeightmapDetails(inout UnpackedGbuffer result, DetailDesc detailNo,
                                  ...
                                  half3 vertical_normal, float3 point_to_eye_norm,
    ...
    UvWithDerivates microdetails_get_uv_and_derivates(float3 tc_world_pos)
    {
      return make_uv_with_derivates(get_microdetails_world_tc(tc_world_pos));
    }
    void calculateHeightmapMicrodetails(inout UnpackedGbuffer result, DetailDesc detailNo,
                                        ...
                                        half porosity, half normal_wetness, float wetness)
    {
      UvWithDerivates uv = microdetails_get_uv_and_derivates(tc_world_pos);
