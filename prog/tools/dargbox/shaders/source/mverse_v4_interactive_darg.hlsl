// PS3 XMB Wave Knot v4 -- Interactive Variant
// Based on mverse_v4_darg.hlsl with cursor interaction:
//   - Subtle radial glow on background follows cursor
//   - Ribbons near cursor get wave disturbances and brightness boost
//
...
float4 darg_ps(float2 uv)
{
    ...
    {
        float2 cursorAspect = (darg_cursor.xy - knot_pos);
        cursorAspect.x *= aspect;
        float2 delta = bgUV - cursorAspect;
        float cursorDist2 = dot(delta, delta);
        float3 cursorGlowColor = hsv2rgb(float3(wave_hue + 0.1, 0.5, 0.15));
        col += cursorGlowColor * exp(-cursorDist2 * 8.0) * 0.6;
    }
    ...
    // Cursor in rotated coordinate space (for ribbon interaction)
    float2 cursorRotated = float2(0, 0);
    float cursorActive = 0.0;
    if (darg_cursor.x >= 0.0)
    {
        float2 cUV = darg_cursor.xy - 0.5;
        cUV.x *= aspect;
        cursorRotated = mul(rot, cUV);
        cursorActive = 1.0;
    }
    ...
    for (float i = 0.0; i < 40.0; i++)
    {
        ...
        // Cursor wave disturbance
        float cursorInfluence = 0.0;
        if (cursorActive > 0.5)
        {
            float dac = along - cursorRotated.x;
            float dpc = perp - ribbonY;
            cursorInfluence = exp(-(dac * dac * 200.0 + dpc * dpc * 800.0));
            ribbonY += cursorInfluence * sin(along * 15.0 - darg_time * 4.0) * 0.008;
        }
        ...
        col += rCol * (core * 1.2 + halo) * kB * dotI * (1.0 + cursorInfluence * 1.5);
