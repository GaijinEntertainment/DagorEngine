...
// Warm variant: darker orange background, vertical wave orientation at 90% horizontal
// Interactive: cursor creates localized wave disturbances on the SDF surface
...
float sdf(float3 p)
{
    ...
    // Cursor wave interaction
    if (darg_cursor.x >= 0.0)
    {
        // Horizontal gate: effect vanishes when cursor is >10% away from wave center (x=0.9)
        float hDist = abs(darg_cursor.x - 0.9);
        float hGate = smoothstep(0.10, 0.01, hDist);

        // Vertical proximity: map cursor screen-Y to SDF p.x at current depth
        // Ray mapping: d.x = (uv.y - 0.5), so p.x = (uv.y - 0.5) * p.z after p*=2
        float cursorPx = (darg_cursor.y - 0.5) * p.z;
        float dx = p.x - cursorPx;
        float vFalloff = exp(-dx * dx * 1.5);

        // Position-dependent variation: different ripple character across the screen
        float screenY = p.x / max(abs(p.z), 0.5);
        float posPhase = screenY * 7.0;
        float posFreq = 1.0 + screenY * 0.8;

        // Primary ripple: traveling waves from cursor, frequency varies with screen height
        float r1 = sin(dx * 3.5 * posFreq + p.z * 0.4 - darg_time * 3.5 + posPhase);
        // Secondary ripple: finer detail at a different rate
        float r2 = sin(dx * 6.0 * posFreq - p.z * 0.2 + darg_time * 2.0 + posPhase * 1.7) * 0.4;

        // Amplitude ramps up strongly as cursor approaches waves (hGate^2)
        o += hGate * vFalloff * (r1 + r2) * 4.0 * hGate;
    }
...
{
    // Ray setup: origin at 0, direction rotated for vertical waves at 90% horizontal
    float3 o = float3(0, 0, 0);
    float3 d = float3(
        (uv.y - 0.5),
        (uv.x - 0.9) * (darg_resolution.x / darg_resolution.y),
        1.0
    );
    ...
    // Dark orange 4-point gradient background
    float3 c = lerp(
        lerp(float3(0.15, 0.06, 0.02), float3(0.08, 0.03, 0.01), uv.x),
        lerp(float3(0.10, 0.04, 0.01), float3(0.20, 0.10, 0.04), uv.x),
        uv.y
    );
    // Blend with warm orange-white based on wave alpha
    c = lerp(c, float3(1.0, 0.85, 0.6), m);

    // Dust / starfield (reduced brightness)
    #ifdef DUST
    c += get_dust(uv, float2(2000.0, 2000.0), mg.y) * 0.15;
    #endif
