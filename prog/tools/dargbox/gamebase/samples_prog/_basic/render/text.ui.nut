...
function sampleSText(params={}) {
  return {
    ...
    size = const [400,SIZE_TO_CONTENT]
    ...
    children = [
      {
        ...
        children = [
          ...
          { size = const [flex(), 50] flow=FLOW_HORIZONTAL gap = 20 children = [
              {text = "Vert aligned static text" rendObj = ROBJ_INSCRIPTION valign = ALIGN_CENTER  size = const [400,flex()] children={ rendObj=ROBJ_FRAME size=flex()}}
              {text="'ROBJ_TEXT'. Dynamic text. valign=ALIGN_CENTER" rendObj=ROBJ_TEXT }
            ]
          }
          { size = const [flex(), 50] flow=FLOW_HORIZONTAL gap = 20 children = [
              {text = "Vert aligned dynamic text" rendObj = ROBJ_TEXT valign = ALIGN_BOTTOM  size = const [400,flex()] children={ rendObj=ROBJ_FRAME size=flex()}}
