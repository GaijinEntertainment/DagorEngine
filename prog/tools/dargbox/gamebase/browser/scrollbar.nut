...
function calcBarSize(bar, axis) {
  return axis == 0 ? [flex(), bar._height] : [bar._width, flex()]
}
...
function scrollbar(scroll_handler, options={}) {
  ...
  return function() {
    ...
    let color = Knob.colorCalc(stateFlags.get())
...
function makeSideScroll(content, options = DEF_SIDE_SCROLL_OPTIONS) {
  ...
  function contentRoot() {
    let bhv = [Behaviors.WheelScroll, Behaviors.ScrollEvent]

    return ContentRoot.__merge({
      size = options.size
      behavior = bhv
