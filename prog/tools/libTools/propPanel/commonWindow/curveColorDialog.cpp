...
CurveColorDialog::CurveColorDialog(void *phandle, const char caption[]) :
  DialogWindow(phandle, hdpi::_pxScaled(310), hdpi::_pxScaled(560), caption, false), colorCurve(NULL), presets(midmem)
{
  updatePresets();
  fillPanel(CurveColorDialog::getPanel());
}
...
void CurveColorDialog::onChange(int pcb_id, ContainerPropertyControl *panel)
{
  switch (pcb_id)
  {
    case ID_COLOR_MODE:
...
void CurveColorDialog::onClick(int pcb_id, ContainerPropertyControl *panel)
{
  switch (pcb_id)
  {
    ...
    case ID_SAVE_PRESET:
      if (colorCurve)
      {
        ...
        DialogWindow dialog(nullptr, hdpi::_pxScaled(300), hdpi::_pxScaled(130), "Preset name");
        ContainerPropertyControl *d_panel = dialog.getPanel();
...
void CurveColorDialog::onDoubleClick(int pcb_id, ContainerPropertyControl *panel)
{
  if (pcb_id == ID_CURVE_CONTAINER)
    getPanel()->setPostEvent(ID_CURVE_CONTROL);
}

void CurveColorDialog::onPostEvent(int pcb_id, ContainerPropertyControl *panel)
{
  if (pcb_id == ID_CURVE_CONTROL && colorCurve)
    colorCurve->showEditDialog();
}

void CurveColorDialog::fillPanel(ContainerPropertyControl *panel)
{
  ...
  // The container is needed for handling the change and double click events of the color curve control.
  ContainerPropertyControl *container = panel->createContainer(ID_CURVE_CONTAINER, true, hdpi::_pxScaled(135));
  del_it(colorCurve);
  colorCurve = new ColorCurveControl(container, container->getWindow(), 10, 10, hdpi::_pxS(270), hdpi::_pxS(270));

  panel->createCustomControlHolder(ID_CURVE_CONTROL, this);
...
static inline int fixColorVal(float val)
{
  if (val > 255)
    return 255;
  if (val < 0)
...
void CurveColorDialog::customControlUpdate(int id)
{
  if (colorCurve)
    colorCurve->updateImgui();
}
