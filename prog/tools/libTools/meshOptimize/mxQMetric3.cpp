...
static inline bool FEQ(double a, double b, double e = FEQ_EPS) { return fabs(a - b) < e; }
static inline bool FEQ2(double a, double b, double e = FEQ_EPS2) { return fabs(a - b) < e; }
...
Mat3 makeTM(const Vec3 &a, const Vec3 &b, const Vec3 &c)
{
  Mat3 r;
  r.setcol(0, a);
  r.setcol(1, b);
...
Mat4 makeTM(const Vec4 &a, const Vec4 &b, const Vec4 &c, const Vec4 &d)
{
  Mat4 r;
  r.setcol(0, a);
  r.setcol(1, b);
...
Mat3 MxQuadric3::tensor() const { return makeTM(Vec3(a2, ab, ac), Vec3(ab, b2, bc), Vec3(ac, bc, c2)); }

Mat4 MxQuadric3::homogeneous() const
{
  return makeTM(Vec4(a2, ab, ac, ad), Vec4(ab, b2, bc, bd), Vec4(ac, bc, c2, cd), Vec4(ad, bd, cd, d2));
}
...
double MxQuadric3::evaluate(double x, double y, double z) const
{
  ...
  return x * x * a2 + 2 * x * y * ab + 2 * x * z * ac + 2 * x * ad + y * y * b2 + 2 * y * z * bc + 2 * y * bd + z * z * c2 +
         2 * z * cd + d2;
}

double invert(Mat3 &r, const Mat3 &a)
{
  double d = a.det();
  if (FEQ(d, 0.0, 1e-12))
    return d;
...
bool MxQuadric3::optimize(float &v, const Vec3 &v1, const Vec3 &v2) const
{
  Vec3 d = v1 - v2;
  Mat3 A = tensor();
