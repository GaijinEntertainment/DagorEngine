...
ECSEntityObjectPropertyPanel::ECSEntityObjectPropertyPanel(ECSBasicObjectEditor &editor,
  PropPanel::ContainerPropertyControl &main_panel) :
  editor(editor), mainPanel(main_panel)
{}

void ECSEntityObjectPropertyPanel::fillComponentPropertyPanel(PropPanel::ContainerPropertyControl &panel, int &control_id,
  const ecs::EntityComponentRef &component, const char *name, int parent_control_id, bool read_only)
{
  ...
  switch (ctype)
  {
    case ecs::ComponentTypeInfo<bool>::type:
      ...
      panel.createCheckBox(control_id, name, component.get<bool>(), !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<int>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createEditInt(control_id, name, component.get<int>(), !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<float>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createEditFloat(control_id, name, component.get<float>(), 2, !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<Point2>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createPoint2(control_id, name, component.get<Point2>(), 2, !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<Point3>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createPoint3(control_id, name, component.get<Point3>(), 2, !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<Point4>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createPoint4(control_id, name, component.get<Point4>(), 2, !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<E3DCOLOR>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createColorBox(control_id, name, component.get<E3DCOLOR>(), !read_only);
      ++control_id;
      break;
    ...
    case ecs::ComponentTypeInfo<ecs::string>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createEditBox(control_id, name, component.get<ecs::string>().c_str(), !read_only);
      ++control_id;
      break;

    case ecs::ComponentTypeInfo<ecs::EntityId>::type:
      ecs_property_panel_mapping.addControl(control_id, name, parent_control_id);
      panel.createEditInt(control_id, name, (unsigned)component.get<ecs::EntityId>(), !read_only);
      ++control_id;
      break;
    ...
    case ecs::ComponentTypeInfo<ecs::Object>::type:
    {
      ...
      for (const eastl::pair<eastl::string, ecs::ChildComponent> &child : component.get<ecs::Object>())
      {
        ...
        fillComponentPropertyPanel(*objectPropertyPanel, control_id, child.second.getEntityComponentRef(),
          ecs::get_key_string(child.first).c_str(), extensibleContainerControlId, read_only);
      }
      ...
      objectGroupPanel->createButton(control_id, "Add", !read_only);
      ++control_id;
    }
    break;
    ...
    case ecs::ComponentTypeInfo<ecs::Array>::type:
    {
      ...
      for (const ecs::ChildComponent &child : component.get<ecs::Array>())
      {
        ...
        fillComponentPropertyPanel(*objectPropertyPanel, control_id, child.getEntityComponentRef(), arrayElementName,
          extensibleContainerControlId, read_only);
        ++arrayIndex;
      }
...
void ECSEntityObjectPropertyPanel::fillProps(ecs::EntityId editable_object_eid, bool read_only)
{
  ...
  if (g_entity_mgr->has(editable_object_eid, ECS_HASH("transform")))
  {
    ...
    editor.createPanelTransform(CM_OBJED_MODE_MOVE, read_only);
    editor.createPanelTransform(CM_OBJED_MODE_ROTATE, read_only);
    editor.createPanelTransform(CM_OBJED_MODE_SCALE, read_only);
  }

  createSceneControl(editable_object_eid);
  ...
  for (ecs::ComponentsIterator it = g_entity_mgr->getComponentsIterator(editable_object_eid); it; ++it)
  {
    ...
    fillComponentPropertyPanel(mainPanel, controlId, comp.second, comp.first, 0, read_only);
  }
  ...
  mainPanel.createButton(PID_TEMPLATE_ADD, "Add", !read_only);
  mainPanel.createButton(PID_TEMPLATE_REMOVE, "Remove", !read_only, false);
  updateTemplateRemoveButton();
}

void ECSEntityObjectPropertyPanel::setComponentValueByPropertyPanel(int control_id, ecs::EntityComponentRef &component)
{
  const auto compData = g_entity_mgr->getDataComponents().getComponentById(component.getComponentId());
  if ((compData.flags & ecs::DataComponent::IS_COPY) != 0)
    return;
...
void ECSEntityObjectPropertyPanel::createSceneControl(ecs::EntityId editable_object_eid)
{
  ecs::Scene::SceneId sceneId = ecs::Scene::C_INVALID_SCENE_ID;
  if (const ecs::Scene::EntityRecord *erecord =
        ecs::g_scenes ? ecs::g_scenes->getActiveScene().findEntityRecord(editable_object_eid) : nullptr)
  {
    sceneId = erecord->sceneId;
  }
  ...
  const auto foundIt = eastl::find_if(editor.getEcsScenes().cbegin(), editor.getEcsScenes().cend(),
    [sceneId](auto &&val) { return val.second == sceneId; });
  mainPanel.createCombo(PID_SCENE_LIST, "Scene", sceneComboEntries,
    foundIt != editor.getEcsScenes().cend() ? foundIt->first.c_str() : sceneComboEntries[0].c_str());
}

void ECSEntityObjectPropertyPanel::onPPChange(int pid, bool edit_finished, ecs::EntityId editable_object_eid)
{
  ...
  if (pid == PID_SCENE_LIST && ecs::g_scenes)
  {
    const eastl::string comboVal{mainPanel.getText(PID_SCENE_LIST).c_str()};
    if (auto it = editor.getEcsScenes().find(comboVal); it != editor.getEcsScenes().cend())
    {
      ecs::g_scenes->setEntityParent(it->second, {editable_object_eid});
    }
    else
    {
      logerr("While setting scene (%s) for an entity from combobox was not found in scene list", comboVal);
    }
  }
  ...
  editor.saveComponent(editable_object_eid, rootComponentName);
}

void ECSEntityObjectPropertyPanel::onAddTemplatePressed(ecs::EntityId editable_object_eid)
{
  ECSTemplateSelectorDialog dialog("Template selector", &editor);
  ...
  editor.saveAddTemplate(editable_object_eid, templateToAdd);
  editor.invalidateObjectProps();
}

void ECSEntityObjectPropertyPanel::onRemoveTemplatePressed(ecs::EntityId editable_object_eid)
{
  ...
  g_entity_mgr->reCreateEntityFromAsync(editable_object_eid, newTemplate.c_str());
  editor.saveDelTemplate(editable_object_eid, templateToRemove, true);
  editor.invalidateObjectProps();
}

void ECSEntityObjectPropertyPanel::updateTemplateRemoveButton()
{
  dag::ConstSpan<String> templateNames = mainPanel.getStrings(PID_TEMPLATE_LIST);
  const int selectionIndex = mainPanel.getInt(PID_TEMPLATE_LIST);
...
bool ECSEntityObjectPropertyPanel::addComponentArrayElementPressed(ecs::Array &component_array)
{
  int memberType;
...
bool ECSEntityObjectPropertyPanel::removeComponentArrayElementPressed(ecs::Array &component_array, int element_index)
{
...
bool ECSEntityObjectPropertyPanel::addComponentObjectMemberPressed(ecs::Object &component_object)
{
  String memberName;
  int memberType;
...
bool ECSEntityObjectPropertyPanel::removeComponentObjectMemberPressed(ecs::Object &component_object, const char *member_name)
{
...
bool ECSEntityObjectPropertyPanel::renameComponentObjectMemberPressed(ecs::Object &component_object, const char *member_name)
{
  ecs::Object::const_iterator it = component_object.find(member_name);
  if (it == component_object.end())
    return false;
...
void ECSEntityObjectPropertyPanel::onPPBtnPressed(int pid, ecs::EntityId editable_object_eid)
{
  if (pid == PID_TEMPLATE_ADD)
    onAddTemplatePressed(editable_object_eid);
  else if (pid == PID_TEMPLATE_REMOVE)
    onRemoveTemplatePressed(editable_object_eid);
  else
  {
