...
NvStripifier::NvStripifier() {} //-V730
...
NvFaceInfo *NvStripifier::FindOtherFace(NvEdgeInfoVec &edgeInfos, int v0, int v1, NvFaceInfo *faceInfo)
{
  ...
  G_ANALYSIS_ASSUME(edgeInfo != NULL);
...
void NvStripifier::BuildStripifyInfo(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos, const unsigned short maxIndex)
{
  ...
  intptr_t numIndices = indices.size();
  ...
  intptr_t numTriangles = numIndices / 3;
  int index = 0;
  bool bFaceUpdated[3];
...
NvFaceInfo *NvStripifier::FindGoodResetPoint(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos)
{
  ...
  intptr_t numFaces = faceInfos.size();
  int startPoint;
...
void NvStripInfo::Combine(const NvFaceInfoVec &forward, const NvFaceInfoVec &backward)
{
  ...
  // add backward faces
  intptr_t numFaces = backward.size(), i;
...
bool NvStripInfo::SharesEdge(const NvFaceInfo *faceInfo, NvEdgeInfoVec &edgeInfos)
{
  ...
  if (IsInStrip(currEdge->m_face0) || IsInStrip(currEdge->m_face1)) //-V522
    return true;
...
void NvStripifier::CommitStrips(NvStripInfoVec &allStrips, const NvStripInfoVec &strips)
{
  ...
  intptr_t numStrips = strips.size();
...
bool NvStripifier::FindTraversal(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos, NvStripInfo *strip, NvStripStartInfo &startInfo)
{
  ...
  if (edgeIter != NULL && startInfo.m_startFace)
  {
...
void NvStripifier::CreateStrips(const NvStripInfoVec &allStrips, IntVec &stripIndices, const bool bStitchStrips,
  unsigned int &numSeparateStrips, const bool bRestart, const unsigned int restartVal)
{
  assert(numSeparateStrips == 0);

  NvFaceInfo tLastFace(0, 0, 0);
  NvFaceInfo tPrevStripLastFace(0, 0, 0);
  intptr_t nStripCount = allStrips.size();
  ...
  for (int i = 0; i < nStripCount; i++)
  {
    ...
    intptr_t nStripFaceCount = strip->m_faces.size();
...
void NvStripifier::Stripify(const WordVec &in_indices, const int in_cacheSize, const int in_minStripLength,
  const unsigned short maxIndex, NvStripInfoVec &outStrips, NvFaceInfoVec &outFaceList)
{
  meshJump = 0.0f;
...
//
void NvStripifier::SplitUpStripsAndOptimize(NvStripInfoVec &allStrips, NvStripInfoVec &outStrips, NvEdgeInfoVec &edgeInfos,
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips, NvFaceInfoVec &allFaceInfos, NvEdgeInfoVec &allEdgeInfos, int numSamples)
{
  ...
  while (!done)
  {
    ...
    for (i = 0; i < numExperiments; i++)
    {
      if (i != bestIndex)
      {
        intptr_t numStrips = experiments[i].size();
