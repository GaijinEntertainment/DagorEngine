...
VPROG drv3d_dx11::create_vertex_shader_unpacked(const struct ShaderSource &, const void *, uint32_t, uint32_t, bool) { return -1; }
FSHADER drv3d_dx11::create_pixel_shader_unpacked(const struct ShaderSource &, const void *, uint32_t, uint32_t, int32_t, bool)
{
  return -1;
}
...
{
  return d3d::compile_compute_shader_hlsl(hlsl_text, len, entry, profile, metadata, shader_bin, out_err);
}

DLL_EXPORT bool compile_compute_shader_xboxone(const char *, unsigned, const char *, const char *, Tab<uint8_t> &, Tab<uint32_t> &,
  String &)
{
  return false;
}
