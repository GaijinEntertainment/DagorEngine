...
class My2LoadCB : public ImpSceneCB
{
public:
  Tab<TEXTUREID> mtex;
...
My2LoadCB::My2LoadCB(const char *fn, AScene *sc, int flg, bool fatal_on_error) :
...
void My2LoadCB::nomem_error(const char *fn)
{
  if (fatalOnError_)
...
void My2LoadCB::open_error(const char *fn)
{
  if (fatalOnError_)
...
void My2LoadCB::format_error(const char *fn)
{
  if (fatalOnError_)
...
void My2LoadCB::read_error(const char *fn)
{
  if (fatalOnError_)
...
int My2LoadCB::load_textures(int num)
{
...
int My2LoadCB::load_material(const char *name)
{
  if (flags & LASF_NOMATS)
    return 0;
  return 1;
...
int My2LoadCB::load_node_data() { return 1; }

int My2LoadCB::load_node_tm() { return 1; }

int My2LoadCB::load_node_script() { return 1; }

int My2LoadCB::load_node_mat(int num)
{
  if (flags & LASF_NOMATS)
    return 0;
  number = num;
...
int My2LoadCB::load_node_obj() { return 1; }

int My2LoadCB::load_texture(int i, const char *fn)
{
  if (i == 0)
  {
    mtex.resize(number);
...
int My2LoadCB::load_material(ImpMat &m)
{
...
int My2LoadCB::load_node_children()
{
  int a = -1;
  nstk.push_back(a);
...
int My2LoadCB::load_node_data(char *name, uint16_t id, int cnum, int flg)
{
  if (cnum)
  {
...
int My2LoadCB::load_node_tm(TMatrix &tm)
{
  cnode->tm = tm;
  return 1;
}

int My2LoadCB::load_node_submat(int i, uint16_t id)
{
  if (number > 1)
  {
    if (i == 0)
...
int My2LoadCB::load_node_script(const char *s)
{
  cnode->script = s;
...
int My2LoadCB::load_node_mesh(Mesh *m)
{
...
int My2LoadCB::load_node_splines(SplineShape *s)
{
...
int My2LoadCB::load_node_light(D3dLight &l)
{
...
int My2LoadCB::load_node_bones(int n)
{
  number = n;
  return 1;
}

int My2LoadCB::load_node_bonesinf(int n)
{
  number2 = n;
  return 1;
}
...
int My2LoadCB::load_node_bone(int i, uint16_t id, TMatrix &tm)
{
  if (flags & LASF_NOMESHES)
  {
    return 1;
...
int My2LoadCB::load_node_bonesinf(real *inf)
{
  if (flags & LASF_NOMESHES)
  {
    memfree(inf, tmpmem);
...
bool My2LoadCB::load_node_morph(int num)
{
  number = num;
  return true;
}
...
bool My2LoadCB::load_node_morph_target(int i, uint16_t nodeid, const char *name)
{
  if (flags & LASF_NOMESHES)
    return true;
  ...
  h->morphTargets[i].node = (Node *)(int)nodeid;
...
static void set_bones(Node *n, Node *root)
{
  if (n->obj && n->obj->isSubOf(OCID_MESHHOLDER))
  {
    ...
    for (int i = 0; i < h->morphTargets.size(); ++i)
      h->morphTargets[i].node = root->find_node(int(h->morphTargets[i].node));
...
{
  My2LoadCB cb(fn, &sc, flg, fatal_on_error);
  if (!load_scene(fn, cb))
  {
    if (fatal_on_error)
