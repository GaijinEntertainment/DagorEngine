...
public:
  ...
  void load()
  {
    ...
    const char *navmesh_layers = gameBlk->getStr("navmesh_layers", nullptr);
    if (!navmesh_layers || !*navmesh_layers)
      return;
    ...
    const char *navmesh_obstacles = gameBlk->getStr("navmesh_obstacles", nullptr);
    if (!navmesh_obstacles || !*navmesh_obstacles)
      return;
...
public:
  ...
  virtual bool catchEvent(unsigned event_huid, void *userData)
  {
    ...
    else if (event_huid == HUID_BeforeMainLoop && delayRiResInit)
    {
      ...
      for (int i = 0; i < p.size(); i++)
        p[i]->init1();
      rendinst::prepare_rt_rigen_data_render(grs_cur_view.pos, grs_cur_view.itm);
  ...
  virtual void renderGeometry(Stage stage)
  {
    ...
    uint64_t lh_mask = IObjEntityFilter::getLayerHiddenMask();
    uint32_t forceLodBits = 0;
    if (force_lod_no >= 0)
      forceLodBits = (force_lod_no + 1) << rendinst::LAYER_FORCE_LOD_SHIFT;
    ...
    switch (stage)
    {
      ...
      case STG_RENDER_STATIC_OPAQUE:
      {
        ...
        rendinst::renderRIGen(rendinst::RenderPass::Normal, ::grs_cur_view.pos, ::grs_cur_view.itm,
          rendinst::LAYER_OPAQUE | rendinst::LAYER_NOT_EXTRA | forceLodBits);
        if (hmlService)
      ...
      case STG_RENDER_STATIC_DECALS:
        ...
        ShaderGlobal::setBlock(frameBlkId, ShaderGlobal::LAYER_FRAME);
        rendinst::renderRIGen(rendinst::RenderPass::Normal, ::grs_cur_view.pos, ::grs_cur_view.itm,
          rendinst::LAYER_DECALS | forceLodBits);
        ShaderGlobal::setBlock(frame_block, ShaderGlobal::LAYER_FRAME);
        break;
      ...
      case STG_RENDER_STATIC_TRANS:
        ...
        ShaderGlobal::setBlock(frameBlkId, ShaderGlobal::LAYER_FRAME);
        rendinst::renderRIGen(rendinst::RenderPass::Normal, ::grs_cur_view.pos, ::grs_cur_view.itm,
          rendinst::LAYER_TRANSPARENT | forceLodBits);
        ShaderGlobal::setBlock(frame_block, ShaderGlobal::LAYER_FRAME);
        break;
      ...
      case STG_RENDER_STATIC_DISTORTION:
        ...
        ShaderGlobal::setBlock(frameBlkId, ShaderGlobal::LAYER_FRAME);
        rendinst::renderRIGen(rendinst::RenderPass::Normal, ::grs_cur_view.pos, ::grs_cur_view.itm,
          rendinst::LAYER_DISTORTION | forceLodBits);
        ShaderGlobal::setBlock(frame_block, ShaderGlobal::LAYER_FRAME);
        break;
      ...
      case STG_RENDER_SHADOWS:
        frame_block = ShaderGlobal::getBlock(ShaderGlobal::LAYER_FRAME);
        ShaderGlobal::setBlock(frameBlkId, ShaderGlobal::LAYER_FRAME);
        rendinst::renderRIGen(rendinst::RenderPass::ToShadow, ::grs_cur_view.pos, ::grs_cur_view.itm,
          rendinst::LAYER_OPAQUE | rendinst::LAYER_NOT_EXTRA | forceLodBits);
        ShaderGlobal::setBlock(frame_block, ShaderGlobal::LAYER_FRAME);
        break;
    }
  ...
  virtual rendinstgenland::AssetData *getLandClassGameRes(const char *name)
  {
    ...
    ::reset_required_res_list_restriction();
    return (rendinstgenland::AssetData *)res;
  }
  virtual void releaseLandClassGameRes(rendinstgenland::AssetData *res)
  {
    if (res)
  ...
  virtual bool createRtRIGenData(float ofs_x, float ofs_z, float grid2world, int cell_sz, int cell_num_x, int cell_num_z,
    dag::ConstSpan<rendinstgenland::AssetData *> land_cls, float dens_map_px, float dens_map_pz)
  {
  ...
  virtual EditableHugeBitMap2d *getRIGenBitMask(rendinstgenland::AssetData *land_cls)
  {
  ...
  virtual bool setupColliderParams(int mode, const BBox3 &area)
  {
    ...
    if (mode == 1)
    {
      ...
      {
        rendinst::prepare_rt_rigen_data_render(area.center(), grs_cur_view.itm);
...
protected:
    ...
    dag::ConstSpan<bool> isDynamicImpostorList, int layer_idx)
  {
    ...
    rendinstgen::RotationPaletteManager *rotationPaletteMgr = rendinstgen::get_rotation_palette_manager();
          ...
          if (int cnt = riLdCnt[k][i * SUBCELL_DIV * SUBCELL_DIV + j].size())
          {
            ...
            if (pool.isPos)
              for (int m = 0; m < cnt; m++)
              {
                ...
                if (pool.hasPaletteRotation)
                {
                  rendinstgen::RotationPaletteManager::Palette palette =
                    rendinstgen::get_rotation_palette_manager()->getPalette({layer_idx, poolId});
                  G_ASSERT(palette.count > 0);
                  rendinstgen::RotationPaletteManager::clamp_euler_angles(palette, pool.entitiesTmList[ent_idx[m]].tm, &palette_id);
                }
                int16_t data[4];
                size_t size =
                  rendinstgen::pack_entity_pos_inst_16(packData, Point3(p.pos.x, p.pos.y, p.pos.z), p.scale, palette_id, data);
              ...
              for (int m = 0; m < cnt; m++)
              {
                ...
                int16_t data[12];
                size_t size = rendinstgen::pack_entity_tm_16(packData, tm, data);
              ...
              for (int m = 0; m < cnt; m++)
              {
                const TMatrix &tm = pool.entitiesTmList[ent_idx[m]].tm;
                int32_t data[12];
                size_t size = rendinstgen::pack_entity_tm_32(packData, tm, data);
                G_ASSERT(size == 12 * sizeof(data[0]));
                for (int i = 0; i < 12; ++i)
    ...
    for (int i = 0; i < mask_num; i++)
    {
      ...
      cwr.writePtr64e(0);         // PATCHABLE_DATA64(rendinstgenland::AssetData*, asset);
