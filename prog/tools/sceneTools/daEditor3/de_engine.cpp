...
void *DagorEdAppWindow::addToolbar(int height) { return 0; }
...
CToolWindow *DagorEdAppWindow::createToolbar(ControlEventHandler *eh, void *hwnd)
{
  ...
  return new CToolWindow(eh, hwnd, 0, 0, w, h, "");
}


void *DagorEdAppWindow::addRawPropPanel(int width)
{
...
void DagorEdAppWindow::addPropPanel(int type, int width)
{
  panelsToAdd.push_back(type);
  panelsToAddWidth.push_back(width);
}
...
void DagorEdAppWindow::managePropPanels()
{
  ...
  for (int i = 0; i < panelsToAdd.size(); ++i)
  {
    ...
    {
      hwnd = addRawPropPanel(panelsToAddWidth[i]);
    ...
    else
    {
      ...
      mManager->resizeWindow(hwnd, panelsToAddWidth[i]);
      mManager->fixWindow(hwnd, true);
    }
...
CPanelWindow *DagorEdAppWindow::createPropPanel(ControlEventHandler *eh, void *hwnd)
{
  unsigned w, h;
  mManager->getWindowClientSize(hwnd, w, h);
  return new CPanelWindow(eh, hwnd, 0, 0, w, h, "");
}
...
CDialogWindow *DagorEdAppWindow::createDialog(int w, int h, const char *caption) { return new CDialogWindow(0, w, h, caption); }
