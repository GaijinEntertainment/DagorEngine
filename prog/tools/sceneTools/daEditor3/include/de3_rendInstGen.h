...
public:
  ...
  virtual rendinstgenland::AssetData *getLandClassGameRes(const char *name) = 0;
  virtual void releaseLandClassGameRes(rendinstgenland::AssetData *res) = 0;
  ...
  virtual bool createRtRIGenData(float ofs_x, float ofs_z, float grid2world, int cell_sz, int cell_num_x, int cell_num_z,
    dag::ConstSpan<rendinstgenland::AssetData *> land_cls, float dens_map_px, float dens_map_pz) = 0;
  virtual void releaseRtRIGenData() = 0;

  virtual EditableHugeBitMap2d *getRIGenBitMask(rendinstgenland::AssetData *land_cls) = 0;
