...
#define FIRST_COL 230
#define FOUTH_COL (FIRST_COL + 280)

#define CONTROLS_TOP 50
#define RADIO_TOP    (CONTROLS_TOP - 5)

#define HOTKEYS_BASE 600
#define CHECKS_BASE  300

#define VERT_INDENT 26
#define RADIO_WIDTH 200
...
PluginShowDialog::PluginShowDialog(void *phandle, const Tab<String> &tags, const StriMap<DeWorkspace::TabInt> &plugins) :
  CDialogWindow(phandle, DIALOG_WIDTH, DIALOG_HEIGHT, "Plugins visibility list"),
  plugins(tmpmem),
  ...
  mByRadio(false)
{
  ...
  mCheckComboPanel = mPanel->createContainer(CHECK_COMBO_GROUP_ID, false, 2);
  mPanel->createSeparator(0);
...
void PluginShowDialog::resizeTable()
{
  ...
  if (_static)
    _static->moveTo(_static->getX() + COMBO_SHIFT, _static->getY());
  rg->setWidth(DIALOG_WIDTH / COL_COUNT);
  mCheckComboPanel->moveTo(DIALOG_WIDTH / COL_COUNT, mCheckComboPanel->getY());
  mCheckComboPanel->setWidth(2 * DIALOG_WIDTH / COL_COUNT);
}
...
void PluginShowDialog::resizeLine(int idx)
{
  ...
  if (_check)
    _check->setWidth(_check->getWidth() + COMBO_SHIFT);

  PropertyControlBase *_combo = mPanel->getById(HOTKEYS_BASE + idx);
  if (_combo)
  {
    _combo->setWidth(COMBO_WIDTH);
    _combo->moveTo(_combo->getX() + COMBO_SHIFT, _combo->getY());
  }
}
