  ...
  noWaterService(false)
{
  ...
  console->setCaption("DaEditor3 console");
...
void DagorEdAppWindow::makeDafaultLayout()
{
  ...
  void *hVp2 = mManager->splitWindow(0, 0, 0.5f, WA_TOP);
  void *hVp3 = mManager->splitWindow(0, 0, 0.5f, WA_LEFT);
  void *hVp1 = mManager->splitWindow(hVp3, 0, 0.5f, WA_TOP);

  void *hPlugTools = mManager->splitWindow(0, 0, TOOLBAR_HEIGHT, WA_TOP);
  mManager->fixWindow(hPlugTools, true);

  void *hTabbar = mManager->splitWindow(0, 0, TABBAR_HEIGHT, WA_TOP);
  mManager->fixWindow(hTabbar, true);

  void *hToolbar = mManager->splitWindow(0, 0, TOOLBAR_HEIGHT, WA_TOP);
...
IWndEmbeddedWindow *DagorEdAppWindow::onWmCreateWindow(void *handle, int type)
{
  switch (type)
  {
    case TOOLBAR_TYPE:
    {
      ...
      mToolPanel = new CToolWindow(this, handle, 0, 0, w, h, "");
    ...
    case TABBAR_TYPE:
    {
      ...
      unsigned w, h;
      mManager->getWindowClientSize(handle, w, h);
      mTabWindow = new CToolWindow(this, handle, 0, 0, w, h, "");

      mTabPanel = mTabWindow->createTabPanel(TAB_PANEL_ID, "");
      mTabPanel->moveTo(2, 0);
      mTabWindow->setWidthById(TAB_PANEL_ID, mTabWindow->getWidth() - 4);
    ...
    case PLUGTOOLS_TYPE:
    {
      ...
      mPlugTools = new CToolWindow(this, handle, 0, 0, w, h, "");
...
void DagorEdAppWindow::fillMenu(IMenu *menu)
{
  ...
  menu->addItem(CM_HELP, CM_ABOUT, "About DaEditor3");
  ...
  switch (id)
  {
    ...
    case CM_VIEWPORT_LAYOUT_4:
    {
      ...
      if (curVp)
      {
        ...
        void *hVp2 = mManager->splitWindow(hVp1, 0, 0.5f, WA_BOTTOM);
        void *hVp3 = mManager->splitWindow(hVp1, 0, 0.5f, WA_RIGHT);
        void *hVp4 = mManager->splitWindow(hVp2, 0, 0.5f, WA_RIGHT);
    ...
    case CM_WINDOW_VIEWPORT:
      if (!ged.getViewportCount())
      {
        void *hwndViewPort = mManager->splitWindow(0, 0, VIEWPORT_HEIGHT, WA_BOTTOM);
...
void DagorEdAppWindow::startWithWorkspace(const char *wspName)
{
  ...
  {
    StartupDlg dlg(mManager->getMainWindow(), "DaEditor3 start", *getWorkspace(), WSP_FILE_PATH, wspName);
...
void DagorEdAppWindow::getDocTitleText(String &text)
{
  ...
  if (sceneFname[0])
    text.printf(300, "DaEditor3x  [%c%c%c%c]  - %s", _DUMP4C(drv), sceneFname);
  else
...
bool DagorEdAppWindow::loadProject(const char *filename)
{
  ...
  for (i = 0; i < plugin.size(); ++i, con.incDone())
  {
    ...
    {
      if (plugLocalBlk)
        plugin[i].p->setVisible(d3d::get_driver_code() == _MAKE4C('STUB') ? false : plugLocalBlk->getBool("visible", true));
    }
    DAGOR_CATCH(...)
    {
    ...
    if (plugin[i].p->acceptSaveLoad())
    {
      ...
      {
        wingw::message_box(wingw::MBS_EXCL, "Fatal error", "DaEditor3 failed to start:\n%s is broken", plugin_fn);
        quit_game(-1);
      }
      ...
      {
        if (!plugLocalBlk)
          plugin[i].p->setVisible(d3d::get_driver_code() == _MAKE4C('STUB') ? false : locBlk.getBool("visible", true));
...
bool DagorEdAppWindow::saveProject(const char *filename)
{
  ...
  if (d3d::get_driver_code() != _MAKE4C('STUB'))
  {
    String localSetDir(filename);
    ::location_from_path(localSetDir);
...
void DagorEdAppWindow::autoSaveProject(const char *filename)
{
  ...
  if (isSaved && d3d::get_driver_code() != _MAKE4C('STUB'))
