...
void EnvironmentPlugin::renderGeometry(Stage stage)
{
  ...
  switch (stage)
  {
    case STG_BEFORE_RENDER:
      if (!isAcesPlugin)
      {
        if (envParams.fog.used && previewFog)
        {
          // Directional fog.

          ShaderGlobal::set_color4(skyFogColorAndDensityVarId,
            Color4(envParams.fog.skyColor.r / 255.f, envParams.fog.skyColor.g / 255.f, envParams.fog.skyColor.b / 255.f,
              envParams.fog.skyDensity));

          ShaderGlobal::set_color4(sunFogColorAndDensityVarId,
            Color4(envParams.fog.sunColor.r / 255.f, envParams.fog.sunColor.g / 255.f, envParams.fog.sunColor.b / 255.f,
              envParams.fog.sunDensity));

          ShaderGlobal::set_color4(sunFogAzimuthSincosVarId, Color4(cosf(sunAzimuth), 0, sinf(sunAzimuth), 0));

          if (envParams.fog.useSunFog)
          {
            shadervars::set_progammable_fog(sunAzimuth, envParams.fog.skyDensity, envParams.fog.sunDensity,
              color3(envParams.fog.skyColor), color3(envParams.fog.sunColor));
          }
          else
          {
            shadervars::set_progammable_fog(sunAzimuth, envParams.fog.skyDensity, envParams.fog.skyDensity,
              color3(envParams.fog.skyColor), color3(envParams.fog.skyColor));
          }
        }
        else
        {
          ShaderGlobal::set_color4(skyFogColorAndDensityVarId, Color4(0.f, 0.f, 0.f, 0.f));
          ShaderGlobal::set_color4(sunFogColorAndDensityVarId, Color4(0.f, 0.f, 0.f, 0.f));
          ShaderGlobal::set_color4(sunFogAzimuthSincosVarId, Color4(0.f, 0.f, 0.f, 0.f));
        }


        if (previewFog && !is_equal_float(envParams.fog.lfogDensity, 0))
          shadervars::set_layered_fog(sunAzimuth, envParams.fog.lfogDensity, envParams.fog.lfogMinHeight, envParams.fog.lfogMaxHeight,
            color3(envParams.fog.lfogSkyColor), color3(envParams.fog.lfogSunColor));
        else
          shadervars::set_layered_fog(0, 0, -10, 10, color3(envParams.fog.lfogSkyColor), color3(envParams.fog.lfogSunColor));
      }
      else if (!skiesSrv)
      {
        ...
        float interp = (view * Point2(cosf(sunAzimuth), sinf(sunAzimuth)) + 1) / 2.0f;

        ::grs_cur_fog.color = e3dcolor(color4(currentEnvironmentAces.envParams.fog.skyColor) * (1 - interp) +
                                       color4(currentEnvironmentAces.envParams.fog.sunColor) * interp);

        ::grs_cur_fog.dens =
          currentEnvironmentAces.envParams.fog.skyDensity * (1 - interp) + currentEnvironmentAces.envParams.fog.sunDensity * interp;


        // Directional fog.

        ShaderGlobal::set_color4(skyFogColorAndDensityVarId,
          Color4(currentEnvironmentAces.envParams.fog.skyColor.r / 255.f, currentEnvironmentAces.envParams.fog.skyColor.g / 255.f,
            currentEnvironmentAces.envParams.fog.skyColor.b / 255.f, currentEnvironmentAces.envParams.fog.skyDensity));

        ShaderGlobal::set_color4(sunFogColorAndDensityVarId,
          Color4(currentEnvironmentAces.envParams.fog.sunColor.r / 255.f, currentEnvironmentAces.envParams.fog.sunColor.g / 255.f,
            currentEnvironmentAces.envParams.fog.sunColor.b / 255.f, currentEnvironmentAces.envParams.fog.sunDensity));


        ShaderGlobal::set_color4(sunFogAzimuthSincosVarId, Color4(sinf(sunAzimuth), 0, cosf(sunAzimuth), 0));

        shadervars::set_progammable_fog(sunAzimuth, currentEnvironmentAces.envParams.fog.skyDensity,
          currentEnvironmentAces.envParams.fog.sunDensity, color3(currentEnvironmentAces.envParams.fog.skyColor),
          color3(currentEnvironmentAces.envParams.fog.sunColor));
      }
      else
...
{
  if (!panel)
    EDITORCORE->addPropPanel(wtype_id, PROPBAR_WIDTH);
  else
...
{
  CDialogWindow *dlg = DAGORED2->createDialog(250, 120, "Connect To Game Http Server");
...
void EnvironmentPlugin::handleViewportPaint(IGenViewportWnd *wnd)
{
  ...
  if (skiesSrv)
  {
    ...
    StdGuiRender::render_box(0, h - 30, w, h);
    StdGuiRender::set_color(COLOR_BLACK);
    StdGuiRender::draw_strf_to(10, h - 10, desc);
  }
