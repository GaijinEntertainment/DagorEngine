...
SplashScreen::SplashScreen()
{
  ...
  bitmap = ::LoadImage(::GetModuleHandle(NULL), bmpPath, IMAGE_BITMAP, hdpi::_pxS(640), hdpi::_pxS(175), LR_LOADFROMFILE);
...
int __stdcall SplashScreen::wndProc(void *h_wnd, unsigned msg, void *w_param, void *l_param)
{
  if (msg == WM_PAINT)
  {
    ...
    ::TextOut(dc, _pxS(20), _pxS(145), IDagorEd2Engine::getBuildVersion(), (int)strlen(IDagorEd2Engine::getBuildVersion()));

    Tab<wchar_t> copyright_str;
    convert_utf8_to_u16_buf(copyright_str, COPYRIGHT_STR, -1);
    ::TextOutW(dc, _pxS(450), _pxS(145), copyright_str.data(), (int)wcslen(copyright_str.data()));
