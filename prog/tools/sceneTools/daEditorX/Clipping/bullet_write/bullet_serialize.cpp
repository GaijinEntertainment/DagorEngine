  ...
  void save(mkbindump::BinDumpSaveCB &cwr)
  {
    ...
    if (!no_grid_actors)
    {
      ...
      public:
        ...
        virtual void leaf_callback(SingleActor *a, int uo, int vo)
        {
          a->calcBbox(a->box);
          actors.push_back(a);
        }
    ...
    // save version
    cwr.writeInt32e(BULLET_SCENE_CLIPPING_VERSION);
...
class BulletCollisionBuilder : public IClippingDumpBuilder
{
public:
  ...
  virtual void start(const CollisionBuildSettings &stg)
  {
  ...
  virtual bool finishAndWrite(const char *temp_fname, IGenSave &cwr, unsigned target_code)
  {
...

    {
      ...
      else
      {
        ...
        collSaver.writeInt(mkbindump::le2be32_cond(BULLET_SCENE_CLIPPING_VERSION | 0x80000000, memCwr.WRITE_BE));
  ...
  virtual IClippingDumpBuilder *getSeparateRayTracer() { return raytracer_dump_builder; }

private:
  BulletSceneCollisionSaver *collisionSaver;
  IClippingDumpBuilder *raytracer_dump_builder;
  int64_t t0;
