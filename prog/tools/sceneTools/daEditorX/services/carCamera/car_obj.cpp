...
void VehicleViewer::renderInfo()
{
  ...
  renderInfoTest(String(256, "Speed: %.0f km/h", car->getVel().length() * 3.6), Point2(w - _pxS(50), h - _pxS(100)), COLOR_WHITE);

  if (car->getMachinery())
    renderInfoTest(String(256, "Gear step: %d", car->getMachinery()->getGearStep()), Point2(w - _pxS(50), h - _pxS(75)), COLOR_WHITE);

  if (driver)
    renderInfoTest(String(256, "Acceleration: %s", driver->getAccelState() ? "on" : "off"), Point2(w - _pxS(50), h - _pxS(50)),
