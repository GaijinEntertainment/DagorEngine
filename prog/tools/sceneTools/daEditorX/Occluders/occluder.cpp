...
const BBox3 occplugin::Occluder::normBox(Point3(-0.5, -0.5, -0.5), Point3(0.5, 0.5, 0.5));
static BBox3 normQuad(Point3(-0.5, -0.5, 0), Point3(0.5, 0.5, 0));

E3DCOLOR occplugin::Occluder::norm_col(200, 10, 10), occplugin::Occluder::sel_col(255, 255, 255);


void occplugin::Occluder::initStatics()
{
...
occplugin::Occluder::Occluder(bool box_occluder) { box = box_occluder; }

occplugin::Occluder::Occluder(const Occluder &src) : RenderableEditableObject(src), box(src.box) {}

void occplugin::Occluder::onAdd(ObjectEditor *objEditor)
{
  if (name.empty())
    objEditor->setUniqName(this, "Occluder");
}

bool occplugin::Occluder::getQuad(Point3 v[4])
{
  if (box)
    return false;
...
void occplugin::Occluder::render()
{
...
void occplugin::Occluder::renderCommonGeom(DynRenderBuffer &dynBuf, const TMatrix4 &gtm)
{
  bool curPlug = DAGORED2->curPlugin() == Plugin::self;
...
void occplugin::Occluder::renderBox(DynRenderBuffer &db, const TMatrix &tm, E3DCOLOR c)
{
  for (int i = 0; i < 8; i++)
  {
...
void occplugin::Occluder::renderQuad(DynRenderBuffer &db, const TMatrix &tm, E3DCOLOR c)
{
  for (int i = 0; i < 4; i++)
  {
...
void occplugin::Occluder::renderQuad(DynRenderBuffer &db, const Point3 v[4], E3DCOLOR c, bool ring)
{
  if (!ring)
  {
    boxVert[0].pos = v[0];
...
bool occplugin::Occluder::isSelectedByRectangle(IGenViewportWnd *vp, const EcRect &rect) const
{
  Point2 cp[8];
  BBox2 box2;
  real z;
...
bool occplugin::Occluder::isSelectedByPointClick(IGenViewportWnd *vp, int x, int y) const
{
  Point3 dir, p0;
  float out_t;
  ...
  dag::ConstSpan<RenderableEditableObject *> objects)
{
  ...
  {
    occplugin::Occluder *o = RTTI_cast<occplugin::Occluder>(objects[i]);
    if (!o)
    {
      one_type = false;
      continue;
...
void occplugin::Occluder::save(DataBlock &blk)
{
...
void occplugin::Occluder::update(float dt) {}
