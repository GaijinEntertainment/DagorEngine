...
AboutDlg::AboutDlg(void *hwnd) : mpHandle(hwnd)
{
  ...
  if (slash)
    strcpy(slash, "\\..\\commonData\\tex\\daeditor.bmp");

  mBitmapHandle = ::LoadImage(::GetModuleHandle(NULL), bmpPath, IMAGE_BITMAP, hdpi::_pxS(32), hdpi::_pxS(32), LR_LOADFROMFILE);
...
void AboutDlg::show()
{
  mHandle = CreateWindowEx(0, ABOUT_CLASS_NAME, "About DaEditorX", WS_SYSMENU, (GetSystemMetrics(SM_CXSCREEN) - DIALOG_WIDTH) / 2,
