...
macro INIT_SSAO()
  ...
  (ps)
  {
    ...
    //upscale_depth_cutoff@f4 = ( upscale_edge_cutoff_near, upscale_edge_cutoff_far,
    // (upscale_edge_cutoff_near-upscale_edge_cutoff_far)*(zn_zfar.x+upscale_edge_cutoff_near_dist*zn_zfar.y),
    // zn_zfar.x+upscale_edge_cutoff_near_dist*zn_zfar.y);
  }
  (vs)
  {
...
macro USE_SSAO()
  ...
  hlsl(ps) {
    ...
    SSAO_TYPE getSSAO(float fullResDepth, float2 ssaotc)
    {
      float totalWeight = 0;
      SSAO_TYPE aoOut = 0;
      int2 ssaotci = int2(lowres_rt_params.xy*ssaotc);
      #define DO_STEP2(valD, valAO) aoOut += doStep( valD, valAO, fullResDepth, totalWeight )
      #define DO_STEP(offset, dval) {dval = texelFetchOffset( downsampled_far_depth_tex, ssaotci, 0, offset ).r; SSAO_TYPE aoVal = texelFetchOffset( ssao_tex, ssaotci, 0, offset ).SSAO_ATTRS; DO_STEP2(dval, aoVal);}
      ...
      float4 lowResDepths = downsampled_far_depth_tex.GatherRed(downsampled_far_depth_tex_samplerstate, ssaotcfl).wzxy;
    ...
    half getSSAO(float fullResDepth, float2 ssaotc)
    {
      ...
      sampledDownsampledDepth.x = tex2Dlod(downsampled_far_depth_tex, float4(ssaoPointTc,0,0)).x;
      sampledDownsampledDepth.y = tex2Dlod(downsampled_far_depth_tex, float4(ssaoPointTc+float2(lowres_rt_params.z, 0),0,0)).x;
      sampledDownsampledDepth.z = tex2Dlod(downsampled_far_depth_tex, float4(ssaoPointTc+float2(0, lowres_rt_params.w),0,0)).x;
      sampledDownsampledDepth.w = tex2Dlod(downsampled_far_depth_tex, float4(ssaoPointTc+lowres_rt_params.zw,0,0)).x;
      ...
      float4 sampledDownsampledDepth = textureGather(downsampled_far_depth_tex, ssaotc);
