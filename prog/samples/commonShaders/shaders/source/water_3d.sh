...
shader water_nv2
{
    ...
    // WaveWorks related textures & constants
      ...
      VS_OUTPUT water_nv_vs(INPUT_VERTEXID_POSXZ USE_INSTANCE_ID)
      {
        ...
        } else
        {
          ...
          shoreWavesMultiplier = 0;
          float4 nvsf_blend_weights0123, nvsf_blend_weights4567;
          get_blend_factor(worldPos.xzy, worldPosDistance, nvsf_tex_coord_cascade01, nvsf_tex_coord_cascade23, nvsf_tex_coord_cascade45, nvsf_blend_weights0123, nvsf_blend_weights4567);
          displacement = 0;
        }
        ##else
  ...
  half4 water_nv_ps(VS_OUTPUT In HW_USE_SCREEN_POS) : SV_Target0
  {
    ...
    ##if water_cascades == five
    nvsf_blendfactors4567.xyz = saturate(0.25*(24.0-dist*UVScaleCascade4567.yzw));
    nvsf_blendfactors4567 *= nvsf_blendfactors4567;
    nvsf_blendfactors4567.xyz *= saturate(0.5 + 0.7*oceanWavesMultiplier);
    ##endif
    ...
    float floorZ = linearize_z(tex2Dlod(depth_gbuf, float4(screenTc,0,0)).x, zn_zfar.zw);
    ...
    seabedColor.rgb = max(float3(0,0,0), unpack_hdr(seabedColor).rgb-fog_add);//currently works not precise
    #else
