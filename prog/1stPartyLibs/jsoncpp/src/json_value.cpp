...
#define JSON_ASSERT( condition ) if (!(condition)) throw std::runtime_error( #condition );
#else
...
#else
static void fail_message(const char*) {}
# define JSON_FAIL_MESSAGE( message ) fail_message(message);
#endif
...
Value::Value( const eastl::string &value )
   : type_( stringValue )
   , allocated_( true )
   , comments_( 0 )
# ifdef JSON_VALUE_USE_INTERNAL_MAP
...
Value::swap( Value &other )
{
   ...
   type_ = other.type_;
   other.type_ = temp;
   eastl::swap( value_, other.value_ );
...
Value::asInt64() const
{
   ...
   case uintValue:
      ...
      return value_.uint_;
   case realValue:
      JSON_ASSERT_MESSAGE( value_.real_ >= minInt64  &&  value_.real_ <= (double)maxInt64, "Real out of Int64 range" );
      return Int( value_.real_ );
   case booleanValue:
      return value_.bool_ ? 1 : 0;
   case stringValue:
...
Value::asUInt64() const
{
   ...
   case uintValue:
      return value_.uint_;
   case realValue:
      JSON_ASSERT_MESSAGE( value_.real_ >= 0  &&  value_.real_ <= (double)maxUInt64,  "Real out of UInt64 range" );
      return UInt( value_.real_ );
   case booleanValue:
      return value_.bool_ ? 1 : 0;
   case stringValue:
...
Value &
Value::operator[]( const eastl::string &key )
{
...
const Value &
Value::operator[]( const eastl::string &key ) const
{
   return (*this)[ key.c_str() ];
}
...
Value 
Value::get( const eastl::string &key,
            const Value &defaultValue ) const
{
...
Value
Value::removeMember( const eastl::string &key )
{
...
bool 
Value::isMember( const eastl::string &key ) const
{
...
Value::getMemberNames() const
{
   ...
   for ( ; it != itEnd; ++it )
      members.push_back( eastl::string( (*it).first.c_str() ) );
#else
   ...
   for ( ; !ValueInternalMap::equals( it, itEnd ); ValueInternalMap::increment(it) )
      members.push_back( eastl::string( ValueInternalMap::key( it ) ) );
#endif
   return members;
}
//
...
void 
Value::setComment( const eastl::string &comment,
                   CommentPlacement placement )
{
...
PathArgument::PathArgument( const eastl::string &key )
   : key_( key.c_str() )
   , kind_( kindKey )
{
}
...
void 
Path::makePath( const eastl::string &path,
                const InArgs &in )
{
   ...
   while ( current != end )
   {
      ...
      else
      {
         ...
         while ( current != end  &&  !strchr( "[.", *current ) )
            ++current;
         args_.push_back( eastl::string( beginName, current ) );
      }
   }
}
...
void 
Path::addPathInArg( const eastl::string &/*path*/, 
                    const InArgs &in, 
...
void 
Path::invalidPath( const eastl::string &/*path*/, 
                   int /*location*/ )
{
   // Error: invalid path.
}
