...
static const SQRegFunctionFromStr mathlib_funcs[] = {
    { math_sqrt,   "pure fastcall sqrt(x: number): number",   "Returns the square root of x" },
    { math_sin,    "pure fastcall sin(x: number): number",    "Returns the sine of x (in radians)" },
    { math_cos,    "pure fastcall cos(x: number): number",    "Returns the cosine of x (in radians)" },
    { math_asin,   "pure fastcall asin(x: number): number",   "Returns the arcsine of x" },
    { math_acos,   "pure fastcall acos(x: number): number",   "Returns the arccosine of x" },
    { math_log,    "pure fastcall log(x: number): number",    "Returns the natural logarithm of x" },
    { math_log10,  "pure fastcall log10(x: number): number",  "Returns the base-10 logarithm of x" },
    { math_tan,    "pure fastcall tan(x: number): number",    "Returns the tangent of x (in radians)" },
    { math_atan,   "pure fastcall atan(x: number): number",   "Returns the arctangent of x" },
    { math_atan2,  "pure fastcall atan2(y: number, x: number): number", "Returns the arctangent of y/x using the signs to determine the quadrant" },
    { math_pow,    "pure fastcall pow(x: number, y: number): number", "Returns x raised to the power y" },
    { math_floor,  "pure fastcall floor(x: number): number",  "Rounds x downward to the nearest integer" },
    { math_ceil,   "pure fastcall ceil(x: number): number",   "Rounds x upward to the nearest integer" },
    { math_round,  "pure fastcall round(x: number): number",  "Rounds x to the nearest integer" },
    { math_exp,    "pure fastcall exp(x: number): number",    "Returns e raised to the power of x" },
    { math_srand,  "fastcall srand(seed: int)",               "Sets the seed for rand()" },
    { math_rand,   "fastcall rand(): int",                    "Returns a random integer in range 0..RAND_MAX" },
    { math_fabs,   "pure fastcall fabs(x: number): float",    "Returns the absolute value of float(x)" },
    { math_abs,    "pure fastcall abs(x: number): number",    "Returns the absolute value of x" },
    { math_min,    "pure fastcall min(x: number, y: number, ...: number): number", "Returns the minimum value from the arguments" },
