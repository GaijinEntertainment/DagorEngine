...
let function calculate_allocated_memory(obj) {
  ...
  switch(typeName) {
    ...
    case "table": sum += bytes_per_object + bytes_per_allocation * 2 + 32 + math.max(items, 4) * bytes_per_object * 4; break
    ...
    case "class": sum += bytes_per_object + bytes_per_allocation * 2 + 32 + math.max(items, 4) * bytes_per_object * 4; break
