...
struct DafxModFx : BaseParticleEffect
{
  ...
  dafx::SystemId sid;
  dafx::InstanceId iid;
  eastl::shared_ptr<dafx::SystemDesc const> pdesc;
  ...
  float currentCalcdDistanceScale = 1.f;
#if DAFX_USE_GRAVITY_ZONE
  Point3 gravityUp = Point3(0, 1, 0);
  Matrix3 rotateToLocalSpace = Matrix3::IDENT;
#endif
  int gameResId = 0;
  ...
  DafxModFx(const DafxModFx &r) :
    sid(r.sid), iid(r.iid), ctx(r.ctx), pdesc(r.pdesc), sinfo(r.sinfo), textures(r.textures), gameResId(r.gameResId)
  {
  ...
  void setLocalGravity(const TMatrix *tm = nullptr)
  {
...
#if DAFX_USE_GRAVITY_ZONE
    if (tm)
      rotateToLocalSpace = transpose(Matrix3(tm->array));

    Point3 invUp = rotateToLocalSpace * gravityUp;
#else
    G_ASSERT(tm);
    Matrix3 itm = transpose(Matrix3(tm->array));
    Point3 invUp = itm.getcol(1);
#endif

    invUp.normalizeF();
    dafx::set_instance_value(g_dafx_ctx, iid, it->second, &invUp, sizeof(invUp));
  }
  ...
  void setGravityTm(const Matrix3 &tm)
  {
    G_UNUSED(tm);
#if DAFX_USE_GRAVITY_ZONE
    bool gravZoneEnabled = sinfo.sflags & ((1 << MODFX_SFLAG_GRAVITY_ZONE_PER_EMITTER) | (1 << MODFX_SFLAG_GRAVITY_ZONE_PER_PARTICLE));
    if (!gravZoneEnabled)
      return;

    gravityUp = tm.getcol(1);
    setLocalGravity();
  ...
  {
    auto it = sinfo.valueOffsets.find(dafx_ex::SystemInfo::VAL_POS_SPLINE_DATA);
    if (it != sinfo.valueOffsets.end())
      dafx::set_instance_value(g_dafx_ctx, iid, it->second, &data, sizeof(TMatrix4));
  }
  ...
  void setParam(unsigned id, void *value) override
  {
    ...
    switch (id)
    {
      case _MAKE4C('PFXR'):
      {
        ...
        if (pdesc)
          sid = dafx::get_system_by_name(g_dafx_ctx, name, forceRecreate ? nullptr : pdesc.get());
      ...
      case _MAKE4C('PFXE'):
      {
        ...
        if (iid)
        {
          ...
          currentCalcdDistanceScale = sinfo.distanceScale.calcScale(distanceToCamOnSpawn);
#if DAFX_USE_GRAVITY_ZONE
          gravityUp = Point3(0, 1, 0);
          rotateToLocalSpace = Matrix3::IDENT;
#endif
  ...
  void *getParam(unsigned id, void *value) override
  {
    switch (id)
    {
      case _MAKE4C('PFXM'): *(int *)value = 1; break;
      case _MAKE4C('PFXQ'): *(dafx::SystemDesc const **)value = pdesc.get(); break;
      ...
      case _MAKE4C('FXLD'):
        if (value)
          ((eastl::vector<dafx::SystemDesc const *> *)value)->push_back(pdesc.get());
        break;
      case _MAKE4C('FXIC'):
        if (value)
  ...
  void setTransform(TMatrix value, dafx_ex::TransformType tr_type)
  {
    ...
    if (!is_equal_float(currentCalcdDistanceScale, 1.f))
    {
      value.col[0] *= currentCalcdDistanceScale;
      value.col[1] *= currentCalcdDistanceScale;
      value.col[2] *= currentCalcdDistanceScale;
    }
