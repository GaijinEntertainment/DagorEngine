...
{
  float3 wpos = mul_tm_pos(rp.pos, rparams.tm);
  if (!DAFX_SPARK_DECL_LOCAL_SPACE_ENABLED(rparams.flags))
  {
#if TEST_SPARKS_USE_NTM
    float4x4 normalizedTm = get_normalized_tm(rparams.tm);
    rp.velocity = mul_tm_dir(rp.velocity, normalizedTm);
#else
    rp.velocity = mul_tm_dir(rp.velocity, rparams.tm);
#endif
