...
macro USE_DAFX_SYSTEM_COMMON()
  ...
  hlsl(cs) {
    ...
    DAFX_INLINE
    bool apply_depth_collision(GlobalData_cref gparams, float3 pos, float3 vel, float restitution, float friction, out float3 out_pos, out float3 out_vel, out float min_dist)
    {
      out_pos = pos;
      out_vel = vel;
      min_dist = 1000000.0;

      float4 ndc = mul(float4(pos, 1), gparams.globtm_prev);
      ndc.xyz /= ndc.w;
      float4 ndcNew = mul(float4(pos + vel * gparams.dt, 1), gparams.globtm_prev);
      ndcNew.xyz /= ndcNew.w;
      ...
      out_vel += tangent * max(velTang - max(pow2(velTang), MIN_CALM_FRICTION) * friction * gparams.dt * pow2(max(normal.y, 0)), 0);
      return true;
    }
  }
endmacro
