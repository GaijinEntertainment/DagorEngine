...
float3x3 sparkfx_gravity_zone_tm(SIM_MODULE_CONTEXT)
{
  ...
  if (sparams.gravity_zone == GRAVITY_ZONE_DEFAULT || sparams.gravity_zone == GRAVITY_ZONE_PER_EMITTER)
    ...
    float3 wpos = DAFX_SPARK_DECL_LOCAL_SPACE_ENABLED(rparams.flags) ? mul_tm_pos(rp.pos, rparams.tm) : rp.pos;
...
float4 finish_sim_module(SIM_MODULE_CONTEXT, float dt)
{
  ...
  if (DAFX_SPARK_DECL_LOCAL_SPACE_ENABLED(rparams.flags))
    wpos = mul_tm_pos(rp.pos, rparams.tm);
...
{
#ifndef __cplusplus
  if (DAFX_SPARK_DECL_COLLISION_DISABLED(rparams.flags))
    return;
  ...
  bool relaxedCollison = DAFX_SPARK_DECL_COLLISION_RELAXED(rparams.flags);
  md.isCollided = apply_depth_collision(gparams, dt, rp.pos, rp.velocity, restitution, friction, relaxedCollison, rp.pos, rp.velocity, md.minCollisionDist);
  #else
  md.isCollided = false;
  #endif
#if SIM_MODULE_DATA_FORCE
...
void sim_module_directional_wind(SIM_MODULE_CONTEXT)
{
  ...
  float3 windVec = DAFX_SPARK_DECL_LOCAL_SPACE_ENABLED(rparams.flags) ? mul_tm_dir(windDir, rparams.tm) : windDir;
