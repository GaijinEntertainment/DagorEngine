...
#include "modfx_gpu_placement_flags.hlsli"
#include "modfx_flags_decl.hlsli"
...
#define DECL_ENABLED(p, a) ( bool( (p) & ( 1 << (a) ) ) )
#define FLAG_ENABLED(p, a) ( bool( (p) & ( 1 << (a) ) ) )
#define MOD_ENABLED(p, a) ( bool( p[((a)/32)] & ( 1 << ((a)%32) ) ) )
...
#define MODFX_RDECL_ORIENTATION_VEC 14
#define MODFX_RDECL_ORIENTATION_VEC_SIZE 4
#define MODFX_RDECL_ORIENTATION_VEC_ENABLED(a) DECL_ENABLED(a, MODFX_RDECL_ORIENTATION_VEC)
...
#define MODFX_FRAME_PERMANENT_FLAGS (MODFX_FRAME_FLAGS_FLIP_X | MODFX_FRAME_FLAGS_FLIP_Y)

#define MODFX_SIM_FLAGS_COLLIDED ( 1 << 0 )
#define MODFX_SIM_FLAGS_SHOULD_STOP_ROTATION ( 1 << 1 )
...
struct ModfxDeclMotionVectors
{
  float motion_vectors_strength;
  uint quality_mask;
};
...
struct ModfxDeclShapeStaticVelocityAlignedInit
{
  float3 right_vec;
  float3 orientation_vec;
};

struct ModfxDeclAdvancedTranslucency
{
  uint advanced_translucency_flags;
  float3 translucency_color_mul;

  float min_radian;
  float max_radian;
  float inv_max_radian_diff;

  float alpha_min;
  float alpha_max;
  float trans_min;
  float trans_max;
  float alpha_diff_inv;
};
// General translucency flag(s)
#define MODFX_TRANSLUCENCY_WEIGH_BY_VOL                      0
// Normal based translucency flags
#define MODFX_TRANSLUCENCY_USE_NORMAL_BASED_TRANSLUCENCY     1
#define MODFX_TRANSLUCENCY_INVERT_ACCEPTED_DEGREES           2
#define MODFX_TRANSLUCENCY_USE_VIEW_VEC_TRANSLUCENCY         3
// Alpha based translucency flags
#define MODFX_TRANSLUCENCY_USE_ALPHA_BASED_TRANSLUCENCY      4
...
struct ModfxDeclXrayParams
{
  uint blend_color1;
  uint blend_color2;
  float inv_max_distance_times_power;
};

struct ModfxDeclWaterFxParams
{
  uint use_height_tex_1;
  float height_scale;
  float3 detail_params;
};
...
struct ModfxDeclParticleHeightLimit
{
  float height_limit;
};

struct ModfxDeclFakeBrightness
{
  // x - dark_dark_val
  // y - dark_bright_val
  // z - bright_dark_val
  // w - bright_bright_val
  float4 brightness_values;
};

struct ModfxDeclZfarFadeout
{
  float negative_inv_dist_diff;
  float one_plus_min_dist_div_by_diff;
};
...
#define MODFX_RMOD_SHAPE_STATIC_VELOCITY_ALIGNED 17
#define MODFX_RMOD_SHAPE_VELOCITY_MOTION 18
#define MODFX_RMOD_OMNI_LIGHT_INIT 19
#define MODFX_RMOD_LIGHTING_INIT 20
#define MODFX_RMOD_DISTORTION_STRENGTH 21
#define MODFX_RMOD_DISTORTION_FADE_COLOR_COLORSTRENGTH 22
#define MODFX_RMOD_RIBBON_PARAMS 23
#define MODFX_RMOD_VOLSHAPE_PARAMS 24
#define MODFX_RMOD_RENDER_PLACEMENT_PARAMS 25
#define MODFX_RMOD_VOLFOG_INJECTION 26
#define MODFX_RMOD_MOTION_VECTORS 27
#define MODFX_RMOD_XRAY_PARAMS 28
#define MODFX_RMOD_ADVANCED_TRANSLUCENCY 29
#define MODFX_RMOD_WATER_FX_PARAMS 30
#define MODFX_RMOD_FAKE_BRIGHTNESS 31
#define MODFX_RMOD_FAKE_BRIGHTNESS_POS 32
#define MODFX_RMOD_ZFAR_FADEOUT 33

#define MODFX_RMOD_TOTAL_COUNT 34
...
void dafx_preload_parent_ren_data( BufferData_cref buf, uint parent_rofs, DAFX_OREF(ModfxParentRenData) data )
{
  ...
  _LL( MODFX_RMOD_SHAPE_STATIC_VELOCITY_ALIGNED );
  ...
  _LL( MODFX_RMOD_MOTION_VECTORS );
  _LL( MODFX_RMOD_XRAY_PARAMS );
  _LL( MODFX_RMOD_ADVANCED_TRANSLUCENCY );
  _LL( MODFX_RMOD_WATER_FX_PARAMS );
  _LL( MODFX_RMOD_FAKE_BRIGHTNESS );
  _LL( MODFX_RMOD_FAKE_BRIGHTNESS_POS );
  _LL( MODFX_RMOD_ZFAR_FADEOUT );
}
...
#define MODFX_SMOD_SHAPE_STATIC_VELOCITY_ALIGNED_INIT 44

#define MODFX_SMOD_PART_TRIMMING 45

#define MODFX_SMOD_COLOR_EMISSION_OVERRIDE 46

#define MODFX_SMOD_VELOCITY_FORCE_FIELD_VORTEX 47
#define MODFX_SMOD_VELOCITY_FORCE_FIELD_VORTEX_ROTATION_SPEED 48
#define MODFX_SMOD_VELOCITY_FORCE_FIELD_VORTEX_PULL_SPEED 49

#define MODFX_SMOD_VELOCITY_FORCE_FIELD_NOISE 50
#define MODFX_SMOD_VELOCITY_FORCE_FIELD_NOISE_CURVE 51

#define MODFX_SMOD_COLLISION_DECAY 52

#define MODFX_SMOD_ALPHA_BY_VELOCITY 53
#define MODFX_SMOD_ALPHA_BY_VELOCITY_CURVE 54

#define MODFX_SMOD_CAMERA_VELOCITY 55

#define MODFX_SMOD_GRAVITY_TM 56

#define MODFX_SMOD_HEIGHT_LIMIT 57

#define MODFX_SMOD_POS_SPLINE_BASED 58

#define MODFX_SMOD_POS_RAIN_MAP 59

#define MODFX_SMOD_TOTAL_COUNT 60
...
void dafx_preload_parent_sim_data( BufferData_cref buf, uint parent_sofs, DAFX_OREF(ModfxParentSimData) data )
{
  ...
  _LL( MODFX_SMOD_SHAPE_STATIC_VELOCITY_ALIGNED_INIT );
  ...
  _LL( MODFX_SMOD_HEIGHT_LIMIT );

  _LL( MODFX_SMOD_POS_SPLINE_BASED );

  _LL( MODFX_SMOD_POS_RAIN_MAP );
}
