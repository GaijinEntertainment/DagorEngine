...
let mkOptText = @(text) {rendObj = ROBJ_TEXT text size = static [flex(1), SIZE_TO_CONTENT] halign = ALIGN_RIGHT fontSize = hdpx(25)}
...
function mkSettingsOption(title, widget){
  return {
    ...
    children = [mkOptText(title), {size= static [flex(2), flex()] children = widget}],
    size = static [flex(), hdpx(35)] gap = hdpx(20)
  }
}
return {
  mkSaveData
