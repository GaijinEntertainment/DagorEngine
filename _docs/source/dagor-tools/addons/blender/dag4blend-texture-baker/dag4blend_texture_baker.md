...
![Where to Find It](_images/dag4blend_text_baker_01.png)

## Settings

Here, various parameters for the upcoming bake are organized into groups.

![Settings](_images/dag4blend_text_baker_02.png)
...
![Inputs: Self Bake](_images/dag4blend_text_baker_03.png)

![Inputs: HP to LP](_images/dag4blend_text_baker_04.png)
...
- **Extrusion**: Defines how much the search area for the high-poly will be
  ...
  ![Inputs: Extrusion](_images/dag4blend_text_baker_05.png)

  ![Inputs: Extrusion](_images/dag4blend_text_baker_06.png)
...
![Active Node](_images/dag4blend_text_baker_07.png)

![Active Node](_images/dag4blend_text_baker_08.png)
...
![Outputs](_images/dag4blend_text_baker_09.png)
...
![Operators](_images/dag4blend_text_baker_10.png)

- **Convert Principled BSDF**: As the name suggests, this converts *Blender*’s
  standard shader, allowing it to be baked as `rendinst_simple`. Useful when the
  material was mixed directly in *Blender* or when an asset was imported from
  GLB/glTF from another program along with textures.

- **Cleanup geometry**: Removes unsupported shader polygons from the source
  geometry. How do you know if a shader is supported? In the Shader Editor, the
  material’s Shader node should have outputs for `_tex_d_rgb`, `_tex_d_alpha`,
  `_tex_met_gloss`, and `_tex_mask`. This feature is primarily designed for
  decals, which cannot be baked. *Blender* cannot bake through transparent
  polygons; the top polygon will completely overwrite the information beneath
  it.
  ...
  `rendinst_mask_layered` can be replaced with `rendinst_perlin_layered`. This
  does not have to be done manually. In the directory with the installed add-on,
  there is a text file with remapping parameters:
...
![Limitations](_images/dag4blend_text_baker_11.png)
...
![Limitations](_images/dag4blend_text_baker_12.png)
