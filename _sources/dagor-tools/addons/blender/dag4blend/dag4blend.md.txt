...
<img src="./_images/inst_preferences.png" alt="Preferences" width="75%">
...
<img src="./_images/inst_projects.png" alt="Projects" width="75%">
...
- **Shading mode:** – shaders behave slightly different between projects. The
  ...
  *WarThunder-like* shaders.

- **Palettes** – there are default global and local palettes.

- **Experimental Features** – new tools that are functional but have some
  limitations. Currently, these include a *composites editor* and *texture
  baking tools* tools:

<img src="_images/inst_exp_features.png" alt="Experimental Features" width="75%">
...
<img src="_images/inst_scene.png" alt="Scene" width="75%">
...
and the shaders will be in *WarThunder-like* mode. To correct this, re-select
...
<img src="./_images/log_text_field.png" alt="Text field" width="100%">

The import/export functions (and future ones) write execution information to a text  **log** field.

```{note} The **log** does not clear itself, so remember to clear it
periodically after ensuring that there are no unchecked errors and warnings. To
clear it, you can simply delete the text object. The next time you need it, the
toolset will create a new one. This approach can be faster than selecting all
the lines and deleting them with `Backspace`/`Delete`.

```

To edit object properties/dagormats/proxymats as text, the toolset creates a
special text object. The name of the object is provided in the tooltip.
...
<img src="./_images/mat_dagormat.png" alt="Dagormat" width="50%">
...
<img src="./_images/mat_dagormat_main.png" alt="Dagormat&#9656;Main" width="100%">
...
<img src="./_images/mat_dagormat_shader.png" alt="Dagormat&#9656;Shader" width="100%">
...
Viewer* or Explorer; quotes will be removed automatically.

<img src="./_images/mat_dagormat_textures.png" alt="Dagormat&#9656;Textures" width="50%">
...
<img src="./_images/mat_dagormat_optional.png" alt="Dagormat&#9656;Optional" width="50%">

### Tools

Here you can edit the active material in text format.

<img src="./_images/mat_dagormat_tools_1.png" alt="Dagormat&#9656;Tools" width="50%">

<img src="./_images/mat_dagormat_tools_2.png" alt="Dagormat&#9656;Tools" width="50%">
...
<img src="./_images/mat_dagormat_proxy.png" alt="Dagormat&#9656;Proxy" width="50%">
...
proxymat folder. It can still be edited as text or temporarily unchecked as **is
proxymat**.
...
the checkbox and saving it to the desired folder.

```{note} Proxymats in *Blender* use the file name, not a user-defined one. Upon
import, shaders are automatically renamed if the name differs.

```

## Object Properties

Located in the N-panel under the `Dagor` tab, it shows parameters of the active
object and is hidden if there is no active object.

<img src="./_images/obj_prop.png" alt="Object Properties" width="35%">
...
editor. The **open presets folder** button opens the folder containing all
...
```{note} Incorrect values in Object Properties are recorded in a new **broken
...
```{note} Use **Cleanup...** only for exporting composite parts for 3D Max! It
can cause issues. In a `.dag` file for the game engine, there should not be
multiple objects with the same name.

```

- **Name** field  – context-sensitive, appearing only when exporting to a single
  `.dag` file. This field is hidden in other export modes.

- **Collection** field – also context-sensitive. You do not need to select a
...
1. **Limit by:** Visible
    ...
    **Example input:**
    ...
    <img src="./_images/batch_exp_visible.png" alt="Limit by&#9656;Visible" width="30%">
...
1. **Limit by:** Sel.Joined
    ...
    **Example input:**
    ...
    <img src="./_images/batch_exp_sel_joined.png" alt="Limit by&#9656;Sel.Joined" width="30%">
...
1. **Limit by:** Sel.Separated
    ...
    **Example input:**
    ...
    <img src="./_images/batch_exp_sel_separated.png" alt="Limit by&#9656;Sel.Separated" width="50%">
...
1. **Limit by:** Col.Separated
    ...
    **Example input:**
    ...
    <img src="./_images/batch_exp_col_separated.png" alt="Limit by&#9656;Col.Separated" width="75%">
...
2. **Limit by:** Col.Joined
    ...
    **Export object:** export all contents of the selected collection to a file
    with the name of this collection. Created mainly for Asset Viewer of assets
    created from several `.dag` files.

    **Example input:**

    <img src="./_images/batch_exp_col_joined.png" alt="Limit by&#9656;Col.Joined" width="100%">
    ...
    **Example output:**
    ...
    (transfer with `Ctrl` in Outliner), i.e. they are not copies, but the same
    collections. The rest of the temporary lods can be prepared in the same way.
    It's much faster than selecting objects and typing the name manually.

      ```{note} Since they are the same collections and not copies, do not
      delete them with `DEL`, but remove them with `RMB`&#9656;`Unlink`, so that
...
<img src="./_images/coll_prop.png" alt="Collection Properties" width="30%">
...
<img src="./_images/coll_prop_override_ex_1.png" alt="Override name: example 1" width="50%">
...
different subfolders. Here, you can specify a subpath that will be appended to
the main export path. If there is no need to rename the collection, simply end
the subpath with `*`, which will be replaced by the collection name during
export:

<img src="./_images/coll_prop_override_ex_2.png" alt="Override name: example 2" width="50%">
...
<img src="./_images/coll_prop_override_ex_3.png" alt="Override name: example 3" width="50%">
...
Used by the composite editor. When importing, it records the node type, and
during export, it ensures a unique type (to avoid double interpretation in the
presence of asset namesakes, typically found among composites and rendInsts).
For manually created item intended for composite export, set this manually.

<img src="./_images/coll_prop_override_type.png" alt="Type" width="30%">
...
```{note} Currently, geometry nodes cannot be combined with custom normals, as
applying the modifier erases them. If you're placing instances using geometry
nodes, don't forget to add the **Realize instances** node; otherwise, the
geometry will be lost during export. Technically, before this node, they are
similar to “empty” without their own geometry.
...
<img src="./_images/sm_groups_1.png" alt="Smoothing groups: Init" width="30%">

When smoothing exist, the interface works similarly to 3D Max, but without
...
<img src="./_images/sm_groups_2.png" alt="Smoothing groups" width="30%">
...
```{note} Selecting by smoothing group adds to the existing selection, rather
than replacing it. So, if you need to select only group 3, for example, clear
the selection first.
...
<img src="./_images/sm_groups_3.png" alt="Smoothing groups" width="50%">
...
Texture slots are still used as in `rendinst_simple`, regardless of the chosen
shader. Nodes do not overlap but are arranged meaningfully. Images from all
slots are now added to the Shader Editor, so with Node Wrangler enabled, they
can be viewed by pressing `Shift`+`Ctrl`+`LMB`.
...
- **Replace existing** – if a collection with the asset name exists in the
  scene, it will replace its contents with those from the `.dag` file instead of
  creating a duplicate with a `.001+` suffix. Useful for quickly reverting
  multiple changes.

<img src="./_images/file_import.png" alt="File&#9656;Import" width="20%">
...
<img src="./_images/batch_imp.png" alt="Batch Import" width="35%">

Options:

- **Search in subfolders** – checks all subdirectories for files. Use
  cautiously, as it can hang *Blender* if too many matches are found.
...
- **Preserve paths** – saves the full path to the `.dag` file (including the
  ...
  folders and needing to export them back to original locations.
...
Viewer or Explorer. Instead of searching for it through `File`&#9656;`Import`,
you can copy the path from Asset Viewer or Explorer (`RMB`&#9656;`Copy as path`)
and paste it into **Path** field of the batch import panel. This action will
automatically transfer the file name to **Masks**, and clicking **IMPORT** will
load only the desired asset.
...
<img src="./_images/batch_imp_add_feat.png" alt="Batch Import: Additional Features" width="50%">
...
<img src="./_images/tools.png" alt="Tools" width="35%">

- **Optimize material slots** – merges identical material slots and removes
  unused ones.

- **Merge Duplicates** – merges identical materials with different indices
  (e.g., `Material` and `Material.001`). These duplicate materials often arise
  when copying objects using `ctrl+c`/`ctrl+v`, as this operation saves the
...
- **Save textures** – collects all textures with valid paths from selected
  objects and saves them in the `/textures/` subfolder of the export folder.
...
Allows baking textures from complex shaders to `rendinst_simple`. Commonly used
for final lods or porting assets to mobile projects.

For more details about *Texture Baker* tool, see
[here](../dag4blend-texture-baker/dag4blend_texture_baker.md).

<img src="./_images/bake.png" alt="Bake" width="75%">

### Composite Editor

For more details about *Composite Editor*, see
[here](../dag4blend-comp-editor/dag4blend_comp_editor.md).

<img src="./_images/comp_editor.png" alt="Composites Editor" width="35%">
